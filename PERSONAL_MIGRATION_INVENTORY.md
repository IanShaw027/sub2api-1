...
> **按产品能力取舍的交互面板**见 canvas `migration-decisions`。
> **按产品能力逐条提炼的具体功能点**（302 条，每条含用户/管理员可感知的具体行为 + 涉及文件）见
> `docs/migration-details/features-01-data-account-auth.md` 至 `features-07-admin-media.md`，7 份报告覆盖全部 34 个能力中
> 有实际改动的 30 个（其余 4 个是部署/文档/待定/建议放弃类目，无需功能点级核对）。

## 差异有效性核验（2026-08-13 补充，四轮）

### 第一轮：文件级假差异（PR 往返收敛）

用 `git merge-tree --write-tree upstream/main personal-dev` 计算"把上游最新合入 personal-dev 后的真实增量"，
与三点 diff（本清单基准，2894 文件）对比：仅 9 个文件已被上游吸收收敛（PR 流回后两边一致），从取舍面板剔除。
净 diff 本身不含 1340 次提交 / 75 次合并的中间 churn——历史再乱，净 diff 已经折叠干净。

### 第二轮：不靠路径猜测，逐文件核对内容（响应"差异不一定等于特殊处理"的质疑）

第一轮曾按路径规则把文件分成 func/生成/测试/i18n冗余/一次性产物，但路径规则可能出错。逐文件核实后发现并修正 2 处：

- `backend/ent/migrate/{ai_skill_center,auth_identity_fk_ondelete}_test.go` —— 路径在 `backend/ent/` 下被误判为"生成代码"，
  实际是**手写的**、验证外键级联与索引是否与迁移一致的测试，改判为「测试」桶（跟随对应迁移批次，而非当作生成代码忽略）。
- `backend/cmd/server/wire_gen.go` —— 路径规则漏判，内容头部确认为 `Code generated by Wire. DO NOT EDIT.`，改判为「生成代码」桶。

核实方法：对全部 2803 个真实增量文件（剔除假差异后）逐一 `git show` 检查文件头 200 字节是否含生成代码标记，
与路径分类结果双向比对，只有以上 2 处不一致，其余 2801 个文件的路径分类与内容验证一致。

同时用忽略空白的完整 unified diff 逐行剥离注释与空行，得到"有效代码行"：全库层面 +75.2 万行原始新增中
+65 万行是真代码（仅 12 个文件是纯空白/注释改动、零真实代码变化，已从功能决策面剔除，多为文档/测试里的无意义改动）。
按此口径重新统计手写功能代码：

  | 类别 | 文件 | 有效代码行 |
  |---|---|---|
  | ★ 手写功能代码（决策面，逐文件真实行数） | 1389 | **+143,606**（第三轮修正后；原路径估算 +234k，因改为逐文件精确归因而非按文件数比例分摊，更准确） |
  | ent + wire 生成代码 | 187 | 随 `go generate ./ent` / wire 重生成，不计入决策 |
  | Go + 前端测试（含 2 处改判的手写迁移校验测试） | 1226 | 跟随对应实现批次迁移，不单独决策 |
  | i18n monolith / 锁文件 / 一次性产物 | ~76 | 直接删除或忽略 |

### 第三轮：merge-tree 冲突标记污染修正

`git merge-tree --write-tree upstream/main personal-dev` 在计算合并结果时，对 80 个文件产生了**未解决的合并冲突**
（内容里混进了字面的 `<<<<<<< / ||||||| / ======= / >>>>>>>` 标记），包括 `group.go`、`admin_service.go`、
`billing_service.go`、`pricing_service.go`、`channel.go` 等核心文件和一整批 `openai_gateway_grok*.go` / `grok_quota_service.go`。
这些文件的"合并后内容"实际不可编译，第二轮的逐文件核对未覆盖到这一层。

用 `git grep "^<<<<<<< "` 直接在对比基准树里扫描确认了完整的 80 个文件清单，对这些文件改用两点 diff
（`upstream/main` 直接对比 `personal-dev` tip，绕开被污染的合并树版本）重新计算有效代码行数，
上表"手写功能代码"行数已按此修正（+144,834 → **+143,606**，冲突标记本身此前被误计成了新增代码行）。
两个独立的深挖代理（分别核对能力 9 与能力 28/29）在各自范围内又交叉验证发现了同一批冲突文件中的子集，
与本轮扫描结果完全吻合。

### 第四轮：逐能力提炼具体功能点（响应"文件行数不等于用户能感知的功能"的进一步质疑）

前三轮解决的是"文件/行数统计准不准"，但决策真正需要的是"这个能力到底做了什么用户能感知的具体事情"。
派发 7 个深挖代理，对 30 个有实际改动的能力（合计约 1389 个功能文件）逐文件读取 diff 内容，
提炼出 **302 条具体功能点**——每条描述用户或管理员能感知的具体行为（前端可见/后台管理/纯后端能力），
纯内部重构/无感知改动的文件单独归类，不硬编功能。报告按能力分组见 `docs/migration-details/features-0{1..7}-*.md`，
`migration-decisions` canvas 已按此重建为功能点级别的决策面（不再是文件数/行数的抽象聚合）。

取舍面板已按上述四轮口径重建，每个能力显示的"功能代码 X 文件 / +Y 行"与"具体功能点清单"均为逐文件核实后的真实值。
