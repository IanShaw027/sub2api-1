...
| **订阅 USD→CNY 汇率** | CNY 通道下订阅套餐每 1 USD 收取的 CNY | `0`（未配置） |

#### 订阅 USD→CNY 汇率（重要）

订阅套餐的 `price` 在系统中按 **USD** 理解。

- 用户通过 **CNY** 支付通道（常见：支付宝/微信）购买**订阅**时，网关扣款金额为 `套餐价格_USD × subscription_usd_to_cny_rate`。
- CNY 订阅下单**必须**配置**正数**汇率。汇率为 `0` 或未配置时，创建订单会返回 `INVALID_PAYMENT_CONFIG`（fail-closed，避免把 USD 数字当作 CNY 收款）。
- 余额充值不受此字段影响（余额本身按 CNY 金额处理）。
- 非 CNY 币种的订阅订单不走该换算。

在启用 CNY 通道销售订阅前，请先在 **设置 → 支付设置** 中配置该汇率。若你的套餐价已是 CNY 语义且仍走 CNY 网关，仅在与目录币种策略一致时才可将汇率设为 `1.0`（更推荐目录统一 USD + 真实汇率）。
