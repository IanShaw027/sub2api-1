...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	userRepository := repository.NewUserRepository(db, userCache)
	...
	apiKeyRepository := repository.NewApiKeyRepository(db, apiKeyCache)
	groupRepository := repository.NewGroupRepository(db)
	userSubscriptionRepository := repository.NewUserSubscriptionRepository(db)
	apiKeyService := service.NewApiKeyService(apiKeyRepository, userRepository, groupRepository, userSubscriptionRepository, apiKeyCache, configConfig)
	apiKeyHandler := handler.NewAPIKeyHandler(apiKeyService)
	usageLogCache := repository.NewUsageLogCache(client)
	usageLogRepository := repository.NewUsageLogRepository(db, usageLogCache)
