...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	mediaAssetRepository := repository.NewMediaAssetRepository(client)
	...
	mediaObjectStoreFactory := repository.NewS3MediaStoreFactory()
	mediaStorageResolver := service.ProvideMediaStorageResolver(backupService, mediaObjectStoreFactory)
	mediaService := service.ProvideMediaService(mediaAssetRepository, mediaStorageResolver, configConfig)
	...
	userHandler := handler.ProvideUserHandler(userService, authService, emailService, emailCache, affiliateService, serviceUserPlatformQuotaRepository, userRPMCache, concurrencyService)
	...
	antigravityGatewayService := service.ProvideAntigravityGatewayService(accountRepository, gatewayCache, schedulerSnapshotService, antigravityTokenProvider, rateLimitService, httpUpstream, settingService, internal500CounterCache, tlsFingerprintProfileService, tlsFingerprintRouterService)
	geminiMessagesCompatService := service.ProvideGeminiMessagesCompatService(accountRepository, groupRepository, gatewayCache, schedulerSnapshotService, geminiTokenProvider, rateLimitService, httpUpstream, antigravityGatewayService, configConfig, tlsFingerprintProfileService, tlsFingerprintRouterService)
	...
	adminUserHandler := handler.ProvideAdminUserHandler(adminService, concurrencyService, serviceUserPlatformQuotaRepository, billingCache, totpService, userService, settingService, dashboardService)
	...
	accountUsageService := service.ProvideAccountUsageService(accountRepository, usageLogRepository, claudeUsageFetcher, geminiQuotaService, antigravityQuotaFetcher, kiroTokenProvider, grokQuotaFetcher, grokQuotaService, openAIQuotaService, usageCache, identityCache, compositeTokenCacheInvalidator, proxyRepository, tlsFingerprintProfileService, settingService, openAIGatewayService)
	...
	accountTestService := service.ProvideAccountTestService(accountRepository, geminiTokenProvider, claudeTokenProvider, grokTokenProvider, antigravityGatewayService, httpUpstream, configConfig, tlsFingerprintProfileService, tlsFingerprintRouterService, openAIGatewayService, settingService, kiroTokenProvider, pluginManager)
	crsSyncService := service.NewCRSSyncService(accountRepository, proxyRepository, oAuthService, openAIOAuthService, geminiOAuthService, configConfig)
	accountHandler := admin.ProvideAccountHandler(adminService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, grokOAuthService, rateLimitService, accountUsageService, accountTestService, concurrencyService, crsSyncService, sessionLimitCache, rpmCache, compositeTokenCacheInvalidator, grokQuotaService, opsService, kiroTokenProvider)
	...
	invoiceService := service.ProvideInvoiceService(client, configConfig, notificationEmailService, mediaService)
	paymentService := service.ProvidePaymentService(client, registry, defaultLoadBalancer, redeemService, subscriptionService, paymentConfigService, userRepository, groupRepository, affiliateService, notificationEmailService, invoiceService)
	settingHandler := handler.ProvideAdminSettingHandler(settingService, emailService, turnstileService, aliyunCaptchaService, opsService, paymentConfigService, paymentService, userAttributeService, notificationEmailService, totpService, userService, mediaService)
	...
	invoiceHandler := admin.NewInvoiceHandler(invoiceService, mediaService)
	ticketService := service.ProvideTicketService(client, configConfig, notificationEmailService, userGroupRateRepository)
	ticketHandler := admin.NewTicketHandler(ticketService, mediaService)
	affiliateHandler := admin.NewAffiliateHandler(affiliateService, adminService)
	complianceHandler := admin.NewComplianceHandler(settingService)
	auditLogRepository := repository.NewAuditLogRepository(db)
	auditLogService := service.ProvideAuditLogService(auditLogRepository, settingService)
	auditLogHandler := admin.NewAuditLogHandler(auditLogService, totpService)
	capacityRepository := repository.NewCapacityForecastRepository(db)
	openAICapacityProvider := service.ProvideOpenAICapacityProvider(accountRepository, openAIQuotaService, capacityRepository)
	anthropicCapacityProvider := service.ProvideAnthropicCapacityProvider(accountUsageService, capacityRepository)
	grokCapacityProvider := service.ProvideGrokCapacityProvider(grokQuotaService, capacityRepository)
	antigravityCapacityProvider := service.ProvideAntigravityCapacityProvider(accountRepository, antigravityQuotaFetcher, capacityRepository)
	geminiCapacityProvider := service.ProvideGeminiCapacityProvider(accountRepository, geminiQuotaService, usageLogRepository, capacityRepository)
	capacityProviderRegistry := service.ProvideCapacityProviderRegistry(openAICapacityProvider, anthropicCapacityProvider, grokCapacityProvider, antigravityCapacityProvider, geminiCapacityProvider)
	capacityForecastService := service.ProvideCapacityForecastService(capacityRepository, accountRepository, capacityProviderRegistry, redisClient, timingWheelService, leaderLockCache, db)
	openAIOAuthCapacityService := service.NewOpenAIOAuthCapacityService(accountRepository, capacityForecastService)
	capacityHandler := admin.NewCapacityHandler(capacityForecastService, openAIOAuthCapacityService)
	...
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, kiroOAuthHandler, cnProviderHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, tlsFingerprintRouterHandler, pluginHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, promptAdminHandler, paymentHandler, invoiceHandler, ticketHandler, affiliateHandler, complianceHandler, auditLogHandler, capacityHandler, ipSecurityHandler, upstreamBillingProbeService, ollamaCloudUsageService, accountDeviceService)
	...
	handlerInvoiceHandler := handler.NewInvoiceHandler(invoiceService, mediaService, paymentConfigService)
	handlerTicketHandler := handler.NewTicketHandler(ticketService, mediaService)
	...
	creationSessionRepository := repository.NewCreationSessionRepository(client)
	creationMessageRepository := repository.NewCreationMessageRepository(client)
