...
func TestAccountHandlerRefreshSingleAccount_KiroUsesTokenProvider(t *testing.T) {
	...
	handler.SetKiroTokenProvider(provider)
...
func TestAccountHandlerRefreshSingleAccount_GrokUsesGrokOAuthService(t *testing.T) {
	...
	handler := NewAccountHandler(adminSvc, nil, nil, nil, nil, grokSvc, nil, nil, nil, nil, nil, nil, nil, nil)
	...
	require.Equal(t, "grok-access-token", adminSvc.lastUpdateAccountInput.Credentials["access_token"])
	require.Equal(t, "https://grok.old", adminSvc.lastUpdateAccountInput.Credentials["base_url"])
}
...
func TestAccountHandlerSetKiroOverage(t *testing.T) {
	...
	handler := NewAccountHandler(adminSvc, nil, nil, nil, nil, nil, nil, &service.AccountUsageService{}, nil, nil, nil, nil, nil, nil)
	router := gin.New()
