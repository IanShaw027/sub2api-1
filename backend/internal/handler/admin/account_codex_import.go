...
func buildCodexIdentityKeys(accountID, userID, email, accessToken string) []string {
	...
	// Preserve the local email+account composite identity as a stronger key than
	// the shared account fallback, while keeping upstream's chatgpt_user_id-first
	// ordering and account: last to avoid cross-member updates in the same team.
	if email != "" && accountID != "" {
