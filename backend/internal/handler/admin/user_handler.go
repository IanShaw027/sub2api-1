...
func (h *UserHandler) GetUserPlatformQuotas(c *gin.Context) {
	...
	if h.userPlatformQuotaRepo == nil {
		response.Error(c, http.StatusServiceUnavailable, "platform quota service not available")
		return
	}
...
func (h *UserHandler) UpdateUserPlatformQuotas(c *gin.Context) {
	...
	if len(req.Quotas) > len(service.AllowedQuotaPlatforms) {
		response.BadRequest(c, "quotas length must be <= "+strconv.Itoa(len(service.AllowedQuotaPlatforms)))
		return
	}
	...
	// 增加一次 DB 查询和逻辑复杂度。由于 AllowedQuotaPlatforms 只有少量元素，
