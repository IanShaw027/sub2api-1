...
func (h *AuthHandler) OIDCOAuthCallback(c *gin.Context) {
	...
	email := oidcSyntheticEmailFromIdentityKey(oidcIdentityKey(issuer, subject))
...
func (h *AuthHandler) CompleteOIDCOAuthRegistration(c *gin.Context) {
	...
	if err := applyPendingOAuthAdoptionAndConsumeSession(c.Request.Context(), client, h.authService, h.userService, h, session, decision, user.ID); err != nil {
		respondPendingOAuthBindingApplyError(c, err)
		return
	}
...
func (k oidcJWK) publicKey() (any, error) {
	...
	case "EC":
		...
		encoded := make([]byte, 1+2*pointSize)
		encoded[0] = 4
		x.FillBytes(encoded[1 : 1+pointSize])
		y.FillBytes(encoded[1+pointSize:])
