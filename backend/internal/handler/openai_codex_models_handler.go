...
func (h *OpenAIGatewayHandler) CodexModels(c *gin.Context) {
	...
	for {
		...
		requestCtx := service.WithTLSFingerprintInboundUserAgent(c.Request.Context(), c.GetHeader("User-Agent"))
		manifest, err := h.gatewayService.FetchCodexModelsManifest(requestCtx, account, c.Query("client_version"), c.GetHeader("If-None-Match"))
		if err != nil {
			if c.Request.Context().Err() != nil {
				return
			}
			if service.IsRetryableCodexModelsManifestError(err) && switchCount < maxAccountSwitches {
				...
				if account.ParentAccountID != nil {
					failedAccountIDs[*account.ParentAccountID] = struct{}{}
				}
				switchCount++
				lastUpstreamErr = err
				continue
