...
func TestOpenAIRecordUsageInputsCarryMandatoryAccountingFields(t *testing.T) {
	files := []string{
	...
	requiredKeys := []string{"QuotaPlatform", "CyberBlocked", "RequestPayloadHash", "ChannelUsageFields"}
	...
	for _, name := range files {
		t.Run(name, func(t *testing.T) {
			...
			require.Empty(t, missing, "OpenAI usage post-billing must receive mandatory accounting fields")
		})
	}
}
