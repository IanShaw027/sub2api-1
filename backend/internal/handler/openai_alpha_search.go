...
func (h *OpenAIGatewayHandler) AlphaSearch(c *gin.Context) {
	...
	mappedBodyForSearch := newOpenAIModelMappedBodyCache(body, h.gatewayService.ReplaceModelInBody)
	forwardBody := mappedBodyForSearch(channelMapping.Mapped, channelMapping.MappedModel)
	...
	for {
		...
		accountRelease, acquireStatus := h.acquireResponsesAccountSlot(c, apiKey.GroupID, apiKey.ID, sessionHash, "", selection, false, &streamStarted, reqLog)
