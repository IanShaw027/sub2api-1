	...
	for {
		...
		hooks := &service.OpenAIWSIngressHooks{
			...
			AfterTurn: func(turn int, turnPayload []byte, result *service.OpenAIForwardResult, turnErr error) {
				...
				h.gatewayService.ReportOpenAIAccountScheduleResult(account.ID, turnErr == nil, result.FirstTokenMs)
				inboundEndpoint := GetInboundEndpoint(c)
				upstreamEndpoint := resolveOpenAIUpstreamEndpoint(c, account, result)
				quotaPlatform := service.QuotaPlatform(c.Request.Context(), apiKey)
				cyberBlocked := service.GetOpsCyberPolicy(c) != nil
...
func (h *OpenAIGatewayHandler) ensureForwardErrorResponse(c *gin.Context, streamStarted bool, forwardErr error) bool {
	...
	jsonKeepaliveCommitted := service.StopOpenAIImagesJSONKeepaliveCommitted(c)
	if c.Writer.Written() {
		if !jsonKeepaliveCommitted && !openAIStreamingResponseStarted(c, streamStarted) {
			return false
		}
		if !jsonKeepaliveCommitted {
			streamStarted = true
		}
	}
