...
func TestOpenAIGatewayHandlerImages2API_RequiresWeb2APIEnabled(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	require.Equal(t, http.StatusForbidden, rec.Code)
	require.Equal(t, "permission_error", gjson.GetBytes(rec.Body.Bytes(), "error.type").String())
	require.Contains(t, rec.Body.String(), service.OpenAIImages2APIDisabledMessage())
}

func TestOpenAIGatewayHandlerImages_RequiresCodexEnabled(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	require.Equal(t, http.StatusForbidden, rec.Code)
	require.Equal(t, "permission_error", gjson.GetBytes(rec.Body.Bytes(), "error.type").String())
	require.Contains(t, rec.Body.String(), service.OpenAIImagesCodexDisabledMessage())
}
