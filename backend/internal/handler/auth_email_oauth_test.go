...
func TestEmailOAuthCallbackCreatesPasswordRegistrationSessionForNewEmail(t *testing.T) {
	...
	handler, client := newOAuthPendingFlowTestHandlerWithDependencies(t, oauthPendingFlowTestHandlerOptions{
		...
		affiliateFactory: func(_ *dbent.Client, _ *service.SettingService) *service.AffiliateService {
			return service.NewAffiliateService(affiliateRepo, &oauthPendingFlowSettingRepoStub{
	...
	req.AddCookie(&http.Cookie{Name: emailOAuthAffiliateCookie, Value: encodeCookieValue("AFF222222345")})
	c.Request = req
	...
	require.Equal(t, "AFF222222345", pendingSessionStringValue(session.UpstreamIdentityClaims, "aff_code"))
...
func TestEmailOAuthCallbackRejectsNewRegistrationWhenRegistrationDisabled(t *testing.T) {
	handler, client := newOAuthPendingFlowTestHandlerWithDependencies(t, oauthPendingFlowTestHandlerOptions{
		settingValues: map[string]string{
	...
	require.Equal(t, http.StatusFound, recorder.Code)
	location := recorder.Header().Get("Location")
	require.Contains(t, location, "error=REGISTRATION_DISABLED")
	userCount, err := client.User.Query().Where(dbuser.EmailEQ("disabled@example.com")).Count(ctx)
	require.NoError(t, err)
	require.Zero(t, userCount)
	sessionCount, err := client.PendingAuthSession.Query().Count(ctx)
	require.NoError(t, err)
...
func TestCompleteEmailOAuthRegistrationUsesAffiliateCodeFromPendingSession(t *testing.T) {
	affiliateRepo := newOAuthEmailAffiliateRepoStub(map[string]int64{"AFF222222456": 2002})
	handler, client := newOAuthPendingFlowTestHandlerWithDependencies(t, oauthPendingFlowTestHandlerOptions{
...
func (r *oauthEmailAffiliateRepoStub) AccrueQuota(context.Context, service.AffiliateAccrualInput) (float64, error) {
...
func (r *oauthEmailAffiliateRepoStub) ListInviteeLedger(context.Context, int64, int64, int) ([]service.AffiliateLedgerEntry, error) {
	panic("unexpected ListInviteeLedger call")
}

func (r *oauthEmailAffiliateRepoStub) CountRebatedInvitees(context.Context, int64) (int, error) {
	panic("unexpected CountRebatedInvitees call")
}

func (r *oauthEmailAffiliateRepoStub) ListAdminAffiliateStats(context.Context, service.AdminAffiliateListParams) ([]service.AdminAffiliateStatsRow, int64, error) {
