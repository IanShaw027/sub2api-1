...
func TestResolveCLIVersionDefaultsToStable(t *testing.T) {
	t.Setenv(CLIVersionEnv, "")
	require.Equal(t, CLIStableVersion, ResolveCLIVersion())
}
...
func TestApplyCLIProxyHeaders(t *testing.T) {
	...
	require.Equal(t, CLIStableVersion, req.Header.Get("x-grok-client-version"))
	...
	require.Equal(t, CLIUserAgent(CLIStableVersion), req.Header.Get("User-Agent"))
}
