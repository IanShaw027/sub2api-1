...
func (c *ssoDeviceFakeClient) Do(req *http.Request) (*http.Response, error) {
	...
	case SSOAccountsURL:
		...
		return ssoDeviceResponse(http.StatusOK, http.Header{"Set-Cookie": {"session=web-session; Domain=.x.ai; Path=/; Secure"}}, `{}`), nil
	case SSODeviceURL:
