...
type GatewayOpenAIWSConfig struct {
	...
	// MaxIngressConnectionsPerAPIKey bounds live ingress sessions across instances. Zero disables it.
...
func (c *Config) Validate() error {
	...
	if c.Billing.UserPlatformQuotaCacheTTLSeconds <= 0 {
		return fmt.Errorf("billing.user_platform_quota_cache_ttl_seconds must be positive")
	}
	...
	if c.Gateway.StreamDataIntervalTimeout != 0 &&
		(c.Gateway.StreamDataIntervalTimeout < 30 || c.Gateway.StreamDataIntervalTimeout > 2000) {
		return fmt.Errorf("gateway.stream_data_interval_timeout must be 0 or between 30-2000 seconds")
	}
