...
func TestServerTimingFinalizesOnDirectWriteHeader(t *testing.T) {
	recorder := runServerTimingRequest(t, true, "/api/v1/admin/status", "", "admin", func(c *gin.Context) {
		c.Writer.WriteHeader(http.StatusNoContent)
	})
	if got := recorder.Header().Get(servertiming.HeaderName); got == "" {
		t.Fatal("timing header was not written before direct response commit")
	}
}
