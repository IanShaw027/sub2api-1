...
func registerContentModerationRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	risk := admin.Group("/risk-control")
	{
		...
		risk.GET("/hashes", h.Admin.ContentModeration.ListFlaggedHashes)
		risk.DELETE("/hashes/batch", h.Admin.ContentModeration.DeleteFlaggedHashes)
...
func registerAccountRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	accounts := admin.Group("/accounts")
	{
		...
		accounts.POST("/import/archive", h.Admin.Account.ImportArchive)
...
func registerSettingsRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	adminSettings := admin.Group("/settings")
	{
		...
		// 临时不可调度规则窗口阈值配置
		adminSettings.GET("/temp-unsched-threshold", h.Admin.Setting.GetTempUnschedThresholdSettings)
		adminSettings.PUT("/temp-unsched-threshold", h.Admin.Setting.UpdateTempUnschedThresholdSettings)
		// 请求整流器配置
