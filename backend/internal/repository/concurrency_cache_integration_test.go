...
func (s *ConcurrencyCacheSuite) TestAccountActiveIndex_AcquireAndRelease() {
	...
	require.NoError(s.T(), err, "release keeps a bounded tombstone so concurrent acquire cannot lose its index")
	require.NoError(s.T(), s.rdb.ZAdd(s.ctx, accountActiveIndexKey, redis.Z{Score: float64(now - 1), Member: member}).Err())
	require.NoError(s.T(), s.cache.CleanupExpiredAccountSlotKeys(s.ctx))
	_, err = s.rdb.ZScore(s.ctx, accountActiveIndexKey, member).Result()
	require.ErrorIs(s.T(), err, redis.Nil, "expired empty account index member should be reconciled")
}
...
func (s *ConcurrencyCacheSuite) TestAccountActiveIndex_WaitLifecycle() {
	...
	require.NoError(s.T(), s.cache.CleanupExpiredAccountSlotKeys(s.ctx))
	_, err = s.rdb.ZScore(s.ctx, accountActiveIndexKey, member).Result()
	require.ErrorIs(s.T(), err, redis.Nil, "expired empty wait index member should be reconciled")
}
...
func (s *ConcurrencyCacheSuite) TestUserActiveIndex_AcquireAndRelease() {
	...
	now, err := s.rawCache.redisUnixSeconds(s.ctx)
	require.NoError(s.T(), err)
	require.NoError(s.T(), s.rdb.ZAdd(s.ctx, userActiveIndexKey, redis.Z{Score: float64(now - 1), Member: member}).Err())
	require.NoError(s.T(), s.cache.CleanupExpiredAccountSlotKeys(s.ctx))
	_, err = s.rdb.ZScore(s.ctx, userActiveIndexKey, member).Result()
	require.ErrorIs(s.T(), err, redis.Nil, "expired empty user index member should be reconciled")
}
...
func (s *ConcurrencyCacheSuite) TestGroupSlot_TrackReleaseAndActiveIndex() {
	groupID := int64(302)
	member := strconv.FormatInt(groupID, 10)

	require.NoError(s.T(), s.rawCache.AcquireGroupSlot(s.ctx, groupID, "group-req"))
	count, err := s.rawCache.GetGroupConcurrency(s.ctx, groupID)
	require.NoError(s.T(), err)
	require.Equal(s.T(), 1, count)

	_, err = s.rdb.ZScore(s.ctx, groupActiveIndexKey, member).Result()
	require.NoError(s.T(), err, "group acquire should register the active index")

	require.NoError(s.T(), s.rawCache.ReleaseGroupSlot(s.ctx, groupID, "group-req"))
	count, err = s.rawCache.GetGroupConcurrency(s.ctx, groupID)
	require.NoError(s.T(), err)
	...
	now, err := s.rawCache.redisUnixSeconds(s.ctx)
	require.NoError(s.T(), err)
	require.NoError(s.T(), s.rdb.ZAdd(s.ctx, groupActiveIndexKey, redis.Z{Score: float64(now - 1), Member: member}).Err())
	require.NoError(s.T(), s.cache.CleanupExpiredAccountSlotKeys(s.ctx))
	_, err = s.rdb.ZScore(s.ctx, groupActiveIndexKey, member).Result()
	require.ErrorIs(s.T(), err, redis.Nil, "expired empty group index member should be reconciled")
}
...
func (s *ConcurrencyCacheSuite) TestCleanupExpiredAccountSlotKeys_ReapsGroupIndex() {
	now, err := s.rawCache.redisUnixSeconds(s.ctx)
	require.NoError(s.T(), err)
	expiredScore := float64(now - 10)
	groupKeyWithFresh := groupSlotKey(501)

	require.NoError(s.T(), s.rdb.ZAdd(s.ctx, groupKeyWithFresh,
		redis.Z{Score: float64(now), Member: "fresh"},
	).Err())
	require.NoError(s.T(), s.rdb.ZAdd(s.ctx, groupActiveIndexKey,
		redis.Z{Score: expiredScore, Member: "501"},
		redis.Z{Score: expiredScore, Member: "502"},
	).Err())

	require.NoError(s.T(), s.cache.CleanupExpiredAccountSlotKeys(s.ctx))

	score, err := s.rdb.ZScore(s.ctx, groupActiveIndexKey, "501").Result()
	require.NoError(s.T(), err)
	require.Greater(s.T(), int64(score), now, "loaded group should be re-scheduled")
	_, err = s.rdb.ZScore(s.ctx, groupActiveIndexKey, "502").Result()
	require.ErrorIs(s.T(), err, redis.Nil, "idle group member should be reaped")
}
...
func (s *ConcurrencyCacheSuite) TestCleanupStaleProcessSlots_ReconcilesExpiredIndexMembers() {
	...
	accountMembers, err := s.rdb.ZRange(s.ctx, accountKey, 0, -1).Result()
	require.NoError(s.T(), err)
	require.Equal(s.T(), []string{"oldproc-1"}, accountMembers)
	userMembers, err := s.rdb.ZRange(s.ctx, userKey, 0, -1).Result()
	require.NoError(s.T(), err)
	require.Equal(s.T(), []string{"oldproc-2"}, userMembers)
	accountWait, err := s.rdb.Get(s.ctx, accountWaitKey).Int()
	require.NoError(s.T(), err)
	require.Equal(s.T(), 4, accountWait)

	accountScore, err := s.rdb.ZScore(s.ctx, accountActiveIndexKey, strconv.FormatInt(accountID, 10)).Result()
	require.NoError(s.T(), err)
	require.Greater(s.T(), int64(accountScore), now)
	userScore, err := s.rdb.ZScore(s.ctx, userActiveIndexKey, strconv.FormatInt(userID, 10)).Result()
	require.NoError(s.T(), err)
	require.Greater(s.T(), int64(userScore), now)
}
...
func (s *ConcurrencyCacheSuite) TestCleanupStaleProcessSlots_PreservesFreshForeignSlotKey() {
	accountID := int64(903)
	accountSlotKey := fmt.Sprintf("%s%d", accountSlotKeyPrefix, accountID)
	now, err := s.rawCache.redisUnixSeconds(s.ctx)
	require.NoError(s.T(), err)
...
func (s *ConcurrencyCacheSuite) TestCleanupStaleProcessSlots_PreservesIndexedGroupSlots() {
	groupID := int64(904)
	groupKey := groupSlotKey(groupID)
	now, err := s.rawCache.redisUnixSeconds(s.ctx)
	require.NoError(s.T(), err)
	...
	members, err := s.rdb.ZRange(s.ctx, groupKey, 0, -1).Result()
	require.NoError(s.T(), err)
	require.ElementsMatch(s.T(), []string{"activeproc-1", "oldproc-1"}, members)
	score, err := s.rdb.ZScore(s.ctx, groupActiveIndexKey, strconv.FormatInt(groupID, 10)).Result()
	require.NoError(s.T(), err)
	require.Greater(s.T(), int64(score), now)
}

func (s *ConcurrencyCacheSuite) TestActiveIndexCleanupDoesNotRemoveConcurrentRefresh() {
	...
	// A delayed older touch must not shorten the newer deadline either.
	s.rawCache.touchActiveIndexAt(s.ctx, accountActiveIndexKey, accountID, now+10)
	score, err = s.rdb.ZScore(s.ctx, accountActiveIndexKey, member).Result()
	require.NoError(s.T(), err)
	require.EqualValues(s.T(), now+100, int64(score))
}
