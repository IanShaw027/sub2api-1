...
const usageLogSelectColumns = "id, user_id, api_key_id, account_id, request_id, model, requested_model, upstream_model, group_id, subscription_id, input_tokens, output_tokens, cache_creation_tokens, cache_read_tokens, cache_creation_5m_tokens, cache_creation_1h_tokens, image_output_tokens, image_output_cost, input_cost, output_cost, cache_creation_cost, cache_read_cost, total_cost, actual_cost, rate_multiplier, billed_by_higher_priced_upstream, account_rate_multiplier, billing_type, request_type, stream, openai_ws_mode, duration_ms, first_token_ms, user_agent, ip_address, image_count, image_size, image_input_size, image_output_size, image_size_source, image_size_breakdown, video_count, video_resolution, video_duration_seconds, service_tier, reasoning_effort, inbound_endpoint, upstream_endpoint, cache_ttl_overridden, long_context_billing_applied, channel_id, model_mapping_chain, billing_tier, billing_mode, video_seconds, video_unit_price, account_stats_cost, created_at, openai_ws_profile, openai_ws_conn_reused, provider"
...
var usageLogInsertArgTypes = [...]string{
	...
	"boolean",     // billed_by_higher_priced_upstream
...
func buildExcludeAdminCondition(userIDColumn string, argIndex int) string {
	return fmt.Sprintf(
		"NOT EXISTS (SELECT 1 FROM users _ua WHERE _ua.id = %s AND _ua.role = $%d)",
		userIDColumn,
		argIndex,
	)
}

func appendExcludeAdminWhereCondition(conditions []string, args []any, userIDColumn string, excludeAdmin bool) ([]string, []any) {
	if !excludeAdmin {
		return conditions, args
	}
	conditions = append(conditions, buildExcludeAdminCondition(userIDColumn, len(args)+1))
	args = append(args, service.RoleAdmin)
	return conditions, args
}

func appendExcludeAdminQueryFilter(query string, args []any, userIDColumn string, excludeAdmin bool) (string, []any) {
	if !excludeAdmin {
		return query, args
	}
	query += fmt.Sprintf(" AND %s", buildExcludeAdminCondition(userIDColumn, len(args)+1))
	args = append(args, service.RoleAdmin)
	return query, args
}
...
func prepareUsageLogInsert(log *service.UsageLog) usageLogInsertPrepared {
	...
	var imageSizeBreakdown any
	if len(log.ImageSizeBreakdown) > 0 {
		if payload, err := json.Marshal(log.ImageSizeBreakdown); err == nil {
			imageSizeBreakdown = string(payload)
		}
	}
	...
	return usageLogInsertPrepared{
		...
		args: []any{
			...
			log.ActualCost,
			rateMultiplier,
			log.BilledByHigherPricedUpstream,
			...
			log.VideoCount,
			videoResolution,
			nullInt(log.VideoDurationSeconds),
			serviceTier,
			reasoningEffort,
			inboundEndpoint,
			upstreamEndpoint,
			...
			strings.TrimSpace(log.Provider),
		},
	}
}
...
func (r *usageLogRepository) GetDashboardStats(ctx context.Context) (*DashboardStats, error) {
	...
	if err := r.fillDashboardPaymentFlowStats(ctx, stats, time.Unix(0, 0).UTC(), now.UTC(), todayStart.UTC(), todayEnd.UTC()); err != nil {
		return nil, err
	}
...
func (r *usageLogRepository) GetDashboardStatsWithRange(ctx context.Context, start, end time.Time) (*DashboardStats, error) {
	...
	if err := r.fillDashboardConsumptionStats(ctx, stats, startUTC, endUTC, todayStart.UTC(), todayEnd.UTC()); err != nil {
		return nil, err
	}
	if err := r.fillDashboardPaymentFlowStats(ctx, stats, startUTC, endUTC, todayStart.UTC(), todayEnd.UTC()); err != nil {
		return nil, err
	}
...
func (r *usageLogRepository) fillDashboardConsumptionStats(ctx context.Context, stats *DashboardStats, startUTC, endUTC, todayUTC, todayEnd time.Time) error {
	query := `
		WITH scoped AS (
			SELECT
				created_at,
		...
		FROM scoped
	`
	balanceType := int8(service.BillingTypeBalance)
	subscriptionType := int8(service.BillingTypeSubscription)
	return scanSingleRow(
		ctx,
		r.sql,
		query,
		[]any{startUTC, endUTC, todayUTC, todayEnd, balanceType, subscriptionType},
		&stats.TotalBalanceActualCost,
		&stats.TotalSubscriptionActualCost,
		&stats.TodayBalanceActualCost,
		&stats.TodaySubscriptionActualCost,
...
func (r *usageLogRepository) fillDashboardPaymentFlowStats(ctx context.Context, stats *DashboardStats, startUTC, endUTC, todayUTC, todayEnd time.Time) error {
	query := `
		SELECT
			COALESCE((
			...
			COALESCE((
				SELECT SUM(pay_amount)
				FROM payment_orders
				WHERE paid_at >= $3::timestamptz AND paid_at < $4::timestamptz
...
func (r *usageLogRepository) GetUsageTrendWithFilters(ctx context.Context, startTime, endTime time.Time, granularity string, userID, apiKeyID, accountID, groupID int64, model string, requestType *int16, stream *bool, billingType *int8, billingMode string) (results []TrendDataPoint, err error) {
	return r.getUsageTrendWithFilters(ctx, startTime, endTime, granularity, userID, apiKeyID, accountID, groupID, model, "", requestType, stream, billingType, billingMode)
}
...
func (r *usageLogRepository) GetModelStatsWithFilters(ctx context.Context, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID int64, requestType *int16, stream *bool, billingType *int8, billingMode string) (results []ModelStat, err error) {
	return r.getModelStatsWithFiltersBySource(ctx, startTime, endTime, userID, apiKeyID, accountID, groupID, "", requestType, stream, billingType, billingMode, usagestats.ModelSourceRequested)
}
...
func (r *usageLogRepository) GetModelStatsWithFiltersBySource(ctx context.Context, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID int64, requestType *int16, stream *bool, billingType *int8, billingMode string, source string) (results []ModelStat, err error) {
	return r.getModelStatsWithFiltersBySource(ctx, startTime, endTime, userID, apiKeyID, accountID, groupID, "", requestType, stream, billingType, billingMode, source)
}
...
func (r *usageLogRepository) GetModelStatsWithUsageFiltersBySource(ctx context.Context, startTime, endTime time.Time, filters UsageLogFilters, source string) (results []ModelStat, err error) {
	return r.getModelStatsWithFiltersBySource(ctx, startTime, endTime, filters.UserID, filters.APIKeyID, filters.AccountID, filters.GroupID, filters.Model, filters.RequestType, filters.Stream, filters.BillingType, filters.BillingMode, source)
}

func (r *usageLogRepository) getModelStatsWithFiltersBySource(ctx context.Context, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID int64, model string, requestType *int16, stream *bool, billingType *int8, billingMode string, source string) (results []ModelStat, err error) {
...
func (r *usageLogRepository) GetGroupStatsWithFilters(ctx context.Context, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID int64, requestType *int16, stream *bool, billingType *int8, billingMode string) (results []usagestats.GroupStat, err error) {
	return r.getGroupStatsWithFilters(ctx, startTime, endTime, userID, apiKeyID, accountID, groupID, "", requestType, stream, billingType, billingMode)
}
...
func (r *usageLogRepository) GetUserBreakdownStats(ctx context.Context, startTime, endTime time.Time, dim usagestats.UserBreakdownDimension, limit int) (results []usagestats.UserBreakdownItem, err error) {
	...
	query, args = appendExcludeAdminQueryFilter(query, args, "ul.user_id", dim.ExcludeAdmin)
...
func (r *usageLogRepository) GetStatsWithFilters(ctx context.Context, filters UsageLogFilters) (*UsageStats, error) {
	...
	var upstreamEndpoints []EndpointStat
	var endpointPaths []EndpointStat
	...
	runEndpoints := func(c context.Context) {
		res, err := r.getEndpointStatsByColumnWithFilters(c, "inbound_endpoint", start, end, filters.UserID, filters.APIKeyID, filters.AccountID, filters.GroupID, filters.Model, filters.ModelFilterSource, filters.RequestType, filters.Stream, filters.BillingType, filters.BillingMode, filters.ExcludeAdmin)
		if err != nil {
	...
	runUpstream := func(c context.Context) {
		res, err := r.getEndpointStatsByColumnWithFilters(c, "upstream_endpoint", start, end, filters.UserID, filters.APIKeyID, filters.AccountID, filters.GroupID, filters.Model, filters.ModelFilterSource, filters.RequestType, filters.Stream, filters.BillingType, filters.BillingMode, filters.ExcludeAdmin)
		if err != nil {
			if !errors.Is(err, context.Canceled) && !errors.Is(err, context.DeadlineExceeded) {
	...
	runPaths := func(c context.Context) {
		res, err := r.getEndpointPathStatsWithFilters(c, start, end, filters.UserID, filters.APIKeyID, filters.AccountID, filters.GroupID, filters.Model, filters.ModelFilterSource, filters.RequestType, filters.Stream, filters.BillingType, filters.BillingMode, filters.ExcludeAdmin)
		if err != nil {
			if !errors.Is(err, context.Canceled) && !errors.Is(err, context.DeadlineExceeded) {
...
func (r *usageLogRepository) getEndpointStatsByColumnWithFilters(ctx context.Context, endpointColumn string, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID int64, model string, modelSource string, requestType *int16, stream *bool, billingType *int8, billingMode string, excludeAdmin bool) (results []EndpointStat, err error) {
	...
	query, args = appendExcludeAdminQueryFilter(query, args, "user_id", excludeAdmin)
	query, args = appendUsageLogModelQueryFilter(query, args, model, modelSource)
	query, args = appendRequestTypeOrStreamQueryFilter(query, args, requestType, stream)
	if billingType != nil {
		query += fmt.Sprintf(" AND billing_type = $%d", len(args)+1)
...
func (r *usageLogRepository) getEndpointPathStatsWithFilters(ctx context.Context, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID int64, model string, modelSource string, requestType *int16, stream *bool, billingType *int8, billingMode string, excludeAdmin bool) (results []EndpointStat, err error) {
	actualCostExpr := "COALESCE(SUM(actual_cost), 0) as actual_cost"
	if accountID > 0 && userID == 0 && apiKeyID == 0 {
		actualCostExpr = "COALESCE(SUM(COALESCE(account_stats_cost, total_cost) * COALESCE(account_rate_multiplier, 1)), 0) as actual_cost"
	}
...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	...
	log := &service.UsageLog{
		...
		ActualCost:                   actualCost,
		RateMultiplier:               rateMultiplier,
		BilledByHigherPricedUpstream: billedByHigherPricedUpstream,
	...
	log.VideoCount = videoCountSnapshot
...
func buildRequestTypeFilterConditionWithAlias(startArgIndex int, requestType int16, alias string) (string, []any) {
	...
	case service.RequestTypeImage:
		return fmt.Sprintf("(%srequest_type = $%d OR (%srequest_type = %d AND %s))", prefix, startArgIndex, prefix, int16(service.RequestTypeCyberBlocked), legacyImageRequestTypeSQLPredicateWithAlias(alias)), []any{requestTypeArg}
	case service.RequestTypeCyberBlocked:
		return fmt.Sprintf("((%srequest_type = $%d AND NOT %s) OR %srequest_type = %d)", prefix, startArgIndex, legacyImageRequestTypeSQLPredicateWithAlias(alias), prefix, int16(service.RequestTypeCyberBlockedMoved)), []any{requestTypeArg}
	default:
...
func legacyImageRequestTypeSQLPredicate() string {
	return legacyImageRequestTypeSQLPredicateWithAlias("")
}

func legacyImageRequestTypeSQLPredicateWithAlias(alias string) string {
	prefix := ""
	if alias != "" {
		prefix = alias + "."
	}
	return fmt.Sprintf("(%simage_count > 0 OR %simage_output_tokens > 0 OR %sbilling_mode = 'image')", prefix, prefix, prefix)
}
