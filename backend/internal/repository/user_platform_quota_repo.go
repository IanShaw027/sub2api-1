...
//  2. 对每条 record 先尝试 UPDATE active 行；
//     未命中时优先重激活一条 soft-deleted 历史行；仍未命中时 INSERT 新行
//
...
func (r *userPlatformQuotaRepository) UpsertForUser(ctx context.Context, userID int64, records []UserPlatformQuotaRecord) error {
	return r.withTx(ctx, func(txCtx context.Context, txClient *dbent.Client) error {
		...
		for _, rec := range records {
			...
			if affected == 0 {
				affected, err = reactivateSoftDeletedRow(txCtx, txClient, userID, rec, now)
				if err != nil {
					return err
				}
			}
...
// reactivateSoftDeletedRow 重新启用一条最近的 soft-deleted 历史行，保留 usage/window 状态。
// 受影响行数为 0 时表示不存在可复用历史行，调用方再走 insert 路径。
func reactivateSoftDeletedRow(ctx context.Context, client *dbent.Client, userID int64, rec UserPlatformQuotaRecord, now time.Time) (int64, error) {
	const query = `UPDATE user_platform_quotas
		SET daily_limit_usd = $1, weekly_limit_usd = $2, monthly_limit_usd = $3,
		    deleted_at = NULL, updated_at = $4
		WHERE id = (
			...
			WHERE user_id = $5 AND platform = $6 AND deleted_at IS NOT NULL
			ORDER BY deleted_at DESC NULLS LAST, updated_at DESC, id DESC
			LIMIT 1
		)`
	res, err := client.ExecContext(ctx, query,
		rec.DailyLimitUSD, rec.WeeklyLimitUSD, rec.MonthlyLimitUSD, now,
