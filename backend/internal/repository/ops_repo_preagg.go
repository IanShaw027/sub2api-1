...
error_agg AS (
  SELECT
    ...
    COUNT(*) FILTER (WHERE COALESCE(client_status_code, 0) >= 400 AND (is_business_limited OR COALESCE(effective_status_code, 0) IN (429, 529))) AS business_limited_count,
    COUNT(*) FILTER (WHERE COALESCE(client_status_code, 0) >= 400 AND NOT (is_business_limited OR COALESCE(effective_status_code, 0) IN (429, 529))) AS error_count_sla,
    ...
    COUNT(*) FILTER (WHERE COALESCE(client_status_code, 0) >= 400 AND error_owner = 'provider' AND NOT is_business_limited AND COALESCE(effective_status_code, 0) = 429) AS upstream_429_count,
    COUNT(*) FILTER (WHERE COALESCE(client_status_code, 0) >= 400 AND error_owner = 'provider' AND NOT is_business_limited AND COALESCE(effective_status_code, 0) = 529) AS upstream_529_count
  FROM error_base
  GROUP BY GROUPING SETS (
    (bucket_start),
    (bucket_start, platform),
