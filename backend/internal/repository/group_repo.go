...
func (r *groupRepository) Create(ctx context.Context, groupIn *service.Group) error {
	builder := r.client.Group.Create().
		...
		SetRefundRateMultiplier(groupIn.RefundRateMultiplier).
		...
		SetImageGenerationRoute(groupIn.EffectiveImageGenerationRoute()).
...
func (r *groupRepository) listWithAccountCountSort(ctx context.Context, q *dbent.GroupQuery, params pagination.PaginationParams, total int) ([]service.Group, *pagination.PaginationResult, error) {
	...
	var openAIImageMainModels map[int64]string
	if models, loadErr := loadGroupOpenAIImageMainModels(ctx, r.sql, groupIDs); loadErr == nil {
		openAIImageMainModels = models
	}
	...
	if len(openAIImageMainModels) > 0 {
		applyOpenAIImageMainModelsToGroups(outGroups, openAIImageMainModels)
	}
