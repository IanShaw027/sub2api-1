...
func (r *scheduledTestPlanRepository) Disable(ctx context.Context, id int64) error {
	_, err := r.db.ExecContext(ctx, `UPDATE scheduled_test_plans SET enabled = false, updated_at = NOW() WHERE id = $1`, id)
	return err
}
