	...
	query := `
		WITH hourly AS (
			...
			COALESCE(SUM(cache_read_tokens), 0) AS cache_read_tokens,
				...
				COALESCE(SUM(actual_cost) FILTER (WHERE billing_type = $4), 0) AS balance_actual_cost,
				COALESCE(SUM(actual_cost) FILTER (WHERE billing_type = $5), 0) AS subscription_actual_cost,
		...
		SELECT
			...
			hourly.actual_cost,
			hourly.account_cost,
			hourly.balance_actual_cost,
			hourly.subscription_actual_cost,
		...
		DO UPDATE SET
			...
			balance_actual_cost = EXCLUDED.balance_actual_cost,
			subscription_actual_cost = EXCLUDED.subscription_actual_cost,
			total_duration_ms = EXCLUDED.total_duration_ms,
			active_users = EXCLUDED.active_users,
			computed_at = EXCLUDED.computed_at
	`
	_, err := r.sql.ExecContext(ctx, query, start, end, tzName, service.BillingTypeBalance, service.BillingTypeSubscription)
	return err
}
	...
	query := `
		WITH daily AS (
			SELECT
				...
				COALESCE(SUM(balance_actual_cost), 0) AS balance_actual_cost,
				COALESCE(SUM(subscription_actual_cost), 0) AS subscription_actual_cost,
		...
		SELECT
			...
			daily.actual_cost,
			daily.account_cost,
			daily.balance_actual_cost,
			daily.subscription_actual_cost,
