	...
	m, err := r.activeQuery().
		...
		WithGroup(func(q *dbent.GroupQuery) {
			q.Select(
				...
				group.FieldVideoGenerationRoute,
				group.FieldVideoPrice480pPerSec,
				group.FieldVideoPrice720pPerSec,
				group.FieldVideoPrice1080pPerSec,
...
func groupEntityToService(g *dbent.Group) *service.Group {
	...
	out := &service.Group{
		...
		ImageGenerationRoute: func() string {
			if g.Platform == service.PlatformGrok && strings.EqualFold(strings.TrimSpace(g.ImageGenerationRoute), "native") {
				return "native"
			}
			return service.NormalizeGroupImageGenerationRoute(g.ImageGenerationRoute)
		}(),
		...
		VideoGenerationRoute:            service.NormalizeGroupVideoGenerationRoute(g.VideoGenerationRoute),
