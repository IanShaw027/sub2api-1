...
func TestBuildOpsSystemLogsCleanupWhere_EscapesQueryWildcards(t *testing.T) {
	filter := &service.OpsSystemLogCleanupFilter{Query: "%"}
	...
	if got, want := args[0], `%\%%`; got != want {
		t.Fatalf("query arg = %#v, want %#v; where=%s", got, want, where)
	}
}
