...
func TestGatewayCacheCyberBlockWritesScopeAndExactKeyTogether(t *testing.T) {
	...
	matched, err = store.FindCyberSessionBlocked(ctx, []string{"block-2"})
	require.NoError(t, err)
	require.Equal(t, "block-2", matched)
	require.Greater(t, server.TTL(cyberSessionScopePrefix+"scope-1"), time.Duration(0))
	require.Greater(t, server.TTL(cyberSessionBlockPrefix+"block-1"), time.Duration(0))
