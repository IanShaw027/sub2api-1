...
SELECT
  ...
  COALESCE(COUNT(*) FILTER (WHERE COALESCE(status_code, 0) >= 400 AND (is_business_limited OR COALESCE(upstream_status_code, status_code, 0) IN (429, 529))), 0) AS business_limited,
  COALESCE(COUNT(*) FILTER (WHERE COALESCE(status_code, 0) >= 400 AND NOT (is_business_limited OR COALESCE(upstream_status_code, status_code, 0) IN (429, 529))), 0) AS error_sla,
  ...
  COALESCE(COUNT(*) FILTER (WHERE COALESCE(status_code, 0) >= 400 AND error_owner = 'provider' AND NOT is_business_limited AND COALESCE(upstream_status_code, status_code, 0) = 429), 0) AS upstream_429,
  COALESCE(COUNT(*) FILTER (WHERE COALESCE(status_code, 0) >= 400 AND error_owner = 'provider' AND NOT is_business_limited AND COALESCE(upstream_status_code, status_code, 0) = 529), 0) AS upstream_529
FROM ops_error_logs
` + where
