...
func (r *opsRepository) GetThroughputTrend(ctx context.Context, filter *service.OpsDashboardFilter, bucketSeconds int) (*service.OpsThroughputTrendResponse, error) {
	...
	stickyWhere, stickyArgs, _ := buildStickyScheduleWhere(filter, start, end, next+len(errorArgs))

	usageBucketExpr := opsBucketExprForUsage(bucketSeconds)
	errorBucketExpr := opsBucketExprForError(bucketSeconds)
	stickyBucketExpr := opsBucketExprForStickySchedule(bucketSeconds)
...
sticky_buckets AS (
  SELECT ` + stickyBucketExpr + ` AS bucket,
...
func opsBucketExprForStickySchedule(bucketSeconds int) string {
...
SELECT
  ...
  COUNT(*) FILTER (WHERE COALESCE(status_code, 0) >= 400 AND (is_business_limited OR COALESCE(upstream_status_code, status_code, 0) IN (429, 529))) AS business_limited,
...
func (r *opsRepository) GetErrorDistribution(ctx context.Context, filter *service.OpsDashboardFilter) (*service.OpsErrorDistributionResponse, error) {
	...
	where, args, _ := buildErrorWhere(filter, start, end, 1)
	excludedExpr := "(is_business_limited OR COALESCE(upstream_status_code, status_code, 0) IN (429, 529))"

	q := `
SELECT
  COALESCE(upstream_status_code, status_code, 0) AS status_code,
  COUNT(*) AS total,
  COUNT(*) FILTER (WHERE NOT ` + excludedExpr + `) AS sla,
  COUNT(*) FILTER (WHERE ` + excludedExpr + `) AS business_limited
FROM ops_error_logs
` + where + `
  AND COALESCE(status_code, 0) >= 400
GROUP BY 1
...
LIMIT 20`
	...
	ownerQuery := `
SELECT
  COALESCE(NULLIF(TRIM(error_owner), ''), 'platform') AS owner,
  COUNT(*) AS total,
  COUNT(*) FILTER (WHERE NOT ` + excludedExpr + `) AS sla,
  COUNT(*) FILTER (WHERE ` + excludedExpr + `) AS business_limited
FROM ops_error_logs
...
ORDER BY total DESC`
	...
	ownerRows, err := r.db.QueryContext(ctx, ownerQuery, args...)
	if err != nil {
		return nil, err
	}
	defer func() { _ = ownerRows.Close() }()

	owners := make([]*service.OpsErrorOwnerDistributionItem, 0, 8)
	for ownerRows.Next() {
		var owner string
		var cntTotal, cntSLA, cntBiz int64
		if err := ownerRows.Scan(&owner, &cntTotal, &cntSLA, &cntBiz); err != nil {
			return nil, err
		}
		owners = append(owners, &service.OpsErrorOwnerDistributionItem{
			Owner:           owner,
			Total:           cntTotal,
			SLA:             cntSLA,
			BusinessLimited: cntBiz,
