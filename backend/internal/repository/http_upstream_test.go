...
func TestHTTPUpstreamDoAppliesGrokCLIIdentityBeforeOAuthRoundTrip(t *testing.T) {
	...
	for _, endpoint := range []string{"responses", "chat/completions"} {
		t.Run(endpoint, func(t *testing.T) {
			...
			require.Equal(t, "legacy-client/1.0", capturedHeaders.Get("User-Agent"))
		})
	}
}
...
func TestApplyGrokCLIProxyHeaders(t *testing.T) {
	...
	t.Run("accepts a valid operator override", func(t *testing.T) {
		t.Setenv("XAI_GROK_CLI_VERSION", "1.0.5-alpha.1")
		req, err := http.NewRequest(http.MethodPost, "https://cli-chat-proxy.grok.com/v1/chat/completions", nil)
		require.NoError(t, err)

		applyGrokCLIProxyHeaders(req)

		require.Equal(t, "1.0.5-alpha.1", req.Header.Get("x-grok-client-version"))
		require.Equal(t, xai.CLIUserAgent("1.0.5-alpha.1"), req.Header.Get("User-Agent"))
	})
	...
	t.Run("rejects a prerelease override below the pinned version", func(t *testing.T) {
		t.Setenv("XAI_GROK_CLI_VERSION", "1.0.4-beta.1")
		req, err := http.NewRequest(http.MethodPost, "https://cli-chat-proxy.grok.com/v1/responses", nil)
		require.NoError(t, err)
	...
	t.Run("does not overwrite already-stamped profile identity", func(t *testing.T) {
		...
		req, err := http.NewRequest(http.MethodPost, "https://cli-chat-proxy.grok.com/v1/responses", nil)
		require.NoError(t, err)
		req.Header.Set("User-Agent", "xai-grok-workspace/0.2.200")
		req.Header.Set("x-grok-client-version", "0.2.200")
		req.Header.Set("x-grok-client-identifier", "grok-shell-learned")
