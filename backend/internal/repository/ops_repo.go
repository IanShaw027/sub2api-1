...
func (r *opsRepository) InsertRetryAttempt(ctx context.Context, input *service.OpsInsertRetryAttemptInput) (int64, error) {
	...
	if mode == "" {
		return 0, fmt.Errorf("invalid retry mode")
	}
...
RETURNING id
`, input.RequestedByUserID, input.SourceErrorID, mode, nullInt64(input.PinnedAccountID), status, startedAt.UTC()).Scan(&id)
	if err != nil {
		return 0, err
	}
	return id, nil
...
func (r *opsRepository) UpdateRetryAttempt(ctx context.Context, input *service.OpsUpdateRetryAttemptInput) error {
	...
	if input.ID <= 0 {
		return fmt.Errorf("invalid retry attempt id")
	}
	...
	if !input.FinishedAt.IsZero() {
		finishedAt = sql.NullTime{Time: input.FinishedAt.UTC(), Valid: true}
	}
	success := sql.NullBool{}
	if input.Success != nil {
		success = sql.NullBool{Bool: *input.Success, Valid: true}
	}
	httpStatus := sql.NullInt64{}
	if input.HTTPStatusCode != nil {
		httpStatus = sql.NullInt64{Int64: int64(*input.HTTPStatusCode), Valid: true}
	}
	upstreamRequestID := nullString(input.UpstreamRequestID)
	usedAccountID := nullInt64(input.UsedAccountID)
	responsePreview := nullString(input.ResponsePreview)
	responseTruncated := false
	if input.ResponseTruncated != nil {
		responseTruncated = *input.ResponseTruncated
	}
	resultRequestID := nullString(input.ResultRequestID)
	resultErrorID := nullInt64(input.ResultErrorID)
	errorMessage := nullString(input.ErrorMessage)
...
SET
  ...
  error_message = $13
WHERE id = $1
`, input.ID, status, finishedAt, input.DurationMs, success, httpStatus, upstreamRequestID, usedAccountID, responsePreview, responseTruncated, resultRequestID, resultErrorID, errorMessage)
	return err
}
...
func (r *opsRepository) GetLatestRetryAttemptForError(ctx context.Context, sourceErrorID int64) (*service.OpsRetryAttempt, error) {
	...
	if sourceErrorID <= 0 {
		return nil, fmt.Errorf("invalid source error id")
	}
...
`, sourceErrorID)
	...
	return scanOpsRetryAttempt(row.Scan)
}
...
`, sourceErrorID, limit)
	...
	out := make([]*service.OpsRetryAttempt, 0, limit)
	for rows.Next() {
		item, scanErr := scanOpsRetryAttempt(rows.Scan)
		if scanErr != nil {
			return nil, scanErr
		}
		out = append(out, item)
...
func scanOpsRetryAttempt(scan func(dest ...any) error) (*service.OpsRetryAttempt, error) {
	var item service.OpsRetryAttempt
	var requestedByUserID sql.NullInt64
	var sourceErrorID sql.NullInt64
	...
	if requestedByUserID.Valid {
		item.RequestedByUserID = requestedByUserID.Int64
	}
	if sourceErrorID.Valid {
		item.SourceErrorID = sourceErrorID.Int64
	}
	...
	if upstreamRequestID.Valid {
		v := strings.TrimSpace(upstreamRequestID.String)
		if v != "" {
			item.UpstreamRequestID = &v
		}
	}
	...
	if responsePreview.Valid {
		v := strings.TrimSpace(responsePreview.String)
		if v != "" {
			item.ResponsePreview = &v
		}
	}
	...
	if resultRequestID.Valid {
		v := strings.TrimSpace(resultRequestID.String)
		if v != "" {
			item.ResultRequestID = &v
		}
	}
	...
	if errorMessage.Valid {
		v := strings.TrimSpace(errorMessage.String)
		if v != "" {
			item.ErrorMessage = &v
		}
	}
...
func buildOpsErrorLogsWhere(filter *service.OpsErrorLogFilter) (string, []any) {
	...
	effectiveStatusExpr := "COALESCE(e.upstream_status_code, e.status_code, 0)"
	excludedExpr := "(COALESCE(e.is_business_limited,false) = true OR " + effectiveStatusExpr + " IN (429, 529))"
	...
	// Excluded = business-limited errors + normal rate limits (429/529).
	view := ""
	if filter != nil {
	...
	switch view {
	case "", "errors":
		clauses = append(clauses, "NOT "+excludedExpr)
	case "excluded":
		clauses = append(clauses, excludedExpr)
	case "all":
		// no-op
	default:
	...
	if len(filter.StatusCodes) > 0 {
		...
		clauses = append(clauses, effectiveStatusExpr+" = ANY($"+itoa(len(args))+")")
		...
		clauses = append(clauses, "NOT ("+effectiveStatusExpr+" = ANY($"+itoa(len(args))+"))")
	}
...
func buildOpsSystemLogsCleanupWhere(filter *service.OpsSystemLogCleanupFilter) (string, []any, bool) {
	...
	listFilter := &service.OpsSystemLogFilter{
		...
		Model:           filter.Model,
		Query:           escapeLikePattern(filter.Query),
	}
