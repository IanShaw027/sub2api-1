...
func (s *FrontendServer) serveIndexHTML(c *gin.Context) {
	...
	cached := s.cache.Get()
	if cached != nil {
		// Never 304 index.html: the body embeds a per-request CSP nonce, but
		// SecurityHeaders issues a new nonce on every response. A 304 would
		// reuse HTML whose nonce no longer matches the CSP header and the
		// browser would block window.__APP_CONFIG__.
