...
func HasToolContinuationOutputInRawPayload(payload []byte) bool {
	if len(payload) == 0 {
		return false
	}
	...
	if input.IsArray() {
		for _, item := range input.Array() {
			if rawInputItemHasToolContinuationOutput(item) {
				return true
			}
		}
		return false
	...
	if input.Type == gjson.JSON {
		return rawInputItemHasToolContinuationOutput(input)
	}
	return false
}

func rawInputItemHasToolContinuationOutput(item gjson.Result) bool {
...
// HasFunctionCallOutput 保留旧命名，但实际覆盖所有 tool continuation
// output item 类型，用于触发续链校验。
...
// FunctionCallOutputCallIDs 提取 input 中 tool continuation output 的 call_id 集合。
...
// HasFunctionCallOutputMissingCallID 判断是否存在缺少 call_id 的 tool
// continuation output。
