...
func (s *OpenAIGatewayService) buildUpstreamRequestOpenAIPassthrough(
	...
	if account.UsesOpenAICodexProtocol() || account.Type == AccountTypeOAuth {
...
func shouldFailoverOpenAIPassthroughResponse(account *Account, statusCode int, responseBody []byte) bool {
	...
	if isOpenAIAccessStateFailover(statusCode, "", responseBody) {
...
func (s *OpenAIGatewayService) handleFailoverErrorResponsePassthrough(
	...
	failoverErr := s.newOpenAIAccountFailoverError(
		...
		upstreamMsg,
		shouldDisable,
		s.shouldRetryOpenAIOAuth429OnSameAccountWithResponse(account, resp.StatusCode, shouldDisable, resp.Header, body) || (!shouldDisable && account.IsPoolMode() && account.IsPoolModeRetryableStatus(resp.StatusCode)),
	)
...
func openAIStreamFailedEventSemanticStatus(payload []byte, message string) int {
	...
	case isOpenAIAccessStateFailover(0, message, payload):
		return http.StatusForbidden
...
func openAIStream403AccountFailure(payload []byte, message string) bool {
	return isOpenAIAccessStateFailover(0, message, payload) || openAIStreamCredentialAuthFailure(payload)
}
...
func openAIStreamFailedEventShouldFailover(payload []byte, message string) bool {
	...
	if isOpenAIAccessStateFailover(0, message, payload) {
		return true
	}
