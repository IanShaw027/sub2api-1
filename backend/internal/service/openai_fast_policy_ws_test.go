...
func TestWSResponseCreate_PassNormalizesFastAliasToPriority(t *testing.T) {
	settings := &OpenAIFastPolicySettings{
		Rules: []OpenAIFastPolicyRule{{
	...
	frame := []byte(`{"type":"response.create","model":"gpt-4.1","service_tier":"fast","input":[{"type":"input_text","text":"hi"}]}`)
	updated, blocked, err := svc.applyOpenAIFastPolicyToWSResponseCreate(context.Background(), account, "gpt-4.1", frame)
	require.NoError(t, err)
	require.Nil(t, blocked)
	require.Equal(t, "priority", gjson.GetBytes(updated, "service_tier").String(), "ws helper should normalize fast alias on pass just like HTTP")
}
