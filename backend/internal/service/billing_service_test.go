...
func TestGetModelPricing_CodexAutoReviewAliasVariantsFallBackToGPT53CodexPricing(t *testing.T) {
	svc := newTestBillingService()

	for _, model := range []string{"openai/codex-auto-review", "codex-auto-review-openai-compact"} {
...
func TestGetModelPricingWithChannel_DoesNotMutateFallbackPricing(t *testing.T) {
	...
	before, err := svc.GetModelPricing("gpt-5.2")
	require.NoError(t, err)
	require.InDelta(t, 1.75e-6, before.InputPricePerToken, 1e-12)
	require.InDelta(t, 3.5e-6, before.InputPricePerTokenPriority, 1e-12)
	require.InDelta(t, 14e-6, before.OutputPricePerToken, 1e-12)
	require.InDelta(t, 28e-6, before.OutputPricePerTokenPriority, 1e-12)

	channelPricing := &ChannelModelPricing{
		InputPrice:       testPtrFloat64(99e-6),
		OutputPrice:      testPtrFloat64(88e-6),
		CacheReadPrice:   testPtrFloat64(0.9e-6),
		ImageOutputPrice: testPtrFloat64(77e-6),
	}
	overridden, err := svc.GetModelPricingWithChannel("gpt-5.2", channelPricing)
	require.NoError(t, err)
	require.InDelta(t, 99e-6, overridden.InputPricePerToken, 1e-12)
	require.Zero(t, overridden.InputPricePerTokenPriority)

	after, err := svc.GetModelPricing("gpt-5.2")
	require.NoError(t, err)
	require.InDelta(t, 1.75e-6, after.InputPricePerToken, 1e-12)
	require.InDelta(t, 3.5e-6, after.InputPricePerTokenPriority, 1e-12)
	require.InDelta(t, 14e-6, after.OutputPricePerToken, 1e-12)
	require.InDelta(t, 28e-6, after.OutputPricePerTokenPriority, 1e-12)
	require.False(t, after.ImageOutputPriceExplicit)
}
...
func TestGetModelPricingWithChannel_OverrideInputPriceOnly(t *testing.T) {
	...
	// Channel pricing expresses the normal resolved unit price. Priority
	// continues to use service-tier multiplier until explicit channel priority
	// fields exist.
...
func TestGetModelPricingWithChannel_CacheReadPriceDoesNotImplyPriorityOverride(t *testing.T) {
	svc := newTestBillingService()
	...
	// CacheReadPrice sets only the normal channel unit price; priority uses the
	// service-tier multiplier rather than pretending an explicit priority price
	// was configured.
