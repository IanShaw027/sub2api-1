...
func TestOpenAIGatewayServiceForwardImages_ImageRateLimitReturnsFailoverAndCoolsCapability(t *testing.T) {
	...
	require.Empty(t, repo.modelRateLimitCalls, "image route 429 should not mark the old model-rate-limit scope")
	require.Len(t, repo.extraUpdateCalls, 1)
	require.Equal(t, account.ID, repo.extraUpdateCalls[0].accountID)
	require.Contains(t, repo.extraUpdateCalls[0].updates, "openai_image_codex_rate_limit_reset_at")
	require.Contains(t, repo.extraUpdateCalls[0].updates, "openai_image_codex_rate_limited_at")
	require.Contains(t, repo.extraUpdateCalls[0].updates, "openai_image_codex_reset_after_seconds")
	require.Contains(t, repo.extraUpdateCalls[0].updates, "openai_image_web2api_rate_limit_reset_at")
	require.Contains(t, account.Extra, "openai_image_codex_rate_limit_reset_at")
}
