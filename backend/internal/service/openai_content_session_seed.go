...
// deriveOpenAIAnchoredContentSessionSeed returns the content-derived seed only
// when it contains a meaningful user/input anchor. This prevents model-only
// requests from becoming a tenant-wide cache routing identity.
...
// deriveOpenAIStablePrefixSessionSeed hashes only the reusable request prefix.
// User and assistant content are excluded, while the returned value remains
// opaque so prompts never leak through routing identifiers or logs.
func deriveOpenAIStablePrefixSessionSeed(body []byte) string {
	...
	digest := sha256.Sum256([]byte(b.String()))
	return contentStablePrefixSessionSeedPrefix + hex.EncodeToString(digest[:16])
}
