	...
	case RedeemTypeSubscription:
		...
		if s.subscriptionService != nil {
			if err := s.subscriptionService.invalidateSubscriptionCaches(userID, groupID); err != nil {
				logger.LegacyPrintf("service.redeem", "[Redeem] invalidate subscription caches failed for user %d group %d: %v", userID, groupID, err)
			}
			return
		}
		if s.billingCacheService == nil {
		...
		go func() {
			...
			if err := s.billingCacheService.InvalidateSubscription(cacheCtx, userID, groupID); err != nil {
				logger.LegacyPrintf("service.redeem", "[Redeem] invalidate billing subscription cache failed for user %d group %d: %v", userID, groupID, err)
			}
			if err := s.billingCacheService.PublishSubscriptionCacheInvalidation(cacheCtx, subCacheKey(userID, groupID)); err != nil {
				logger.LegacyPrintf("service.redeem", "[Redeem] publish subscription cache invalidation failed for user %d group %d: %v", userID, groupID, err)
			}
		}()
	}
}
...
func (s *RedeemService) GetStats(ctx context.Context) (map[string]any, error) {
	repoStats, err := s.redeemRepo.GetStats(ctx)
	if err != nil {
		return nil, err
	}
	if repoStats == nil {
		repoStats = &RedeemCodeStats{}
	}
	byType := repoStats.ByType
	if byType == nil {
		byType = map[string]int64{}
	}
	stats := map[string]any{
		"total_codes":             repoStats.TotalCodes,
		"unused_codes":            repoStats.UnusedCodes,
		"used_codes":              repoStats.UsedCodes,
		"total_value":             repoStats.TotalValue,
		"active_codes":            repoStats.UnusedCodes,
		"expired_codes":           repoStats.ExpiredCodes,
		"total_value_distributed": repoStats.TotalValueDistributed,
		"by_type":                 byType,
	}
...
func (s *RedeemService) GetUserHistoryPaginated(ctx context.Context, userID int64, page, pageSize int, codeType string) ([]RedeemCode, int64, float64, error) {
	params := pagination.PaginationParams{Page: page, PageSize: pageSize}
	codes, result, err := s.redeemRepo.ListByUserPaginated(ctx, userID, params, codeType)
	if err != nil {
		return nil, 0, 0, fmt.Errorf("get user redeem history paginated: %w", err)
	}
	totalRecharged, err := s.redeemRepo.SumPositiveBalanceByUser(ctx, userID)
	if err != nil {
		return nil, 0, 0, fmt.Errorf("sum user redeem history recharged total: %w", err)
	}
