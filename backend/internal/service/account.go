...
// UsesOpenAICodexProtocol reports ChatGPT/Codex inference credentials.
// Implicit-platform OAuth is treated as OpenAI (legacy accounts). Setup tokens
// share the forwarding contract only on OpenAI. Grok/Kiro/Anthropic OAuth must
// not inherit Codex identity or routing.
func (a *Account) UsesOpenAICodexProtocol() bool {
	if a == nil {
		return false
	}
	if a.Type == AccountTypeOAuth && (a.Platform == "" || a.IsOpenAI()) {
		return true
	}
...
// 全平台可用：anthropic / openai / gemini / antigravity / grok / kiro。
func (a *Account) IsTLSFingerprintEnabled() bool {
	if a == nil || !supportsAccountTLSFingerprint(a.Platform) {
		return false
	}
