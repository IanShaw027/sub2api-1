...
func (a *Account) GeminiOAuthType() string {
	...
	return strings.TrimSpace(a.GetCredential("oauth_type"))
}
...
// conservative tier-based inference when oauth_type is missing.
// It intentionally does not infer from project_id alone.
func (a *Account) GeminiOAuthTypeSafe() string {
	if a.Platform != PlatformGemini || a.Type != AccountTypeOAuth {
		return ""
	}
	...
	rawTier := a.GeminiTierID()
	switch {
	case canonicalGeminiTierIDForOAuthType("google_one", rawTier) != "":
		return "google_one"
	case canonicalGeminiTierIDForOAuthType("code_assist", rawTier) != "":
		return "code_assist"
	default:
		return ""
	}
