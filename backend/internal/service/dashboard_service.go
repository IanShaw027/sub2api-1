...
type DashboardService struct {
	...
	groupUsage     *dashboardGroupUsageSummaryCacheEntry
	aggEnabled     bool
...
func (s *DashboardService) GetDashboardStatsWithRange(ctx context.Context, start, end time.Time) (*usagestats.DashboardStats, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("get dashboard stats with range fallback: %w", err)
	}
	s.applyAggregationStatus(ctx, stats)
	return stats, nil
}

func (s *DashboardService) GetUsageTrendWithFilters(ctx context.Context, startTime, endTime time.Time, granularity string, userID, apiKeyID, accountID, groupID int64, model string, requestType *int16, stream *bool, billingType *int8, billingMode string) ([]usagestats.TrendDataPoint, error) {
	trend, err := s.usageRepo.GetUsageTrendWithFilters(ctx, startTime, endTime, granularity, userID, apiKeyID, accountID, groupID, model, requestType, stream, billingType, billingMode)
	if err != nil {
...
func (s *DashboardService) GetModelStatsWithFilters(ctx context.Context, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID int64, requestType *int16, stream *bool, billingType *int8, billingMode string) ([]usagestats.ModelStat, error) {
	stats, err := s.usageRepo.GetModelStatsWithFilters(ctx, startTime, endTime, userID, apiKeyID, accountID, groupID, requestType, stream, billingType, billingMode)
	if err != nil {
...
func (s *DashboardService) GetModelStatsWithFiltersBySource(ctx context.Context, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID int64, requestType *int16, stream *bool, billingType *int8, billingMode string, modelSource string) ([]usagestats.ModelStat, error) {
	normalizedSource := usagestats.NormalizeModelSource(modelSource)
	if normalizedSource == usagestats.ModelSourceRequested {
		return s.GetModelStatsWithFilters(ctx, startTime, endTime, userID, apiKeyID, accountID, groupID, requestType, stream, billingType, billingMode)
	}

	type modelStatsBySourceRepo interface {
		GetModelStatsWithFiltersBySource(ctx context.Context, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID int64, requestType *int16, stream *bool, billingType *int8, billingMode string, source string) ([]usagestats.ModelStat, error)
	}

	if sourceRepo, ok := s.usageRepo.(modelStatsBySourceRepo); ok {
		stats, err := sourceRepo.GetModelStatsWithFiltersBySource(ctx, startTime, endTime, userID, apiKeyID, accountID, groupID, requestType, stream, billingType, billingMode, normalizedSource)
		if err != nil {
...
func (s *DashboardService) GetGroupStatsWithFilters(ctx context.Context, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID int64, requestType *int16, stream *bool, billingType *int8, billingMode string) ([]usagestats.GroupStat, error) {
	stats, err := s.usageRepo.GetGroupStatsWithFilters(ctx, startTime, endTime, userID, apiKeyID, accountID, groupID, requestType, stream, billingType, billingMode)
	if err != nil {
...
func (s *DashboardService) GetGroupUsageSummary(ctx context.Context, todayStart time.Time) ([]usagestats.GroupUsageSummary, error) {
	if cached, ok := s.getCachedGroupUsageSummary(todayStart); ok {
		return cached, nil
	}
	...
	s.storeCachedGroupUsageSummary(todayStart, results)
...
func (s *DashboardService) getCachedGroupUsageSummary(todayStart time.Time) ([]usagestats.GroupUsageSummary, bool) {
	...
	s.groupUsageMu.Lock()
	defer s.groupUsageMu.Unlock()
	if s.groupUsage == nil {
		return nil, false
	}
	if !s.groupUsage.TodayStart.Equal(todayStart) {
		return nil, false
	}
	if time.Since(s.groupUsage.UpdatedAt) > s.cacheTTL {
		s.groupUsage = nil
		return nil, false
	}
	return cloneGroupUsageSummaries(s.groupUsage.Results), true
}

func (s *DashboardService) storeCachedGroupUsageSummary(todayStart time.Time, results []usagestats.GroupUsageSummary) {
	if s == nil {
		return
	}
	s.groupUsageMu.Lock()
	defer s.groupUsageMu.Unlock()
	s.groupUsage = &dashboardGroupUsageSummaryCacheEntry{
