...
func TestOpenAIAgentIdentityPassthroughKeepsSessionAndPromptCacheHeaders(t *testing.T) {
	...
	require.Equal(t, openaiOutboundSessionID(context.Background(), account, 0, "client-session"), req.Header.Get("session_id"))
	require.Equal(t, openaiOutboundSessionID(context.Background(), account, 0, "client-conversation"), req.Header.Get("conversation_id"))
