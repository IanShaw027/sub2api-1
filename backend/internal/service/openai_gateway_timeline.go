...
func openAIWSDiagnosticStartTimelineFields(groupID, apiKeyID int64, v openAIWSDiagnosticStartLog) map[string]any {
	return map[string]any{
		...
		"delta_candidate":                       v.DeltaCandidate,
		"delta_fallback_reason":                 strings.TrimSpace(v.DeltaFallbackReason),
		"delta_prefix_match":                    v.DeltaPrefixMatch,
		"delta_conn_match":                      v.DeltaConnMatch,
		"delta_most_recent_match":               v.DeltaMostRecentMatch,
		"delta_non_input_match":                 v.DeltaNonInputMatch,
		"delta_raw_client_equiv":                v.DeltaRawClientEquiv,
		"delta_break_boundary":                  strings.TrimSpace(v.DeltaBreakBoundary),
		"delta_break_item_type":                 strings.TrimSpace(v.DeltaBreakItemType),
		"delta_store_fallback_reason":           strings.TrimSpace(v.DeltaStoreFallbackReason),
		"delta_conn_reanchor_blockers":          strings.TrimSpace(v.DeltaConnReanchorBlockers),
		"pool_total":                            v.PoolSnapshot.TotalConns,
		"pool_acquire_state":                    strings.TrimSpace(v.PoolAcquireState),
		"pool_neutral":                          v.PoolSnapshot.NeutralConns,
		"pool_session_bound":                    v.PoolSnapshot.SessionBoundConns,
		"pool_idle_neutral":                     v.PoolSnapshot.IdleNeutralConns,
		"pool_idle_session_bound":               v.PoolSnapshot.IdleSessionBoundConns,
		"pool_neutral_stock":                    v.PoolSnapshot.NeutralStockConns,
		"pool_matching":                         v.PoolSnapshot.MatchingConns,
		"pool_matching_idle":                    v.PoolSnapshot.MatchingIdleConns,
		"pool_leased":                           v.PoolSnapshot.LeasedConns,
		"pool_waiters":                          v.PoolSnapshot.Waiters,
		"pool_creating":                         v.PoolSnapshot.Creating,
		"pool_prewarm_active":                   v.PoolSnapshot.PrewarmActive,
		"pool_prewarm_failures":                 v.PoolSnapshot.PrewarmFailures,
		"pool_neutral_target":                   v.PoolSnapshot.NeutralPrewarmTarget,
		"pool_effective_max":                    v.PoolSnapshot.EffectiveMaxConns,
	}
}
