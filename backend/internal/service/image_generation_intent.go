...
func openAIJSONToolsContainImageGeneration(tools gjson.Result) bool {
	...
	tools.ForEach(func(_, item gjson.Result) bool {
		if isOpenAIImageGenerationType(openAIJSONString(item.Get("type"))) ||
			(openAIJSONString(item.Get("type")) == "namespace" && isOpenAIImageGenNamespaceName(openAIJSONString(item.Get("name")))) {
			found = true
			return false
		}
		return true
...
func openAIRequestBodyHasImageGenerationDeclaration(body []byte) bool {
	...
	return openAIJSONToolsContainImageGeneration(gjson.GetBytes(body, "tools")) ||
		openAIJSONInputContainsImageGenNamespace(gjson.GetBytes(body, "input")) ||
...
func openAIJSONToolsContainImageGenNamespace(tools gjson.Result) bool {
	...
	found := false
	tools.ForEach(func(_, item gjson.Result) bool {
		if openAIJSONString(item.Get("type")) == "namespace" && isOpenAIImageGenNamespaceName(openAIJSONString(item.Get("name"))) {
			found = true
			return false
		}
		return true
...
func openAIJSONToolChoiceSelectsImageGeneration(choice gjson.Result) bool {
	...
	if isOpenAIImageGenerationType(openAIJSONString(choice.Get("tool.type"))) {
		return true
	}
