...
func (s *emailBindCacheStub) GetVerificationCode(_ context.Context, email string) (*service.VerificationCodeData, error) {
	...
	if _, consumed := s.consumed[email]; consumed {
		return nil, nil
	}
	return s.data, nil
}
...
func (s *emailBindCacheStub) VerifyAndConsumeVerificationCode(_ context.Context, email, code string, maxAttempts int) (service.VerificationCodeCheckResult, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.err != nil {
