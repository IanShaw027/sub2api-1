...
func (p *OpenAITokenProvider) GetAccessToken(ctx context.Context, account *Account) (string, error) {
	...
	needsRefresh := !account.IsOpenAIPersonalAccessToken() && (missingAccessToken || expiresAt == nil || time.Until(*expiresAt) <= openAITokenRefreshSkew)
	if needsRefresh && strings.TrimSpace(account.GetOpenAIRefreshToken()) == "" {
		if expiresAt != nil && !time.Now().Before(*expiresAt) {
			p.disableAccountMissingRefreshToken(account, "missing refresh_token on request path")
	...
	if needsRefresh && p.refreshAPI != nil && p.executor != nil {
		...
		p.metrics.refreshRequests.Add(1)
		p.metrics.touchNow()
		leaseToken, locked, lockErr := p.tokenCache.AcquireRefreshLock(ctx, cacheKey, 30*time.Second)
