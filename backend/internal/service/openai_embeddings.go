...
func (s *OpenAIGatewayService) ForwardEmbeddings(
	...
	resp, err := s.doAccountHTTP(ctx, c, account, upstreamReq, proxyURL, "embeddings")
	if err != nil {
	...
	if resp.StatusCode >= 400 {
		...
		if s.shouldFailoverOpenAIUpstreamResponse(resp.StatusCode, upstreamMsg, respBody) {
			...
			shouldDisable := s.handleOpenAIAccountUpstreamError(ctx, account, resp.StatusCode, resp.Header, respBody, canonicalOpenAIAccountSchedulingModel(account, originalModel))
			...
			if isOpenAIAccessStateFailover(resp.StatusCode, upstreamMsg, respBody) {
