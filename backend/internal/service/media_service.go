...
func validMediaBizType(bizType string) bool {
	switch bizType {
	case MediaBizInvoice, MediaBizTicket, MediaBizAvatar, MediaBizImageTask:
		return true
	default:
		return false
	}
...
func clampMediaTTLMinutes(minutes int) int {
	if minutes <= 0 {
	...
	if minutes > MaxMediaTTLMinutes {
		return MaxMediaTTLMinutes
	}
	return minutes
}
...
func (s *MediaService) Upload(ctx context.Context, in UploadMediaInput) (*MediaAsset, error) {
	...
	if bizType == MediaBizInvoice && !in.ActorIsAdmin {
...
func (s *MediaService) CreateDownloadGrant(ctx context.Context, in CreateDownloadGrantInput) (*MediaDownloadGrant, error) {
	...
	ttlMin := clampMediaTTLMinutes(in.TTLMinutes)
	expiresAt := s.now().Add(time.Duration(ttlMin) * time.Minute).Unix()
	sig := s.signDownload(in.AssetID, expiresAt)
	q := url.Values{}
...
func (s *MediaService) OpenSignedDownloadStream(ctx context.Context, in OpenSignedDownloadInput) (*MediaAsset, io.ReadCloser, error) {
	if len(s.signKey) == 0 {
		return nil, nil, infraerrors.InternalServer("MEDIA_SIGNING_KEY_MISSING", "media download signing key is not configured")
	}
	if in.Expires <= 0 || strings.TrimSpace(in.Sig) == "" {
		return nil, nil, ErrMediaDownloadInvalid
	}
	if !s.validDownloadSig(in.AssetID, in.Expires, in.Sig) {
		return nil, nil, ErrMediaDownloadInvalid
	}
...
func (s *MediaService) signDownload(id, expires int64) string {
	mac := hmac.New(sha256.New, s.signKey)
	_, _ = mac.Write([]byte(mediaDownloadPayload(id, expires)))
	return base64.RawURLEncoding.EncodeToString(mac.Sum(nil))
}

func (s *MediaService) validDownloadSig(id, expires int64, sig string) bool {
	expected := s.signDownload(id, expires)
...
func allowedMediaMIME(bizType, mime string) bool {
	...
	case MediaBizAvatar, MediaBizImageTask:
