...
func TestOpenAIUpstreamAccessStateClassification(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			body := []byte(tt.body)
			require.Equal(t, tt.want, isOpenAIHTTPUpstreamAccessStateError(0, "", body))
			if !tt.want {
				return
			}
...
func TestOpenAIUpstreamAccessStateDoesNotScanEchoedJSON(t *testing.T) {
	...
	require.False(t, isOpenAIHTTPUpstreamAccessStateError(0, "", body))
...
func TestOpenAIHTTPAccessStateDoesNotTrustBadRequestMessage(t *testing.T) {
	...
	require.False(t, isOpenAIHTTPUpstreamAccessStateError(http.StatusBadRequest, "", body), "free-form stream messages are not durable account evidence")
...
func TestOpenAIStreamEchoedAccessStateMessageDoesNotDisableOrFailover(t *testing.T) {
	...
	require.False(t, isOpenAIHTTPUpstreamAccessStateError(0, message, payload))
