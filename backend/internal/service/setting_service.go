...
func (s *SettingService) refreshCachedSettings(settings *SystemSettings) {
	...
	gatewayForwardingCache.Store(&cachedGatewayForwardingSettings{
		...
		claudeTelemetryMode:              normalizeClaudeTelemetryMode(settings.ClaudeTelemetryMode),
...
func (s *SettingService) getGatewayForwardingSettingsCached(ctx context.Context) gatewayForwardingSettingsResult {
	...
	if cached, ok := gatewayForwardingCache.Load().(*cachedGatewayForwardingSettings); ok && cached != nil {
		if time.Now().UnixNano() < cached.expiresAt {
			return gatewayForwardingSettingsResult{
				...
				claudeTelemetryMode:              normalizeClaudeTelemetryMode(cached.claudeTelemetryMode),
			}
		}
	}
	val, _, _ := gatewayForwardingSF.Do("gateway_forwarding", func() (any, error) {
		...
		if err != nil {
			...
			return gatewayForwardingSettingsResult{fp: true, claudeOAuthSystemPromptInjection: true, rewriteMessageCacheControl: s.defaultRewriteMessageCacheControl(), clientDatelineNormalization: true, claudeTelemetryMode: ClaudeTelemetryModeDrop}, nil
		}
		fp := true
		...
		claudeTelemetryMode := normalizeClaudeTelemetryMode(values[SettingKeyClaudeTelemetryMode])
		gatewayForwardingCache.Store(&cachedGatewayForwardingSettings{
			...
			claudeTelemetryMode:              claudeTelemetryMode,
			expiresAt:                        time.Now().Add(gatewayForwardingCacheTTL).UnixNano(),
		})
		return gatewayForwardingSettingsResult{
			fp:                               fp,
	...
	return gatewayForwardingSettingsResult{fp: true, claudeOAuthSystemPromptInjection: true, clientDatelineNormalization: true, claudeTelemetryMode: ClaudeTelemetryModeDrop}
}
...
func normalizeClaudeTelemetryMode(mode string) string {
	switch strings.ToLower(strings.TrimSpace(mode)) {
	case ClaudeTelemetryModeForward:
		return ClaudeTelemetryModeForward
	default:
		return ClaudeTelemetryModeDrop
	}
}

// GetClaudeTelemetryMode returns the DB-backed runtime mode for Claude Code telemetry.
func (s *SettingService) GetClaudeTelemetryMode(ctx context.Context) string {
	if s == nil || s.settingRepo == nil {
		return ClaudeTelemetryModeDrop
	}
...
func (s *SettingService) SetOpenAIFastPolicySettings(ctx context.Context, settings *OpenAIFastPolicySettings) error {
	...
	validActions := map[string]bool{
		BetaPolicyActionPass: true, BetaPolicyActionFilter: true, BetaPolicyActionBlock: true, OpenAIFastPolicyActionForcePriority: true,
	}
	validScopes := map[string]bool{
		BetaPolicyScopeAll: true, BetaPolicyScopeOAuth: true, BetaPolicyScopeAPIKey: true, BetaPolicyScopeBedrock: true,
	}
...
// 永远返回包含全部允许 quota platform key 的 map（值可能为零值/nil 字段，表示"上层未配置 = 不限制"）。
//
...
func (s *SettingService) GetDefaultPlatformQuotas(ctx context.Context) (map[string]*DefaultPlatformQuotaSetting, error) {
	...
	return out, nil // 补齐全部允许 quota platform key，保持与旧实现一致的下游契约
}
