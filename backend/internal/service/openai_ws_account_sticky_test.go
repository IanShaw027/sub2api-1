...
func TestOpenAIGatewayService_SelectAccountByPreviousResponseID_QuotaAutoPausedMiss(t *testing.T) {
	...
	require.NoError(t, store.BindResponseAccount(ctx, groupID, 0, "resp_prev_quota", account.ID, time.Hour))

	selection, err := svc.SelectAccountByPreviousResponseID(ctx, &groupID, 0, "resp_prev_quota", "gpt-5.1", nil, false)
	require.NoError(t, err)
	...
	boundAccountID, getErr := store.GetResponseAccount(ctx, groupID, 0, "resp_prev_quota")
...
func TestOpenAIGatewayService_SelectAccountByPreviousResponseID_ForceHTTPDurableStickyEnabled(t *testing.T) {
	ctx := context.Background()
	groupID := int64(23)
	account := Account{
		ID:          12,
	...
	require.NoError(t, store.BindResponseAccount(ctx, groupID, 0, "resp_prev_force_http_durable", account.ID, time.Hour))

	selection, err := svc.SelectAccountByPreviousResponseID(ctx, &groupID, 0, "resp_prev_force_http_durable", "gpt-5.1", nil, false)
	require.NoError(t, err)
	require.NotNil(t, selection)
	require.Equal(t, account.ID, selection.Account.ID)
}
...
func TestOpenAIGatewayService_SelectAccountByPreviousResponseID_CapabilityMismatchKeepsSticky(t *testing.T) {
	...
	require.NoError(t, store.BindResponseAccount(ctx, groupID, 0, "resp_prev_capability", account.ID, time.Hour))
	...
	require.NoError(t, err)
	require.Nil(t, selection)
	boundAccountID, getErr := store.GetResponseAccount(ctx, groupID, 0, "resp_prev_capability")
	require.NoError(t, getErr)
	require.Equal(t, account.ID, boundAccountID)
}

func TestOpenAISelectAccountByPreviousResponseID_CapabilityMismatchKeepsSticky(t *testing.T) {
	TestOpenAIGatewayService_SelectAccountByPreviousResponseID_CapabilityMismatchKeepsSticky(t)
}
