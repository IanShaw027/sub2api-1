...
func ProvideGrokQuotaService(
	...
	svc := NewGrokQuotaService(accountRepo, proxyRepo, tokenProvider, httpUpstream, tlsFPProfileService)
...
func ProvideRateLimitService(
	...
	svc.SetTempUnschedCounterCache(tempUnschedCounter)
...
// auto-pause cache sink without making OpsService depend on SettingService.
...
func ProvideSettingService(settingRepo SettingRepository, groupRepo GroupRepository, proxyRepo ProxyRepository, cfg *config.Config) *SettingService {
	...
	if err := svc.LoadOpenAIWSPoolRuntimeSettings(context.Background()); err != nil {
		logger.LegacyPrintf("service.setting", "Warning: load openai ws pool runtime settings failed: %v", err)
	}
...
func ProvideBillingCacheService(
	...
	svc.SetBalanceCacheOutboxRepository(balanceOutboxRepo)
	return svc
}
