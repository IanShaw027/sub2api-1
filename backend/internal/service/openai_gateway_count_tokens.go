...
func (s *OpenAIGatewayService) ForwardCountTokensAsAnthropic(
	...
	if account.Type == AccountTypeAPIKey && !account.SupportsOpenAIEndpointCapability(OpenAIEndpointCapabilityResponsesInputTokens) {
		writeAnthropicCountTokensError(c, http.StatusNotFound, "not_found_error", "Token counting is not supported by this OpenAI account")
		return nil
	}
