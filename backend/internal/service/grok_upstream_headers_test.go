...
func TestResolveGrokUpstreamUserAgentNeverPassthrough(t *testing.T) {
	...
	require.Equal(t, xai.CLIUserAgent(xai.CLIStableVersion), resolveGrokUpstreamUserAgent(c))
	require.Equal(t, xai.CLIUserAgent(xai.CLIStableVersion), resolveGrokUpstreamUserAgent(nil))
}
...
func TestApplyGrokTLSProfileHeadersIgnoresProfileUserAgent(t *testing.T) {
	t.Setenv(xai.CLIVersionEnv, "")

	req, err := http.NewRequest(http.MethodPost, "https://api.x.ai/v1/responses", nil)
	require.NoError(t, err)

	applyGrokTLSProfileHeaders(req, &tlsfingerprint.Profile{
		UserAgent:  "grok-native/1.0",
		Originator: "grok_desktop",
	})

	require.Equal(t, xai.CLIUserAgent(xai.CLIStableVersion), req.Header.Get("User-Agent"))
	require.Equal(t, "grok_desktop", req.Header.Get("Originator"))
}
