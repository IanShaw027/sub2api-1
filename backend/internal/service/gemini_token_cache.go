...
type GeminiTokenCache interface {
	...
	AcquireRefreshLock(ctx context.Context, cacheKey string, ttl time.Duration) (string, bool, error)
	ReleaseRefreshLock(ctx context.Context, cacheKey string, leaseToken string) error
}
