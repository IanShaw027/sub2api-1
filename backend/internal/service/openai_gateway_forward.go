...
func (s *OpenAIGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*OpenAIForwardResult, error) {
	...
	billingModel, upstreamModel := resolveOpenAIForwardMappedModels(account, clientModel, isCompactRequest)
	...
	if isCompactRequest && !compactApplied {
		if compactModel := s.resolveOpenAICompactFallbackModel(account, clientModel); compactModel != "" {
			upstreamModel = compactModel
			compactApplied = compactModel != clientModel
		}
	}
	...
	if account.UsesOpenAICodexProtocol() {
		...
		// 先清空上一 attempt 的 staged IDs，再写入本账号结果（含 nil），避免
		// failover 从收敛账号切到 off 账号时残留上一账号身份。
	...
	for {
		...
		if resp.StatusCode >= 400 {
			...
			if retryBody, fallbackModel, retry := s.prepareOpenAICompactFallbackRetry(
				c, account, clientModel, body, resp.StatusCode, upstreamMsg, respBody, compactModelFallbackRetried,
			); retry {
			...
			if s.shouldFailoverOpenAIUpstreamResponse(resp.StatusCode, upstreamMsg, respBody) {
				...
				failoverErr := s.newOpenAIAccountFailoverError(
					...
					upstreamMsg,
					shouldDisable,
					s.shouldRetryOpenAIOAuth429OnSameAccountWithResponse(account, resp.StatusCode, shouldDisable, resp.Header, respBody) || (!shouldDisable && account.IsPoolMode() && (account.IsPoolModeRetryableStatus(resp.StatusCode) || isOpenAITransientProcessingError(resp.StatusCode, upstreamMsg, respBody))),
				)
		...
		if reqStream {
			...
			if err != nil {
				if signal, ok := asOpenAICompactFallbackSignal(err); ok {
					if retryBody, fallbackModel, retry := s.prepareOpenAICompactFallbackRetry(
						c, account, clientModel, body, http.StatusBadRequest, signal.message, signal.payload, compactModelFallbackRetried,
					); retry {
...
func (s *OpenAIGatewayService) buildUpstreamRequest(ctx context.Context, c *gin.Context, account *Account, body []byte, token string, isStream bool, promptCacheKey string, isCodexCLI bool) (*http.Request, error) {
	...
	if account.UsesOpenAICodexProtocol() {
		...
		if isOpenAIResponsesCompactPath(c) {
			...
			req.Header.Set("session_id", openaiOutboundSessionIDFromProfileAccount(profile, apiKeyID, codexAccountIdentitySource(c, account), compactSession))
		} else {
			req.Header.Set("accept", "text/event-stream")
		}
		if promptCacheKey != "" {
			isolated := openaiOutboundSessionIDFromProfileAccount(profile, apiKeyID, codexAccountIdentitySource(c, account), promptCacheKey)
