...
// resolveOpenAIForwardMappedModels is the account/model/requireCompact chain
// used by Forward. billingModel is the ordinary account mapping used for
// usage billing. upstreamModel is the model actually sent, and must match
// resolveOpenAIAccountUpstreamModelForRequest so scheduler, channel
// restriction, and Forward cannot disagree.
...
func (s *OpenAIGatewayService) selectAccountWithLoadAwareness(ctx context.Context, groupID *int64, platform string, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, requiredCapability OpenAIEndpointCapability, useUpstreamTokenCost bool) (*AccountSelectionResult, error) {
	...
	if s.concurrencyService == nil || !cfg.LoadBatchEnabled {
		...
		result, err := s.tryAcquireAccountSlot(ctx, account.ID, groupID, account.EffectiveConcurrency())
			...
			if err == nil {
				...
				if !clearSticky && isOpenAICompatibleAccountEligibleForRequest(ctx, account, platform, requestedModel, false, requiredCapability) {
					...
					if account == nil {
						...
						result, err := s.tryAcquireAccountSlot(ctx, accountID, groupID, account.EffectiveConcurrency())
						if err == nil && result != nil && result.Acquired {
						...
						if waitingCount < cfg.StickySessionMaxWaiting {
							return s.newSelectionResult(ctx, account, false, nil, newAccountWaitPlan(account, cfg.StickySessionWaitTimeout, cfg.StickySessionMaxWaiting))
						}
	...
	tryAcquireFromLoadMap := func(loadMap map[int64]*AccountLoadInfo) (*AccountSelectionResult, bool, error) {
		...
		for _, item := range selectionOrder {
			...
			result, err := s.tryAcquireAccountSlot(ctx, fresh.ID, groupID, fresh.EffectiveConcurrency())
			if err == nil && result != nil && result.Acquired {
				selection, selectErr := s.newAcquiredSelectionResult(ctx, fresh, result.ReleaseFunc)
				if selectErr != nil {
					return nil, true, selectErr
				}
				if sessionHash != "" && !stickySpillover && !gatewayProfitControlGateActive(ctx) && !PreserveStickyBindingFromContext(ctx) {
...
func (s *OpenAIGatewayService) tryAcquireAccountSlot(ctx context.Context, accountID int64, groupID *int64, maxConcurrency int) (*AcquireResult, error) {
	if s.concurrencyService == nil {
		return &AcquireResult{Acquired: false}, ErrConcurrencyUnavailable
	}
