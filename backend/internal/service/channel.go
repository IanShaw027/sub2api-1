...
const (
	...
	BillingModeVideo      BillingMode = "video"       // 视频计费（按分辨率每秒）
	BillingModeSearch     BillingMode = "search"      // 搜索/工具调用计费（per 1k calls）
	BillingModeAudio      BillingMode = "audio"       // 语音/音频计费（realtime/tts/stt）
)

// IsValid 检查 BillingMode 是否为合法值
func (m BillingMode) IsValid() bool {
	switch m {
	case BillingModeToken, BillingModePerRequest, BillingModeImage, BillingModeVideo, BillingModeSearch, BillingModeAudio, "":
		return true
	}
	return false
}
...
func ValidateIntervals(intervals []PricingInterval, mode BillingMode) error {
	...
	// per_request / image / video / search / audio 模式按显式价格或 tier，不做 token 区间重叠校验
	if mode == BillingModePerRequest || mode == BillingModeImage || mode == BillingModeVideo || mode == BillingModeSearch || mode == BillingModeAudio {
		return nil
	}
...
type SupportedModel struct {
	...
	PricingLookupName string               // 实际计费/全局 fallback 查价模型；为空时等同 Name
	IsCapability      bool                 // true 表示来自 mapping/capability，不是纯 pricing catalog
}
...
func (c *Channel) SupportedModels() []SupportedModel {
	...
	add := func(platform, displayName, pricingLookupName string, pricing *ChannelModelPricing, isCapability bool) {
		key := dedupKey{platform: platform, name: strings.ToLower(displayName)}
		if _, ok := seen[key]; ok {
			return
		}
		if pricingLookupName == "" {
			pricingLookupName = displayName
		}
		seen[key] = struct{}{}
		result = append(result, SupportedModel{
			...
			Platform:          platform,
			Pricing:           pricing,
			PricingLookupName: pricingLookupName,
	...
	for platform, mapping := range c.ModelMapping {
		...
		for src, target := range mapping {
			...
			if isWild {
				...
				for _, candidate := range pidx.names {
					if strings.HasPrefix(strings.ToLower(candidate), prefixLower) {
						...
						add(platform, display, candidate, pricing, true)
					}
				}
				continue
			}
			...
			add(platform, displayName, pricingKey, pricing, true)
		}
	}
	...
	for platform, pidx := range idx {
		for _, name := range pidx.names {
			...
			add(platform, display, name, pricing, false)
		}
	}
