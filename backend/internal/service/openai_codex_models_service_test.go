...
type codexModelsHTTPUpstreamStub struct {
	...
	doWithTLS func(req *http.Request, proxyURL string, accountID int64, accountConcurrency int, profile *tlsfingerprint.Profile) (*http.Response, error)
}
...
func TestFetchCodexModelsManifestUsesTLSRuntimeForOAuthAndAPIKey(t *testing.T) {
	tests := []struct {
		name    string
		account *Account
	}{
		{name: "oauth", account: newCodexModelsTestAccount()},
		{name: "api key", account: newCodexModelsAPIKeyTestAccount("https://upstream.example")},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			tt.account.Extra = map[string]any{"enable_tls_fingerprint": true}
			var gotProfile *tlsfingerprint.Profile
			var gotUserAgent, gotOriginator string
			upstream := &codexModelsHTTPUpstreamStub{doWithTLS: func(req *http.Request, _ string, _ int64, _ int, profile *tlsfingerprint.Profile) (*http.Response, error) {
				gotProfile = profile
			...
			ctx := WithTLSFingerprintInboundUserAgent(context.Background(), "codex_cli_rs/0.144.0")
			if _, err := s.FetchCodexModelsManifest(ctx, tt.account, "0.144.0", ""); err != nil {
				t.Fatalf("FetchCodexModelsManifest returned error: %v", err)
			}
			if gotProfile == nil {
				t.Fatal("Codex manifest request did not use the resolved TLS profile")
			}
			if gotUserAgent == "" || gotOriginator == "" {
				t.Fatalf("Codex identity headers were not paired: user-agent=%q originator=%q", gotUserAgent, gotOriginator)
			}
		})
	}
}
