...
func (s *OpenAIGatewayService) FetchCodexModelsManifest(ctx context.Context, account *Account, clientVersion, ifNoneMatch string) (*CodexModelsManifest, error) {
	...
	tlsRuntime := s.resolveOpenAITLSFingerprintRuntime(ctx, nil, account, "http")
...
func (s *OpenAIGatewayService) fetchCodexModelsManifestUpstream(ctx context.Context, request codexModelsManifestRequest, ifNoneMatch string) (*CodexModelsManifest, error) {
	...
	if s.httpUpstream != nil {
		...
		resp, err = s.httpUpstream.DoWithTLS(req, request.proxyURL, request.accountID, request.accountConcurrency, request.tlsRuntime.Profile)
	} else {
