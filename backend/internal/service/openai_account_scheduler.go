...
func (s *defaultOpenAIAccountScheduler) Select(
	...
	if req.GuardianParentAccountID > 0 {
		...
		selection, _, err := s.selectBySessionHash(ctx, parentReq, &decision)
		if err != nil {
			return nil, decision, err
		}
		if selection != nil && selection.Account != nil {
...
func (s *defaultOpenAIAccountScheduler) selectBySessionHash(
	...
	result, acquireErr := s.service.tryAcquireAccountSlot(ctx, accountID, req.GroupID, account.EffectiveConcurrency())
...
func (s *defaultOpenAIAccountScheduler) tryAcquireOpenAISelectionOrderWithBudget(
	...
	for i := 0; i < len(selectionOrder); i++ {
		...
		result, attempted, acquireErr := s.tryAcquireOpenAIAccountSlot(ctx, candidate.account.ID, req.GroupID, candidate.account.EffectiveConcurrency(), budget)
		if !attempted {
			break
		}
		if acquireErr != nil {
		...
		if fresh.Concurrency != candidate.account.Concurrency {
			release(result)
			result, attempted, acquireErr = s.tryAcquireOpenAIAccountSlot(ctx, fresh.ID, req.GroupID, fresh.EffectiveConcurrency(), budget)
			if !attempted {
				continue
			}
			if acquireErr != nil {
...
func (s *defaultOpenAIAccountScheduler) tryAcquireOpenAIAccountSlot(
	...
	result, err := s.service.tryAcquireAccountSlot(ctx, accountID, groupID, maxConcurrency)
...
func (s *defaultOpenAIAccountScheduler) tryFallbackToWeightedSticky(
	...
	for _, accountID := range []int64{req.StickyPreviousAccountID, req.StickyAccountID} {
		...
		result, acquireErr := s.service.tryAcquireAccountSlot(ctx, account.ID, req.GroupID, account.EffectiveConcurrency())
		if acquireErr != nil {
...
func (s *OpenAIGatewayService) selectAccountWithSchedulerOnce(
	...
	return scheduler.Select(ctx, OpenAIAccountScheduleRequest{
		...
		PreserveStickyBinding:   PreserveStickyBindingFromContext(ctx) || preserveGuardianParentBinding,
		RequirePrivacySet:       s.openAIGroupRequiresPrivacySet(ctx, groupID),
