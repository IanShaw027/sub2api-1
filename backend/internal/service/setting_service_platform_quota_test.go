...
func TestGetDefaultPlatformQuotas_ReturnsAllowedPlatforms(t *testing.T) {
	...
	svc := newSettingServiceForPlatformQuotaTest(map[string]string{
		// 新 JSON 格式：anthropic daily=10.5, openai monthly=0, 其余平台无配置
	...
	// Grok 无配置 → daily = nil
	if v := got["grok"].DailyLimitUSD; v != nil {
		t.Errorf("grok daily want nil (not configured), got %v", *v)
	}
}
...
// 再由 GetDefaultPlatformQuotas（读）正确往返——覆盖真实 write→read 路径，锁住补齐契约。
