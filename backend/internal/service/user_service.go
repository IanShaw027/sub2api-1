...
func (s *UserService) RecordLastActiveForUser(ctx context.Context, user *User) {
	...
	activeAt := time.Now()
	if err := s.userRepo.UpdateUserLastActiveAt(ctx, user.ID, activeAt); err != nil {
		slog.Warn("record user last active failed", "user_id", user.ID, "error", err)
		return
	}
