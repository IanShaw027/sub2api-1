...
func newGatewayTextEndpointServiceForTest(upstream *anthropicHTTPUpstreamRecorder) *GatewayService {
...
func TestGatewayForwardAsChatCompletions_AutoRouteCCUpstreamUsesRawChatEndpoint(t *testing.T) {
	...
	svc := newGatewayTextEndpointServiceForTest(upstream)
...
func TestGatewayForwardMessages_OpenAICompatCCUpstreamRequiresTextEndpointAutoRoute(t *testing.T) {
	...
	c, _ := newGatewayTextEndpointContext(http.MethodPost, "/v1/messages", body)
	upstream := &anthropicHTTPUpstreamRecorder{
		resp: &http.Response{
			StatusCode: http.StatusOK,
	...
	require.Contains(t, err.Error(), "text_endpoint_auto_route")
	require.Nil(t, upstream.lastReq, "CC-only upstream without auto-route must not hit Anthropic /v1/messages")
}
