...
func (s *openAIRecordUsageUserPlatformQuotaRepoStub) BatchSnapshotUsage(context.Context, []UserPlatformQuotaSnapshot, time.Time) error {
...
func TestOpenAIGatewayServiceRecordUsage_MixedOutputSizesBillPerImageTier(t *testing.T) {
	imagePrice1K, imagePrice4K := 0.11, 0.44
	usageRepo := &openAIRecordUsageLogRepoStub{inserted: true}
	svc := newOpenAIRecordUsageServiceForTest(usageRepo, &openAIRecordUsageUserRepoStub{}, &openAIRecordUsageSubRepoStub{}, nil)

	err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
		Result: &OpenAIForwardResult{
			RequestID:        "resp_image_mixed_sizes",
			Model:            "gpt-image-2",
			ImageCount:       2,
			ImageOutputSizes: []string{"1024x1024", "3840x2160"},
			Duration:         time.Second,
		},
		APIKey: &APIKey{ID: 11203, GroupID: i64p(1203), Group: &Group{
			ID: 1203, RateMultiplier: 1, ImagePrice1K: &imagePrice1K, ImagePrice4K: &imagePrice4K,
		}},
		User: &User{ID: 21203}, Account: &Account{ID: 31203},
	})

	require.NoError(t, err)
	require.Equal(t, map[string]int{ImageBillingSize1K: 1, ImageBillingSize4K: 1}, usageRepo.lastLog.ImageSizeBreakdown)
	require.InDelta(t, 0.55, usageRepo.lastLog.TotalCost, 1e-12)
...
func TestOpenAIGatewayServiceRecordUsage_ImageToolBillsMainModelTokensPlusExactImageCost(t *testing.T) {
	imagePrice := 0.02
	groupID := int64(212)
	...
	err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
		Result: &OpenAIForwardResult{
			RequestID:         "resp_image_tool_mixed",
			Model:             "gpt-image-2",
			TokenBillingModel: "gpt-5.4",
			Usage: OpenAIUsage{
				InputTokens:       1000,
				OutputTokens:      400,
				ImageOutputTokens: 250,
			},
			ImageCount: 2,
			ImageSize:  "1K",
			Duration:   time.Second,
	...
	require.Equal(t, string(BillingModeImage), *usageRepo.lastLog.BillingMode)
	require.Equal(t, 2, usageRepo.lastLog.ImageCount)
	require.Equal(t, 250, usageRepo.lastLog.ImageOutputTokens)
	expectedTokenCost := expectedOpenAICost(t, svc, "gpt-5.4", OpenAIUsage{
		InputTokens:  1000,
		OutputTokens: 400,
	}, 0.15)
	require.InDelta(t, expectedTokenCost.InputCost, usageRepo.lastLog.InputCost, 1e-12)
	require.InDelta(t, expectedTokenCost.OutputCost, usageRepo.lastLog.OutputCost, 1e-12)
	require.InDelta(t, 0.0, usageRepo.lastLog.ImageOutputCost, 1e-12)
	require.InDelta(t, expectedTokenCost.TotalCost+0.04, usageRepo.lastLog.TotalCost, 1e-12)
...
func TestOpenAIGatewayServiceRecordUsage_ImageWebBridgeUsesExactImageCostOnly(t *testing.T) {
	...
	err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
		Result: &OpenAIForwardResult{
			RequestID:            "resp_image_web_bridge",
			Model:                "gpt-image-2",
			TokenBillingModel:    "gpt-5.4",
			EffectiveRequestType: RequestTypeImageWebBridge,
			Usage: OpenAIUsage{
				InputTokens:       1000,
				OutputTokens:      400,
				ImageOutputTokens: 250,
		...
		APIKey: &APIKey{
			...
			Group: &Group{
				...
				Platform:          PlatformOpenAI,
				RateMultiplier:    0.15,
				Images2APIPrice1K: &imagePrice,
			},
		},
		User:    &User{ID: 2209},
		Account: &Account{ID: 3209},
	})
...
func TestOpenAIGatewayServiceRecordUsage_ImageSharedMultiplierIgnoresGroupRate(t *testing.T) {
	imagePrice := 0.2
	groupID := int64(121)
...
func TestOpenAIGatewayServiceRecordUsage_ImageSharedMultiplierIgnoresUserGroupOverride(t *testing.T) {
	imagePrice := 0.5
	userRate := 0.2
	groupID := int64(125)
