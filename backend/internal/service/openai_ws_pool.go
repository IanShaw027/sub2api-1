...
var openAIWSConnEvictHook atomic.Pointer[func(connID string, reason string)]

// RegisterOpenAIWSConnEvictHook 注册连接淘汰回调（幂等覆盖）。
// 用于让 conn_id -> last_response_id 等外部映射在连接删除时同步失效。
func RegisterOpenAIWSConnEvictHook(hook func(connID string, reason string)) {
	if hook == nil {
...
func triggerOpenAIWSConnEvict(connID string, reason string) {
	id := stringsTrim(connID)
	if id == "" {
		return
	}
	evictReason := stringsTrim(reason)
	if evictReason == "" {
		evictReason = "unknown"
	}
...
func deleteOpenAIWSAccountConnLocked(ap *openAIWSAccountPool, connID string, reasons ...string) {
	...
	delete(ap.conns, id)
	triggerOpenAIWSConnEvict(id, reason)
}
...
func (l *openAIWSConnLease) ConnIdleDuration() time.Duration {
	if l == nil || l.conn == nil {
		return 0
	}
	return l.conn.idleDuration(time.Now())
}

func (l *openAIWSConnLease) ConnLeaseCount() int64 {
	if l == nil || l.conn == nil {
		return 0
	}
	return l.conn.leaseCount.Load()
}
...
func (c *openAIWSConn) isNeutralStock() bool {
	if c == nil || c.profile != openAIWSConnProfileNeutral {
		return false
	}
	return c.neutralStock.Load()
}

func (c *openAIWSConn) markNeutralStock() {
	if c == nil || c.profile != openAIWSConnProfileNeutral {
		return
	}
	c.neutralStock.Store(true)
}

func (c *openAIWSConn) clearNeutralStock() {
	if c == nil {
		return
	}
	c.neutralStock.Store(false)
}
...
type openAIWSPoolMetrics struct {
	...
	acquireStaleEvict     atomic.Int64
...
func (p *openAIWSConnPool) SnapshotMetrics() OpenAIWSPoolMetricsSnapshot {
	...
	return OpenAIWSPoolMetricsSnapshot{
		...
		AcquireStaleEvictTotal:  p.metrics.acquireStaleEvict.Load(),
...
func (p *openAIWSConnPool) runBackgroundCleanupWorker() {
	...
	for {
		timer := time.NewTimer(p.backgroundSweepInterval())
		select {
		case <-timer.C:
...
// ReconcileShrink 立即对所有账号池按当前 idle TTL / neutral target 收缩多余空闲连接。
...
func (p *openAIWSConnPool) acquire(ctx context.Context, req openAIWSAcquireRequest, retry int) (*openAIWSConnLease, error) {
	...
	now := time.Now()
	if ap.lastCleanupAt.IsZero() || now.Sub(ap.lastCleanupAt) >= p.acquireCleanupInterval() {
		evicted = p.cleanupAccountLocked(ap, now, accountConcurrency)
		ap.lastCleanupAt = now
	} else if !req.ForceNewConn {
		evicted = append(evicted, p.cleanupExpiredReusableConnsLocked(ap, now)...)
	}
	...
	if allowReuse {
		if forcePreferredConn {
			...
			if preferredConn.tryAcquire() {
				...
				ap.mu.Unlock()
				closeOpenAIWSConns(evicted)
				return p.finalizeReusedLease(accountID, preferredConn, req, connPick, 0), nil
			}
			...
			return p.finalizeReusedLease(accountID, preferredConn, req, connPick, queueWait), nil
		}
		...
		if preferredConnID != "" {
			if conn, ok := ap.conns[preferredConnID]; ok && conn.matchesAcquire(req) {
				if conn.tryAcquire() {
					...
					ap.mu.Unlock()
					closeOpenAIWSConns(evicted)
					return p.finalizeReusedLease(accountID, conn, req, connPick, 0), nil
				}
				if affinityOnlyReuse {
					...
					queueWait := time.Since(waitStart)
					p.metrics.acquireQueueWaitMs.Add(queueWait.Milliseconds())
					return p.finalizeReusedLease(accountID, conn, req, connPick, queueWait), nil
				}
			}
		}
		...
		if !affinityOnlyReuse {
			...
			if best != nil && best.tryAcquire() {
				...
				ap.mu.Unlock()
				closeOpenAIWSConns(evicted)
				return p.finalizeReusedLease(accountID, best, req, connPick, 0), nil
			}
			for _, conn := range ap.conns {
...
func (p *openAIWSConnPool) pickOldestIdleNeutralStockConnLocked(ap *openAIWSAccountPool) *openAIWSConn {
	if ap == nil || len(ap.conns) == 0 {
		return nil
	}
	var oldest *openAIWSConn
	for _, conn := range ap.conns {
		if conn == nil || !conn.isNeutralStock() {
			continue
		}
...
func countNeutralStockConnsLocked(ap *openAIWSAccountPool) int {
	if ap == nil {
		return 0
	}
	count := 0
...
func countNeutralStockConnsByReuseKeyLocked(ap *openAIWSAccountPool, reuseKey string) int {
	if ap == nil {
		return 0
	}
	count := 0
	for _, conn := range ap.conns {
		...
		if stringsTrim(conn.reuseKey) == stringsTrim(reuseKey) {
			count++
		}
	}
	return count
...
func (p *openAIWSConnPool) finalizeReusedLease(accountID int64, conn *openAIWSConn, req openAIWSAcquireRequest, connPick, queueWait time.Duration) *openAIWSConnLease {
	if conn != nil && req.Profile == openAIWSConnProfileNeutral {
		conn.clearNeutralStock()
	}
	if conn != nil {
		conn.markLease()
	}
...
func (p *openAIWSConnPool) neutralAcquireStaleIdle() time.Duration {
	if p != nil && p.cfg != nil && p.cfg.Gateway.OpenAIWS.NeutralAcquireStaleIdleSeconds > 0 {
		return time.Duration(p.cfg.Gateway.OpenAIWS.NeutralAcquireStaleIdleSeconds) * time.Second
	}
	return openAIWSNeutralAcquireStaleIdle
}

func (p *openAIWSConnPool) acquireCleanupInterval() time.Duration {
	interval := openAIWSAcquireCleanupInterval
	if ttl := p.sessionIdleTTL(); ttl > 0 && ttl < interval {
		return ttl
	}
	return interval
}

func (p *openAIWSConnPool) backgroundSweepInterval() time.Duration {
	interval := openAIWSBackgroundSweepTicker
	if ttl := p.sessionIdleTTL(); ttl > 0 && ttl < interval {
		return ttl
	}
	return interval
...
func (p *openAIWSConnPool) neutralPrewarmBaseConcurrency(concurrency int) int {
	if concurrency <= 0 {
		return 0
	}
	effective := concurrency - stickyReserveSlots(concurrency, p.stickyReservePercent())
	if effective < 0 {
		return 0
	}
	return effective
...
func (p *openAIWSConnPool) neutralPrewarmTargetForConcurrency(concurrency int) int {
	...
	percent := p.neutralPrewarmPercent()
	if percent <= 0 {
		return 0
	}
	baseConcurrency := p.neutralPrewarmBaseConcurrency(concurrency)
	if baseConcurrency <= 0 {
		return 0
	}
	target := baseConcurrency * percent / 100
	if minIdle := p.minIdlePerAccount(); minIdle > 0 && target < minIdle {
		target = minIdle
	}
	// maxIdlePerAccount 现在只限制 neutral 预热库存，不压缩已归还的
	// session-bound 粘性复用连接；后者仅受 idle TTL / max-age 清理。
	if maxIdle := p.maxIdlePerAccount(); maxIdle > 0 && target > maxIdle {
		target = maxIdle
	}
	if target > baseConcurrency {
...
func (p *openAIWSConnPool) logConnEvict(conn *openAIWSConn, reason string) {
	...
	now := time.Now()
	logOpenAIWSModeInfo(
		"conn_evict conn_id=%s profile=%s reason=%s conn_age_ms=%d conn_idle_ms=%d conn_lease_count=%d",
		normalizeOpenAIWSLogValue(conn.id),
		normalizeOpenAIWSLogValue(string(conn.profile)),
		normalizeOpenAIWSLogValue(reason),
		conn.age(now).Milliseconds(),
		conn.idleDuration(now).Milliseconds(),
		conn.leaseCount.Load(),
	)
}

func (p *openAIWSConnPool) idleConnEvictionReason(conn *openAIWSConn, now time.Time) string {
	if conn == nil {
		return ""
	}
	if maxAge := p.maxConnAge(); maxAge > 0 && conn.age(now) >= maxAge {
		return "conn_max_age"
	}
	switch conn.profile {
	case openAIWSConnProfileSessionBound:
		if ttl := p.sessionIdleTTL(); ttl > 0 && now.Sub(conn.lastUsedAt()) >= ttl {
			return "session_idle_ttl"
		}
	case openAIWSConnProfileNeutral:
		if ttl := p.neutralIdleTTL(); ttl > 0 && now.Sub(conn.lastUsedAt()) >= ttl {
			return "neutral_idle_ttl"
		}
	}
	return ""
}

func (p *openAIWSConnPool) acquireIdleConnEvictionReason(conn *openAIWSConn, now time.Time) string {
	if reason := p.idleConnEvictionReason(conn, now); reason != "" {
		return reason
	}
	staleIdle := p.neutralAcquireStaleIdle()
	if conn != nil && conn.profile == openAIWSConnProfileNeutral && staleIdle > 0 &&
		now.Sub(conn.lastUsedAt()) >= staleIdle {
		return "neutral_acquire_stale_idle"
	}
	return ""
}

func (p *openAIWSConnPool) removeConnLocked(ap *openAIWSAccountPool, conn *openAIWSConn, reason string) {
	if ap == nil || conn == nil {
		return
	}
	p.logConnEvict(conn, reason)
	deleteOpenAIWSAccountConnLocked(ap, conn.id, reason)
	if len(ap.pinnedConns) > 0 {
		delete(ap.pinnedConns, conn.id)
	}
}

func (p *openAIWSConnPool) cleanupExpiredReusableConnsLocked(ap *openAIWSAccountPool, now time.Time) []*openAIWSConn {
	if ap == nil {
		return nil
	}
	...
	for id, conn := range ap.conns {
		...
		select {
		case <-conn.closedCh:
			p.removeConnLocked(ap, conn, "closed")
		...
		reason := p.acquireIdleConnEvictionReason(conn, now)
		if reason == "" {
			continue
		}
		p.removeConnLocked(ap, conn, reason)
		evicted = append(evicted, conn)
		p.metrics.scaleDownTotal.Add(1)
		if reason == "neutral_acquire_stale_idle" {
			p.metrics.acquireStaleEvict.Add(1)
		}
	}
	return evicted
}
...
func (p *openAIWSConnPool) cleanupAccountLocked(ap *openAIWSAccountPool, now time.Time, accountConcurrency int) []*openAIWSConn {
	...
	for id, conn := range ap.conns {
		...
		if maxAge > 0 && conn.age(now) >= maxAge {
			...
			deleteOpenAIWSAccountConnLocked(ap, id, "conn_max_age")
			if len(ap.pinnedConns) > 0 {
				delete(ap.pinnedConns, id)
			}
			evicted = append(evicted, conn)
		...
		if conn.profile == openAIWSConnProfileSessionBound && sessionIdleTTL > 0 && now.Sub(conn.lastUsedAt()) >= sessionIdleTTL {
			...
			deleteOpenAIWSAccountConnLocked(ap, id, "session_idle_ttl")
			if len(ap.pinnedConns) > 0 {
				delete(ap.pinnedConns, id)
			}
			evicted = append(evicted, conn)
		...
		if conn.profile == openAIWSConnProfileNeutral && neutralIdleTTL > 0 && now.Sub(conn.lastUsedAt()) >= neutralIdleTTL {
			...
			deleteOpenAIWSAccountConnLocked(ap, id, "neutral_idle_ttl")
			if len(ap.pinnedConns) > 0 {
				delete(ap.pinnedConns, id)
			}
			evicted = append(evicted, conn)
	...
	for countNeutralStockConnsLocked(ap) > neutralTarget {
		idle := p.pickOldestIdleNeutralStockConnLocked(ap)
		if idle == nil {
			break
		}
		p.logConnEvict(idle, "neutral_over_target")
		deleteOpenAIWSAccountConnLocked(ap, idle.id, "neutral_over_target")
		if len(ap.pinnedConns) > 0 {
	...
	if accountConcurrency <= 0 && maxIdle == 0 && p != nil && p.cfg != nil && p.cfg.Gateway.OpenAIWS.MaxConnsPerAccount > 0 {
		for len(ap.conns) > 0 {
			...
			deleteOpenAIWSAccountConnLocked(ap, idle.id, "idle_over_max")
			if len(ap.pinnedConns) > 0 {
				delete(ap.pinnedConns, idle.id)
			}
			evicted = append(evicted, idle)
...
func (p *openAIWSConnPool) evictConn(accountID int64, connID string, reasons ...string) {
	...
	if ok && ap != nil {
		...
		if c, exists := ap.conns[connID]; exists {
			conn = c
			deleteOpenAIWSAccountConnLocked(ap, connID, reason)
			if len(ap.pinnedConns) > 0 {
...
func (p *openAIWSConnPool) dialTimeout() time.Duration {
	...
	return defaultOpenAIWSDialTimeout
}
