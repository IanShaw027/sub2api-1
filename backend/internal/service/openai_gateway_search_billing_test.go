...
func TestOpenAIGatewayServiceCalculateOpenAIRecordUsageCost_SearchUsesGroupPrice(t *testing.T) {
	...
	groupID := int64(123)
	breakdown, err := svc.calculateOpenAIRecordUsageCost(context.Background(), &OpenAIForwardResult{SearchCount: 2, Duration: time.Second}, &APIKey{GroupID: &groupID, Group: &Group{ID: groupID, SearchPricePer1k: &pricePer1k}}, "", 1, 1, 1, UsageTokens{}, "", RequestTypeUnknown)
