...
// OpsMetricsDaily defines the schema for ops_metrics_daily.
type OpsMetricsDaily struct {
	ent.Schema
}

func (OpsMetricsDaily) Annotations() []schema.Annotation {
	return []schema.Annotation{
		entsql.Annotation{Table: "ops_metrics_daily"},
	}
}

func (OpsMetricsDaily) Fields() []ent.Field {
	return []ent.Field{
...
func (OpsMetricsDaily) Indexes() []ent.Index {
	return []ent.Index{
		index.Fields("bucket_date", "platform").Unique(),
		index.Fields("bucket_date"),
		index.Fields("platform", "bucket_date"),
	}
}
