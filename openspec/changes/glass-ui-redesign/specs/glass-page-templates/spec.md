...
旧 `/studio?mode=...` 路由仅作兼容重定向。每条页面路由 MUST 归入且仅归入下列模板之一，并使用对应布局骨架：LandingLayout（`/home /model-plaza /legal/:documentId /key-usage`）、AuthLayout（`/login /register /email-verify /forgot-password /reset-password /auth/dingtalk/email-completion`）、CallbackStatus（`/auth/callback /auth/linuxdo/callback /auth/wechat/callback /auth/wechat/payment/callback /auth/dingtalk/callback /auth/oidc/callback /payment/result`）、SetupWizard（`/setup`）、DashboardPage（`/dashboard /admin/dashboard /admin/ops /admin/orders/dashboard /monitor`）、ListPage（`/keys /usage /orders /invoices /tickets /subscriptions /available-channels /admin/users /admin/groups /admin/accounts /admin/subscriptions /admin/channels/pricing /admin/channels/monitor /admin/plugins /admin/announcements /admin/proxies /admin/redeem /admin/promo-codes /admin/usage /admin/audit-logs /admin/risk-control /admin/prompt-audit /admin/affiliates/invites /admin/affiliates/rebates /admin/affiliates/transfers /admin/orders /admin/orders/invoices /admin/orders/plans /admin/tickets`）、DetailPage（`/invoices/:id /tickets/:id /tickets/new /admin/tickets/:id`）、SettingsPage（`/admin/settings /profile`）、ActionPage（`/redeem /affiliate /batch-image`）、PaymentFlow（`/purchase /payment/qrcode /payment/stripe /payment/airwallex /payment/stripe-popup`）、WorkspacePage（`/studio/chat /studio/image /studio/video /studio/voice /studio/gallery`）、EmbedPage（`/custom/:id`、`/purchase` iframe 模式）、NotFound（`/:pathMatch(.*)*`）。`route-matrix.md` MUST 与路由表一致。
...
ActionPage MUST 为最大宽 720 居中列（gap 14）：玻璃卡（标题块 + 36px 字段 + 34px primary）→ 结果 `.notice-success/-danger` → 说明卡（`.card-title` + 12.5 muted 列表）→ `MiniStatCard` / 3 列统计；邀请页邀请链接为 `EndpointCard` 式复制行，返利记录为 ListPage 表格。PaymentFlow：套餐卡玻璃卡圆角 16 内边距 20，选中 `.glass-ring`，价格 28/800 tabular，特性行 13 + 12px success 对勾，42px 全宽 primary；金额输入 40px `.field.input-lg` 带 `$` 前缀；支付品牌按钮 32px；二维码页 440 居中卡、220px 二维码在 `--surface-secondary` 圆角 12 盒内、轮询状态圆点徽章；结果页与 CallbackStatus 同款。EmbedPage：玻璃卡圆角 16 包裹 iframe / Markdown，`.card-header` 标题 + 34px secondary"新窗口打开"；创作中心不使用 EmbedPage 或后台批量任务模板，作为统一 AppLayout 下的 WorkspacePage；侧栏「创作中心」分类包含对话、图片、视频、语音、灵感墙五个子路由，使用共享顶栏、面包屑、PageHeader 与 Glass 控件，不再提供独立品牌导航；仅对话显示会话侧栏，图片和视频使用结果墙与底部输入区。媒体默认本地保存，主动发布才上传并公开；详细契约见 `docs/CREATION_WORKSPACE.md`。
