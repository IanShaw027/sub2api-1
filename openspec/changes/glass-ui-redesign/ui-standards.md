...
- 管理员侧栏支持管理员/用户模式，普通用户不显示模式切换。创作中心（包括批量生图）只属于用户导航；管理员菜单按业务类别分组。
- 菜单选中态使用 `--nav-active-bg`，不以额外边框/阴影包围文字。顶栏随滚动进度在 0-48px 内过渡到主题实底，并显示底部边线；真实父路由面包屑可点击。
- 日期筛选影响区间统计、趋势、排行、展开明细与事件；实时、今日、累计及存量指标必须用对应标签，不得给它们套不匹配的区间折线。范围 API 的 `total_*` 以服务端契约为准，不按字段名字猜测为终身累计。
- DataTable 普通列内容默认最大 360px，操作列 240px；可用 `Column.maxWidth` 显式覆盖。宽表滚动承载，不强制压缩数字、徽标或进度条，不依赖原型固定列数。
- 日期弹层逃离父级裁切并限制于视口内；图标操作同时提供可访问名称及 title/tooltip。外部点击不抢焦点，Escape 关闭弹层后返回触发器。
