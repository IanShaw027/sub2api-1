...
- [ ] 11.8 `/admin/announcements`（`AnnouncementsView.vue`：ListPage + Markdown 编辑器 `.field` 文本域 220 + 预览卡 + 受众 / 排期 `SettingRow`）；验证同上；提交 `feat(glass): admin lists batch 1 (tasks 11.1–11.8)`
...
批次 1（仪表盘 / 监控 / 记录）
- [ ] 12.1 `/dashboard`（`user/DashboardView.vue` + `components/user/dashboard/*` + `components/charts/*`：DashboardPage 配方；平台拆分改为 `1fr 1fr 1fr` 嵌板而非卡中卡；`TokenUsageTrend` / `ModelDistributionChart` 用 `chartTheme()`；图例 12 muted）；验证亮 / 暗 / 390 截图 + spec
...
- [ ] 15.2 `scripts/i18n-diff.mjs`：zh/en 键差集与重复键检查；验证输出 `zh-only: 0, en-only: 0`
