// Render each artboard of the design prototype to PNG. Usage: node protoshot.js [theme]
...
const theme = process.argv[2] || 'glass-light'
const OUT = path.join(S, 'reference', theme); fs.mkdirSync(OUT, { recursive: true })
// static server for design dir
const mime = { '.html': 'text/html', '.js': 'application/javascript', '.css': 'text/css' }
const srv = http.createServer((req, res) => {
  const p = path.join(S, 'design', decodeURIComponent(req.url.split('?')[0]))
