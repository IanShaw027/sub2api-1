...
router.beforeEach(async (to, _from, next) => {
  ...
  if ((to.meta.requiresPayment || to.meta.requiresRiskControl || to.meta.requiresTicket || to.meta.requiresCreationCenter) && !appStore.publicSettingsLoaded) {
    try {
      await appStore.fetchPublicSettings()
    } catch (error) {
      console.warn('Failed to load public settings in route guard', error)
  ...
  if (
    to.meta.requiresCreationCenter &&
    appStore.publicSettingsLoaded &&
    appStore.cachedPublicSettings?.creation_center_enabled !== true
  ) {
    next(authStore.isAdmin ? '/admin/dashboard' : '/dashboard')
    return
  }
