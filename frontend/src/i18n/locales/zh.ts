export default {
  ...
  admin: {
    ...
    riskControl: {
      ...
      retryCount: '失败重试次数',
      apiKeyRateLimitPolicy: '审核服务失败策略',
      apiKeyRateLimitPolicyHint: '在前置拦截（pre-block）模式下，当内容审核链路无法完成时生效（无可用审核 Key、本地限流、上游 429/5xx、超时、空结果等，不仅限于 429）。选择「拦截」会对客户端报错；选择「放行」会在审核不可用时 fail-open 继续原请求。除非你明确接受审核故障时的放行，否则请保持拦截。',
      apiKeyRateLimitPolicyError: '拦截请求（fail closed）',
      apiKeyRateLimitPolicyAllow: '放行原请求（fail open）',
      ...
      preHashCheckHint: '在审核前比对已命中哈希；预拦截模式会阻断，仅观察模式只记录命中并放行。',
      ...
      hashAction: '拦截类型',
      hashCategory: '命中类别',
      hashMatchedKeyword: '命中关键词',
      hashSource: '来源（用户 / 模型 / 分组）',
      hashFieldEmpty: '—',
      hashCreatedAt: '创建时间',
      hashExpiresAt: '过期时间',
      hashHits7d: '7 天命中',
      hashHits30d: '30 天命中',
      noFlaggedHashes: '暂无风险哈希',
      deleteFlaggedHash: '删除指定哈希',
      deleteSelectedFlaggedHashes: '删除已选（{count}）',
      deleteSelectedFlaggedHashesConfirm: '确定删除选中的 {count} 个风险哈希吗？',
      ...
      flaggedHashesLoadFailed: '加载风险哈希失败',
      flaggedHashesDeleted: '已删除 {count} 个风险哈希',
      ...
      blockedKeywordsDescription: '匹配忽略大小写；同一行使用 && 分隔时，各片段需在「邻近窗口」内共现才算命中（默认窗口见下方设置）；命中后会按下方策略决定是否调用上游审计接口。',
      ...
      defaultProximityWindow: '默认邻近窗口（字符）',
      defaultProximityWindowHint: '&& 组合关键词未显式指定 :N 时使用的默认邻近窗口，范围 1~2000。窗口越小越严格，可降低长文本中两词偶然共现的误判。',
      keywordSyntaxTitle: '组合与窗口语法说明',
      keywordSyntaxRuleAnd: '① 单行用 && 连接多个词，表示这些词需全部出现才命中（如「账号 && 出售」）。',
      keywordSyntaxRuleWindow: '② && 命中还要求各词落在同一「邻近窗口」内共现，避免长文本中相隔很远的两词被误判；未指定时使用上面的默认窗口。',
      keywordSyntaxRuleOverride: '③ 可用 :N 后缀为单条规则覆盖窗口，如「绕过 && 风控:40」表示窗口 40 字符（上限 2000）。',
      keywordSyntaxRuleException: '④ 例外短语（白名单）：某次命中若被更长的例外短语完整包含则跳过，如「勒索病毒」可让「勒索」不误判。',
    ...
    accounts: {
      ...
      bulkEdit: {
        ...
        singlePlatformRequired: '批量编辑必须明确限定到单一平台，请缩小选择范围或添加平台筛选。'
      },
      ...
      quotaControl: {
        ...
        tlsFingerprint: {
          ...
          bindingMatrix: '按 OS / 客户端 绑定模板',
          bindingMatrixHint: '路由器判定请求对应哪个 OS/客户端；此矩阵决定本账号在该维度使用哪个模板。不同账号可绑定不同模板。',
          addBinding: '添加绑定',
          noBindings: '未配置分维度绑定，将使用上方的路由/默认模板。',
          clientTypePlaceholder: '客户端（如 codex-cli）',
          defaultOS: '默认操作系统',
          defaultOSNone: '未设置（用上方模板）',
          defaultOSHint: '该平台无入站 User-Agent，请选择本账号模拟的 OS 维度。'
        },
        sessionIdMasking: {
          label: '会话 ID 伪装',
    ...
    proxies: {
      ...
      batchTestAllFilteredConfirm: '当前未选择代理。确认要对当前过滤结果中的全部 {count} 个代理执行批量测试吗？',
      ...
      batchQualityAllFilteredConfirm: '当前未选择代理。确认要对当前过滤结果中的全部 {count} 个代理执行批量质量检测吗？',
    ...
    settings: {
      ...
      gatewayForwarding: {
        ...
        claudeTelemetryModeHint: '控制 /api/event_logging/batch 遥测请求。直接丢弃会返回 200 不访问上游；清洗后代发只会使用 Anthropic OAuth/Setup Token 账号，API Key 账号不会参与。',
        grokDefaultBaseURLMode: 'Grok 默认上游 Base URL',
        grokDefaultBaseURLModeHint: '仅影响文本推理（responses/chat），用于未显式指定账号 base_url 的 Grok OAuth 账号。可选择官方 API、区域 API 或 Grok Build CLI；图片/视频 Imagine 始终走 api.x.ai，usage/billing 始终走 CLI 代理。账号显式 base_url 对推理优先。',
        grokDefaultBaseURLModeAPI: '官方 API（api.x.ai/v1）',
        grokDefaultBaseURLModeUSEast1: '区域 API（us-east-1.api.x.ai/v1）',
      ...
      site: {
        ...
        downloadToolsUrlHint: '工具下载页的链接。留空则隐藏下载工具入口。',
    ...
    tlsFingerprintProfiles: {
      ...
      allPlatforms: '全部平台',
      allOS: '全部系统',
      allClientTypes: '全部客户端',
      filterNamePlaceholder: '搜索名称…',
      ...
      form: {
        ...
        osAny: '不限 / 通用',
        clientType: '客户端类型',
        clientTypePlaceholder: '例如 codex-cli / chatgpt-desktop / claude-code',
        ...
        userAgentPlaceholder: '例如 codex_cli_rs/0.125.0 (Ubuntu 22.4.0; x86_64)',
        userAgentHint: '可选。启用该模板时用此 User-Agent 请求上游；留空则使用内置默认（Codex CLI）兜底。',
        ...
        signatureAlgorithmsCert: '证书签名算法',
        ...
        pskModes: 'PSK 模式',
        extensions: '扩展',
        extensionPayloads: '扩展 Payload（JSON）',
      ...
      capture: {
        ...
        newTask: '新建采集任务',
        backToTasks: '返回任务',
        copyValueSuccess: '已复制到剪贴板',
        copyValueFailed: '复制失败',
        startSuccess: '采集任务已启动',
        startFailed: '启动采集任务失败',
        stopSuccess: '采集任务已停止',
        stopFailed: '停止采集任务失败',
        restartSuccess: '采集任务已重新开启',
        restartFailed: '重新开启采集任务失败',
        deleteSuccess: '采集任务已删除',
        deleteFailed: '删除采集任务失败',
        deleteConfirmMessage: '确定要删除采集任务 "{name}" 吗？关联的采样数据也将被删除。',
        loadFailed: '加载采集任务失败',
    ...
    tlsFingerprintRouters: {
      ...
      form: {
        ...
        dimensionOS: '判定 OS',
        dimensionNone: '不判定',
        dimensionClientType: '判定客户端',
        dimensionClientTypePlaceholder: '如 codex-cli（经账号矩阵解析）',
        caseSensitive: '区分大小写',
  ...
  payment: {
    ...
    admin: {
      ...
      alreadyRefunded: '已退款',
      deductBalance: '扣除余额',
      deductSubscriptionBenefit: '扣除订阅权益',
      deductBalanceHint: '从用户余额中扣回充值金额',
      userBalance: '用户余额',
      orderAmount: '订单金额',
      userRequestedRefundAmount: '用户申请',
