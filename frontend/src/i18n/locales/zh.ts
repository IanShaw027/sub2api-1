export default {
  ...
  admin: {
    ...
    accounts: {
      ...
      kiro: {
        manualUpdateAction: '更新凭据',
        manualUpdateDialogTitle: '更新 Kiro 凭据',
        manualUpdateTitle: '当前仅支持手动更新凭据',
        manualUpdateDesc: '当前后端还没有接入 Kiro 的管理端 OAuth 重新授权流程。请在账号编辑页手动更新已保存的 Kiro 凭据。',
        manualUpdateStepRefresh: '必须提供有效的 refresh token；上游登录状态变化后应及时替换。',
        manualUpdateStepIDC: '如果账号使用 IDC 认证，还需要同步维护 Client ID 和 Client Secret。',
        manualUpdateStepVersions: '当上游客户端环境变化时，请一并检查 access token、过期时间、区域、machine ID 和版本字段。',
        authMethodLabel: '认证方式',
        authMethodHint: 'Social 适合常规 Kiro 账号；IDC 需要额外的 AWS/IDC 客户端参数。',
        authMethodSocial: 'Social',
        authMethodIDC: 'IDC',
        refreshTokenLabel: 'Refresh Token',
        refreshTokenPlaceholder: '粘贴 Kiro refresh token',
        refreshTokenPlaceholderEdit: '粘贴新的 refresh token，或保留当前值',
        refreshTokenRequired: '必须填写 Kiro refresh token',
        accessTokenLabel: 'Access Token',
        accessTokenPlaceholder: '可选，填写当前 Kiro access token',
        accessTokenPlaceholderEdit: '留空则保留当前 access token',
        accessTokenHintCreate: '可选。不填时服务端会在首次调用前尝试用 refresh token 刷新。',
        accessTokenHintEdit: '可选。留空保留当前 access token；填写后会覆盖现有值。',
        clientIdLabel: 'Client ID',
        clientIdPlaceholder: 'Kiro client ID',
        clientSecretLabel: 'Client Secret',
        clientSecretPlaceholder: 'Kiro client secret',
        clientSecretPlaceholderEdit: '留空则保留当前 client secret',
        idcClientRequired: 'Kiro IDC 模式必须填写 Client ID 和 Client Secret',
        expiresAtLabel: 'Expires At',
        expiresAtHintCreate: '可选。填写当前 access token 的过期时间，避免首次使用时立即触发刷新。',
        expiresAtHintEdit: '留空会清除当前 expires_at，让服务端基于 refresh token 重新获取。',
