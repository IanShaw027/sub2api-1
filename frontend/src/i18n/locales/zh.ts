export default {
  ...
  admin: {
    ...
    accounts: {
      ...
      reAuthorizeUnavailableKiro: 'Kiro 账号不支持浏览器式的管理端重新授权流程。你可以先尝试刷新已保存的 refresh token，失败后再到编辑页手动更新凭据。',
      kiro: {
        ...
        manualUpdateDesc: '当前后端还没有接入 Kiro 的浏览器式管理端 OAuth 重新授权流程。你可以先尝试刷新当前已保存的 refresh token；如果上游登录状态已经变化，再打开账号编辑页手动更新凭据。',
        ...
        accessTokenAutoClearHintEdit: '如果你改了刷新令牌、认证方式或 IDC 客户端配置，并且这里留空，保存时会自动清掉旧访问令牌，让后端重新刷新。',
    ...
    settings: {
      ...
      site: {
        ...
        supportQRCodesHint: "支持上传多个客服二维码，可选填写备注，备注会显示在二维码下方。",
        addSupportQRCode: "添加二维码",
        uploadQRCode: "上传二维码",
        supportQRCodeImageHint: "支持 PNG 或 JPG，最大 500KB，建议使用正方形图片。",
        supportQRCodeNote: "备注",
        supportQRCodeNotePlaceholder: "可选，显示在二维码下方",
        docUrl: "文档链接",
      ...
      authSourceDefaults: {
        title: "来源附加授权",
        description:
          "按第三方认证来源配置附加余额、并发和订阅。注册时会在用户默认值基础上叠加，首次绑定开启后也会叠加；不同来源可累计，同一来源只发一次。",
        ...
        grantOnSignupLabel: "注册时叠加授权",
        grantOnSignupHint: "新用户通过该来源注册时，在用户默认值基础上追加发放。",
        grantOnFirstBindLabel: "首次绑定时叠加授权",
        grantOnFirstBindHint:
          "已有账号首次绑定该来源时追加发放；以后重复绑定同一来源不会再获得。",
        bonusBalanceLabel: "附加余额",
        bonusConcurrencyLabel: "附加并发数",
        defaultSubscriptionsLabel: "附加订阅",
        defaultSubscriptionsHint:
          "仅对当前认证来源生效，会和用户默认订阅一起叠加发放。",
  ...
  tickets: {
    ...
    resubmit: "重新提交",
    reply: "发送回复",
    createConversationTitle: "创建工单",
    createConversationSubtitle: "提交后会自动生成系统消息并进入对话流。",
    createSystemMessage: "请在右侧填写工单信息。提交后，系统会自动创建工单并记录提交消息。",
    detailConversationTitle: "工单对话",
    ...
    fields: {
      ...
      orderNo: "订单号",
      expectedAmount: "期望退款金额",
      reason: "退款原因",
      evidence: "证据说明",
      currentConcurrency: "当前并发",
      targetConcurrency: "申请并发",
      usageScenario: "使用场景",
      peakWindow: "峰值时段",
      currentRate: "当前倍率",
      baseRate: "基础倍率",
    ...
    validation: {
      ...
      formIncomplete: "请填写当前类别的必填信息",
    },
    messages: {
      created: "工单创建成功",
      withdrawn: "工单已撤回，可继续修改",
      resubmitted: "工单已重新提交",
      closed: "工单已关闭",
