export default {
  ...
  skills: {
    ...
    admin: {
      ...
      settlement: {
        ...
        replayAction: 'Replay Settlement',
        replaySuccess: 'Settlement replay has been triggered.',
        replayFailed: 'Failed to replay settlement.',
  ...
  usage: {
    ...
    videoUnitPrice: 'Video unit price',
    videoSubtotal: 'Video subtotal',
    cacheRead: 'Read',
    cacheWrite: 'Write',
  ...
  admin: {
    ...
    riskControl: {
      ...
      apiKeyRateLimitPolicy: 'Audit Service Failure Policy',
      apiKeyRateLimitPolicyHint: 'Applied in pre-block mode when the content-moderation audit path cannot complete (no usable audit keys, local key limits, upstream 429/5xx, timeouts, or empty audit results—not only 429). Error blocks the client request; Allow fails open and continues the original request. Prefer Error unless you explicitly accept open-pass during audit outages.',
      apiKeyRateLimitPolicyError: 'Block request (fail closed)',
      apiKeyRateLimitPolicyAllow: 'Allow original request (fail open)',
      ...
      preHashCheckHint: 'Known flagged hashes are checked before moderation. Pre-block mode blocks; observe mode records the hit and allows the request.',
      ...
      hashAction: 'Block type',
      hashCategory: 'Category',
      hashMatchedKeyword: 'Matched keyword',
      hashSource: 'Source (user / model / group)',
      hashFieldEmpty: '—',
      hashCreatedAt: 'Created',
      hashExpiresAt: 'Expires',
      hashHits7d: '7d hits',
      hashHits30d: '30d hits',
      noFlaggedHashes: 'No risk hashes',
      deleteFlaggedHash: 'Delete hash',
      deleteSelectedFlaggedHashes: 'Delete selected ({count})',
      deleteSelectedFlaggedHashesConfirm: 'Delete {count} selected risk hashes?',
      ...
      flaggedHashesLoadFailed: 'Failed to load risk hashes',
      flaggedHashesDeleted: 'Deleted {count} risk hashes',
      ...
      cyberPolicyExcludeBanHint: 'When enabled, cyber_policy hits no longer count toward auto-ban violations: no ban judgment on the hit itself, and history rows are excluded from the rolling count. Logs are still written; cyber_policy notice emails are currently disabled.',
      ...
      blockedKeywordsPlaceholder: 'One keyword or rule per line, e.g.:\nbadword1\nfull risky phrase\naccount && sale\nbypass && risk-control:40',
      blockedKeywordsDescription: 'Matching is case-insensitive. Use && on one line when all terms must co-occur within a "proximity window" (see the default below) before the rule hits. Whether the upstream moderation API is invoked after a hit depends on the strategy below.',
      ...
      defaultProximityWindow: 'Default proximity window (chars)',
      defaultProximityWindowHint: 'Default proximity window used by && rules that do not specify a :N suffix. Range 1–2000. A smaller window is stricter and reduces false positives from two terms that merely co-occur far apart in long text.',
      keywordSyntaxTitle: 'Combination & window syntax',
      keywordSyntaxRuleAnd: '1. Join multiple terms on one line with && — all of them must appear to hit (e.g. "account && sale").',
      keywordSyntaxRuleWindow: '2. An && hit also requires the terms to co-occur within a "proximity window", so terms far apart in long text are not falsely flagged; the default window above is used when unspecified.',
      keywordSyntaxRuleOverride: '3. Append :N to override the window for a single rule, e.g. "bypass && risk-control:40" means a 40-character window (max 2000).',
      keywordSyntaxRuleException: '4. Exception phrases (allowlist): a hit fully contained within a longer exception phrase is skipped, e.g. "ransomware" prevents "ransom" from misfiring.',
      ...
      filters: {
        search: 'Search user/key/current input',
        from: 'From',
        to: 'To',
        allGroups: 'All Groups',
    ...
    accounts: {
      ...
      quotaControl: {
        ...
        tlsFingerprint: {
          ...
          bindingMatrix: 'Per-OS / Client Bindings',
          bindingMatrixHint: 'The router decides which OS/client a request maps to; this matrix decides which profile this account uses for that dimension. Different accounts can bind different profiles.',
          addBinding: 'Add binding',
          noBindings: 'No per-dimension bindings. The router/default profile above is used.',
    ...
    settings: {
      ...
      gatewayForwarding: {
        ...
        claudeTelemetryMode: 'Claude Code Telemetry Handling',
        claudeTelemetryModeDrop: 'Drop directly (default)',
        claudeTelemetryModeForward: 'Sanitize and forward',
        claudeTelemetryModeHint: 'Controls /api/event_logging/batch telemetry requests. Drop returns 200 without upstream forwarding; forward sanitizes and uses only Anthropic OAuth/Setup Token accounts, never API key accounts.',
        grokDefaultBaseURLMode: 'Grok Default Upstream Base URL',
        grokDefaultBaseURLModeHint: 'Affects text inference only (responses/chat) for Grok OAuth accounts without an explicit account base_url. Choose the official API, a regional API, or the Grok Build CLI. Image/video Imagine always uses api.x.ai; usage/billing always uses the CLI proxy. Explicit per-account base_url wins for inference.',
        grokDefaultBaseURLModeAPI: 'Official API (api.x.ai/v1)',
        grokDefaultBaseURLModeUSEast1: 'Regional API (us-east-1.api.x.ai/v1)',
      ...
      site: {
        ...
        downloadToolsUrlPlaceholder: 'https://docs.example.com/cli',
        downloadToolsUrlHint: 'Link to your tool downloads page. Leave empty to hide the download entry.',
        siteLogo: 'Site Logo',
    ...
    tlsFingerprintProfiles: {
      ...
      allPlatforms: 'All platforms',
      allOS: 'All OS',
      allClientTypes: 'All clients',
      filterNamePlaceholder: 'Search name…',
      ...
      form: {
        ...
        clientTypePlaceholder: 'e.g. codex-cli / chatgpt-desktop / claude-code',
        ...
        userAgentPlaceholder: 'e.g. codex_cli_rs/0.125.0 (Ubuntu 22.4.0; x86_64)',
        userAgentHint: 'Optional. Sent as the upstream User-Agent when this template is applied; empty falls back to the built-in default (Codex CLI).',
        ...
        signatureAlgorithmsCert: 'Signature Algorithms Cert',
        ...
        extensionPayloads: 'Extension Payloads (JSON)',
      ...
      capture: {
        ...
        restart: 'Restart',
        delete: 'Delete',
        newTask: 'New capture task',
        backToTasks: 'Back to tasks',
        copyValueSuccess: 'Copied to clipboard',
        copyValueFailed: 'Copy failed',
        ...
        restartSuccess: 'Capture task restarted',
        restartFailed: 'Failed to restart capture task',
        deleteSuccess: 'Capture task deleted',
        deleteFailed: 'Failed to delete capture task',
        deleteConfirmMessage: 'Are you sure you want to delete capture task "{name}"? All associated samples will also be deleted.',
    ...
    tlsFingerprintRouters: {
      ...
      form: {
        ...
        dimensionOS: 'Decided OS',
        dimensionNone: 'None',
        dimensionClientType: 'Decided Client',
        dimensionClientTypePlaceholder: 'e.g. codex-cli (resolved via account matrix)',
