...
describe('useOpenAIOAuth.buildAccountName', () => {
  it('uses manual name first and otherwise prefers email with team workspace', () => {
    const oauth = useOpenAIOAuth()
    expect(oauth.buildAccountName({ email: 'user@example.com', workspace_name: 'Team A' }, ' Manual ')).toBe('Manual')
    expect(oauth.buildAccountName({ email: 'user@example.com', workspace_name: 'Team A', plan_type: 'team' })).toBe(
      'user@example.com (Team A)'
    )
    expect(oauth.buildAccountName({ email: 'user@example.com', workspace_name: 'Personal', plan_type: 'free' })).toBe(
      'user@example.com'
    )
