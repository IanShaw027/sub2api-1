  ...
  <AuthLayout>
    <div class="space-y-6">
      ...
      <div
        ...
        class="rounded-xl border border-warning/25 bg-warning-soft p-4 dark:border-amber-800/50 dark:bg-amber-900/20"
      >
      ...
      <form v-else @submit.prevent="handleRegister" class="space-y-5">
        ...
        <div v-if="invitationCodeEnabled">
          ...
          <div class="relative">
            <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3.5">
              <Icon name="key" size="md" :class="invitationValidation.valid ? 'text-success' : 'text-ink-faint dark:text-dark-500'" />
            </div>
            <input
              id="invitation_code"
            ...
            <div v-if="invitationValidating" class="absolute inset-y-0 right-0 flex items-center pr-3.5">
              <svg class="h-4 w-4 animate-spin text-ink-faint" fill="none" viewBox="0 0 24 24">
          ...
          <transition name="fade">
            <div v-if="invitationValidation.valid" class="mt-2 flex items-center gap-2 rounded-lg bg-success-soft px-3 py-2 dark:bg-green-900/20">
              <Icon name="checkCircle" size="sm" class="text-success dark:text-green-400" />
              <span class="text-sm text-green-800 dark:text-green-400">
        ...
        <div v-if="promoCodeEnabled">
          <label for="promo_code" class="input-label">
            ...
            <span class="ml-1 text-xs font-normal text-ink-faint dark:text-dark-500">({{ t('common.optional') }})</span>
          </label>
          <div class="relative">
            <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3.5">
              <Icon name="gift" size="md" :class="promoValidation.valid ? 'text-success' : 'text-ink-faint dark:text-dark-500'" />
            </div>
            <input
              id="promo_code"
          ...
          <transition name="fade">
            <div v-if="promoValidation.valid" class="mt-2 flex items-center gap-2 rounded-lg bg-success-soft px-3 py-2 dark:bg-green-900/20">
              <Icon name="gift" size="sm" class="text-success dark:text-green-400" />
              <span class="text-sm text-green-800 dark:text-green-400">
...
const siteName = ref<string>('Clomio')
...
onMounted(async () => {
  ...
  try {
    ...
    siteName.value = settings.site_name || 'Clomio'
