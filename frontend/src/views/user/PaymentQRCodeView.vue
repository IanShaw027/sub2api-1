<template>
  <AppLayout>
    <div class="mx-auto flex max-w-md flex-col items-center space-y-6 py-8">
      <h2 class="text-xl font-semibold text-ink dark:text-white">
        {{ qrUrl ? scanTitle : t('payment.qr.payInNewWindow') }}
      </h2>
      <div v-if="qrUrl" class="rounded-card bg-white p-6 shadow-lg dark:bg-dark-800">
        <canvas ref="qrCanvas" class="mx-auto"></canvas>
      </div>
      <!-- Scan prompt for QR code -->
      <p v-if="qrUrl && !expired && scanHint" class="text-center text-sm text-ink-soft dark:text-ink-soft">
        {{ scanHint }}
      </p>
      ...
      <div v-else class="text-center">
        <p class="text-sm text-ink-soft dark:text-ink-soft">{{ qrUrl ? t('payment.qr.expiresIn') : t('payment.qr.payInNewWindowHint') }}</p>
        <p class="mt-1 text-2xl font-bold tabular-nums text-ink dark:text-white">{{ countdownDisplay }}</p>
        <p class="mt-2 text-sm text-ink-faint">{{ t('payment.qr.waitingPayment') }}</p>
      </div>
...
onMounted(() => {
  ...
  payUrl.value = assertPaymentLaunchUrl(String(route.query.pay_url || ''))
