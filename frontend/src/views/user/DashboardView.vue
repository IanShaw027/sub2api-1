...
const loadStats = async () => { loading.value = true; try { await authStore.refreshUser({ touchActive: true }); stats.value = await usageAPI.getDashboardStats() } catch (error) { console.error('Failed to load dashboard stats:', error) } finally { loading.value = false } }
