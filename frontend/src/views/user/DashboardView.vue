  ...
  <AppLayout>
    <div class="space-y-6">
      ...
      <template v-else-if="stats">
        <UserDashboardStats :stats="stats" :balance="user?.balance || 0" :is-simple="authStore.isSimpleMode" :platform-quotas="platformQuotas" :live-rpm-used="liveRpm?.user_rpm_used" :live-rpm-limit="liveRpm?.user_rpm_limit" :current-concurrency="liveRpm?.current_concurrency" />
...
const loadLiveRpm = async () => { try { liveRpm.value = await getMyRPMStatus() } catch (error) { console.warn('Failed to load live RPM:', error) } }
const refreshAll = () => { loadStats(); loadCharts(); loadRecent(); loadPlatformQuotas(); loadLiveRpm() }
