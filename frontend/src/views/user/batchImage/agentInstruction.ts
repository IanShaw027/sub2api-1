/**
 * Builds the "AI agent" system-prompt text shown/copied from the batch-image
 * usage guide modal. Extracted verbatim from BatchImageGuideView.vue
 * (glass-ui-redesign task 12.12) — text and behaviour are unchanged.
 */
export function joinEndpointPath(base: string, path: string): string {
  return `${base.replace(/\/+$/, '')}/${path.replace(/^\/+/, '')}`
}

export function buildAgentInstruction(endpointBase: string): string {
  return `---
