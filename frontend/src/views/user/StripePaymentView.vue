<template>
 <component :is="isPopup ? 'div' : AppLayout" :class="isPopup ? 'min-h-screen bg-background' : ''">
 <div class="stripe-card mx-auto space-y-6 py-8" :class="isPopup ? 'px-4' : ''">
 <div v-if="loading" class="flex items-center justify-center py-20">
 <div class="h-8 w-8 animate-spin rounded-full border-4 border-[var(--accent)] border-t-transparent"></div>
 </div>
 <div v-else-if="initError" class="glass-card p-8 text-center">
 ...
 <h3 class="font-semibold text-lg text-foreground">{{ t('payment.stripeLoadFailed') }}</h3>
 <p class="mt-2 text-sm text-muted">{{ initError }}</p>
 <button class="btn-glass-primary mt-6" @click="router.push('/purchase')">{{ t('payment.result.backToRecharge') }}</button>
 </div>
 <template v-else>
 <!-- 金额头部 -->
 <div v-if="order" class="glass-card overflow-hidden">
 <div class="px-6 py-6 text-center" :style="{ background: STRIPE_BRAND_GRADIENT }">
 ...
 <p class="font-semibold text-lg text-foreground">{{ t('payment.qr.scanWxpay') }}</p>
 <div class="relative rounded-lg border-2 bg-success-50 p-4" :style="{ borderColor: WECHAT_BRAND_COLOR }">
 <img :src="wechatQrUrl" alt="WeChat Pay QR" class="h-56 w-56 rounded" />
 <div class="pointer-events-none absolute inset-0 flex items-center justify-center">
 <span class="rounded-full p-2 shadow ring-2 ring-white" :style="{ background: WECHAT_BRAND_COLOR }">
 ...
 <p class="text-center text-sm text-muted">{{ t('payment.qr.scanWxpayHint') }}</p>
 </div>
 </div>
 <div class="glass-card p-4 text-center">
 <p class="text-sm text-muted">{{ t('payment.qr.waitingPayment') }}</p>
 </div>
 </template>
 ...
 <div class="h-10 w-10 animate-spin rounded-full border-4 border-t-transparent" :style="{ borderColor: ALIPAY_BRAND_COLOR, borderTopColor: 'transparent' }"></div>
 <p class="text-sm text-muted">{{ t('payment.qr.payInNewWindowHint') }}</p>
 </div>
 </div>
 </template>
 ...
 <p class="text-sm text-muted">{{ t('payment.stripeSuccessProcessing') }}</p>
 </div>
 </div>
 </template>
 ...
 <p v-if="stripeError" class="mt-4 text-sm text-danger-text ">{{ stripeError }}</p>
 ...
 <button class="btn-glass-secondary" @click="router.push('/purchase')">{{ t('payment.result.backToRecharge') }}</button>
 </div>
 </template>

 <!-- 错误状态 -->
 <div v-if="stripeError && !showPaymentElement" class="glass-card p-4">
 <p class="text-sm text-danger-text ">{{ stripeError }}</p>
 <button class="btn-glass-secondary mt-3 w-full" @click="router.push('/purchase')">{{ t('payment.result.backToRecharge') }}</button>
 </div>
 </template>
 </div>
 </component>
...
import { ALIPAY_BRAND_COLOR, WECHAT_BRAND_COLOR, STRIPE_BRAND_GRADIENT } from '@/components/payment/paymentBrandColors'
