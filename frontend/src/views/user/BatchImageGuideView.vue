<template>
  <AppLayout>
    <TablePageLayout>
      <template #filters>
        <div class="flex flex-col gap-3">
          ...
          <div
            ...
            class="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-line bg-card px-3 py-2 shadow-xs dark:border-dark-700 dark:bg-dark-800"
          >
            <span class="text-sm text-ink-body dark:text-dark-300">
              已选择 <span class="font-medium text-ink dark:text-white">{{ selectedJobIds.size }}</span> 个任务
            </span>
        ...
        >
          ...
          <template #cell-id="{ row }">
	            <div class="flex w-[220px] items-start gap-1" :class="row.is_child ? 'pl-6' : ''">
	              <button
	                ...
	                type="button"
	                class="mt-1 flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-md text-ink-soft transition-colors hover:bg-page hover:text-ink focus:outline-none focus-visible:ring-2 focus-visible:ring-accent/25 dark:hover:bg-dark-700 dark:hover:text-white"
	              ...
	              <button type="button" class="min-w-0 flex-1 rounded-lg py-1 text-left transition-colors hover:bg-page focus:outline-none focus-visible:ring-2 focus-visible:ring-accent/25 dark:hover:bg-dark-700" @click="selectJob(row.id)">
	                <span
	                  class="flex min-w-0 items-center gap-2 text-sm font-medium"
	                  :class="row.task_name ? 'text-ink dark:text-white' : 'text-ink-soft dark:text-dark-400'"
                >
                  <span class="min-w-0 truncate">{{ row.task_name || defaultTaskName(row.created_at) }}</span>
                  <span v-if="row.child_count > 0 && !row.is_child" class="flex-shrink-0 rounded-full bg-line px-2 py-0.5 text-xs font-normal text-ink-body dark:bg-dark-700">
                    {{ row.child_count }} 子任务
                  </span>
                  <span v-if="row.is_child" class="flex-shrink-0 rounded-full bg-amber-50 px-2 py-0.5 text-xs font-normal text-amber-700 dark:bg-amber-900/20 dark:text-amber-300">
	                ...
	                <span class="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-ink-soft dark:text-dark-400">
          ...
          <template #cell-model="{ row }">
	            <div class="mx-auto max-w-[180px] text-center">
	              <p class="truncate text-sm text-ink-body dark:text-dark-300" :title="row.model">{{ row.model }}</p>
	            </div>
	          </template>

          <template #cell-api_key_name="{ value }">
            <span class="block truncate text-center text-sm text-ink-body dark:text-dark-300">
              {{ value || '未记录' }}
            </span>
          </template>
          ...
          <template #cell-counts="{ row }">
            <div class="flex items-center justify-center gap-2 text-sm tabular-nums">
              ...
              <span :class="displayJob(row).fail_count > 0 ? 'text-red-600 dark:text-red-300' : 'text-ink-faint dark:text-dark-500'">{{ displayJob(row).fail_count }}</span>
              <span class="text-xs text-ink-faint dark:text-dark-500">共 {{ displayJob(row).item_count }}</span>
            </div>
          </template>

          <template #cell-cost="{ row }">
            <span class="block text-center text-sm text-ink-body dark:text-dark-300">
              {{ costLabel(displayJob(row)) }}
            </span>
          </template>

          <template #cell-downloaded="{ row }">
            <span class="block text-center text-sm" :class="row.downloaded_at ? 'text-emerald-700 dark:text-emerald-300' : 'text-ink-soft dark:text-dark-400'">
              ...
              <button
                type="button"
                class="batch-row-action flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-ink-soft transition-colors hover:bg-page hover:text-brand-600 focus:outline-none focus-visible:ring-2 focus-visible:ring-accent/25 dark:hover:bg-dark-700 dark:hover:text-brand-400"
                title="查看详情"
              ...
              <button
                type="button"
                class="batch-row-action flex flex-col items-center gap-0.5 rounded-lg p-1.5 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-accent/25"
                :class="canDownload(row) ? 'text-ink-soft hover:bg-green-50 hover:text-green-600 dark:hover:bg-green-900/20 dark:hover:text-green-400' : 'text-ink-faint dark:text-dark-500'"
              ...
              <div v-if="canRetry(row) || canDeleteRecord(row)">
                <button
                  type="button"
                  class="batch-row-action flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-ink-soft transition-colors hover:bg-page hover:text-ink focus:outline-none focus-visible:ring-2 focus-visible:ring-accent/25 dark:hover:bg-dark-700 dark:hover:text-white"
                  :class="{ 'bg-page text-ink dark:bg-dark-700 dark:text-white': openMoreJobId === row.id }"
                  title="更多操作"
          ...
          <template #empty>
            <div class="flex min-h-[260px] flex-col items-center justify-center py-6 md:min-h-[300px]">
              <Icon name="sparkles" size="xl" class="mb-4 h-12 w-12 text-ink-faint dark:text-dark-500" />
              <p class="text-lg font-medium text-ink dark:text-ink">暂无批量任务</p>
      ...
      <template #pagination>
        <div
          ...
          class="flex flex-col gap-3 border-t border-line bg-card px-4 py-3 dark:border-dark-700 dark:bg-dark-800 sm:flex-row sm:items-center sm:justify-between sm:px-6"
        >
          <div class="flex flex-wrap items-center gap-3 text-sm text-ink-body dark:text-dark-300">
            <span>
    ...
    <Teleport to="body">
      <div
        v-if="openMoreJobId"
        class="fixed z-[9999] w-44 overflow-hidden rounded-xl bg-card py-1 text-sm shadow-lg ring-1 ring-black/5 dark:bg-dark-800 dark:ring-white/10"
        :style="moreMenuStyle"
        @click.stop
      >
        <template v-for="job in batchJobs" :key="job.id">
          <template v-if="job.id === openMoreJobId">
            <button
              v-if="canRetry(job)"
              type="button"
              class="flex w-full items-center gap-2 px-3 py-2 text-left text-ink-body transition-colors hover:bg-amber-50 hover:text-amber-700 disabled:opacity-60 dark:text-dark-200 dark:hover:bg-amber-900/20 dark:hover:text-amber-300"
    ...
    <Teleport to="body">
      <div
        ...
        class="batch-prompt-popover fixed z-[9999] rounded-lg border border-line bg-card p-3 text-sm text-ink shadow-xl ring-1 ring-black/5 dark:border-dark-700 dark:bg-dark-900 dark:text-ink dark:ring-white/10"
        :style="promptPopover.style"
        @mouseenter="cancelPromptPopoverClose"
        @mouseleave="schedulePromptPopoverClose"
      >
        <div class="mb-2 flex items-center justify-between gap-3">
          <span class="text-xs font-medium text-ink-soft dark:text-dark-400">完整 Prompt</span>
          <button
            type="button"
            class="rounded-md px-2 py-1 text-xs font-medium text-brand-600 transition-colors hover:bg-brand-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-accent/25 dark:text-brand-300 dark:hover:bg-brand-900/20"
            @click="copyPromptPopover"
          >
            复制
          </button>
        </div>
        <p class="max-h-48 overflow-y-auto whitespace-pre-wrap break-words leading-6 selection:bg-brand-100 selection:text-brand-900 dark:selection:bg-brand-900/60 dark:selection:text-brand-100">
          {{ promptPopover.text }}
        </p>
      </div>
    </Teleport>
    ...
    <BaseDialog :show="!!currentJob" title="任务详情" width="extra-wide" @close="closeDetail">
      <div v-if="currentJob" class="space-y-4">
        <div class="rounded-lg border border-line bg-page/70 px-4 py-3 dark:border-dark-700 dark:bg-dark-900/40">
          <div class="grid gap-x-6 gap-y-3 sm:grid-cols-2 lg:grid-cols-4">
            <div class="min-w-0 text-center">
              <p class="text-xs text-ink-soft dark:text-dark-400">状态</p>
            ...
            <div class="min-w-0 text-center">
              <p class="text-xs text-ink-soft dark:text-dark-400">{{ hasChildJobs(currentJob.id) ? '汇总结果' : '结果' }}</p>
              ...
              <span class="text-ink-faint dark:text-dark-500">/</span>
              <span :class="(currentDisplayJob || currentJob).fail_count > 0 ? 'text-red-600 dark:text-red-300' : 'text-ink-faint dark:text-dark-500'">{{ (currentDisplayJob || currentJob).fail_count }}</span>
            </p>
            </div>
            <div class="min-w-0 text-center">
              <p class="text-xs text-ink-soft dark:text-dark-400">费用</p>
              <p class="mt-1 truncate font-medium text-ink dark:text-white">{{ costLabel(currentDisplayJob || currentJob) }}</p>
            </div>
            <div class="min-w-0 text-center">
              <p class="text-xs text-ink-soft dark:text-dark-400">下载状态</p>
              <p class="mt-1 truncate font-medium text-ink dark:text-white">
        ...
        <div class="flex flex-wrap items-center justify-between gap-3">
          <h3 class="text-sm font-semibold text-ink dark:text-white">明细</h3>
        ...
        <div v-if="items.length" class="overflow-x-auto rounded-lg border border-line bg-card dark:border-dark-700 dark:bg-dark-900">
          <table class="w-full min-w-[860px] table-fixed divide-y divide-line text-sm dark:divide-dark-700">
            <colgroup>
              <col class="w-[18%]" />
              <col class="w-[34%]" />
              <col class="w-[12%]" />
            ...
            <thead class="bg-page dark:bg-dark-800/80">
              <tr>
                <th class="px-3 py-3 text-center text-sm font-medium text-ink-soft dark:text-dark-400">Custom ID</th>
                <th class="px-3 py-3 text-left text-sm font-medium text-ink-soft dark:text-dark-400">Prompt</th>
                <th class="px-3 py-3 text-center text-sm font-medium text-ink-soft dark:text-dark-400">状态</th>
                <th class="px-3 py-3 text-center text-sm font-medium text-ink-soft dark:text-dark-400">预览</th>
                <th class="px-3 py-3 text-center text-sm font-medium text-ink-soft dark:text-dark-400">结果</th>
              </tr>
            </thead>
              ...
              >
                <td class="px-3 py-2.5 text-center">
                  <span
                    ...
                    :class="isRecoveredOriginalFailure(item) ? 'text-ink-faint dark:text-dark-500' : 'text-ink dark:text-white'"
                    :title="item.custom_id"
                  >
                    {{ item.custom_id }}
                  </span>
                </td>
                <td class="px-3 py-2.5 text-left" :class="isRecoveredOriginalFailure(item) ? 'text-ink-faint dark:text-dark-500' : 'text-ink-body dark:text-dark-300'">
                  <div
                ...
                <td class="px-3 py-2.5 text-center">
                  <div class="mx-auto h-12 w-12 overflow-hidden rounded-md border border-line bg-page dark:border-dark-700 dark:bg-dark-800">
                    <button
                    ...
                    <button
                      ...
                      type="button"
                      class="flex h-full w-full items-center justify-center text-ink-soft transition-colors hover:bg-page hover:text-brand-600 disabled:cursor-wait disabled:opacity-70 dark:hover:bg-dark-700"
                    ...
                    <div v-else class="flex h-full w-full items-center justify-center text-ink-faint" :title="item.image_count > 0 ? '不可预览' : '无图片'">
        ...
        <div v-else class="rounded-lg border border-dashed border-line py-10 text-center dark:border-dark-700">
          <Icon name="refresh" size="lg" class="mx-auto mb-3 text-ink-faint" :class="loadingItems ? 'animate-spin' : ''" />
          <p class="text-sm font-medium text-ink-body dark:text-dark-200">
            {{ loadingItems ? '正在加载明细...' : '暂无明细' }}
          </p>
          <p v-if="!loadingItems" class="mt-1 text-sm text-ink-soft dark:text-dark-400">
    ...
    <BaseDialog :show="!!previewImageItem" :title="previewImageItem?.custom_id || '图片预览'" width="extra-wide" :z-index="60" @close="closeImagePreview">
      <div class="space-y-3">
        ...
        <div class="flex min-h-[420px] items-center justify-center rounded-lg bg-page p-4 dark:bg-dark-900">
          <img
      ...
      <form class="space-y-5" @submit.prevent="submitJob">
        <div class="grid gap-4 md:grid-cols-2">
          ...
          <div>
            ...
            <div class="input flex items-center bg-page text-ink-body dark:bg-dark-900">
              1K
            </div>
        ...
        <div class="space-y-3">
          <div class="flex items-center justify-between gap-3">
            ...
            <span class="text-xs text-ink-soft dark:text-dark-400">已添加 {{ promptRows.length }} 条</span>
          </div>
          <div class="rounded-lg border border-line p-3 dark:border-dark-700">
            <textarea
              v-model="promptDraft"
              rows="3"
              class="h-[76px] w-full resize-y rounded-md border border-line px-3 py-2 text-sm leading-5 outline-none focus:border-accent focus:ring-2 focus:ring-accent/25 dark:border-dark-600 dark:bg-dark-900 dark:text-ink dark:focus:border-accent dark:focus:ring-accent/25"
            ...
            <div v-if="referenceImageDrafts.length" class="mt-3 flex flex-wrap gap-2">
              <span
                ...
                class="inline-flex max-w-full items-center gap-1 rounded-md border border-line bg-page px-2 py-1 text-xs text-ink-body dark:border-dark-700 dark:bg-dark-900 dark:text-dark-200"
              >
                <span class="max-w-[180px] truncate">{{ ref.name }}</span>
                <button type="button" class="text-ink-faint hover:text-red-600" title="移除参考图" @click="removeReferenceImageDraft(refIndex)">
                  <Icon name="x" size="xs" />
                </button>
              </span>
            </div>
          ...
          <div v-if="promptRows.length" class="overflow-hidden rounded-lg border border-line dark:border-dark-700">
            <div
              v-for="(row, index) in promptRows"
              :key="row.localId"
              class="flex items-center gap-3 border-b border-line px-3 py-2 last:border-b-0 dark:border-dark-700"
            >
              <span class="w-20 flex-shrink-0 font-mono text-xs text-ink-soft dark:text-dark-400">{{ row.custom_id }}</span>
              <p class="min-w-0 flex-1 truncate text-sm text-ink dark:text-ink">{{ row.prompt }}</p>
              <span v-if="row.output_count > 1" class="flex-shrink-0 text-xs text-ink-soft dark:text-dark-400">
                x{{ row.output_count }}
              </span>
              <span v-if="row.reference_images.length" class="flex-shrink-0 text-xs text-ink-soft dark:text-dark-400">
          ...
          <div v-else class="rounded-lg border border-dashed border-line px-3 py-6 text-center text-sm text-ink-soft dark:border-dark-700">
            还没有添加 prompt。
          </div>
        </div>
    ...
    <BaseDialog :show="showGuideModal" title="批量生图使用说明" width="wide" @close="showGuideModal = false">
	      <div class="space-y-5">
	        <section class="space-y-3">
	          <h3 class="text-sm font-semibold text-ink dark:text-white">当前界面如何使用</h3>
	          <div class="rounded-lg border border-line bg-page p-3 text-sm leading-6 text-ink-body dark:border-dark-700 dark:bg-dark-900/50 dark:text-dark-200">
	        ...
	        <section class="space-y-3">
	          <div class="flex flex-wrap items-center justify-between gap-3">
	            <h3 class="text-sm font-semibold text-ink dark:text-white">给 Codex 的 Skill 说明</h3>
	            <p class="text-xs text-ink-soft dark:text-dark-400">用于告诉 Codex 如何代替用户整理 prompt、提交任务和下载结果。</p>
	          </div>
	        <textarea
	          :value="agentInstruction"
	          readonly
	          class="min-h-[420px] w-full resize-y rounded-md border border-line bg-page p-4 font-mono text-sm leading-6 text-ink outline-none focus:border-accent focus:ring-2 focus:ring-accent/25 dark:border-dark-600 dark:bg-dark-900 dark:text-ink dark:focus:border-accent dark:focus:ring-accent/25"
	        />
	        </section>
	      </div>
      <template #footer>
...
function detailItemRowClass(item: BatchImageDetailItem) {
  if (isRecoveredOriginalFailure(item)) {
    return 'bg-page/80 text-ink-faint hover:bg-line/80 dark:bg-dark-900/60 dark:text-dark-500 dark:hover:bg-dark-800/70'
  }
  return 'hover:bg-page/70 dark:hover:bg-dark-800/60'
}
...
function itemResultClass(item: BatchImageDetailItem) {
  if (isRecoveredOriginalFailure(item)) return 'bg-page text-ink-soft ring-line dark:bg-dark-800 dark:text-dark-400 dark:ring-dark-700'
  ...
  return 'bg-page text-ink-soft ring-line dark:bg-dark-800 dark:text-dark-400 dark:ring-dark-700'
}
