<template>
 ...
 <button type="button" class="btn-glass-secondary" :disabled="loadingJobs" @click="resetFilters">
 {{ t('common.reset') }}
 </button>
 <button type="button" class="btn-glass-secondary" :disabled="loadingKeys || loadingJobs" :title="t('common.refresh')" @click="refreshPage">
 <Icon name="refresh" size="md" :class="loadingKeys || loadingJobs ? 'animate-spin' : ''" />
 </button>
 <button type="button" class="btn-glass-secondary" @click="showGuideModal = true">
 <Icon name="book" size="md" class="mr-2" />
 {{ t('batchImage.actions.usageGuide') }}
 </button>
 <button type="button" class="btn-glass-primary" @click="openCreateModal">
 ...
 >
 <template #count>
 <span class="font-medium text-foreground">{{ selectedJobIds.size }}</span>
 </template>
 </i18n-t>
 ...
 <template #cell-id="{ row }">
	 ...
	 <button type="button" class="min-w-0 flex-1 rounded-lg py-1 text-left transition-colors hover:bg-surface-2 focus:outline-none focus-visible:ring-2 focus-visible:ring-[color-mix(in_oklch,var(--accent)_30%,transparent)]" @click="selectJob(row.id)">
	 <span
 ...
 <button
 type="button"
 class="batch-row-action flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-muted transition-colors hover:bg-surface-2 hover:text-accent focus:outline-none focus-visible:ring-2 focus-visible:ring-[color-mix(in_oklch,var(--accent)_30%,transparent)]"
 ...
 <button
 type="button"
 class="batch-row-action flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-muted transition-colors hover:bg-surface-2 hover:text-foreground focus:outline-none focus-visible:ring-2 focus-visible:ring-[color-mix(in_oklch,var(--accent)_30%,transparent)]"
 ...
 class="flex flex-col gap-3 border-t border-line bg-surface px-4 py-3 sm:flex-row sm:items-center sm:justify-between sm:px-6"
 >
 ...
 <button type="button" class="btn-glass-secondary btn-sm" :disabled="refreshing || loadingItems" @click="refreshDetail">
 ...
 <div class="flex justify-end gap-3">
	 <button type="button" class="btn-glass-secondary" :disabled="!currentJob || !canCancel(currentJob) || cancelling" @click="cancelSelected">
 ...
 <button type="button" class="btn-glass-secondary h-9 justify-center whitespace-nowrap px-4 text-sm" :disabled="!promptDraft.trim()" @click="addPromptRow">
 ...
 <template #footer>
 <div class="flex justify-end gap-3">
 <button type="button" class="btn-glass-secondary" :disabled="submitting" @click="closeCreateModal">{{ t('common.cancel') }}</button>
	 <button type="button" class="btn-glass-primary inline-flex min-w-[120px] justify-center" :disabled="submitting || loadingModels || (parsedItems.length === 0 && !promptDraft.trim()) || !selectedApiKey || !form.model" @click="submitJob">
 ...
 <BaseDialog :show="showGuideModal" :title="t('batchImage.guide.title')" width="wide" @close="showGuideModal = false">
	 ...
	 <h3 class="text-sm font-semibold text-foreground">{{ t('batchImage.guide.uiTitle') }}</h3>
	 <div class="rounded-lg border border-line bg-surface-2 p-3 text-sm leading-6 text-foreground ">
	 ...
	 <section class="space-y-3">
	 <div class="flex flex-wrap items-center justify-between gap-3">
	 <h3 class="text-sm font-semibold text-foreground">{{ t('batchImage.guide.skillTitle') }}</h3>
	 <p class="text-xs text-muted">{{ t('batchImage.guide.skillDesc') }}</p>
	 </div>
	 <textarea
	 :value="agentInstruction"
	 readonly
	 class="min-h-[420px] w-full resize-y rounded-md border border-line bg-surface-2 p-4 font-mono text-sm leading-6 text-foreground outline-none focus:border-[var(--accent)] focus:ring-2 focus:ring-[color-mix(in_oklch,var(--accent)_18%,transparent)]"
	 />
	 </section>
	 </div>
 <template #footer>
 <div class="flex justify-end gap-3">
 <button type="button" class="btn-glass-secondary" @click="showGuideModal = false">{{ t('common.close') }}</button>
 <button type="button" class="btn-glass-primary" @click="copyInstruction">
