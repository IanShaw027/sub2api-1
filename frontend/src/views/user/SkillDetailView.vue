      ...
      <div v-if="skill" class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
        <section class="space-y-6">
          <div class="card overflow-hidden">
            <div class="border-b border-gray-200 bg-gradient-to-br from-white via-cyan-50 to-emerald-50 px-6 py-6 dark:border-dark-700 dark:from-dark-900 dark:via-dark-950 dark:to-emerald-950/20">
              <div class="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                <div class="min-w-0">
                  <div class="flex flex-wrap items-center gap-2">
                    <span class="rounded-full bg-slate-900 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-[0.25em] text-white">
                  ...
                  <h1 class="mt-3 text-3xl font-bold text-gray-900 dark:text-white">{{ skill.name }}</h1>
                  <p class="mt-2 text-base text-gray-600 dark:text-gray-300">{{ skill.tagline || skill.description }}</p>
                </div>
                <div class="flex flex-col gap-3 lg:items-end">
                  <div
                    v-if="actionVersion"
                    class="rounded-2xl border border-white/70 bg-white/80 px-4 py-3 shadow-sm backdrop-blur dark:border-dark-700 dark:bg-dark-900/75"
                  >
                    <p class="text-[11px] font-medium uppercase tracking-[0.25em] text-gray-500 dark:text-gray-400">
                      {{ t('skills.detail.actionVersion', '操作版本') }}
                    </p>
                    <div class="mt-2 flex flex-wrap items-center gap-2">
                      <span class="text-sm font-semibold text-gray-900 dark:text-white">{{ actionVersion.version }}</span>
            ...
            <div class="grid gap-6 px-6 py-6 lg:grid-cols-2">
              <div class="space-y-4">
                <div class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-dark-700 dark:bg-dark-900">
                  <h2 class="text-sm font-semibold text-gray-900 dark:text-white">{{ t('skills.detail.about', '技能说明') }}</h2>
                  <p class="mt-3 whitespace-pre-wrap text-sm leading-6 text-gray-600 dark:text-gray-400">{{ skill.description }}</p>
                </div>

                <div v-if="skill.readme" class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-dark-700 dark:bg-dark-900">
                  <h2 class="text-sm font-semibold text-gray-900 dark:text-white">{{ t('skills.detail.readme', '使用说明') }}</h2>
                  <p class="mt-3 whitespace-pre-wrap text-sm leading-6 text-gray-600 dark:text-gray-400">{{ skill.readme }}</p>
                </div>

                <div v-if="showSource" class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-dark-700 dark:bg-dark-900">
                  <h2 class="text-sm font-semibold text-gray-900 dark:text-white">{{ t('skills.detail.sourcePreview', '源内容预览') }}</h2>
                  <div class="mt-3 space-y-3">
                    <template v-if="skill.content?.type === 'prompt_chat'">
                      <pre class="overflow-x-auto rounded-xl bg-gray-50 p-4 text-xs leading-6 text-gray-700 dark:bg-dark-950 dark:text-gray-200">{{ skill.content.system_prompt }}</pre>
                      <pre class="overflow-x-auto rounded-xl bg-gray-50 p-4 text-xs leading-6 text-gray-700 dark:bg-dark-950 dark:text-gray-200">{{ skill.content.user_prompt_template }}</pre>
                    </template>
                    <template v-else-if="skill.content?.type === 'prompt_image'">
                      <pre class="overflow-x-auto rounded-xl bg-gray-50 p-4 text-xs leading-6 text-gray-700 dark:bg-dark-950 dark:text-gray-200">{{ skill.content.prompt_template }}</pre>
                      <pre
                        v-if="skill.content.negative_prompt_template"
                        class="overflow-x-auto rounded-xl bg-gray-50 p-4 text-xs leading-6 text-gray-700 dark:bg-dark-950 dark:text-gray-200"
                      >{{ skill.content.negative_prompt_template }}</pre>
                    </template>
                    <template v-else-if="skill.content?.type === 'script'">
                      <pre class="overflow-x-auto rounded-xl bg-gray-50 p-4 text-xs leading-6 text-gray-700 dark:bg-dark-950 dark:text-gray-200">{{ skill.content.source_code }}</pre>
                    </template>
                  </div>
                </div>
                ...
                <div
                  v-else
                  class="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800 dark:border-amber-900/40 dark:bg-amber-900/20 dark:text-amber-100"
                >
              ...
              <div class="space-y-4">
                <div class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-dark-700 dark:bg-dark-900">
                  <h2 class="text-sm font-semibold text-gray-900 dark:text-white">{{ t('skills.detail.exposedVariables', '可配置变量') }}</h2>
                  <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ t('skills.detail.exposedVariablesHint', '这些变量会在运行或调用前展示给用户填写。') }}</p>
                  <div class="mt-4">
                    <SkillVariableForm v-model="variableValues" :schema="skill.variable_schema" />
                  </div>
                </div>

                <div v-if="skill.install_note" class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-dark-700 dark:bg-dark-900">
                  <h2 class="text-sm font-semibold text-gray-900 dark:text-white">{{ t('skills.detail.installNote', '安装说明') }}</h2>
                  <p class="mt-3 whitespace-pre-wrap text-sm leading-6 text-gray-600 dark:text-gray-400">{{ skill.install_note }}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <aside class="space-y-4">
          <div class="card p-5">
            <h2 class="text-sm font-semibold text-gray-900 dark:text-white">{{ t('skills.detail.meta', '元信息') }}</h2>
            <dl class="mt-4 space-y-3 text-sm">
              <div class="flex items-center justify-between gap-3">
                <dt class="text-gray-500 dark:text-gray-400">{{ t('common.name', '名称') }}</dt>
                <dd class="text-right font-medium text-gray-900 dark:text-white">{{ skill.author.name || '-' }}</dd>
              </div>
              <div class="flex items-center justify-between gap-3">
                <dt class="text-gray-500 dark:text-gray-400">{{ t('skills.market.category', '分类') }}</dt>
                <dd class="text-right font-medium text-gray-900 dark:text-white">{{ skill.category || '-' }}</dd>
              </div>
              <div class="flex items-center justify-between gap-3">
                <dt class="text-gray-500 dark:text-gray-400">{{ t('skills.detail.latestVersion', '当前版本') }}</dt>
                <dd class="text-right font-medium text-gray-900 dark:text-white">{{ skill.latest_version?.version || '-' }}</dd>
              </div>
              <div class="flex items-center justify-between gap-3">
                <dt class="text-gray-500 dark:text-gray-400">{{ t('skills.card.installs', '安装') }}</dt>
                <dd class="text-right font-medium text-gray-900 dark:text-white">{{ skill.stats.installs }}</dd>
              </div>
              <div class="flex items-center justify-between gap-3">
                <dt class="text-gray-500 dark:text-gray-400">{{ t('skills.card.runs', '运行') }}</dt>
                <dd class="text-right font-medium text-gray-900 dark:text-white">{{ skill.stats.runs }}</dd>
              </div>
              <div class="flex items-center justify-between gap-3">
                <dt class="text-gray-500 dark:text-gray-400">{{ t('skills.revenue.title', '收益') }}</dt>
                <dd class="text-right font-medium text-gray-900 dark:text-white">{{ formatCurrency(skill.stats.revenue, skill.pricing.currency) }}</dd>
              </div>
            </dl>
          </div>
          ...
          <div class="card p-5">
            ...
            <div class="space-y-3">
              <div
                ...
                :key="version.id"
                class="rounded-2xl border border-gray-200 p-3 dark:border-dark-700"
              >
                <div class="flex items-center justify-between gap-3">
                  <div>
                    <p class="font-medium text-gray-900 dark:text-white">{{ version.version }}</p>
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ version.changelog || t('skills.versions.noChangelog', '暂无变更说明') }}</p>
                  </div>
