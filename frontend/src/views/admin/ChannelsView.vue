  ...
  <AppLayout>
      ...
      <template #table>
        ...
        >
          ...
          <template #cell-description="{ value }">
            <span class="text-sm text-ink-soft dark:text-dark-400">{{ value || '-' }}</span>
          </template>
          ...
          <template #cell-group_count="{ row }">
            <span
              class="inline-flex items-center rounded-chip bg-page px-2 py-0.5 text-xs font-medium text-ink-body dark:bg-dark-600 dark:text-dark-200"
            >
    ...
    >
      <div class="channel-dialog-body">
        <!-- Tab Bar -->
        <div class="flex items-center border-b border-line dark:border-dark-700 flex-shrink-0 -mx-4 sm:-mx-6 px-4 sm:px-6 -mt-3 sm:-mt-4">
          <!-- Basic Settings Tab -->
          <button
            type="button"
        ...
        <form id="channel-form" @submit.prevent="handleSubmit" class="flex-1 overflow-y-auto pt-4">
          ...
          <div v-show="activeTab === 'basic'" class="space-y-5">
            ...
            <div>
              ...
              <p class="mt-1 ml-6 text-xs text-ink-faint">
              ...
              <div class="flex flex-wrap gap-2">
                <label
                  ...
                  :class="activePlatforms.includes(p)
                    ...
                    : 'border-line hover:bg-page dark:border-dark-600 dark:hover:bg-dark-700'"
                >
                  <input
                    type="checkbox"
                    :checked="activePlatforms.includes(p)"
                    class="h-3.5 w-3.5 rounded border-line text-brand-600 focus:ring-accent/25"
            ...
            <div class="border-t border-line pt-4 dark:border-dark-700">
              <div class="flex items-center justify-between">
                <div>
                  <label class="text-sm font-medium text-ink-body dark:text-dark-200">
          ...
          >
            ...
            <div>
              <label class="input-label text-xs">
                ...
                <span v-if="section.group_ids.length > 0" class="ml-1 font-normal text-ink-faint">
                  ({{ t('admin.channels.form.selectedCount', { count: section.group_ids.length }) }})
                </span>
              </label>
              <div class="max-h-40 overflow-auto rounded-lg border border-line bg-page p-2 dark:border-dark-600 dark:bg-dark-900">
                <div v-if="groupsLoading" class="py-2 text-center text-xs text-ink-soft">
                  {{ t('common.loading', 'Loading...') }}
                </div>
                <div v-else-if="getGroupsForPlatform(section.platform).length === 0" class="py-2 text-center text-xs text-ink-soft">
                ...
                <div v-else class="flex flex-wrap gap-1">
                  <label
                    ...
                    :key="group.id"
                    class="inline-flex cursor-pointer items-center gap-1.5 rounded-md border border-line px-2 py-1 text-xs transition-colors hover:bg-page dark:border-dark-600 dark:hover:bg-dark-700"
                    :class="[
                      section.group_ids.includes(group.id) ? 'bg-brand-50 border-brand-300 dark:bg-brand-900/20 dark:border-brand-700' : '',
                  ...
                  >
                    <input
                      ...
                      class="h-3 w-3 rounded border-line text-brand-600 focus:ring-accent/25"
                    ...
                    <span class="text-[10px] text-ink-faint">{{ group.account_count || 0 }}</span>
                    <span
            ...
            <div v-if="section.platform === 'anthropic' && webSearchGlobalEnabled" class="border-t border-line pt-3 dark:border-dark-600">
              <div class="flex items-center justify-between">
                <div>
                  <label class="text-xs font-medium text-ink-body dark:text-dark-200">
            ...
            <div v-if="section.platform === 'openai'" class="border-t border-line pt-3 dark:border-dark-600">
            ...
            <div>
              <div class="mb-1 flex items-center justify-between">
                ...
                <button type="button" @click="addMappingEntry(sIdx)" class="text-xs text-brand-600 hover:text-brand-700">
                  + {{ t('common.add', 'Add') }}
                </button>
              </div>
              <div
                v-if="Object.keys(section.model_mapping).length === 0"
                class="rounded border border-dashed border-line p-2 text-center text-xs text-ink-faint dark:border-dark-500"
              >
              ...
              <div v-else class="space-y-1">
                ...
                >
                  ...
                  <button
                    ...
                    class="rounded p-0.5 text-ink-faint hover:text-red-500"
                  >
                    <Icon name="trash" size="sm" />
                  </button>
                </div>
            ...
            <div>
              <div class="mb-1 flex items-center justify-between">
                ...
                <button type="button" @click="addPricingEntry(sIdx)" class="text-xs text-brand-600 hover:text-brand-700">
                  + {{ t('common.add', 'Add') }}
                </button>
              </div>
              <div
            ...
            <div class="mt-4 border-t border-line pt-4 dark:border-dark-700 space-y-3">
              <div class="flex items-center justify-between">
                <h4 class="text-sm font-medium text-ink-body dark:text-dark-200">
                  {{ t('admin.channels.form.accountStatsPricingRules') }}
                </h4>
                <button
                  type="button"
                  @click="addAccountStatsRule(sIdx)"
                  class="rounded-lg border border-brand-300 px-3 py-1 text-xs font-medium text-brand-600 hover:bg-brand-50 dark:border-brand-600 dark:text-brand-400 dark:hover:bg-brand-900/20"
                >
              ...
              <p
                ...
                class="text-xs italic text-ink-faint dark:text-dark-500"
              >
              ...
              <div
                ...
                :key="ruleIndex"
                class="space-y-3 rounded-lg border border-line p-4 dark:border-dark-600"
              >
                <div class="flex items-center justify-between">
                  <input
                    v-model="rule.name"
                    :placeholder="t('admin.channels.form.ruleName')"
                    class="bg-transparent text-sm font-medium text-ink-body placeholder:text-ink-faint outline-none dark:text-dark-200"
                  />
                ...
                <div>
                  <label class="text-xs text-ink-soft dark:text-dark-400">{{ t('admin.channels.form.ruleGroups') }}</label>
                  <div class="mt-1 flex flex-wrap gap-1">
                    <label
                      ...
                      :class="rule.group_ids.includes(gid)
                        ...
                        : 'border-line hover:bg-page dark:border-dark-600 dark:hover:bg-dark-700'"
                    >
                      <input type="checkbox" :checked="rule.group_ids.includes(gid)" class="h-3 w-3 rounded border-line text-brand-600 focus:ring-accent/25" @change="rule.group_ids.includes(gid) ? rule.group_ids.splice(rule.group_ids.indexOf(gid), 1) : rule.group_ids.push(gid)" />
                      <span :class="['font-medium', platformTextClass(section.platform)]">{{ getGroupNameById(gid) }}</span>
                    </label>
                  </div>
                  <p v-if="section.group_ids.length === 0" class="mt-1 text-xs text-ink-faint">
                    {{ t('admin.channels.form.noGroupsInChannel') }}
                  </p>
                </div>

                <div>
                  <label class="text-xs text-ink-soft dark:text-dark-400">{{ t('admin.channels.form.ruleAccounts') }}</label>
                  <!-- Selected account chips -->
                  <div class="mt-1 flex flex-wrap gap-1">
                    <span
                      v-for="accountId in rule.account_ids"
                      :key="accountId"
                      class="inline-flex items-center gap-1 rounded-md border border-brand-300 bg-brand-50 px-2 py-0.5 text-xs dark:border-brand-700 dark:bg-brand-900/20"
                    >
                      <span :class="['font-medium', platformTextClass(section.platform)]">{{ getRuleAccountLabel(accountId) }}</span>
                      <button type="button" @click="removeRuleAccount(rule, accountId)" class="text-ink-faint hover:text-red-500">
                        <Icon name="x" size="xs" />
                      </button>
                    </span>
                  </div>
                  ...
                  <div class="relative mt-1 rule-account-search-container">
                    ...
                    <div
                      ...
                      class="absolute z-50 mt-1 max-h-48 w-full overflow-auto rounded-lg border border-line bg-card shadow-lg dark:border-dark-600 dark:bg-dark-800"
                    >
                      <button
                      ...
                      >
                        ...
                        <span class="ml-2 text-xs text-ink-faint">#{{ account.id }}</span>
                      </button>
                    </div>
                  </div>
                  <p class="mt-1 text-xs text-ink-faint">
                ...
                <div>
                  <div class="mb-1 flex items-center justify-between">
                    <label class="text-xs text-ink-soft dark:text-dark-400">{{ t('admin.channels.form.ruleModelPricing') }}</label>
                    <button type="button" @click="addRulePricingEntry(sIdx, ruleIndex)" class="text-xs text-brand-600 hover:text-brand-700">
                      + {{ t('common.add') }}
                    </button>
                  </div>
                  <div v-if="rule.pricing.length === 0" class="rounded border border-dashed border-line p-2 text-center text-xs text-ink-faint dark:border-dark-500">
...
// ── Platform config ──
const platformOrder: GroupPlatform[] = ['anthropic', 'openai', 'gemini', 'antigravity', 'sora', 'kiro', 'grok']
...
.channel-tab-active {
  @apply border-brand-600 text-brand-600 dark:border-brand-400 dark:text-brand-400;
}

.channel-tab-inactive {
  @apply border-transparent text-ink-soft hover:text-ink-body hover:border-line dark:text-dark-400 dark:hover:text-dark-200;
}
</style>
