<template>
  <AppLayout>
    <div class="space-y-6">
      ...
      <div class="card">
        <div class="flex flex-wrap items-center border-b border-line px-2 dark:border-dark-700 sm:px-4">
          <button
            ...
            :class="activeTab === tab.key
              ...
              : 'border-transparent text-ink-soft hover:border-line hover:text-ink dark:hover:border-dark-500'"
        ...
        <UsageFilters
          ...
          flat
          :mode="activeTab"
          class="border-b border-line dark:border-dark-700/50"
          :start-date="startDate"
          :end-date="endDate"
          :exporting="exporting"
          ...
          <template #after-reset>
            <div v-if="activeTab !== 'ranking'" ref="columnDropdownRef" class="relative">
              ...
              <div
                ...
                class="absolute right-0 top-full z-50 mt-1 max-h-80 w-52 overflow-y-auto rounded-card border border-line bg-card py-1 shadow-md dark:border-dark-600 dark:bg-dark-800"
              >
                <button
                ...
                >
                  ...
                  <Icon
                    v-if="isCurrentColumnVisible(column.key)"
                    name="check"
                    size="sm"
...
// Error request columns use an independent preference key so usage-table choices do not leak across tabs.
...
const errToggleableColumns = computed(() =>
  errAllColumns.value.filter((column) => !ERR_ALWAYS_VISIBLE.includes(column.key))
)
const errVisibleColumnKeys = computed(() =>
  errAllColumns.value
    .filter((column) => ERR_ALWAYS_VISIBLE.includes(column.key) || !errHiddenColumns.has(column.key))
