<template>
 ...
 : 'border-transparent text-muted hover:border-line hover:text-foreground'"
        ...
        <UsageFilters v-model="filters" ref="usageFiltersRef" flat :mode="activeTab" class="border-b border-line/50" :start-date="startDate" :end-date="endDate" :exporting="exporting" :model-options="modelNameOptions" @change="applyFilters" @refresh="refreshData" @reset="resetFilters" @cleanup="openCleanupDialog" @export="exportToExcel">
...
const filters = ref<AdminUsageQueryParams>({ user_id: undefined, model: undefined, group_id: undefined, request_type: undefined, native_compaction_v2: null, billing_type: null, billing_mode: undefined, exclude_admin: false, start_date: startDate.value, end_date: endDate.value })
...
const resetFilters = () => {
  ...
  filters.value = { start_date: startDate.value, end_date: endDate.value, request_type: undefined, native_compaction_v2: null, billing_type: null, billing_mode: undefined, exclude_admin: false }
  granularity.value = getGranularityForRange(startDate.value, endDate.value)
  applyFilters()
}
