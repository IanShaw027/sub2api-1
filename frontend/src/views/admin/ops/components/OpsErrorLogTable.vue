    ...
    <div class="flex min-h-0 flex-1 flex-col overflow-hidden" :class="flat ? '' : 'card'">
      ...
      >
        ...
        <template #cell-platform="{ row }">
          <span class="text-sm text-foreground ">{{ row.platform || '-' }}</span>
        </template>

        <template #cell-model="{ row }">
          <div v-if="hasModelMapping(row)" class="space-y-0.5 text-xs">
            <div class="break-all font-medium text-foreground ">{{ row.requested_model }}</div>
            <div class="break-all text-muted "><span class="mr-0.5">↳</span>{{ row.upstream_model }}</div>
          </div>
          <span v-else-if="displayModel(row)" class="text-sm font-medium text-foreground ">{{ displayModel(row) }}</span>
        ...
        <template #cell-status="{ row }">
          <div class="flex items-center gap-1.5">
            ...
            <span
              ...
              class="inline-flex items-center rounded px-2 py-0.5 text-xs font-medium bg-surface-2 text-foreground  "
        ...
        <template #cell-message="{ row }">
          <span
            v-if="row.message"
            class="block max-w-[280px] truncate text-sm text-muted "
            :title="row.message"
        ...
        <template #cell-user_agent="{ row }">
          <span
            v-if="row.user_agent"
            class="block max-w-[320px] truncate text-sm text-muted "
        ...
        <template #cell-client_ip="{ row }">
          <div @click.stop>
            <div v-if="row.client_ip">
              <span class="text-sm font-mono text-muted ">{{ row.client_ip }}</span>
        ...
        <template #cell-actions="{ row }">
          <button
            type="button"
            class="rounded p-1 text-muted transition-colors hover:bg-surface-2 hover:text-accent  "
...
function getTypeBadge(log: OpsErrorLog): { label: string; className: string } {
  ...
  if (isUpstreamRow(log)) {
    return { label: t('admin.ops.errorLog.typeUpstream'), className: 'bg-[color-mix(in_oklch,var(--danger)_14%,transparent)] text-danger-text  ' }
  }
  ...
  return { label: fallback, className: 'bg-surface-2 text-foreground  ' }
}
