...
function severityBadgeClass(severity: string | undefined): string {
  ...
  if (s === 'p0' || s === 'critical') return 'bg-[color-mix(in_oklch,var(--danger)_14%,transparent)] text-danger-text  '
  if (s === 'p1' || s === 'warning') return 'bg-[color-mix(in_oklch,var(--warning)_18%,transparent)] text-warning-text  '
  if (s === 'p2' || s === 'info') return 'bg-[color-mix(in_oklch,var(--accent)_12%,transparent)] text-accent  '
  if (s === 'p3') return 'bg-surface-2 text-foreground  '
  ...
  <div class="rounded-xl bg-surface p-6 shadow-sm ring-1 ring-[color-mix(in_oklch,var(--foreground)_8%,transparent)]  ">
    <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
      <div>
        ...
        <p class="mt-1 text-xs text-muted ">{{ t('admin.ops.alertEvents.description') }}</p>
      </div>
    ...
    <div v-else-if="empty" class="rounded-xl border border-dashed border-line p-8 text-center text-sm text-muted  ">
      ...
      <div class="max-h-[600px] overflow-y-auto" @scroll="onScroll">
        <div v-if="!isDesktopViewport" class="divide-y divide-line ">
          <div
            v-for="row in events"
            :key="row.id"
            class="cursor-pointer space-y-2 p-4 hover:bg-surface-2 "
          ...
          >
            ...
            <div class="text-xs font-semibold text-foreground ">{{ row.title || '-' }}</div>
            <div v-if="row.description" class="line-clamp-2 text-[11px] text-muted ">
              {{ row.description }}
            </div>
        ...
        <table v-else class="min-w-full divide-y divide-line ">
          ...
          <tbody class="divide-y divide-line bg-surface  ">
            ...
            >
              ...
              <td class="min-w-[260px] px-4 py-3 text-xs text-foreground ">
        ...
        <div v-else-if="!hasMore && events.length > 0" class="py-3 text-center text-xs text-muted">
          -
        </div>
      </div>
    </div>
    ...
    <BaseDialog
      ...
      @close="closeDetail"
    >
      <div v-if="detailLoading" class="flex items-center justify-center py-10 text-sm text-muted ">
        ...
        <div class="rounded-xl bg-surface-2 p-4 ">
          <div class="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <div v-if="selected.description" class="mt-1 whitespace-pre-wrap text-xs text-muted ">
                {{ selected.description }}
              </div>
            </div>

            <div class="flex flex-wrap gap-2">
              <div class="flex items-center gap-2 rounded-lg bg-surface px-2 py-1 ring-1 ring-line  ">
                <span class="text-[11px] font-bold text-muted ">{{ t('admin.ops.alertEvents.detail.silence') }}</span>
                <Select
          ...
          <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div class="rounded-xl bg-surface-2 p-4 ">
              <div class="text-xs font-bold uppercase tracking-wider text-muted">{{ t('admin.ops.alertEvents.detail.firedAt') }}</div>
              <div class="mt-1 text-sm font-medium text-foreground ">{{ formatDateTime(selected.fired_at || selected.created_at) }}</div>
            </div>
            <div class="rounded-xl bg-surface-2 p-4 ">
              <div class="text-xs font-bold uppercase tracking-wider text-muted">{{ t('admin.ops.alertEvents.detail.resolvedAt') }}</div>
              <div class="mt-1 text-sm font-medium text-foreground ">{{ selected.resolved_at ? formatDateTime(selected.resolved_at) : '-' }}</div>
            </div>
            <div class="rounded-xl bg-surface-2 p-4 ">
              <div class="text-xs font-bold uppercase tracking-wider text-muted">{{ t('admin.ops.alertEvents.detail.ruleId') }}</div>
              <div class="mt-1 flex flex-wrap items-center gap-2">
                <div class="font-mono text-sm font-bold text-foreground ">#{{ selected.rule_id }}</div>
                <a
                  class="inline-flex items-center gap-1 rounded-md bg-surface px-2 py-1 text-[11px] font-bold text-foreground ring-1 ring-line hover:bg-surface-2    "
            ...
            <div class="rounded-xl bg-surface-2 p-4 ">
              <div class="text-xs font-bold uppercase tracking-wider text-muted">{{ t('admin.ops.alertEvents.detail.dimensions') }}</div>
        ...
        <div class="rounded-xl border border-line bg-surface p-4  ">
          <div class="mb-3 flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <div class="mt-0.5 text-xs text-muted ">{{ t('admin.ops.alertEvents.detail.historyHint') }}</div>
            </div>
            <Select :model-value="historyRange" :options="historyRangeOptions" class="w-[140px]" @change="historyRange = String($event || '7d')" />
          </div>

          <div v-if="historyLoading" class="py-6 text-center text-xs text-muted ">
            {{ t('admin.ops.alertEvents.detail.historyLoading') }}
          </div>
          <div v-else-if="history.length === 0" class="py-6 text-center text-xs text-muted ">
          ...
          <div v-else class="overflow-hidden rounded-lg border border-line ">
            <table class="min-w-full divide-y divide-line ">
              <thead class="bg-surface-2 ">
                <tr>
                  <th class="px-3 py-2 text-left text-[11px] font-bold uppercase tracking-wider text-muted ">{{ t('admin.ops.alertEvents.table.time') }}</th>
                  <th class="px-3 py-2 text-left text-[11px] font-bold uppercase tracking-wider text-muted ">{{ t('admin.ops.alertEvents.table.status') }}</th>
                  <th class="px-3 py-2 text-left text-[11px] font-bold uppercase tracking-wider text-muted ">{{ t('admin.ops.alertEvents.table.metric') }}</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-line ">
                <tr v-for="it in history" :key="it.id" class="hover:bg-surface-2 ">
                  <td class="px-3 py-2 text-xs text-muted ">{{ formatDateTime(it.fired_at || it.created_at) }}</td>
