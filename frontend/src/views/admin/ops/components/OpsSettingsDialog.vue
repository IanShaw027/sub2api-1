...
<template>
  <UiModal :open="show" :title="t('admin.ops.settings.title')" width="xl" @close="emit('close')">
    ...
    <div v-else-if="runtimeSettings && emailConfig && advancedSettings" class="space-y-6">
      <!-- 验证错误 -->
      <div v-if="!validation.valid" class="rounded-lg border border-[color-mix(in_oklch,var(--warning)_35%,transparent)] bg-[color-mix(in_oklch,var(--warning)_18%,transparent)] p-3 text-xs text-warning-text   ">
      ...
      <div class="rounded-xl bg-surface-2 p-4 ">
        ...
        <div>
          ...
          <input
            ...
            class="input"
          />
          <p class="mt-1 text-xs text-muted">{{ t('admin.ops.settings.evaluationIntervalHint') }}</p>
        </div>
      </div>
          ...
          <div v-if="emailConfig.alert.enabled">
            ...
            <div class="mt-2 flex flex-wrap gap-2">
              <span
                ...
                :key="email"
                class="inline-flex items-center gap-2 rounded-full bg-[color-mix(in_oklch,var(--accent)_12%,transparent)] px-3 py-1 text-xs font-medium text-accent  "
              >
                {{ email }}
                <button type="button" class="text-accent hover:text-accent" @click="removeRecipient('alert', email)">×</button>
              </span>
            </div>
        ...
        <div class="space-y-4">
          ...
          <div v-if="emailConfig.report.enabled">
            ...
            <div class="mt-2 flex flex-wrap gap-2">
              ...
              >
                {{ email }}
                <button type="button" class="text-accent hover:text-accent" @click="removeRecipient('report', email)">×</button>
              </span>
            </div>
            <p class="mt-2 text-xs text-muted ">
              {{ t('admin.ops.settings.recipientsHint') }}
          ...
          <div v-if="emailConfig.report.enabled" class="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div class="flex items-center justify-between">
              <label class="text-sm font-medium text-foreground ">{{ t('admin.ops.settings.dailySummary') }}</label>
            ...
            <div class="flex items-center justify-between">
              <label class="text-sm font-medium text-foreground ">{{ t('admin.ops.settings.weeklySummary') }}</label>
        ...
        <div class="space-y-4">
          <div>
            ...
            <input
              ...
              class="input"
            />
            <p class="mt-1 text-xs text-muted">{{ t('admin.ops.settings.slaMinPercentHint') }}</p>
          </div>
          ...
          <div>
            ...
            <input
              ...
              class="input"
            />
            <p class="mt-1 text-xs text-muted">{{ t('admin.ops.settings.ttftP99MaxMsHint') }}</p>
          </div>
          ...
          <div>
            ...
            <input
              ...
              class="input"
            />
            <p class="mt-1 text-xs text-muted">{{ t('admin.ops.settings.requestErrorRateMaxPercentHint') }}</p>
          </div>
          ...
          <div>
            ...
            <input
              ...
              class="input"
            />
            <p class="mt-1 text-xs text-muted">{{ t('admin.ops.settings.upstreamErrorRateMaxPercentHint') }}</p>
          </div>
        </div>
      </div>
      ...
      <details class="rounded-xl bg-surface-2 ">
        ...
        <div class="space-y-4 px-4 pb-4">
          <!-- 数据保留策略 -->
          <div class="space-y-3">
            <h5 class="text-xs font-semibold text-foreground ">{{ t('admin.ops.settings.dataRetention') }}</h5>

            <div class="flex items-center justify-between">
              <label class="text-sm font-medium text-foreground ">{{ t('admin.ops.settings.enableCleanup') }}</label>
            ...
            <div v-if="advancedSettings.data_retention.cleanup_enabled">
              ...
              <input
                ...
                placeholder="0 2 * * *"
              />
              <p class="mt-1 text-xs text-muted">{{ t('admin.ops.settings.cleanupScheduleHint') }}</p>
            </div>
            ...
            <p class="text-xs text-muted">{{ t('admin.ops.settings.retentionDaysHint') }}</p>
          </div>

          <!-- 预聚合任务 -->
          <div class="space-y-3">
            <h5 class="text-xs font-semibold text-foreground ">{{ t('admin.ops.settings.aggregation') }}</h5>

            <div class="flex items-center justify-between">
              <div>
                <label class="text-sm font-medium text-foreground ">{{ t('admin.ops.settings.enableAggregation') }}</label>
                <p class="mt-1 text-xs text-muted">{{ t('admin.ops.settings.aggregationHint') }}</p>
              </div>
          ...
          <div class="space-y-3">
            <h5 class="text-xs font-semibold text-foreground ">{{ t('admin.ops.settings.openaiQuotaAutoPause') }}</h5>
            <p class="text-xs text-muted">{{ t('admin.ops.settings.openaiQuotaAutoPauseHint') }}</p>
            ...
            <p class="text-xs text-muted">{{ t('admin.ops.settings.openaiQuotaAutoPauseThresholdHint') }}</p>
          </div>

          <!-- Error Filtering -->
          <div class="space-y-3">
            <h5 class="text-xs font-semibold text-foreground ">{{ t('admin.ops.settings.errorFiltering') }}</h5>

            <div class="flex items-center justify-between">
              <div>
                <label class="text-sm font-medium text-foreground ">{{ t('admin.ops.settings.ignoreCountTokensErrors') }}</label>
            ...
            <div class="flex items-center justify-between">
              <div>
                <label class="text-sm font-medium text-foreground ">{{ t('admin.ops.settings.ignoreContextCanceled') }}</label>
                <p class="mt-1 text-xs text-muted">
            ...
            <div class="flex items-center justify-between">
              <div>
                <label class="text-sm font-medium text-foreground ">{{ t('admin.ops.settings.ignoreNoAvailableAccounts') }}</label>
                <p class="mt-1 text-xs text-muted">
            ...
            <div class="flex items-center justify-between">
              <div>
                <label class="text-sm font-medium text-foreground ">{{ t('admin.ops.settings.ignoreInsufficientBalanceErrors') }}</label>
                <p class="mt-1 text-xs text-muted">
          ...
          <!-- Auto Refresh -->
          <div class="space-y-3">
            <h5 class="text-xs font-semibold text-foreground ">{{ t('admin.ops.settings.autoRefresh') }}</h5>

            <div class="flex items-center justify-between">
              <div>
                <label class="text-sm font-medium text-foreground ">{{ t('admin.ops.settings.enableAutoRefresh') }}</label>
                <p class="mt-1 text-xs text-muted">
          ...
          <!-- Dashboard Cards -->
          <div class="space-y-3">
            <h5 class="text-xs font-semibold text-foreground ">{{ t('admin.ops.settings.dashboardCards') }}</h5>

            <div class="flex items-center justify-between">
              <div>
                <label class="text-sm font-medium text-foreground ">{{ t('admin.ops.settings.displayAlertEvents') }}</label>
                <p class="mt-1 text-xs text-muted">
            ...
            <div class="flex items-center justify-between">
              <div>
                <label class="text-sm font-medium text-foreground ">{{ t('admin.ops.settings.displayOpenAITokenStats') }}</label>
                <p class="mt-1 text-xs text-muted">
