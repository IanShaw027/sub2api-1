  ...
  <UiModal :open="show" :title="title" width="xl" @close="close">
    <div v-if="loading" class="flex items-center justify-center py-16">
      <div class="flex flex-col items-center gap-3">
        ...
        <div class="text-sm font-medium text-muted ">{{ t('admin.ops.errorDetail.loading') }}</div>
      </div>
    </div>
    ...
    <div v-else class="space-y-6 p-6">
      ...
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div class="rounded-xl bg-surface-2 p-4 ">
          <div class="text-xs font-bold uppercase tracking-wider text-muted">{{ t('admin.ops.errorDetail.requestId') }}</div>
        ...
        <div class="rounded-xl bg-surface-2 p-4 ">
          <div class="text-xs font-bold uppercase tracking-wider text-muted">{{ t('admin.ops.errorDetail.time') }}</div>
        ...
        <div class="rounded-xl bg-surface-2 p-4 ">
          <div class="text-xs font-bold uppercase tracking-wider text-muted">{{ t('admin.ops.errorDetail.platform') }}</div>
          <div class="mt-1 text-sm font-medium text-foreground ">
            {{ detail.platform || '—' }}
          </div>
        </div>
        ...
        <div class="rounded-xl bg-surface-2 p-4 ">
          <div class="text-xs font-bold uppercase tracking-wider text-muted">{{ t('admin.ops.errorDetail.group') }}</div>
          <div class="mt-1 text-sm font-medium text-foreground ">
        ...
        <div class="rounded-xl bg-surface-2 p-4 ">
          <div class="text-xs font-bold uppercase tracking-wider text-muted">{{ t('admin.ops.errorDetail.model') }}</div>
          <div class="mt-1 text-sm font-medium text-foreground ">
        ...
        <div class="rounded-xl bg-surface-2 p-4 ">
          <div class="text-xs font-bold uppercase tracking-wider text-muted">{{ t('admin.ops.errorDetail.inboundEndpoint') }}</div>
          <div class="mt-1 break-all font-mono text-sm font-medium text-foreground ">
        ...
        <div class="rounded-xl bg-surface-2 p-4 ">
          <div class="text-xs font-bold uppercase tracking-wider text-muted">{{ t('admin.ops.errorDetail.upstreamEndpoint') }}</div>
          <div class="mt-1 break-all font-mono text-sm font-medium text-foreground ">
        ...
        <div class="rounded-xl bg-surface-2 p-4 ">
          <div class="text-xs font-bold uppercase tracking-wider text-muted">{{ t('admin.ops.errorDetail.status') }}</div>
          <div class="mt-1">
        ...
        <div class="rounded-xl bg-surface-2 p-4 ">
          <div class="text-xs font-bold uppercase tracking-wider text-muted">{{ t('admin.ops.errorDetail.upstreamStatus') }}</div>
          <div class="mt-1">
        ...
        <div class="rounded-xl bg-surface-2 p-4 ">
          <div class="text-xs font-bold uppercase tracking-wider text-muted">{{ t('admin.ops.errorDetail.requestType') }}</div>
          <div class="mt-1 text-sm font-medium text-foreground ">
        ...
        <div class="rounded-xl bg-surface-2 p-4 ">
          <div class="text-xs font-bold uppercase tracking-wider text-muted">{{ t('admin.ops.errorDetail.message') }}</div>
          <div class="mt-1 break-words text-sm font-medium text-foreground " :title="rootCauseMessage">
            {{ rootCauseMessage || '—' }}
          </div>
        </div>

        <div v-if="detail.api_key_prefix" class="rounded-xl bg-surface-2 p-4 ">
          <div class="text-xs font-bold uppercase tracking-wider text-muted">{{ t('admin.ops.errorDetail.apiKeyPrefix') }}</div>
      ...
      <div v-if="rootCauseMessage" class="rounded-xl bg-[color-mix(in_oklch,var(--warning)_18%,transparent)] p-6 ">
        <h3 class="text-sm font-black uppercase tracking-wider text-warning-text ">{{ t('admin.ops.errorDetail.rootCause') }}</h3>
        <div class="mt-3 break-words text-sm font-medium text-warning-text ">{{ rootCauseMessage }}</div>
      </div>

      <div class="rounded-xl bg-surface-2 p-6 ">
        <h3 class="text-sm font-black uppercase tracking-wider text-foreground ">{{ t('admin.ops.errorDetail.diagnosticPayloads') }}</h3>
        <div v-if="!diagnosticPayloadSections.length" class="mt-4 text-sm text-muted ">{{ t('common.noData') }}</div>
      ...
      <div v-if="showUpstreamList" class="rounded-xl bg-surface-2 p-6 ">
        <div class="flex flex-wrap items-center justify-between gap-2">
          ...
          <div class="text-xs text-muted " v-if="correlatedUpstreamLoading">{{ t('common.loading') }}</div>
        </div>

        <div v-if="!correlatedUpstreamLoading && !correlatedUpstreamErrors.length" class="mt-3 text-sm text-muted ">
          {{ t('common.noData') }}
        </div>
          ...
          >
            <div class="flex flex-wrap items-center justify-between gap-2">
              <div class="text-xs font-black text-foreground ">
                #{{ idx + 1 }}
                <span v-if="ev.type" class="ml-2 rounded-md bg-surface-2 px-2 py-0.5 font-mono text-[10px] font-bold text-foreground  ">{{ ev.type }}</span>
              </div>
              <div class="flex items-center gap-2">
                ...
                <button
                  type="button"
                  class="inline-flex items-center gap-1.5 rounded-md px-1.5 py-1 text-[10px] font-bold text-accent hover:bg-[color-mix(in_oklch,var(--accent)_12%,transparent)] disabled:cursor-not-allowed disabled:opacity-60  "
            ...
            <div class="mt-3 grid grid-cols-1 gap-2 text-xs text-muted  sm:grid-cols-2">
              <div>
                <span class="text-muted">{{ t('admin.ops.errorDetail.upstreamEvent.status') }}:</span>
                <span class="ml-1 font-mono">{{ ev.status_code ?? '—' }}</span>
              </div>
              <div>
                <span class="text-muted">{{ t('admin.ops.errorDetail.upstreamEvent.requestId') }}:</span>
