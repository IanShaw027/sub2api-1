...
describe('OpsSystemLogTable host support', () => {
  ...
  it('renders the host and sends it with list and cleanup filters', async () => {
    ...
    const hostLabel = wrapper.findAll('label').find((label) => label.text().includes('admin.ops.systemLogs.filters.host'))
    expect(hostLabel).toBeDefined()
    await hostLabel!.find('input').setValue(' api-node-2 ')

    const searchButton = wrapper.findAll('button').find((button) => button.text() === 'admin.ops.systemLogs.actions.query')
    ...
    const cleanupButton = wrapper.findAll('button').find((button) => button.text() === 'admin.ops.systemLogs.actions.cleanupFiltered')
  ...
  it.each([
    ['zh', zhLocale, '主机'],
    ['en', enLocale, 'Host'],
  ])('defines the Host translation for %s', (_name, locale, expected) => {
    expect(locale.admin.ops.systemLogs.filters.host).toBe(expected)
  })
})
