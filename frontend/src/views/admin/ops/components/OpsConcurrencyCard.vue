...
function getLoadTextClass(loadPct: number): string {
  if (loadPct >= 90) return 'text-danger-text '
  if (loadPct >= 70) return 'text-warning-text '
  if (loadPct >= 50) return 'text-warning-text '
  ...
  <div class="flex h-full flex-col rounded-xl bg-surface p-6 shadow-sm ring-1 ring-[color-mix(in_oklch,var(--foreground)_8%,transparent)]  ">
    ...
    <div class="mb-4 flex shrink-0 items-center justify-between gap-3">
      ...
      <div class="flex items-center gap-2">
        ...
        <!-- 刷新按钮 -->
        <button
          class="flex items-center gap-1 rounded-lg bg-surface-2 px-2 py-1 text-[11px] font-semibold text-foreground transition-colors hover:bg-surface-3 disabled:cursor-not-allowed disabled:opacity-50   "
          :disabled="loading"
    ...
    <!-- 错误提示 -->
    <div v-if="errorMessage" class="mb-3 shrink-0 rounded-xl bg-[color-mix(in_oklch,var(--danger)_14%,transparent)] p-2.5 text-xs text-danger-text  ">
      {{ errorMessage }}
    </div>
    ...
    <div
      ...
      class="flex flex-1 items-center justify-center rounded-xl border border-dashed border-line text-sm text-muted  "
    >
    ...
    <div v-else class="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-line ">
      <!-- 维度标题栏 -->
      <div class="flex shrink-0 items-center justify-between border-b border-line bg-surface-2 px-3 py-2  ">
      ...
      <!-- 空状态 -->
      <div v-if="displayRows.length === 0" class="flex flex-1 items-center justify-center text-sm text-muted ">
      ...
      <div v-else-if="displayDimension === 'user'" class="custom-scrollbar max-h-[360px] flex-1 space-y-2 overflow-y-auto p-3">
        <div v-for="row in (displayRows as UserRow[])" :key="row.key" class="rounded-lg bg-surface-2 p-2.5 ">
          <!-- 用户信息和并发 -->
      ...
      <div v-else-if="displayDimension === 'platform' || displayDimension === 'group'" class="custom-scrollbar max-h-[360px] flex-1 space-y-2 overflow-y-auto p-3">
        <div v-for="row in (displayRows as SummaryRow[])" :key="row.key" class="rounded-lg bg-surface-2 p-3 ">
          <!-- 标题行 -->
          ...
          <!-- 进度条 -->
          <div class="mb-2 h-1.5 w-full overflow-hidden rounded-full bg-surface-3 ">
            <div
          ...
          <div class="flex flex-wrap items-center gap-x-3 gap-y-1 text-[10px]">
            ...
            <div class="flex items-center gap-1">
              ...
              <span class="text-muted ">
                <span class="font-bold text-success-text ">{{ row.available_accounts }}</span
                >/{{ row.total_accounts }}
              </span>
            ...
            <span
              ...
              class="rounded-full bg-[color-mix(in_oklch,var(--warning)_18%,transparent)] px-1.5 py-0.5 font-semibold text-warning-text  "
            >
            ...
            <span
              ...
              class="rounded-full bg-[color-mix(in_oklch,var(--danger)_14%,transparent)] px-1.5 py-0.5 font-semibold text-danger-text  "
            >
      ...
      <div v-else class="custom-scrollbar max-h-[360px] flex-1 space-y-2 overflow-y-auto p-3">
        <div v-for="row in (displayRows as AccountRow[])" :key="row.key" class="rounded-lg bg-surface-2 p-2.5 ">
          <!-- 账号名称和并发 -->
          <div class="mb-1.5 flex items-center justify-between gap-2">
            ...
            <div class="flex shrink-0 items-center gap-2">
              ...
              <span
                ...
                class="inline-flex items-center gap-1 rounded bg-[color-mix(in_oklch,var(--success)_16%,transparent)] px-1.5 py-0.5 text-[10px] font-medium text-success-text  "
              >
              ...
              <span
                ...
                class="inline-flex items-center gap-1 rounded bg-[color-mix(in_oklch,var(--warning)_18%,transparent)] px-1.5 py-0.5 text-[10px] font-medium text-warning-text  "
              >
                <svg class="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <span
                ...
                class="inline-flex items-center gap-1 rounded bg-[color-mix(in_oklch,var(--danger)_14%,transparent)] px-1.5 py-0.5 text-[10px] font-medium text-danger-text  "
              >
                <svg class="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path
                    stroke-linecap="round"
              ...
              <span
                v-else
                class="inline-flex items-center gap-1 rounded bg-surface-2 px-1.5 py-0.5 text-[10px] font-medium text-foreground  "
              >
