...
<template>
  <BaseDialog :show="show" :title="modalTitle" width="full" @close="close">
    <div class="flex h-full min-h-0 flex-col">
      <!-- Filters -->
      <div class="mb-4 flex-shrink-0 border-b border-line pb-4 dark:border-dark-700">
        <div class="grid grid-cols-8 gap-2">
          <div class="col-span-2 compact-select">
            <div class="relative group">
              <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
                <svg
                  class="h-3.5 w-3.5 text-ink-faint transition-colors group-focus-within:text-accent"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
              ...
              <input
                v-model="q"
                type="text"
                class="w-full rounded-lg border-line bg-page/50 py-1.5 pl-9 pr-3 text-xs font-medium text-ink-body transition-all focus:border-accent focus:bg-card focus:ring-2 focus:ring-accent/25 dark:border-dark-700 dark:bg-dark-900 dark:focus:bg-dark-800"
          ...
          <div class="flex items-center justify-end">
            <button type="button" class="rounded-lg bg-page px-3 py-1.5 text-xs font-semibold text-ink-body transition-colors hover:bg-divider dark:bg-dark-700 dark:hover:bg-dark-600" @click="resetFilters">
              {{ t('common.reset') }}
            </button>
          </div>
        </div>
      ...
      <div class="flex min-h-0 flex-1 flex-col">
        <div class="mb-2 flex-shrink-0 text-xs text-ink-soft">
