...
<template>
  <Teleport to="body">
    ...
    >
      <div
        class="relative max-h-[90vh] w-full max-w-5xl overflow-y-auto rounded-2xl bg-white shadow-2xl dark:bg-dark-800"
        @click.stop
      >
        <!-- Header -->
        <div class="sticky top-0 z-10 flex items-center justify-between border-b border-gray-200 bg-white px-6 py-4 dark:border-dark-700 dark:bg-dark-800">
          <div>
            <div class="flex items-center gap-3">
              <h2 class="text-lg font-black text-gray-900 dark:text-white">{{ preset.title }}</h2>
              <span class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ rangeLabel }}</span>
            </div>
            <div class="mt-1 text-[11px] text-gray-400">
              {{ t('admin.ops.requestDetails.subtitle') }}
            </div>
          </div>
          <div class="flex items-center gap-2">
            <button
              @click="fetchData"
              :disabled="loading"
              class="rounded-lg bg-gray-100 px-3 py-2 text-xs font-bold text-gray-600 hover:bg-gray-200 disabled:opacity-50 dark:bg-dark-700 dark:text-gray-300 dark:hover:bg-dark-600"
            >
              {{ t('admin.ops.requestDetails.refresh') }}
            </button>
            <button
              @click="close"
