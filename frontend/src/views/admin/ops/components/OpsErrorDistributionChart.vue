...
<template>
  <div class="flex h-full flex-col rounded-xl bg-surface p-6 shadow-sm ring-1 ring-[color-mix(in_oklch,var(--foreground)_8%,transparent)]  ">
    <div class="mb-4 flex items-center justify-between">
      ...
      <button
        type="button"
        class="inline-flex items-center rounded-lg border border-line bg-surface px-2 py-1 text-[11px] font-semibold text-muted hover:bg-surface-2 disabled:opacity-50    "
        ...
        <div class="mt-4 flex flex-col items-center gap-2">
          ...
          <div class="flex flex-wrap justify-center gap-3">
            <div v-for="item in categories" :key="item.label" class="flex items-center gap-1.5 text-xs">
              ...
              <span class="text-muted ">{{ item.label }} {{ item.count }}</span>
            </div>
          </div>
        </div>
      </div>
