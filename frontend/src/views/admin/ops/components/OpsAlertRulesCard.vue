  ...
  <div class="rounded-3xl bg-surface p-6 shadow-sm ring-1 ring-[color-mix(in_oklch,var(--foreground)_8%,transparent)]  ">
    <div class="mb-4 flex flex-wrap items-start justify-between gap-3 sm:gap-4">
      <div>
        ...
        <p class="mt-1 text-xs text-muted ">{{ t('admin.ops.alertRules.description') }}</p>
      </div>
    ...
    <div v-else-if="sortedRules.length === 0" class="rounded-xl border border-dashed border-line p-8 text-center text-sm text-muted  ">
      {{ t('admin.ops.alertRules.empty') }}
    </div>

    <div v-else class="max-h-[520px] overflow-hidden rounded-xl border border-line ">
        ...
        <div v-if="!isDesktopViewport" class="divide-y divide-line ">
          <div v-for="row in sortedRules" :key="row.id" class="space-y-2 p-4">
            <div class="flex items-start justify-between gap-2">
              ...
              <span class="shrink-0 text-xs font-bold text-foreground ">{{ row.severity }}</span>
            </div>
            <div class="text-xs text-foreground ">
              <span class="font-mono">{{ row.metric_type }}</span>
              <span class="mx-1 text-muted">{{ row.operator }}</span>
            ...
            <div v-if="row.updated_at" class="text-[10px] text-muted">
        ...
        <table v-else class="min-w-full divide-y divide-line ">
          ...
          <tbody class="divide-y divide-line bg-surface  ">
            <tr v-for="row in sortedRules" :key="row.id" class="hover:bg-surface-2 ">
              ...
              <td class="whitespace-nowrap px-4 py-3 text-xs font-bold text-foreground ">
                {{ row.severity }}
              </td>
              <td class="whitespace-nowrap px-4 py-3 text-xs text-foreground ">
                {{ row.enabled ? t('common.enabled') : t('common.disabled') }}
    ...
    >
      <div class="space-y-4">
        <div v-if="!editorValidation.valid" class="rounded-xl bg-[color-mix(in_oklch,var(--danger)_14%,transparent)] p-4 text-xs text-danger-text  ">
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <div>
            ...
            <div v-if="selectedMetricDefinition" class="mt-1 space-y-0.5 text-xs text-muted ">
          ...
          <div class="flex items-center justify-between rounded-xl bg-surface-2 px-4 py-3  md:col-span-2">
            <span class="text-xs font-bold text-foreground ">{{ t('admin.ops.alertRules.form.enabled') }}</span>
            <input v-model="draft!.enabled" type="checkbox" class="h-4 w-4 rounded border-line text-accent focus:ring-primary-500" />
          </div>

          <div class="flex items-center justify-between rounded-xl bg-surface-2 px-4 py-3  md:col-span-2">
            <span class="text-xs font-bold text-foreground ">{{ t('admin.ops.alertRules.form.notifyEmail') }}</span>
            <input v-model="draft!.notify_email" type="checkbox" class="h-4 w-4 rounded border-line text-accent focus:ring-primary-500" />
          </div>
        </div>
      </div>
