    ...
    <div v-if="draft" class="space-y-5">
      ...
      <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
        <div class="md:col-span-2">
          ...
          <input v-model="draft.name" type="text" class="input" />
        </div>

        <div class="md:col-span-2">
          <label class="input-label">{{ t('admin.ops.alertRules.form.description') }}</label>
          <textarea v-model="draft.description" rows="2" class="input"></textarea>
        </div>
        ...
        <div>
          ...
          <input v-model.number="draft.threshold" type="number" step="0.01" class="input" />
        </div>
        <div>
        ...
        <div>
          ...
          <input v-model.number="draft.sustained_minutes" type="number" min="1" max="1440" class="input" />
        </div>
        <div>
          <label class="input-label">{{ t('admin.ops.alertRules.form.cooldown') }}</label>
          <input v-model.number="draft.cooldown_minutes" type="number" min="0" max="1440" class="input" />
        </div>
        ...
        <div class="md:col-span-2 flex flex-wrap items-center gap-6">
          ...
          <label class="inline-flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
            <input v-model="draft.notify_email" type="checkbox" class="h-4 w-4 rounded border-gray-300" />
      ...
      <div class="flex justify-end gap-2">
        <button class="btn btn-secondary" :disabled="saving" @click="showEditor = false">{{ t('common.cancel') }}</button>
