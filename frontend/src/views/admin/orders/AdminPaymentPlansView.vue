      ...
      <DataTable :columns="planColumns" :data="plans" :loading="plansLoading">
        ...
        <template #cell-group_id="{ value }">
          <span v-if="isGroupMissing(value)" class="text-sm">
            <span class="text-ink-faint">#{{ value }}</span>
        ...
        <template #cell-price="{ value, row }">
          <div class="text-sm">
            <span class="font-medium tabular-nums text-ink dark:text-white">${{ (value ?? 0).toFixed(2) }}</span>
            <span v-if="row.original_price" class="ml-1 text-xs tabular-nums text-ink-faint line-through">${{ row.original_price.toFixed(2) }}</span>
          </div>
        </template>
        ...
        <template #cell-for_sale="{ value, row }">
          <button
            ...
            :class="[
              ...
              value ? 'bg-brand-500' : 'bg-line dark:bg-dark-600'
            ]"
        ...
        <template #cell-actions="{ row }">
          <div class="flex items-center gap-2">
            <button @click="openPlanEdit(row)" class="flex flex-col items-center gap-0.5 rounded-control p-1.5 text-ink-soft transition-colors hover:bg-brand-50 hover:text-brand-600 dark:hover:bg-brand-950/30 dark:hover:text-brand-400">
              <Icon name="edit" size="sm" />
              <span class="text-xs">{{ t('common.edit') }}</span>
            </button>
            <button @click="confirmDeletePlan(row)" class="flex flex-col items-center gap-0.5 rounded-control p-1.5 text-ink-soft transition-colors hover:bg-danger-soft hover:text-danger dark:hover:bg-red-900/20 dark:hover:text-red-400">
...
function getPlanNameClass(groupId: number): string {
  ...
  return group ? platformTextClass(group.platform) : 'text-ink dark:text-white'
}

function validityUnitLabelKey(unit: string): string {
  switch (normalizePaymentPlanValidityUnit(unit)) {
    case 'week':
      return 'weeks'
    case 'month':
      return 'months'
