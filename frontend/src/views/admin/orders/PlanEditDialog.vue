<template>
  <BaseDialog :show="show" :title="plan ? t('payment.admin.editPlan') : t('payment.admin.createPlan')" width="wide" @close="emit('close')">
    <form id="plan-form" @submit.prevent="handleSavePlan" class="space-y-4">
        ...
        <div>
          ...
          <Select v-model="planForm.group_id" :options="groupOptions" :placeholder="t('payment.admin.selectGroup')" class="w-full">
            ...
            <template #option="{ option, selected }">
              ...
              <Icon v-if="selected" name="check" size="sm" class="text-brand-500" :stroke-width="2" />
            </template>
          </Select>
        </div>
      </div>
      ...
      <!-- Group Info Preview -->
      <div v-if="selectedGroupInfo" class="rounded-control border border-line bg-page p-3 dark:border-dark-600 dark:bg-dark-800">
        ...
        <div class="grid grid-cols-2 gap-2 text-xs">
          <div><span class="text-ink-soft">{{ t('payment.admin.dailyLimit') }}:</span> <span class="ml-1 font-medium text-ink dark:text-dark-300">{{ selectedGroupInfo.daily_limit_usd != null ? '$' + selectedGroupInfo.daily_limit_usd : t('payment.admin.unlimited') }}</span></div>
          <div><span class="text-ink-soft">{{ t('payment.admin.weeklyLimit') }}:</span> <span class="ml-1 font-medium text-ink dark:text-dark-300">{{ selectedGroupInfo.weekly_limit_usd != null ? '$' + selectedGroupInfo.weekly_limit_usd : t('payment.admin.unlimited') }}</span></div>
          <div><span class="text-ink-soft">{{ t('payment.admin.monthlyLimit') }}:</span> <span class="ml-1 font-medium text-ink dark:text-dark-300">{{ selectedGroupInfo.monthly_limit_usd != null ? '$' + selectedGroupInfo.monthly_limit_usd : t('payment.admin.unlimited') }}</span></div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          ...
          <p v-if="subscriptionCnyPreview" class="mt-1 text-xs font-medium text-brand-600 dark:text-brand-400">
      ...
      <div>
        ...
        <p class="mt-1 text-xs text-ink-soft dark:text-dark-400">{{ t('payment.admin.featuresHint') }}</p>
      </div>
      <div class="flex items-center gap-3">
        <label class="text-sm text-ink dark:text-dark-300">{{ t('payment.admin.forSale') }}</label>
        <button
          type="button"
          :class="[
            ...
            planForm.for_sale ? 'bg-brand-500' : 'bg-line dark:bg-dark-600'
          ]"
...
watch(() => props.show, (visible) => {
  if (!visible) return
  if (props.plan) {
    Object.assign(planForm, { name: props.plan.name, group_id: props.plan.group_id, description: props.plan.description, price: props.plan.price, original_price: props.plan.original_price || 0, validity_days: props.plan.validity_days, validity_unit: normalizePaymentPlanValidityUnit(props.plan.validity_unit), sort_order: props.plan.sort_order || 0, for_sale: props.plan.for_sale })
