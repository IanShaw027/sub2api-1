<template>
  <BaseDialog :show="show" :title="plan ? t('payment.admin.editPlan') : t('payment.admin.createPlan')" width="wide" @close="emit('close')">
    <form id="plan-form" @submit.prevent="handleSavePlan" class="space-y-4">
      ...
      <!-- Group Info Preview -->
      <div v-if="selectedGroupInfo" class="rounded-lg border border-line bg-surface-2 p-3  ">
        ...
        <div class="grid grid-cols-2 gap-2 text-xs">
          <div><span class="text-muted">{{ t('payment.admin.dailyLimit') }}:</span> <span class="ml-1 font-medium text-foreground ">{{ selectedGroupInfo.daily_limit_usd != null ? '$' + selectedGroupInfo.daily_limit_usd : t('payment.admin.unlimited') }}</span></div>
          <div><span class="text-muted">{{ t('payment.admin.weeklyLimit') }}:</span> <span class="ml-1 font-medium text-foreground ">{{ selectedGroupInfo.weekly_limit_usd != null ? '$' + selectedGroupInfo.weekly_limit_usd : t('payment.admin.unlimited') }}</span></div>
          <div><span class="text-muted">{{ t('payment.admin.monthlyLimit') }}:</span> <span class="ml-1 font-medium text-foreground ">{{ selectedGroupInfo.monthly_limit_usd != null ? '$' + selectedGroupInfo.monthly_limit_usd : t('payment.admin.unlimited') }}</span></div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          ...
          <p v-if="subscriptionCnyPreview" class="mt-1 text-xs font-medium text-accent ">
      ...
      <div class="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <p class="mt-1 text-xs text-muted ">{{ t('payment.admin.currencyHint') }}</p>
        </div>
      </div>
      <div>
