  ...
  <AppLayout>
          ...
          <div class="flex flex-1 flex-wrap items-center gap-3">
            ...
            <div class="relative w-full md:w-64">
              <Icon
                name="search"
                size="md"
                class="absolute left-3 top-1/2 -translate-y-1/2 text-ink-faint dark:text-dark-500"
              />
              <input
                v-model="searchQuery"
                type="text"
          ...
          <div class="flex flex-wrap items-center justify-end gap-2">
            ...
            <div class="flex items-center gap-2 md:contents">
              ...
              <div class="relative" ref="filterDropdownRef">
                ...
                <div
                  ...
                  class="absolute right-0 top-full z-50 mt-1 w-48 rounded-card border border-line bg-card py-1 shadow-md dark:border-dark-600 dark:bg-dark-800"
                >
                  <!-- Built-in filters -->
                  <button
                  ...
                  <div
                    ...
                    class="my-1 border-t border-line dark:border-dark-700"
                  ></div>
            ...
            <div class="relative" ref="sortDropdownRef">
              ...
              <div
                ...
                class="absolute right-0 top-full z-50 mt-1 w-56 rounded-card border border-line bg-card py-1 shadow-md dark:border-dark-600 dark:bg-dark-800"
              >
                <button
        ...
        >
          <template #header-concurrency>
            ...
            >
              ...
              <span class="text-[10px] font-normal uppercase tracking-wide text-ink-faint dark:text-dark-500">
                {{ concurrencySortLabel }}
              </span>
              <span class="text-ink-faint dark:text-dark-500">
          ...
          <template #cell-email="{ value, row }">
            <div class="flex items-center gap-2">
              <div
                class="flex h-8 w-8 items-center justify-center overflow-hidden rounded-full bg-brand-100 dark:bg-brand-900/30"
              >
                <img
                ...
                />
                <span v-else class="text-sm font-medium text-brand-700 dark:text-brand-300">
                  {{ value.charAt(0).toUpperCase() }}
                </span>
              </div>
              <button
                type="button"
                class="font-medium text-ink underline decoration-dashed decoration-line underline-offset-4 transition-colors hover:text-brand-600 dark:text-white dark:decoration-dark-500 dark:hover:text-brand-400"
          ...
          <template #cell-notes="{ value }">
            <div class="max-w-xs">
              <span
                ...
                class="block truncate text-sm text-ink-body dark:text-dark-400"
              >
                {{ value.length > 30 ? value.substring(0, 25) + '...' : value }}
              </span>
              <span v-else class="text-sm text-ink-faint">-</span>
            </div>
          </template>
          ...
          >
            <div class="max-w-xs">
              <span
                class="block truncate text-sm text-ink-body dark:text-dark-200"
          ...
          <template #cell-groups="{ row }">
            <div v-if="allGroups.length > 0" class="flex flex-col gap-1">
              ...
              >
                ...
                <span class="text-ink-soft dark:text-dark-400">{{ t('admin.users.exclusiveLabel') }}</span>
                <!-- Hover tooltip（操作菜单未打开时显示） -->
                <div
                  v-if="expandedGroupUserId !== row.id"
                  class="pointer-events-none absolute left-0 top-full z-50 mt-1.5 rounded bg-ink px-2.5 py-1.5 text-xs text-white opacity-0 shadow-lg transition-opacity duration-75 group-hover/ex:opacity-100 dark:bg-dark-600"
                >
                  <div class="absolute left-4 bottom-full border-4 border-transparent border-b-ink dark:border-b-dark-600"></div>
                ...
                <div
                  v-if="expandedGroupUserId === row.id"
                  class="absolute left-0 top-full z-50 mt-1.5 min-w-[160px] overflow-hidden rounded-lg border border-line bg-card py-1 text-xs shadow-xl dark:border-dark-600 dark:bg-dark-700"
                >
                  <div class="border-b border-line px-3 py-1.5 text-[10px] font-medium uppercase tracking-wider text-ink-faint dark:border-dark-600 dark:text-dark-400">
                    {{ t('admin.users.clickToReplace') }}
                  </div>
                  <div
                    v-for="g in getUserGroups(row).exclusive"
                    :key="g.id"
                    class="flex cursor-pointer items-center gap-2 px-3 py-2 text-ink-body transition-colors hover:bg-brand-50 hover:text-brand-600 dark:text-dark-200 dark:hover:bg-brand-900/30 dark:hover:text-brand-400"
              ...
              >
                <Icon name="globe" size="xs" class="h-3.5 w-3.5 text-ink-faint dark:text-dark-500" />
                <span class="font-medium text-ink-body dark:text-dark-300">{{ getUserGroups(row).publicGroups.length }}</span>
                <span class="text-ink-faint dark:text-dark-500">{{ t('admin.users.publicLabel') }}</span>
                <!-- Tooltip: 向下弹出 -->
                <div class="pointer-events-none absolute left-0 top-full z-50 mt-1.5 rounded bg-ink px-2.5 py-1.5 text-xs text-white opacity-0 shadow-lg transition-opacity duration-75 group-hover/pub:opacity-100 dark:bg-dark-600">
                  <div class="absolute left-4 bottom-full border-4 border-transparent border-b-ink dark:border-b-dark-600"></div>
                  <div class="flex flex-col gap-0.5 whitespace-nowrap">
            ...
            <span v-else class="text-xs text-ink-faint dark:text-dark-500">-</span>
          </template>
          ...
          <template #cell-subscriptions="{ row }">
            ...
            <span
              v-else
              class="inline-flex items-center gap-1.5 rounded-md bg-page px-2 py-1 text-xs text-ink-faint dark:bg-dark-700/50 dark:text-dark-500"
            >
          ...
          <template #cell-balance="{ value, row }">
            <div class="flex items-center gap-2">
              <div class="group relative">
                ...
                <!-- Instant tooltip -->
                <div class="pointer-events-none absolute bottom-full left-1/2 z-50 mb-1.5 -translate-x-1/2 whitespace-nowrap rounded bg-ink px-2 py-1 text-xs text-white opacity-0 shadow-lg transition-opacity duration-75 group-hover:opacity-100 dark:bg-dark-600">
                  {{ t('admin.users.balanceHistoryTip') }}
                  <div class="absolute left-1/2 top-full -translate-x-1/2 border-4 border-transparent border-t-ink dark:border-t-dark-600"></div>
                </div>
              </div>
              <button
          ...
          <template #cell-balance_platform_quota="{ row }">
            <button
              type="button"
              class="block text-left underline decoration-dashed decoration-line underline-offset-4 transition-colors hover:decoration-brand-400 dark:decoration-dark-500"
          ...
          <template #header-usage>
            <div class="flex items-center gap-1.5">
              ...
              <div class="usage-sort-trigger relative">
                <button
                  type="button"
                  class="flex items-center gap-1 rounded px-1 py-0.5 transition-colors hover:bg-page dark:hover:bg-dark-700"
                  :class="usageSort ? 'text-brand-600 dark:text-brand-400' : 'text-ink-faint dark:text-dark-500'"
                ...
                <div
                  ...
                  class="absolute right-0 top-full z-50 mt-1 min-w-[140px] rounded-lg border border-line bg-card py-1 shadow-lg dark:border-dark-600 dark:bg-dark-800"
                >
                  <button
                    v-for="metric in USAGE_METRICS"
                    :key="metric"
                    type="button"
                    class="flex w-full items-center justify-between gap-3 px-3 py-1.5 text-left text-xs normal-case tracking-normal hover:bg-page dark:hover:bg-dark-700"
                  ...
                  <div class="mt-1 border-t border-line px-3 py-1 text-[10px] normal-case tracking-normal text-ink-faint dark:border-dark-700 dark:text-dark-500">
          ...
          <template #cell-usage="{ row }">
            <div class="text-sm">
              <div class="flex items-center gap-1.5">
                <span class="text-ink-soft dark:text-dark-400">{{ t('admin.users.todayBalance', '今日余额') }}:</span>
                <span class="font-medium text-ink dark:text-white">
                  {{ formatOptionalCost(row.today_balance_actual_cost) }}
                </span>
              </div>
              <div class="mt-0.5 flex items-center gap-1.5">
                <span class="text-ink-soft dark:text-dark-400">{{ t('admin.users.todaySubscription', '今日订阅') }}:</span>
                <span class="font-medium text-ink dark:text-white">
                  {{ formatOptionalCost(row.today_subscription_actual_cost) }}
                </span>
              </div>
              <div class="mt-0.5 flex items-center gap-1.5">
                <span class="text-ink-soft dark:text-dark-400">{{ t('admin.users.last30Days', '近30日') }}:</span>
                <span class="font-medium text-ink dark:text-white">
          ...
          <template #cell-status="{ value }">
            <div class="flex items-center gap-1.5">
              ...
              ></span>
              <span class="text-sm text-ink-body dark:text-dark-200">
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              ...
              <button
                ...
                :class="[
                  'flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-ink-soft transition-colors',
                  row.status === 'active'
              ...
              <button
                ...
                class="action-menu-trigger flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-ink-soft transition-colors hover:bg-page hover:text-ink dark:hover:bg-dark-700 dark:hover:text-white"
                :class="{ 'bg-page text-ink dark:bg-dark-700 dark:text-white': activeMenuId === row.id }"
              >
    ...
    <Teleport to="body">
      <div
        ...
        class="action-menu-content fixed z-[9999] w-48 overflow-hidden rounded-xl bg-card shadow-lg ring-1 ring-black/5 dark:bg-dark-800 dark:ring-white/10"
          ...
          <template v-for="user in users" :key="user.id">
            <template v-if="user.id === activeMenuId">
              ...
              <button
                ...
                class="flex w-full items-center gap-2 px-4 py-2 text-sm text-ink-body hover:bg-page dark:text-dark-200 dark:hover:bg-dark-700"
              >
                <Icon name="key" size="sm" class="text-ink-faint" :stroke-width="2" />
              ...
              <button
                ...
                class="flex w-full items-center gap-2 px-4 py-2 text-sm text-ink-body hover:bg-page dark:text-dark-200 dark:hover:bg-dark-700"
              >
                <Icon name="users" size="sm" class="text-ink-faint" :stroke-width="2" />
              ...
              <button
                ...
                class="flex w-full items-center gap-2 px-4 py-2 text-sm text-ink-body hover:bg-page dark:text-dark-200 dark:hover:bg-dark-700"
              >
                <Icon name="chartBar" size="sm" class="text-ink-faint" :stroke-width="2" />
              ...
              <button
                ...
                class="flex w-full items-center gap-2 px-4 py-2 text-sm text-ink-body hover:bg-page dark:text-dark-200 dark:hover:bg-dark-700"
              >
                <Icon name="dollar" size="sm" class="text-ink-faint" :stroke-width="2" />
...
const DEFAULT_HIDDEN_COLUMNS = ['notes', 'groups', 'subscriptions', 'balance_platform_quota']
...
const toggleColumn = (key: string) => {
  ...
  if (wasHidden && (key.startsWith('attr_') || key === 'balance_platform_quota')) {
...
const loadUsers = async () => {
  ...
  try {
    ...
    if (
      ...
      ((attributeDefinitions.value.length > 0 && hasVisibleAttributeColumns.value) ||
        hasVisiblePlatformQuotaColumn.value)
    ) {
