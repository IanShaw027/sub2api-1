  ...
  <AppLayout>
    <div class="mx-auto max-w-6xl space-y-6">
      ...
      <div v-if="loading" class="flex items-center justify-center py-12">
        <div
          class="h-8 w-8 animate-spin rounded-full border-b-2 border-brand-600"
        ></div>
      </div>

      <!-- Settings Form -->
      <form v-else @submit.prevent="saveSettings" class="space-y-6" novalidate>
        <!-- Tab Navigation (chrome only — do not alter tab.key / activeTab logic) -->
        <div class="settings-tabs-shell" data-testid="settings-tabs-shell">
          <nav
        ...
        >
          ...
          <div class="card">
            <div
              class="border-b border-line px-6 py-4 dark:border-dark-700"
            >
            ...
            <div class="space-y-4 p-6">
              ...
              >
                <div
                  class="h-4 w-4 animate-spin rounded-full border-b-2 border-brand-600"
                ></div>
                {{ t("common.loading") }}
              </div>
              ...
              <div v-else class="space-y-4">
	            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <label
                      class="mb-1 block text-sm font-medium text-ink-body"
                    >
                      {{ t("admin.settings.adminApiKey.currentKey") }}
                    </label>
                    <code
                      class="rounded bg-page px-2 py-1 font-mono text-sm text-ink dark:bg-dark-700 dark:text-ink"
                    >
                      {{ adminApiKeyMasked }}
                    </code>
                  </div>
                ...
                >
                  ...
                  <div class="flex items-center gap-2">
                    <code
                      class="flex-1 select-all break-all rounded border border-green-300 bg-card px-3 py-2 font-mono text-sm dark:border-green-700 dark:bg-dark-800"
                    >
                      {{ newAdminApiKey }}
                    </code>
                    <button
          ...
          <div class="card">
            <div class="border-b border-line px-6 py-4 dark:border-dark-700">
              <h2 class="text-lg font-semibold text-ink dark:text-white">
            ...
            <div class="space-y-5 p-6">
              <div class="flex items-center justify-between">
                <div>
                  <label class="font-medium text-ink dark:text-white">{{ localText("启用异常 IP 封禁", "Enable suspicious IP blocking") }}</label>
                  <p class="text-sm text-ink-soft dark:text-dark-400">{{ localText("关闭后停止新增封禁，现有封禁仍继续生效。", "When disabled, new detections stop while existing bans remain enforced.") }}</p>
                </div>
                <Toggle v-model="form.ip_multi_account_ban_enabled" />
              </div>
              <div v-if="form.ip_multi_account_ban_enabled" class="grid gap-4 border-t border-line pt-4 dark:border-dark-700 md:grid-cols-2">
                <div>
              ...
              <div class="flex flex-wrap items-center gap-3 border-t border-line pt-4 dark:border-dark-700">
                ...
                <span class="text-xs text-ink-soft dark:text-dark-400">{{ localText("详情包含账号、来源类型、路径和调用次数。", "Details include accounts, source type, paths, and call counts.") }}</span>
              </div>
              <div v-if="ipSecurityBans.length" class="overflow-x-auto rounded border border-line dark:border-dark-700">
                <table class="min-w-full text-left text-sm">
                  <thead class="bg-page dark:bg-dark-800"><tr><th class="px-3 py-2">IP</th><th class="px-3 py-2">{{ localText("状态", "Status") }}</th><th class="px-3 py-2">{{ localText("账号数", "Accounts") }}</th><th class="px-3 py-2">{{ localText("创建时间", "Created") }}</th><th class="px-3 py-2"></th></tr></thead>
                  <tbody>
                    <tr v-for="ban in ipSecurityBans" :key="ban.id" class="border-t border-line dark:border-dark-700">
              ...
              <div v-if="ipSecurityTotal" class="flex items-center justify-between text-sm text-ink-soft">
              ...
              <div v-if="ipSecurityDetail" class="rounded border border-line p-4 dark:border-dark-700">
                <div class="flex flex-wrap items-center justify-between gap-3"><div><div class="font-mono font-semibold">{{ ipSecurityDetail.ban.ip_address }}</div><div class="text-xs text-ink-soft">{{ formatIPSecurityStatus(ipSecurityDetail.ban.status) }} · {{ ipSecurityDetail.ban.reason }}</div></div><button v-if="ipSecurityDetail.ban.status === 'active'" type="button" class="btn btn-danger btn-sm" @click="releaseIPSecurityBan(ipSecurityDetail.ban.id)">{{ localText("解除并加入白名单", "Release and whitelist") }}</button><button v-else-if="ipSecurityDetail.ban.status === 'whitelisted'" type="button" class="btn btn-secondary btn-sm" @click="removeIPSecurityWhitelist(ipSecurityDetail.ban.id)">{{ localText("恢复检测", "Restore detection") }}</button></div>
                <div class="mt-3 space-y-2 text-sm"><div v-for="activity in ipSecurityDetail.activities" :key="`${activity.user_id}-${activity.source}-${activity.api_key_id}-${activity.request_id}-${activity.path}`" class="rounded bg-page p-2 dark:bg-dark-800"><div class="flex flex-wrap justify-between gap-2"><span>{{ activity.user_email || activity.user_username || `#${activity.user_id}` }}</span><span class="font-mono text-xs">{{ activity.source }}<span v-if="activity.api_key_id"> · key #{{ activity.api_key_id }}</span></span></div><div class="mt-1 break-all font-mono text-xs text-ink-soft">{{ activity.method }} {{ activity.path }}</div><div class="mt-1 text-[11px] text-ink-faint">{{ localText("调用", "Calls") }}: {{ activity.request_count }} · {{ formatIPSecurityTime(activity.first_seen_at) }} - {{ formatIPSecurityTime(activity.last_seen_at) }}</div><div class="mt-1 break-all text-[11px] text-ink-faint">peer: {{ activity.peer_ip || '-' }}<span v-if="activity.forwarded_for"> · forwarded: {{ activity.forwarded_for }}</span></div><div v-if="activity.request_id" class="mt-1 break-all text-[11px] text-ink-faint">request: {{ activity.request_id }}</div></div><div v-if="ipSecurityDetail.pages > 1" class="flex items-center justify-end gap-2 pt-2"><button type="button" class="btn btn-secondary btn-xs" :disabled="ipSecurityDetail.page <= 1" @click="loadIPSecurityDetailPage(ipSecurityDetail.page - 1)"><Icon name="chevronLeft" size="sm" /></button><span class="text-xs text-ink-soft">{{ ipSecurityDetail.page }} / {{ ipSecurityDetail.pages }}</span><button type="button" class="btn btn-secondary btn-xs" :disabled="ipSecurityDetail.page >= ipSecurityDetail.pages" @click="loadIPSecurityDetailPage(ipSecurityDetail.page + 1)"><Icon name="chevronRight" size="sm" /></button></div></div>
              </div>
            </div>
          </div>
        <!-- Tab: Security — Registration, Turnstile, LinuxDo -->
          ...
          <div class="card">
            ...
            <div class="space-y-5 p-6">
              ...
              <div class="flex items-center justify-between">
                <div>
                  <label class="font-medium text-ink dark:text-white">{{
              ...
              <!-- Email Verification -->
              <div
                class="flex items-center justify-between border-t border-line pt-4 dark:border-dark-700"
              >
                <div>
                  <label class="font-medium text-ink dark:text-white">{{
              ...
              <div class="border-t border-line pt-4 dark:border-dark-700">
                <label class="font-medium text-ink dark:text-white">{{
                ...
                <div
                  class="mt-3 rounded-lg border border-line bg-card p-2 dark:border-dark-500 dark:bg-dark-700"
                >
                  <div class="flex flex-wrap items-center gap-2">
                    <span
                      v-for="suffix in registrationEmailSuffixWhitelistTags"
                      :key="suffix"
                      class="inline-flex items-center gap-1 rounded bg-page px-2 py-1 text-xs font-mono text-ink-body dark:bg-dark-600 dark:text-ink-body"
                    >
                      <span class="text-ink-faint dark:text-dark-400">@</span>
                      <span>{{ suffix }}</span>
                      <button
                        type="button"
                        class="rounded-full text-ink-soft hover:bg-page hover:text-ink-body dark:hover:bg-dark-500 dark:hover:text-white"
                        @click="
                    ...
                    <div
                      class="flex min-w-[220px] flex-1 items-center gap-1 rounded border border-transparent px-2 py-1 focus-within:border-brand-300 dark:focus-within:border-brand-700"
                    >
                      <span
                        class="font-mono text-sm text-ink-faint dark:text-dark-400"
                        >@</span
                      >
                      <input
                        v-model="registrationEmailSuffixWhitelistDraft"
                        type="text"
                        class="w-full bg-transparent text-sm font-mono text-ink outline-none placeholder:text-ink-faint dark:text-white dark:placeholder:text-ink-soft"
                        :placeholder="
                          t(
              ...
              <div
                ...
                class="border-t border-line pt-4 dark:border-dark-700"
              >
                <label
                  class="mb-2 block text-sm font-medium text-ink-body"
                >
                ...
                <input
                  ...
                  "
                />
                <p class="mt-1.5 text-xs text-ink-soft dark:text-dark-400">
                  ...
                  <div>
                    ...
                    >
                      ...
                      <code
                        ...
                        class="select-all break-all rounded bg-page px-2 py-1 font-mono text-xs text-ink-body dark:bg-dark-800"
                      >
          ...
          <div class="card">
            ...
            <div class="space-y-5 p-6">
              ...
              <div
                ...
                class="space-y-6 border-t border-line pt-4 dark:border-dark-700"
              >
                ...
                <div
                  class="flex items-center justify-between rounded border border-line px-4 py-3 dark:border-dark-700"
                >
                  <div>
                    <label class="font-medium text-ink dark:text-white">
                      {{
                    ...
                    <p class="mt-0.5 text-xs text-ink-soft dark:text-dark-400">
                      {{
                        localText(
                ...
                <div
                  class="space-y-4 rounded-lg border border-line p-4 dark:border-dark-700"
                >
                  <div>
                    <h3 class="font-medium text-ink dark:text-white">
                  ...
                  <div
                    class="rounded-lg border border-line p-4 dark:border-dark-700"
                  >
                    <div class="flex items-start justify-between gap-4">
                      <div>
                        <h4 class="font-medium text-ink dark:text-white">
              ...
              <div class="grid grid-cols-1 gap-6 xl:grid-cols-2">
                <div class="rounded-lg border border-line p-4 dark:border-dark-700">
                  ...
                  <div v-if="form.github_oauth_enabled" class="mt-4 space-y-4">
                    <div class="rounded-lg bg-page px-3 py-2 text-xs text-ink-body dark:bg-dark-800">
                    ...
                    <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
                      <div>
                        <label class="mb-2 block text-sm font-medium text-ink-body">Client ID</label>
                        <input
                      ...
                      <div>
                        <label class="mb-2 block text-sm font-medium text-ink-body">Client Secret</label>
                        <input
                    ...
                    <div>
                      <label class="mb-2 block text-sm font-medium text-ink-body">
                        {{ localText("后端回调地址", "Backend Callback URL") }}
                      </label>
                      <input
                      ...
                      <div class="mt-2 flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-3">
                        ...
                        <code
                          ...
                          class="select-all break-all rounded bg-page px-2 py-1 font-mono text-xs text-ink-body dark:bg-dark-800 dark:text-ink-body"
                        >
                ...
                <div class="rounded-lg border border-line p-4 dark:border-dark-700">
                  ...
                  <div v-if="form.google_oauth_enabled" class="mt-4 space-y-4">
                    <div class="rounded-lg bg-page px-3 py-2 text-xs text-ink-body dark:bg-dark-800 dark:text-ink-body">
                      {{
                        localText(
        ...
        >
          <div class="rounded-xl border border-line bg-page px-5 py-4 dark:border-dark-700 dark:bg-dark-800/50">
            ...
            <div class="space-y-5 p-6">
              ...
              <template v-else>
                ...
                <div
                  ...
                  class="space-y-4 border-t border-line pt-4 dark:border-dark-700"
                >
                  <div>
                    <label
                      class="mb-2 block text-sm font-medium text-ink-body"
                ...
                <div
                  class="flex justify-end border-t border-line pt-4 dark:border-dark-700"
                >
                  <button
                    type="button"
              ...
              <template v-else>
                ...
                >
                  ...
                  <div class="flex items-center justify-between">
                    <div>
                      <label
                        class="text-sm font-medium text-ink-body"
                        >{{
                  ...
                  <div
                    ...
                    class="ml-4 space-y-3 border-l-2 border-line pl-4 dark:border-dark-600"
                  >
                    <div>
                      <label
                        class="text-sm font-medium text-ink-body"
                    ...
                    <button
                      ...
                      class="btn btn-ghost btn-xs text-brand-600 dark:text-brand-400"
                    >
            ...
            <div class="space-y-5 p-6">
              ...
              <template v-else>
                ...
                <div
                  ...
                  :key="rule.beta_token"
                  class="rounded-lg border border-line p-4 dark:border-dark-600"
                >
                  <div class="mb-3 flex items-center gap-2">
                    <span
                      class="text-sm font-medium text-ink dark:text-white"
                    >
                      {{ getBetaDisplayName(rule.beta_token) }}
                    </span>
                    <span
                      class="rounded bg-page px-2 py-0.5 text-xs text-ink-soft dark:bg-dark-700"
                    >
                      {{ rule.beta_token }}
                    </span>
                  </div>
                  ...
                  <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      <label
                        class="mb-1 block text-xs font-medium text-ink-body dark:text-dark-400"
                      >
                  ...
                  <div v-if="rule.action === 'block'" class="mt-3">
                    ...
                    <input
                      ...
                      "
                    />
                    <p class="mt-1 text-xs text-ink-faint dark:text-dark-400">
                  ...
                  <div v-if="betaPresets[rule.beta_token]?.length" class="mt-3">
                    ...
                    <div class="flex flex-wrap gap-2">
                      <button
                        ...
                        :key="preset.label"
                        type="button"
                        class="inline-flex items-center gap-1 rounded-md border border-brand-200 bg-brand-50 px-2.5 py-1 text-xs font-medium text-brand-700 transition-colors hover:bg-brand-100 dark:border-brand-800 dark:bg-brand-900/30 dark:text-brand-300 dark:hover:bg-brand-900/50"
                  ...
                  <div class="mt-3">
                    ...
                    <p class="mb-2 text-xs text-ink-faint dark:text-dark-400">
                    ...
                    <button
                      ...
                      "
                      class="mb-2 inline-flex items-center gap-1 text-xs text-brand-600 transition-colors hover:text-brand-700 dark:text-brand-400 dark:hover:text-brand-300"
                    >
                      <svg
                        class="h-3.5 w-3.5"
                        fill="none"
                    ...
                    <div class="flex flex-wrap items-center gap-1.5">
                      <span class="text-xs text-ink-faint dark:text-dark-400"
                        >{{
                      ...
                      <button
                        ...
                        :key="pattern"
                        type="button"
                        class="rounded border border-line px-2 py-0.5 text-xs text-ink-body transition-colors hover:border-brand-300 hover:bg-brand-50 hover:text-brand-700 dark:border-dark-600 dark:hover:border-brand-700 dark:hover:bg-brand-900/30 dark:hover:text-brand-300"
          ...
          <div class="rounded-xl border border-accent-200 bg-accent-50 px-5 py-4 dark:border-blue-900/60 dark:bg-blue-950/30">
            <h2 class="text-base font-semibold text-accent-900 dark:text-blue-100">
              {{ localText("OpenAI 平台模块", "OpenAI Platform Modules") }}
            </h2>
            <p class="mt-1 text-sm text-accent-700 dark:text-blue-300">
              {{
                localText(
          ...
          <div class="card">
            ...
            <div class="space-y-5 p-6">
              ...
              <div
                ...
                class="rounded-lg border border-dashed border-line p-6 text-center text-sm text-ink-soft dark:border-dark-600"
              >
              ...
              <div>
                ...
                <p class="mt-2 text-xs text-ink-faint dark:text-dark-400">
        ...
        <!-- Tab: Gateway — Claude Code, Scheduling -->
            ...
            <div class="space-y-5 p-6">
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
                  ...
                  >
                    <div class="flex items-start justify-between gap-3">
                      <div>
                        <label
                          class="font-mono text-sm font-medium text-ink dark:text-white"
                        >
                          {{ platform }}
                        </label>
                        <p class="mt-0.5 text-xs text-ink-soft dark:text-dark-400">
                      ...
                      <span
                        class="rounded bg-page px-2 py-0.5 text-[11px] font-medium text-ink-body dark:bg-dark-700 dark:text-ink-body"
                      >
                        %
                      </span>
                    </div>
              ...
              <div class="flex items-center justify-between">
                ...
                <Toggle
                  ...
                  data-testid="openai-advanced-scheduler-enabled"
                />
              </div>

              <div
                v-if="form.openai_advanced_scheduler_enabled"
                class="flex items-center justify-between border-t border-line pt-5 dark:border-dark-700"
              >
                <div>
                  <label class="text-sm font-medium text-ink-body">
                ...
                <Toggle
                  ...
                  data-testid="openai-advanced-scheduler-sticky-weighted"
                />
              </div>
              ...
              >
                ...
                <Toggle
                  ...
                  data-testid="openai-advanced-scheduler-subscription-priority"
                />
              </div>

              <div
                v-if="form.openai_advanced_scheduler_enabled"
                class="border-t border-line pt-5 dark:border-dark-700"
              >
                <div>
                  <label class="text-sm font-medium text-ink-body">
                ...
                <div class="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-5">
                  <label
                    ...
                    class="block"
                  >
                    <span class="text-xs font-medium text-ink-body dark:text-dark-400">
                      {{ field.label }}
                    </span>
                    <input
                      v-model="form[field.key]"
                      :data-testid="`openai-advanced-scheduler-${field.key.replace('openai_advanced_scheduler_', '')}`"
                      class="input mt-1"
                      inputmode="decimal"
              ...
              <div class="grid gap-4 rounded-lg border border-line p-4 dark:border-dark-700">
          ...
          <div class="card">
            ...
            <div class="p-6 space-y-4">
                ...
                <div>
                  ...
                  <p class="mb-2 mt-1 text-xs text-ink-soft dark:text-dark-400">
                  ...
                  >
                    ...
                    />
                    <label class="flex shrink-0 items-center gap-1 text-xs text-ink-body dark:text-dark-400">
                ...
                <div class="flex items-center justify-between">
                  <div class="pr-4">
                    <label
                      class="block text-sm font-medium text-ink-body"
                    >
                      {{
                ...
                <div>
                  ...
                  >
                    ...
                    />
                    <label
                      class="flex shrink-0 items-center gap-1 text-xs text-ink-body dark:text-dark-400"
                      :title="
                        t(
              ...
              <div>
                ...
                <div class="space-y-3">
                  <div
                    ...
                    :key="block.id"
                    class="rounded-lg border border-line bg-page p-4 dark:border-dark-700 dark:bg-dark-800/60"
                  >
                    <div
                      :class="[
                    ...
                    <div v-show="block.expanded">
                      <div class="grid gap-3 md:grid-cols-2">
                        <div>
                          <label
                            class="mb-1 block text-xs font-medium text-ink-body"
                          >
                            {{
                              t(
                      ...
                      >
                        <div class="flex items-center justify-between gap-4">
                          <div>
                            <label
                              class="text-xs font-medium text-ink-body"
                            >
                              {{
                                t(
            ...
            <div class="p-6 space-y-4">
              <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
                <div
                  ...
                  :key="p"
                  class="flex items-center justify-between rounded-lg border border-line bg-page px-3 py-2 dark:border-dark-700 dark:bg-dark-800/60"
                >
                  <span class="font-mono text-sm text-ink-body">{{ p }}</span>
            ...
            <div class="space-y-5 p-6">
              ...
              <div v-if="webSearchConfig.enabled" class="space-y-4">
                ...
                <div
                  ...
                  class="rounded-lg border border-dashed border-line p-4 text-center text-sm text-ink-faint dark:border-dark-600"
                >
                ...
                <div
                  ...
                  :key="pIdx"
                  class="rounded-lg border border-line dark:border-dark-600"
                >
                  <!-- Collapsible header -->
                  <div
                  ...
                  <div
                    ...
                    class="space-y-3 border-t border-line px-4 pb-4 pt-3 dark:border-dark-700"
                  >
                    ...
                    <div>
                      ...
                      <div class="relative">
                        ...
                        >
                          <button
                            type="button"
                            class="rounded p-1 text-ink-faint hover:text-ink-body"
                            :title="
                              apiKeyVisible[pIdx]
                                ? t(
                    ...
                    <div class="flex items-center gap-2">
                      ...
                      <div
                        v-if="
                          ...
                          provider.quota_limit > 0
                        "
                        class="flex-1 rounded-full bg-line dark:bg-dark-600"
                        style="height: 6px"
                      >
                        <div
          ...
          >
            <div
              class="mx-4 w-full max-w-lg rounded-xl bg-card p-6 shadow-xl dark:bg-dark-800"
            >
              <h3
                class="mb-4 text-lg font-semibold text-ink dark:text-white"
              >
              ...
              <div
                v-if="wsTestResult"
                class="mt-4 max-h-80 overflow-y-auto rounded-lg bg-page p-4 dark:bg-dark-700"
              >
                <p
                  class="mb-2 text-sm font-medium text-ink-body"
                >
                  {{
                ...
                <div
                  ...
                  :key="rIdx"
                  class="mt-2 border-t border-line pt-2 first:mt-0 first:border-0 first:pt-0 dark:border-dark-600"
                >
                  <a
                    :href="r.url"
                    target="_blank"
                    class="text-sm font-medium text-accent-600 hover:underline dark:text-blue-400"
                    >{{ r.title }}</a
                  >
                  <p class="mt-0.5 text-xs text-ink-soft dark:text-dark-400">
                    {{ r.snippet }}
            ...
            <div class="space-y-6 p-6">
              ...
              <div class="border-t border-line pt-4 dark:border-dark-700">
                ...
                <div
                  ...
                  class="rounded border border-dashed border-line px-4 py-3 text-sm text-ink-soft dark:border-dark-600"
                >
                ...
                <div v-else class="space-y-3">
                  <div
                    ...
                    class="grid grid-cols-1 gap-3 rounded border border-line p-3 md:grid-cols-[1fr_160px_auto] dark:border-dark-600"
                  >
                    <div>
                      <label
                        class="mb-1 block text-xs font-medium text-ink-body dark:text-dark-400"
              ...
              <div class="border-t border-line pt-4 dark:border-dark-700">
                ...
                <div class="overflow-x-auto">
                  <table class="min-w-full text-sm">
                    <thead>
                      <tr class="text-left text-xs text-ink-soft dark:text-dark-400">
                    ...
                    <tbody class="space-y-2">
                      <tr
                        v-for="p in (['anthropic', 'openai', 'gemini', 'antigravity', 'kiro', 'grok'] as const)"
                        :key="p"
                        class="align-top"
                      >
                        <td class="py-1 pr-4">
                          <span class="font-mono text-xs text-ink-body">{{ p }}</span>
                        </td>
                        <td class="py-1 pr-4">
                          <input
            ...
            <div class="space-y-6 p-6">
              ...
              <div class="space-y-4">
                <div
                  ...
                  class="rounded-xl border border-line p-4 dark:border-dark-700"
                >
                  <div class="gap-4 md:flex md:items-start md:justify-between">
                    ...
                    <div
                      class="mt-4 flex items-center justify-between rounded border border-line px-4 py-3 md:mt-0 md:min-w-[280px] dark:border-dark-700"
                    >
                      <div>
                        <label class="font-medium text-ink dark:text-white">
                  ...
                  <div
                    ...
                    class="mt-4 space-y-4 border-t border-line pt-4 dark:border-dark-700"
                  >
                    <div
                      class="flex items-center justify-between rounded border border-line px-4 py-3 dark:border-dark-700"
                    >
        ...
        <div v-show="activeTab === 'general'" class="space-y-6">
          ...
          <div class="card">
            ...
            <div class="space-y-6 p-6">
              ...
              >
                <div>
                  <h3 class="text-sm font-medium text-ink dark:text-white">
              ...
              <div>
                ...
                <p class="mb-3 text-xs text-ink-soft dark:text-dark-400">
                ...
                <button
                  type="button"
                  class="mt-3 flex w-full items-center justify-center gap-2 rounded-lg border-2 border-dashed border-line px-4 py-2.5 text-sm text-ink-soft transition-colors hover:border-brand-400 hover:text-brand-600 dark:border-dark-600 dark:hover:border-brand-500 dark:hover:text-brand-400"
                  @click="addEndpoint"
                >
                  <svg
                    class="h-4 w-4"
              ...
              <div>
                ...
                <div class="space-y-4">
                  <div
                    ...
                    data-testid="support-qr-grid"
                  >
                    <div
                      v-for="(item, index) in form.support_qr_codes"
                      :key="`support-qr-${index}`"
                      class="rounded-card border border-line bg-page/80 p-4 dark:border-dark-700 dark:bg-dark-900/40"
                    >
                      <div class="flex items-start justify-between gap-3">
                  ...
                  <button
                    type="button"
                    class="flex w-full items-center justify-center gap-2 rounded-lg border-2 border-dashed border-line px-4 py-2.5 text-sm text-ink-soft transition-colors hover:border-brand-400 hover:text-brand-600 dark:border-dark-600 dark:hover:border-brand-500 dark:hover:text-brand-400"
              ...
              <!-- Home Content (deprecated — landing is now native Vue) -->
              <div>
                <label
                  class="mb-2 flex flex-wrap items-center gap-2 text-sm font-medium text-ink-body"
                >
                  {{ t("admin.settings.site.homeContent") }}
                  <span
                    class="inline-flex items-center rounded-full border border-amber-200 bg-amber-50 px-2 py-0.5 text-[11px] font-semibold uppercase tracking-wide text-amber-700 dark:border-amber-700/60 dark:bg-amber-900/30 dark:text-amber-300"
                  >
                    {{ t("admin.settings.site.homeContentDeprecated") }}
                  </span>
                </label>
                <textarea
          ...
          <div class="card">
            ...
            <div class="space-y-4 p-6">
                ...
                <div class="mb-3 flex items-center justify-between">
                  ...
                  <div class="flex items-center gap-2">
                    ...
                    <button
                      v-if="index > 0"
                      type="button"
                      class="rounded p-1 text-ink-faint hover:bg-page hover:text-ink-body dark:hover:bg-dark-700"
              ...
              <button
                type="button"
                class="flex w-full items-center justify-center gap-2 rounded-lg border-2 border-dashed border-line py-3 text-sm text-ink-soft transition-colors hover:border-brand-400 hover:text-brand-600 dark:border-dark-600 dark:hover:border-brand-500 dark:hover:text-brand-400"
                @click="addMenuItem"
              >
                <svg
                  class="h-4 w-4"
	            ...
	            <div class="space-y-6 p-6">
	              <div class="grid grid-cols-1 gap-5 lg:grid-cols-[minmax(0,1fr)_220px]">
	                <div>
	                  ...
	                  <div class="grid grid-cols-2 gap-2 rounded-lg bg-page p-1 dark:bg-dark-700">
                    <button
                      type="button"
                      class="inline-flex items-center justify-center gap-2 rounded-md px-3 py-2 text-sm font-medium transition"
                      :class="
                        form.login_agreement_mode === 'modal'
                          ? 'bg-card text-brand-700 shadow-xs dark:bg-dark-800 dark:text-brand-300'
                          : 'text-ink-body hover:text-ink dark:text-dark-300 dark:hover:text-white'
                      "
              ...
              <div>
                ...
                <div class="mt-4 space-y-3">
                  <div
                    ...
                    :key="doc.id || index"
                    class="rounded-lg border border-line bg-card p-4 dark:border-dark-700 dark:bg-dark-800/60"
                  >
                    <div class="mb-3 flex items-center justify-between gap-3">
                      <div class="flex min-w-0 items-center gap-3">
                        <span class="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-md bg-page text-ink-body dark:bg-dark-700 dark:text-dark-200">
                          <Icon
                            :name="
                              index === 1
                                ? 'shield'
                        ...
                        <div class="min-w-0">
                          <p class="truncate text-sm font-semibold text-ink dark:text-white">
                            {{ doc.title || localText("未命名文档", "Untitled document") }}
                          </p>
                          <p class="truncate text-xs text-ink-soft dark:text-dark-400">
                    ...
                    <div class="grid grid-cols-1 gap-3 lg:grid-cols-2">
                      <div>
                        <label class="mb-1 block text-xs font-medium text-ink-body dark:text-dark-400">
                      ...
                      <div>
                        ...
                        <div class="flex overflow-hidden rounded-lg border border-line bg-card focus-within:border-accent focus-within:ring-[3px] focus-within:ring-accent/25 dark:border-dark-600 dark:bg-dark-900">
                          <span class="inline-flex flex-shrink-0 items-center border-r border-line bg-page px-3 text-sm text-ink-soft dark:border-dark-700 dark:bg-dark-800 dark:text-dark-400">
                            /legal/
                          </span>
                          <input
                            v-model="doc.id"
                            type="text"
                            class="min-w-0 flex-1 border-0 bg-transparent px-3 py-2 text-sm text-ink outline-none placeholder:text-ink-faint focus:ring-0 dark:text-white dark:placeholder:text-dark-500"
        ...
        <div class="card">
          ...
          <div class="space-y-5 p-6">
            ...
            <div v-if="form.affiliate_enabled" class="space-y-6">
              ...
              <!-- 专属用户管理 -->
              <div class="border-t border-line pt-6 dark:border-dark-700">
                <div class="mb-3 flex items-center justify-between">
                  <div>
                ...
                <div class="overflow-hidden rounded-lg border border-line dark:border-dark-700">
                  <table class="min-w-full divide-y divide-line dark:divide-dark-700">
                    <thead class="bg-page dark:bg-dark-800">
                      <tr>
                        ...
                        <th class="px-3 py-2 text-left text-xs font-medium uppercase text-ink-soft">{{ t('admin.settings.features.affiliate.customUsers.col.email') }}</th>
                        <th class="px-3 py-2 text-left text-xs font-medium uppercase text-ink-soft">{{ t('admin.settings.features.affiliate.customUsers.col.username') }}</th>
                        <th class="px-3 py-2 text-left text-xs font-medium uppercase text-ink-soft">{{ t('admin.settings.features.affiliate.customUsers.col.code') }}</th>
                        <th class="px-3 py-2 text-left text-xs font-medium uppercase text-ink-soft">{{ t('admin.settings.features.affiliate.customUsers.col.rate') }}</th>
                        <th class="px-3 py-2 text-left text-xs font-medium uppercase text-ink-soft">{{ t('admin.settings.features.affiliate.customUsers.col.actions') }}</th>
                      </tr>
                    </thead>
                    <tbody class="divide-y divide-line bg-card dark:divide-dark-700 dark:bg-dark-900">
                      <tr v-if="affiliateState.loading">
                        <td colspan="6" class="px-3 py-6 text-center text-sm text-ink-soft">
                          {{ t('common.loading') }}
                        </td>
                      </tr>
                      ...
                      <tr v-for="entry in affiliateState.entries" :key="entry.user_id">
                        ...
                        <td class="px-3 py-2 text-sm text-ink dark:text-white">{{ entry.email }}</td>
                        <td class="px-3 py-2 text-sm text-ink-body">{{ entry.username }}</td>
                        <td class="px-3 py-2 text-sm font-mono">
                          {{ entry.aff_code }}
                          <span
                            v-if="entry.aff_code_custom"
                            class="ml-1 inline-block rounded bg-brand-100 px-1.5 py-0.5 text-[10px] font-medium text-brand-700 dark:bg-brand-900/30 dark:text-brand-300"
                        ...
                        <td class="px-3 py-2 text-sm">
                          ...
                          <span v-else class="text-ink-faint">{{ t('admin.settings.features.affiliate.customUsers.useGlobal') }}</span>
                        </td>
                        <td class="px-3 py-2 text-sm">
                          <div class="flex items-center gap-2">
                            <button type="button" class="text-brand-600 hover:underline" @click="openAffiliateModal(entry)">
                              {{ t('common.edit') }}
                            </button>
                            <button
                              type="button"
                ...
                <div v-if="affiliateState.total > affiliateState.pageSize" class="mt-3 flex items-center justify-between text-sm">
                  ...
                  <div class="flex items-center gap-2">
                    ...
                    <span class="text-ink-soft">{{ affiliateState.page }} / {{ Math.max(1, Math.ceil(affiliateState.total / affiliateState.pageSize)) }}</span>
                    <button
                      type="button"
                      class="btn btn-secondary btn-sm"
        ...
        >
          <div class="w-full max-w-md rounded-lg bg-card p-6 shadow-xl dark:bg-dark-900">
            ...
            <div class="space-y-4">
              <div v-if="affiliateModal.mode === 'add'">
                ...
                <div
                  ...
                  class="flex items-center justify-between rounded-md border border-brand-200 bg-brand-50 px-3 py-2 dark:border-brand-700/50 dark:bg-brand-900/20"
                >
                  <div class="text-sm">
                    <span class="font-medium text-ink dark:text-white">{{ affiliateModal.selectedUser.email }}</span>
                    <span class="ml-1 text-xs text-ink-soft">({{ affiliateModal.selectedUser.username }})</span>
                  </div>
                  <button
                    type="button"
                    class="text-lg leading-none text-ink-faint hover:text-red-600"
                ...
                <template v-else>
                  ...
                  <div
                    ...
                    class="mt-1 max-h-40 overflow-y-auto rounded border border-line dark:border-dark-700"
                  >
                    <button
                      v-for="u in affiliateModal.userResults"
                      :key="u.id"
                      type="button"
                      class="w-full px-3 py-1.5 text-left text-sm hover:bg-page dark:hover:bg-dark-800"
                      @click="selectAffiliateUser(u)"
                    >
                      {{ u.email }} <span class="text-xs text-ink-soft">({{ u.username }})</span>
                    </button>
                  </div>
                </template>
              </div>
            ...
            <div class="mt-6 flex items-center justify-between gap-3">
              <p
                ...
                class="text-xs text-ink-soft dark:text-dark-400"
              >
            ...
            >
              ...
              <p class="mt-1 text-sm text-ink-soft dark:text-dark-400">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  class="ml-2 inline-flex items-center text-brand-600 hover:text-brand-700 dark:text-brand-400 dark:hover:text-brand-300"
                >
                  <svg
              ...
              <template v-if="form.payment_enabled">
                ...
                <div class="grid grid-cols-3 gap-3">
                  ...
                  <div>
                    ...
                    <div
                      class="rounded-lg border border-line bg-page px-3 py-2 text-sm text-ink-body dark:border-dark-600 dark:bg-dark-800 dark:text-ink-body"
                    >
                      {{
                        (form.payment_product_name_prefix || "Clomio") +
                        " 100 " +
                ...
                <div class="grid grid-cols-2 gap-3 sm:grid-cols-5">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      @input="
                        form.payment_subscription_usd_to_cny_rate = Math.max(
                          0,
                          Math.round(
                            parseFloat(
                              ($event.target as HTMLInputElement).value ||
                  ...
                  <div>
                    ...
                    <div class="relative">
                      ...
                      />
                      <span
                        class="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-3 text-ink-faint"
                        >%</span
                      >
                    </div>
                    <p class="mt-0.5 text-xs text-ink-faint">
                  ...
                  <div>
                    ...
                    <div class="flex items-center gap-2">
                      <button
                        type="button"
                        :class="[
                          'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-150 ease-in-out focus:outline-none focus:ring-2 focus:ring-accent/25 focus:ring-offset-2',
                          form.payment_cancel_rate_limit_enabled
                            ? 'bg-brand-500'
                            : 'bg-line dark:bg-dark-600',
                        ]"
                        @click="
                          form.payment_cancel_rate_limit_enabled =
                            !form.payment_cancel_rate_limit_enabled
                      ...
                      <span
                        :class="[
                          ...
                          form.payment_cancel_rate_limit_enabled
                            ? 'text-ink-body'
                            : 'text-ink-faint dark:text-dark-500',
                        ]"
                        >{{
                ...
                <div>
                    ...
                    <button
                      ...
                      :class="[
                        ...
                        isPaymentTypeEnabled(pt.value)
                          ...
                          : 'border-line bg-card text-ink-body hover:border-divider hover:bg-page dark:border-dark-600 dark:bg-dark-800 dark:text-ink-body dark:hover:border-dark-500',
                      ]"
                    >
                      {{ pt.label }}
                    </button>
                  ...
                  <p class="mt-2 text-xs text-ink-faint dark:text-dark-400">
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      class="ml-1 text-brand-500 hover:text-brand-600 dark:text-brand-400 dark:hover:text-brand-300"
                    >
        ...
        <div v-show="activeTab === 'email'" class="space-y-6">
            ...
            <div class="p-6">
              <div class="flex items-start gap-3">
                <Icon
                  name="mail"
                  size="md"
                  class="mt-0.5 flex-shrink-0 text-ink-faint dark:text-dark-400"
                />
                <div>
                  <h3 class="font-medium text-ink dark:text-white">
          ...
          <div v-if="form.email_verify_enabled" class="card">
            <div
              class="flex items-center justify-between border-b border-line px-6 py-4 dark:border-dark-700"
            >
              <div>
                <h2 class="text-lg font-semibold text-ink dark:text-white">
          ...
          <div class="card">
            <div
              class="border-b border-line px-6 py-4 dark:border-dark-700"
            >
              <h3 class="text-base font-medium text-ink dark:text-white">
            ...
            <div class="px-6 py-6 space-y-4">
              <div class="flex items-center justify-between">
                <label
                  class="mb-0 block text-sm font-medium text-ink-body"
              ...
              <div v-if="form.balance_low_notify_enabled">
                ...
                <div class="relative">
                  <span
                    class="absolute left-3 top-1/2 -translate-y-1/2 text-ink-faint"
                    >$</span
                  >
                  <input
              ...
              <div
                class="flex items-center justify-between rounded-card border border-line px-4 py-3 dark:border-dark-700"
              >
                <div>
                  <label
                    class="mb-0 block text-sm font-medium text-ink-body"
                ...
                <div class="space-y-2">
                  ...
                  >
                    ...
                    >
                      ...
                      />
                      <div
                        class="w-9 h-5 bg-line peer-focus:outline-none rounded-full peer dark:bg-dark-600 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-line after:border after:rounded-full after:h-4 after:w-4 after:transition-all dark:after:border-dark-500 peer-checked:bg-brand-600"
                      ></div>
                    </label>
                    <input
                      v-model="entry.email"
...
import { defineAsyncComponent, ref, reactive, computed, onMounted, watch, nextTick } from "vue";
...
function centerActiveSettingsTab() {
...
async function saveSettings() {
  ...
  min_codex_version: form.min_codex_version?.trim() || "",
    ...
    if (openaiFastPolicyLoaded.value) {
      const normalizedRuleUserIDs = openaiFastPolicyForm.rules.map((rule) =>
        normalizeOpenAIFastPolicyUserIDs(rule.user_ids),
      );
      if (normalizedRuleUserIDs.some((userIDs) => userIDs === null)) {
        appStore.showError(
      ...
      payload.openai_fast_policy_settings = {
        rules: openaiFastPolicyForm.rules.map((rule, ruleIndex) => {
          ...
          const userIDs = normalizedRuleUserIDs[ruleIndex] || [];
          return {
...
/* ============ 系统设置 Tab 导航（PR-10 chrome only） ============ */
.settings-tabs-shell {
  @apply sticky z-20 -mx-1 rounded-card border border-line/80 bg-card/90 p-1.5 backdrop-blur-xl;
  top: 4.75rem;
  box-shadow:
...
/* Scroll container: keep native swipe/trackpad scroll, hide manual scrollbar chrome. */
...
.settings-tab {
  @apply relative isolate flex h-10 min-w-[6.75rem] shrink-0 items-center justify-center gap-1.5 whitespace-nowrap rounded-xl border border-transparent px-3 text-sm font-medium text-ink-body outline-none transition-colors duration-150 ease-out dark:text-dark-300;
}
...
.settings-tab:focus-visible {
  @apply ring-2 ring-brand/40 ring-offset-2 ring-offset-white dark:ring-offset-dark-900;
}

.settings-tab-active {
  @apply border-brand-200/80 bg-card text-brand-700 shadow-xs dark:border-brand-400/30 dark:bg-dark-700/95 dark:text-brand-200;
  box-shadow:
...
/* Brand → accent underline (Clomio tokens; replaces hard-coded #14b8a6/#0ea5e9) */
.settings-tab-active::after {
  ...
  background: linear-gradient(
    90deg,
    rgb(var(--c-brand)) 0%,
    rgb(var(--c-brand-cyan)) 55%,
    rgb(var(--c-accent)) 100%
  );
}

.settings-tab-icon {
  @apply flex h-7 w-7 shrink-0 items-center justify-center rounded-lg text-ink-soft transition-colors duration-150 dark:text-dark-400;
}

.settings-tab:hover .settings-tab-icon,
.settings-tab:focus-visible .settings-tab-icon {
  @apply text-ink-body dark:text-dark-200;
}

.settings-tab-active .settings-tab-icon {
  @apply bg-brand-50 text-brand-600 dark:bg-brand-400/10 dark:text-brand-300;
}
