  ...
  <div ref="containerRef" class="relative">
    <div v-if="selectedUserIds.length > 0" class="mb-2 flex flex-wrap gap-2">
      <span
        ...
        :key="userId"
        class="inline-flex max-w-full items-center gap-1.5 rounded-md bg-ink-faint/10 px-2.5 py-1.5 text-xs text-ink-body dark:bg-dark-600 dark:text-ink"
      >
        <span class="max-w-64 truncate font-medium" :title="selectedUserLabel(userId)">
          {{ selectedUserLabel(userId) }}
        </span>
        <span class="shrink-0 text-ink-faint">#{{ userId }}</span>
        <span
        ...
        <button
          type="button"
          class="shrink-0 rounded text-ink-faint hover:text-red-600 dark:hover:text-red-400"
    ...
    <div
      ...
      class="absolute z-50 mt-1 max-h-60 w-full overflow-auto rounded-lg border border-line bg-white shadow-lg dark:border-dark-600 dark:bg-dark-700"
    >
      <div v-if="searchLoading" class="px-4 py-3 text-sm text-ink-soft dark:text-ink-soft">
        {{ t("common.loading") }}
      </div>
      <div
        v-else-if="availableResults.length === 0"
        class="px-4 py-3 text-sm text-ink-soft dark:text-ink-soft"
      >
      ...
      <template v-else>
        <button
          ...
          :key="user.id"
          type="button"
          class="flex w-full items-center justify-between gap-3 px-4 py-2 text-left text-sm hover:bg-page dark:hover:bg-dark-600"
          @click="selectUser(user)"
        >
          <span class="min-w-0 truncate font-medium text-ink dark:text-white">
            {{ user.email }}
            <span v-if="user.deleted" class="ml-1 text-xs font-normal text-ink-faint">
              {{ t("admin.settings.openaiFastPolicy.userDeleted") }}
            </span>
          </span>
          <span class="shrink-0 text-xs text-ink-faint">#{{ user.id }}</span>
        </button>
      </template>
    </div>
  </div>
