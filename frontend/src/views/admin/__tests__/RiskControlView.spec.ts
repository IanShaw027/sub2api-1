...
describe('admin RiskControlView', () => {
  ...
  it('prunes APIKey audit exemptions outside selected group scope when saving', async () => {
    getConfig.mockResolvedValueOnce({
      ...baseConfig(),
      all_groups: false,
      group_ids: [7],
      api_key_exempt_group_ids: [7, 42],
    })
    getGroups.mockResolvedValueOnce([
      { id: 42, name: 'OpenAI VIP', platform: 'openai' },
      { id: 7, name: 'Claude', platform: 'anthropic' },
  ...
  it('disables APIKey audit exemptions outside selected group scope', async () => {
    getConfig.mockResolvedValueOnce({
      ...baseConfig(),
      all_groups: false,
      group_ids: [7],
    ...
    expect(wrapper.get<HTMLInputElement>('[data-test="risk-control-group-api-key-exempt-42"]').element.disabled).toBe(true)
    expect(wrapper.get<HTMLInputElement>('[data-test="risk-control-group-api-key-exempt-7"]').element.disabled).toBe(false)
  })
