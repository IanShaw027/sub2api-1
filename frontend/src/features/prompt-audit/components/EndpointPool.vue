<template>
 <section aria-labelledby="prompt-pool-title" class="border-b border-line py-6">
 <div class="flex flex-wrap items-start justify-between gap-3">
 <div>
 <h2 id="prompt-pool-title" class="text-base font-semibold text-foreground">{{ t('admin.promptAudit.pool.title') }}</h2>
 <p class="mt-1 text-sm text-muted">{{ t('admin.promptAudit.pool.description') }}</p>
 </div>
 ...
 <div v-if="endpoints.length === 0" class="mt-5 rounded-xl border border-dashed border-line px-5 py-10 text-center text-sm text-muted">
 {{ t('admin.promptAudit.pool.empty') }}
 </div>
 <div v-else class="mt-5 overflow-hidden rounded-xl border border-line bg-surface">
 <div class="hidden grid-cols-[minmax(260px,1.45fr)_minmax(210px,1fr)_minmax(190px,.8fr)_minmax(230px,1.15fr)_auto] gap-5 border-b border-l-[3px] border-b-line border-l-transparent bg-surface-2/80 px-5 py-2.5 text-[11px] font-semibold uppercase tracking-[0.08em] text-muted xl:grid">
 ...
 <p class="mb-1 text-[10px] font-semibold uppercase tracking-wider text-muted xl:hidden">{{ t('admin.promptAudit.pool.model') }}</p>
 <p class="truncate text-sm font-medium text-foreground" :title="endpoint.model">{{ endpoint.model }}</p>
 </div>

 <div>
 <p class="mb-1 text-[10px] font-semibold uppercase tracking-wider text-muted xl:hidden">{{ t('admin.promptAudit.pool.limits') }}</p>
 <div class="flex flex-wrap gap-1.5 text-xs text-muted">
 <span class="rounded-md bg-surface-2 px-2 py-1 tabular-nums">{{ endpoint.timeout_ms }} ms</span>
 <span class="rounded-md bg-surface-2 px-2 py-1 tabular-nums">{{ endpoint.input_limit }} chars</span>
 </div>
 </div>
 ...
 <div class="flex flex-wrap items-center justify-end gap-1 border-t border-line pt-3 xl:flex-nowrap xl:border-0 xl:pt-0">
 ...
 <label class="space-y-1 text-sm text-foreground">
