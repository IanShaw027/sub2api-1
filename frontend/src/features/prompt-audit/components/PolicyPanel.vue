<template>
 ...
 <div>
 <h2 id="prompt-policy-title" class="text-base font-semibold text-foreground">{{ t('admin.promptAudit.policy.title') }}</h2>
 <p class="mt-1 text-sm text-muted">{{ t('admin.promptAudit.policy.description') }}</p>
 </div>

 <div class="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(260px,0.45fr)]">
 <div class="rounded-xl border border-line p-4 sm:p-5">
 <fieldset>
 <legend class="text-sm font-medium text-foreground">{{ t('admin.promptAudit.policy.scope') }}</legend>
 <div class="mt-3 flex flex-wrap gap-5 text-sm text-foreground">
 ...
 <div class="mt-3 max-h-52 overflow-y-auto rounded-lg border border-line p-2">
 <label v-for="group in filteredGroups" :key="group.id" class="flex cursor-pointer items-center justify-between gap-3 rounded-md px-2 py-2 text-sm hover:bg-surface-2">
 <span class="flex items-center gap-2 text-foreground">
 <input type="checkbox" :checked="draft.group_ids.includes(group.id)" @change="toggleGroup(group.id)" />
 {{ group.name }}
 </span>
 <span class="text-xs text-muted">{{ group.platform }} · {{ group.status }}</span>
 </label>
 ...
 <label v-for="scanner in SCANNER_CATALOG" :key="scanner.id" class="flex items-center gap-2 rounded-md px-2 py-1.5 text-sm text-foreground hover:bg-surface-2">
 ...
 <div class="space-y-4 rounded-xl border border-line p-4 sm:p-5">
 <label class="block text-sm text-foreground">
