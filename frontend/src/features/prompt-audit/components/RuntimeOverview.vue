<template>
 <section aria-labelledby="prompt-runtime-title" class="border-b border-line py-6">
 <div class="flex flex-wrap items-start justify-between gap-3">
 <div>
 <h2 id="prompt-runtime-title" class="text-base font-semibold text-foreground">
 ...
 <div v-if="error" role="alert" class="notice notice-danger mt-5 text-sm">
 {{ error }}
 </div>
 <div v-else-if="loading && !runtime" class="mt-5 grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6" aria-busy="true">
 <div v-for="index in 6" :key="index" class="h-16 animate-pulse rounded-xl bg-surface-2" />
 </div>
 <template v-else-if="runtime">
 <dl class="mt-5 grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">
 <div v-for="item in statusItems" :key="item.label" class="rounded-xl border border-line bg-surface-2/80 px-3 py-3">
 <dt class="text-xs text-muted">{{ item.label }}</dt>
 <dd class="mt-1.5 flex items-center gap-2 text-sm font-semibold text-foreground">
 ...
 <h3 class="text-sm font-medium text-foreground">{{ t('admin.promptAudit.runtime.guardMetrics') }}</h3>
 ...
 <p class="mt-0.5 text-sm font-semibold tabular-nums text-foreground">{{ metric.value }}</p>
 </div>
 </div>
 ...
 <div class="rounded-xl border border-line px-4 py-3">
 <h3 class="text-sm font-medium text-foreground">{{ t('admin.promptAudit.runtime.latest') }}</h3>
 <p class="mt-2 text-sm text-muted">
 {{ runtime.last_processed_at ? formatDate(runtime.last_processed_at) : t('admin.promptAudit.common.never') }}
 </p>
 <p v-if="runtime.last_error_code" class="mt-1 break-words text-sm text-danger-text">
 ...
 <span v-for="(probe, id) in runtime.endpoints" :key="id" class="rounded-md px-2 py-1 text-xs" :class="probe.ok ? 'bg-[color-mix(in_oklch,var(--success)_12%,transparent)] text-success-text' : 'bg-[color-mix(in_oklch,var(--danger)_12%,transparent)] text-danger-text'">
...
function statusDot(status: string): string {
 ...
 if (status === 'disabled') return 'bg-surface-3'
