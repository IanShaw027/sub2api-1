...
const SaveToggle = defineComponent({
 ...
 h('span', {
 class: [
 'pointer-events-none inline-block h-5 w-5 rounded-full bg-surface shadow transition-transform duration-200 ease-in-out',
 props.modelValue ? 'translate-x-5' : 'translate-x-0',
 ],
 }),
 ]),
 h('span', { class: 'select-none text-foreground' }, props.label),
 ])
 },
})
