<template>
 ...
 <span class="select-icon shrink-0 text-muted transition-transform" :class="isOpen ? 'rotate-180' : ''">
 ...
 <button
 type="button"
 class="dropdown-item select-option select-option-group flex w-full items-center justify-between border-b border-line px-4 py-2 text-left text-sm font-semibold text-foreground hover:bg-surface-2"
 @click="clear"
 >
 <span>{{ allLabel }}</span>
 ...
 type="button"
 role="option"
 class="dropdown-item select-option flex w-full items-center justify-between gap-3 px-4 py-2 text-left text-sm text-foreground hover:bg-surface-2"
 ...
 <small v-if="option.count != null" class="text-xs text-muted">{{ formatCount(option.count) }}</small>
 </button>
 <p v-if="options.length === 0" class="px-4 py-3 text-center text-xs text-muted">{{ t('channelMonitorV2.filters.empty') }}</p>
 </div>
 </Transition>
 </Teleport>
 </div>
