<template>
 ...
 :class="showTrophy ? rankClass : 'text-muted'"
 >
 {{ label }}
 </span>
 </span>
...
/**
 * Rank medal palettes: light / mid / deep fills for the multi-path trophy SVG.
 * Gold/silver/bronze are token-composed (color-mix over --warning/--muted/--danger)
 * instead of literal hex so the medal still reacts to theme + accent-color changes.
 */
const RANK_BASE = {
 1: 'var(--warning)', // gold
 2: 'var(--muted)', // silver
 3: 'color-mix(in oklch, var(--warning) 55%, var(--danger) 35%)', // bronze
} as const

function medalPalette(base: string) {
 return {
 light: `color-mix(in oklch, ${base} 55%, var(--surface) 45%)`,
 mid: base,
 deep: `color-mix(in oklch, ${base} 70%, var(--foreground) 30%)`,
 }
}

const RANK_PALETTES = {
 1: medalPalette(RANK_BASE[1]),
 2: medalPalette(RANK_BASE[2]),
 3: medalPalette(RANK_BASE[3]),
} as const
...
const rankClass = computed(() => {
 if (rankNum.value === 1) return 'text-[var(--warning)]'
 if (rankNum.value === 2) return 'text-muted'
 if (rankNum.value === 3) return 'text-[color-mix(in_oklch,var(--warning)_55%,var(--danger)_35%)]'
 return ''
})
</script>
