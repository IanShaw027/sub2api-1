<template>
 ...
 <p class="page-description mt-1.5 text-xs text-muted">
 ...
 <strong class="text-sm font-medium text-foreground">{{ platformLabel(platform.platform) }}</strong>
 <input
 class="input"
 ...
 :key="group.id"
 class="flex cursor-pointer items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition hover:bg-surface-2"
 >
 <input
 type="checkbox"
 class="h-4 w-4 rounded border-line text-accent focus:ring-accent/40"
 :checked="draft.group_ids.includes(group.id)"
 @change="toggleGroup(group.id)"
 />
 <span class="min-w-0 flex-1 truncate font-medium text-foreground">{{ group.name }}</span>
 <small class="shrink-0 text-xs text-muted">{{ platformLabel(group.platform) }} · #{{ group.id }}</small>
 </label>
 </div>
 <p v-if="groups.length === 0" class="empty-state py-8 text-sm text-muted">{{ t('channelMonitorV2.settings.groupsEmpty') }}</p>
 </div>
 </div>
 ...
 <small class="shrink-0 font-mono text-[10px] text-muted">{{ category }}</small>
 </label>
 </div>
 </div>
