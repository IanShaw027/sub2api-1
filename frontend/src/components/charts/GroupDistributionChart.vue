          ...
          <tbody>
            <template v-for="group in displayGroupStats" :key="group.group_id">
              <tr
                ...
                :class="enableBreakdown && group.group_id > 0 ? 'cursor-pointer hover:bg-page dark:hover:bg-dark-700/40' : ''"
                @click="enableBreakdown && group.group_id > 0 && toggleBreakdown('group', group.group_id)"
              >
                <td
                  class="max-w-[100px] truncate py-1.5 font-medium"
                  :class="enableBreakdown && group.group_id > 0 ? 'text-accent-600 hover:text-accent-800 dark:text-blue-400 dark:hover:text-blue-300' : 'text-ink dark:text-white'"
    ...
    datasets: [
      {
        ...
        backgroundColor: displayGroupStats.value.map((_, i) => chartColor(i)),
        borderWidth: 0
      }
    ]
  }
