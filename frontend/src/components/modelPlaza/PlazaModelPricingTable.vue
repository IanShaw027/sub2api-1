<template>
 ...
 <th
 rowspan="2"
 class="border-r border-line py-2.5 pl-5 pr-4 text-left align-middle"
 >
 ...
 <th
 colspan="3"
 class="border-l border-line pt-2 text-center"
 >
 <div class="border-b border-line pb-2 text-muted">
 ...
 <th
 rowspan="2"
 class="border-l border-line py-2.5 pl-3 pr-5 text-right align-middle"
 >
 {{ t('modelPlaza.table.rate') }}
 </th>
 </tr>
 <tr
 class="border-b border-line text-left text-[11px] font-medium uppercase leading-4 tracking-wide text-muted"
 >
 ...
 <th class="border-l border-line px-3 py-2 font-medium">
 ...
 :key="key"
 class="border-b border-line transition-colors last:border-b-0 hover:bg-surface-2/70"
 >
 <!-- 模型名 + 非 token 计费模式徽章;分时时段行额外标注时段 -->
 <td class="border-r border-line py-2.5 pl-5 pr-4 align-middle">
 <div class="flex flex-wrap items-center gap-1.5">
 <span class="font-medium text-foreground">{{ m.name }}</span>
 <!-- 时段徽章紧跟模型名,其余徽章排在后面,空间不足时先换行的是它们 -->
 <span
 v-if="period"
 class="inline-flex items-center whitespace-nowrap rounded-md bg-surface-2 px-1 py-0.5 font-mono text-[10px] font-medium text-muted"
 ...
 class="rounded-md bg-surface-2 px-1.5 py-0.5 text-[10px] font-medium text-muted"
 >
 {{ billingModeLabel(m) }}
 </span>
 <span
 ...
 <td class="pz-cell px-3 py-2.5 align-middle font-mono font-semibold text-foreground">
 ...
 v-for="(iv, idx) in tokenIntervals(m)"
 :key="idx"
 class="whitespace-nowrap font-mono text-xs leading-5 text-foreground"
 :title="tierHint(m)"
 >
 ...
 class="space-y-0.5 font-mono text-xs text-foreground"
 >
 <div>
 ...
 :key="idx"
 class="inline-flex items-center gap-1 rounded-md bg-surface-2 px-2 py-0.5 font-mono text-xs text-foreground"
 >
 ...
 <td
 class="border-l border-line px-3 py-2.5 align-middle font-mono text-xs text-muted"
 >
 ...
 <td class="px-3 py-2.5 align-middle font-mono text-xs text-muted">
 <template v-if="officialIntervals(m).length">
 <div
 v-for="(iv, idx) in officialIntervals(m)"
 :key="idx"
 ...
 v-for="(iv, idx) in officialIntervals(m)"
 :key="idx"
 class="whitespace-nowrap font-mono text-xs leading-5 text-muted"
 :title="t('modelPlaza.table.tierHint')"
 >
 <template v-if="iv.cache_write_price != null || iv.cache_write_1h_price != null || iv.cache_read_price != null">
 ...
 class="space-y-0.5 font-mono text-xs text-muted"
 >
 <div>
 <span class="mr-1 font-sans font-normal text-muted">{{ t('modelPlaza.table.cacheWrite') }}</span>
 ...
 <td
 class="border-l border-line py-2.5 pl-3 pr-5 text-right align-middle font-mono text-xs"
 >
 <span
 v-if="period"
 ...
 <span class="font-bold text-accent">{{ effectiveRate }}x</span>
 </template>
 <span v-else class="font-bold text-foreground">{{ effectiveRate }}x</span>
 </td>
 </tr>
 </tbody>
 </table>
