<template>
  <header class="glass sticky top-0 z-30 border-b border-line/50 dark:border-dark-700/50">
    <div class="flex h-16 items-center justify-between px-4 md:px-6">
      ...
      <div class="flex items-center gap-4">
        ...
        <div class="hidden lg:block">
          <h1 class="text-lg font-semibold text-ink dark:text-white">
            {{ pageTitle }}
          </h1>
          <p v-if="pageDescription" class="text-xs text-ink-soft dark:text-dark-400">
            {{ pageDescription }}
          </p>
        </div>
      </div>
      ...
      <div class="flex items-center gap-3">
        ...
        <a
          ...
          class="flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-sm font-medium text-ink-body transition-colors hover:bg-page hover:text-ink dark:text-dark-400 dark:hover:bg-dark-800 dark:hover:text-white"
        >
        ...
        <div
          v-if="user"
          class="hidden items-center gap-2 rounded-xl bg-brand-50 px-3 py-1.5 dark:bg-brand-950/40 sm:flex"
        >
          <svg
        ...
        <div v-if="user" class="relative" ref="dropdownRef">
          <button
            ...
            class="flex items-center gap-2 rounded-xl p-1.5 transition-colors hover:bg-page dark:hover:bg-dark-800"
            :aria-label="t('common.userMenu')"
          >
            <div class="flex h-8 w-8 items-center justify-center overflow-hidden rounded-xl bg-gradient-to-br from-brand-500 to-brand-600 text-sm font-medium text-white shadow-xs">
              <img
                v-if="avatarUrl"
                :src="avatarUrl"
                :alt="displayName"
            ...
            <div class="hidden text-left md:block">
              ...
              <div class="text-xs capitalize text-ink-soft dark:text-dark-400">
                {{ user.role }}
              </div>
            </div>
            <Icon name="chevronDown" size="sm" class="hidden text-ink-faint md:block" />
          </button>
          ...
          <transition name="dropdown">
            <div v-if="dropdownOpen" class="dropdown right-0 mt-2 w-56">
              ...
              <div class="border-b border-line px-4 py-3 dark:border-dark-700">
                ...
                <div class="text-xs text-ink-soft dark:text-dark-400">{{ user.email }}</div>
              </div>

              <!-- Balance (mobile only) -->
              <div class="border-b border-line px-4 py-2 dark:border-dark-700 sm:hidden">
                <div class="text-xs text-ink-soft dark:text-dark-400">
                  {{ t('common.balance') }}
                </div>
                <div class="text-sm font-semibold text-brand-600 dark:text-brand-400">
                  ${{ user.balance?.toFixed(2) || '0.00' }}
                </div>
              </div>
              ...
              <div v-if="showOnboardingButton" class="border-t border-line py-1 dark:border-dark-700">
              ...
              <div class="border-t border-line py-1 dark:border-dark-700">
                <button
                  @click="handleLogout"
