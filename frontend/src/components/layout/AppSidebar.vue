...
const flagAiStudio = makeSidebarFlag(FeatureFlags.aiStudio)
...
function buildSelfNavItems(withDashboard: boolean): NavItem[] {
  ...
  items.push(
    ...
    {
      path: '/ai',
      label: t('ai.center.label', 'AI 创作中心'),
      icon: AiShieldIcon,
      expandOnly: true,
...
function finalizeNav(items: NavItem[]): NavItem[] {
  ...
  return authStore.isSimpleMode
    ? visible.filter(item => !item.hideInSimpleMode && !isSimpleModeRouteRestricted(item.path))
    : visible
}
...
const adminNavItems = computed((): NavItem[] => {
  const baseItems: NavItem[] = [
    ...
    },
    {
      ...
      children: [
        ...
        { path: '/admin/orders/invoices', label: t('nav.invoiceApplications'), icon: TicketListIcon },
      ],
    },
  ...
  if (authStore.isSimpleMode) {
    const filtered = visible.filter(item => !item.hideInSimpleMode && !isSimpleModeRouteRestricted(item.path))
