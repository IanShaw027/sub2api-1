<template>
  <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
    <div class="card p-4 flex items-center gap-3">
      ...
      <div class="min-w-0">
        <p class="text-xs font-medium text-ink-soft">{{ t('usage.totalRequests') }}</p>
        <p class="text-xl font-bold tabular-nums text-ink">{{ stats?.total_requests?.toLocaleString() || '0' }}</p>
        <p class="text-xs text-ink-faint">{{ t('usage.inSelectedRange') }}</p>
      </div>
    </div>
    <div class="card p-4 flex items-center gap-3">
      <div class="stat-icon shrink-0 bg-warning-soft text-warning dark:bg-amber-900/30 dark:text-amber-400"><svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m21 7.5-9-5.25L3 7.5m18 0-9 5.25m9-5.25v9l-9 5.25M3 7.5l9 5.25M3 7.5v9l9 5.25m0-9v9" /></svg></div>
      <div class="min-w-0">
        <p class="text-xs font-medium text-ink-soft">{{ t('usage.totalTokens') }}</p>
        <p class="text-xl font-bold tabular-nums text-ink">{{ formatTokens(stats?.total_tokens || 0) }}</p>
        <p class="flex flex-wrap items-center gap-x-1 text-xs text-ink-soft">
          ...
          <span class="group relative inline-flex cursor-help items-center gap-0.5" tabindex="0">
            ...
            <span
              class="pointer-events-none absolute left-1/2 top-full z-30 mt-2 w-56 -translate-x-1/2 rounded-card border border-line bg-card p-3 text-left text-xs text-ink-body opacity-0 shadow-md transition-opacity duration-150 group-hover:opacity-100 group-focus:opacity-100 dark:border-dark-600 dark:bg-dark-800 dark:text-dark-200"
            >
              <span class="mb-2 block font-medium text-ink dark:text-white">
                {{ cacheDetailLabel() }}
              </span>
    ...
    <div class="card p-4 flex items-center gap-3">
      ...
      <div class="min-w-0 flex-1">
        <p class="text-xs font-medium text-ink-soft">{{ t('usage.totalCost') }}</p>
        <p class="text-xl font-bold tabular-nums text-success">
          ${{ (stats?.total_actual_cost || 0).toFixed(4) }}
        </p>
        <p class="text-xs text-ink-faint">
          <template v-if="showAccountCost && totalAccountCost != null">
            <span class="text-warning">{{ t('usage.accountCost') }} ${{ totalAccountCost.toFixed(4) }}</span>
            <span> · </span>
          </template>
          <span>
    ...
    <div class="card p-4 flex items-center gap-3">
      ...
      <div class="min-w-0"><p class="text-xs font-medium text-ink-soft">{{ t('usage.avgDuration') }}</p><p class="text-xl font-bold tabular-nums text-ink">{{ formatDuration(stats?.average_duration_ms || 0) }}</p></div>
    </div>
  </div>
</template>
