<template>
  <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
    <div class="glass-card p-4 flex items-center gap-3">
      <div class="rounded-lg bg-[color-mix(in_oklch,var(--accent)_12%,transparent)] p-2 text-accent">
        <Icon name="document" size="md" />
      </div>
      <div>
        <p class="text-xs font-medium text-muted">{{ t('usage.totalRequests') }}</p>
        <p class="text-xl font-bold">{{ stats?.total_requests?.toLocaleString() || '0' }}</p>
        <p class="text-xs text-muted">{{ t('usage.inSelectedRange') }}</p>
      </div>
    </div>
    <div class="glass-card p-4 flex items-center gap-3">
      <div class="rounded-lg bg-warning-100 p-2 text-warning-text"><svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m21 7.5-9-5.25L3 7.5m18 0-9 5.25m9-5.25v9l-9 5.25M3 7.5l9 5.25M3 7.5v9l9 5.25m0-9v9" /></svg></div>
      <div>
        <p class="text-xs font-medium text-muted">{{ t('usage.totalTokens') }}</p>
        <p class="text-xl font-bold">{{ formatTokens(stats?.total_tokens || 0) }}</p>
        <p class="flex flex-wrap items-center gap-x-1 text-xs text-muted">
    ...
    <div class="glass-card p-4 flex items-center gap-3">
      <div class="rounded-lg bg-[color-mix(in_oklch,var(--success)_16%,transparent)] p-2 text-success-600">
    ...
    <div class="glass-card p-4 flex items-center gap-3">
      <div class="rounded-lg bg-accent-100 p-2 text-accent-600">
        <Icon name="clock" size="md" />
      </div>
      <div><p class="text-xs font-medium text-muted">{{ t('usage.avgDuration') }}</p><p class="text-xl font-bold">{{ formatDuration(stats?.average_duration_ms || 0) }}</p></div>
    </div>
  </div>
</template>
