    ...
    <div class="overflow-x-auto">
      <table class="w-full min-w-max divide-y divide-line">
        <thead class="bg-surface-2">
          <tr>
            <th class="w-16 px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-muted sm:px-6">#</th>
            ...
            <th
              ...
              :key="col.key"
              class="cursor-pointer select-none whitespace-nowrap px-4 py-3 text-right text-xs font-medium uppercase tracking-wider transition-colors hover:bg-surface-2"
              :class="sortBy === col.key ? 'text-accent' : 'text-muted'"
        ...
        <tbody class="divide-y divide-line bg-surface">
          ...
          <tr v-else-if="items.length === 0">
            <td :colspan="sortableColumns.length + 2" class="py-12 text-center text-sm text-muted">
          ...
          >
            <td class="px-4 py-3 sm:px-6">
              ...
              >{{ index + 1 }}</span>
              <span v-else class="inline-block w-6 text-center text-sm tabular-nums text-muted">{{ index + 1 }}</span>
            </td>
            ...
            <td class="whitespace-nowrap px-4 py-3 text-right text-sm tabular-nums text-muted">{{ item.requests.toLocaleString() }}</td>
            <td class="whitespace-nowrap px-4 py-3 text-right text-sm tabular-nums text-muted">{{ fmtTokens(item.input_tokens) }}</td>
