      ...
      <div class="flex flex-1 flex-wrap items-end gap-4">
        ...
        <div ref="userSearchRef" class="usage-filter-dropdown relative w-full sm:w-auto sm:min-w-[240px]">
          ...
          <button
            ...
            type="button"
            @click="clearUser"
            class="absolute right-2 top-9 text-muted"
          ...
          <div
            ...
            class="absolute z-50 mt-1 max-h-60 w-full overflow-auto rounded-lg border bg-surface shadow-[var(--shadow-pop)]"
          >
            <button
              ...
              type="button"
              @click="selectUser(u)"
              class="w-full px-4 py-2 text-left hover:bg-surface-2"
            >
              <span>{{ u.email }}<span v-if="u.deleted" class="ml-1 text-xs text-muted">（{{ t('admin.usage.userDeletedBadge') }}）</span></span>
              <span class="ml-2 text-xs text-muted">#{{ u.id }}</span>
            </button>
          </div>
        </div>
        ...
        <div ref="apiKeySearchRef" class="usage-filter-dropdown relative w-full sm:w-auto sm:min-w-[240px]">
          ...
          >
            ...
            >
              ...
              <span class="ml-2 text-xs text-muted">#{{ k.id }}</span>
            </button>
          </div>
        </div>
        ...
        <div ref="accountSearchRef" class="usage-filter-dropdown relative w-full sm:w-auto sm:min-w-[220px]">
          ...
          >
            ...
            >
              ...
              <span class="ml-2 text-xs text-muted">#{{ a.id }}</span>
            </button>
          </div>
        </div>
        ...
        <!-- Billing Mode Filter (usage list, charts, and user ranking) -->
        <div v-if="mode !== 'errors'" class="w-full sm:w-auto sm:min-w-[200px]">
        ...
        <label v-if="mode === 'ranking'" class="flex items-center gap-2 pb-2 text-sm text-foreground">
          <input
...
interface Props {
   ...
   * ranking 模式:同 usage 但隐藏上游模型审计与清理/导出按钮(用户排行 tab 用)
   */
