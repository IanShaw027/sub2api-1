<template>
  ...
  >
    <div class="settings-rows">
      <!-- Global Toggle --><SettingRow
        :label="t('admin.settings.webSearchEmulation.enabled')"
        :description="t('admin.settings.webSearchEmulation.enabledHint')"
      >
        <Toggle v-model="webSearchConfig.enabled" /> </SettingRow
      ><!-- Providers -->
      <div v-if="webSearchConfig.enabled" class="space-y-4 settings-block">
        <SettingRow :label="t('admin.settings.webSearchEmulation.providers')">
          <button
            type="button"
  ...
  >
    <div class="mx-4 w-full max-w-lg glass-card-solid rounded-hero p-6">
