<template>
  <!-- Codex Settings -->
  <SettingsSection
    :title="t('admin.settings.gatewayForwarding.codexHardeningTitle')"
  >
    <div class="settings-rows">
      ...
      <div class="settings-rows">
        <SettingRow
          :label="t('admin.settings.gatewayForwarding.minCodexVersion')"
        >
          <input
            ...
            :placeholder="
              t('admin.settings.gatewayForwarding.minCodexVersionPlaceholder')
            "
          />
        </SettingRow>
        <SettingRow
          :label="t('admin.settings.gatewayForwarding.maxCodexVersion')"
        >
          <input
            ...
            class="input w-full font-mono text-sm"
            :placeholder="
              t('admin.settings.gatewayForwarding.maxCodexVersionPlaceholder')
            "
          />
        </SettingRow>
      </div>
      <p class="text-xs text-muted settings-block">
        {{ t("admin.settings.gatewayForwarding.codexVersionHint") }}
      </p>
      <SettingRow
        :label="t('admin.settings.gatewayForwarding.codexFingerprintSignals')"
      >
        ...
        >
          <select v-model="row.type" class="input w-32 text-sm">
            ...
            <option value="body_path">
              {{ t("admin.settings.gatewayForwarding.codexFpTypeBodyPath") }}
            </option>
          </select>
          <input
            v-model="row.match"
            ...
            :placeholder="
              t('admin.settings.gatewayForwarding.codexFpMatchPlaceholder')
            "
          />
          <label
      ...
      <SettingRow :label="t('admin.settings.gatewayForwarding.codexBlacklist')">
        <p class="mb-2 mt-1 text-xs text-muted">
        ...
        >
          <input
            ...
            :placeholder="
              t('admin.settings.gatewayForwarding.codexOriginatorPlaceholder')
            "
          />
          <input
            ...
            class="input flex-1 font-mono text-sm"
            :placeholder="
              t('admin.settings.gatewayForwarding.codexUaContainsPlaceholder')
            "
          />
          <button
            type="button"
      ...
      <SettingRow :label="t('admin.settings.gatewayForwarding.codexWhitelist')">
        <p class="mb-2 mt-1 text-xs text-muted">
