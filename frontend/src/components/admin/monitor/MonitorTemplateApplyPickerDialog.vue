<template>
  ...
  >
    ...
    <div v-else-if="monitors.length === 0" class="py-6 text-center text-sm text-muted">
    ...
    <div v-else>
      ...
      <ul class="max-h-80 divide-y divide-line overflow-y-auto rounded-lg border border-line">
        <li
          v-for="m in monitors"
          :key="m.id"
          class="flex cursor-pointer items-center gap-3 px-3 py-2 hover:bg-surface-2"
          @click="toggle(m.id)"
        >
          <input
            type="checkbox"
          ...
          <span v-if="m.provider === 'openai'" class="text-xs text-muted">{{ m.api_mode }}</span>
          <span
            v-if="!m.enabled"
            class="ml-auto rounded bg-surface-2 px-1.5 py-0.5 text-xs text-muted"
          >
