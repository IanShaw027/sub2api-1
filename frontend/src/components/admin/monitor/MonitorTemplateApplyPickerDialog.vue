<template>
  <BaseDialog
    ...
    @close="$emit('close')"
  >
    <p class="mb-3 text-sm text-ink-body">
      {{ t('admin.channelMonitor.template.applyPickerHint') }}
    </p>

    <div v-if="loading" class="py-6 text-center text-sm text-ink-faint">
      {{ t('common.loading') }}
    </div>

    <div v-else-if="monitors.length === 0" class="py-6 text-center text-sm text-ink-faint">
    ...
    <div v-else>
      ...
      <div class="mb-2 flex items-center gap-3 text-xs">
        ...
        <button
          type="button"
          class="text-ink-soft hover:underline"
          @click="selectNone"
        >
      ...
      <ul class="max-h-80 divide-y divide-line overflow-y-auto rounded-lg border border-line dark:divide-dark-700 dark:border-dark-700">
        <li
          v-for="m in monitors"
          :key="m.id"
          class="flex cursor-pointer items-center gap-3 px-3 py-2 hover:bg-page dark:hover:bg-dark-800"
          @click="toggle(m.id)"
        >
          <input
            type="checkbox"
          ...
          />
          <span class="font-medium text-ink dark:text-white">{{ m.name }}</span>
          <span class="text-xs text-ink-faint">{{ m.provider }}</span>
          <span v-if="m.provider === 'openai'" class="text-xs text-ink-faint">{{ m.api_mode }}</span>
          <span
            v-if="!m.enabled"
            class="ml-auto rounded bg-page px-1.5 py-0.5 text-xs text-ink-soft dark:bg-dark-700"
          >
