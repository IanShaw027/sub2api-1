<template>
  ...
  >
    <!-- provider tabs -->
    <div class="mb-4 border-b border-line dark:border-dark-700">
      <div role="tablist" class="flex gap-1">
        ...
        >
          ...
          <span
            ...
            class="ml-1.5 rounded-full bg-ink-faint/10 px-2 py-0.5 text-xs dark:bg-dark-700"
          >
    ...
    <div v-if="!editing" class="space-y-2">
      ...
      <div v-if="loading" class="py-8 text-center text-sm text-ink-faint">
        {{ t('common.loading') }}
      </div>
      ...
      <div
        ...
        v-else
        :key="tpl.id"
        class="rounded-lg border border-line bg-white p-4 dark:border-dark-700 dark:bg-dark-800"
      >
        <div class="flex items-start justify-between gap-3">
          <div class="min-w-0 flex-1">
            <div class="flex items-center gap-2">
              <span class="font-medium text-ink dark:text-white">{{ tpl.name }}</span>
              <span
              ...
              <span
                ...
                class="text-xs text-ink-soft dark:text-ink-soft"
              >
                {{ t('admin.channelMonitor.template.associatedCount', { n: tpl.associated_monitors }) }}
              </span>
            </div>
            <p v-if="tpl.description" class="mt-0.5 text-xs text-ink-soft dark:text-ink-soft">
              {{ tpl.description }}
            </p>
    ...
    <div v-else class="space-y-4">
      ...
      <div v-if="form.provider === PROVIDER_OPENAI" class="rounded-lg border border-accent-100 bg-accent-50/50 p-3 dark:border-blue-500/20 dark:bg-blue-500/10">
...
function tabClass(value: Provider): string {
  return activeProvider.value === value
    ...
    : 'border-b-2 border-transparent text-ink-soft hover:text-ink-body dark:text-ink-soft dark:hover:text-ink'
}
...
function modeBadgeClass(mode: BodyOverrideMode): string {
  switch (mode) {
    ...
    default:
      return 'bg-ink-faint/10 text-ink-body dark:bg-dark-700 dark:text-ink-body'
  }
}
...
function apiModeButtonClass(mode: APIMode): string {
  ...
  if (active) {
    return 'border-brand-500 bg-white text-brand-700 shadow-xs dark:border-brand-400 dark:bg-brand-500/15 dark:text-brand-300'
  }
  return 'border-accent-100 bg-white/70 text-ink-body hover:border-brand-300 dark:border-dark-700 dark:bg-dark-800 dark:text-ink-soft'
}
...
function apiModeBadgeClass(mode: APIMode): string {
  if (normalizeAPIMode(mode) === API_MODE_RESPONSES) {
    return 'bg-accent-100 text-accent-700 dark:bg-blue-500/15 dark:text-blue-300'
  }
