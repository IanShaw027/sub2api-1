<template>
  <BaseDialog :show="show" :title="t('admin.users.groupConfig')" width="wide" @close="$emit('close')">
    <div v-if="user" class="space-y-6">
      <!-- 用户信息头部 -->
      <div class="flex items-center gap-4 rounded-card bg-gradient-to-r from-brand-50 to-brand-100 p-5 dark:from-brand-900/30 dark:to-brand-800/20">
        <div class="flex h-14 w-14 items-center justify-center rounded-full bg-card shadow-xs dark:bg-dark-700">
          <span class="text-2xl font-semibold text-brand-600 dark:text-brand-400">{{ user.email.charAt(0).toUpperCase() }}</span>
        </div>
        <div class="flex-1">
          <p class="text-lg font-semibold text-ink dark:text-white">{{ user.email }}</p>
          <p class="mt-1 text-sm text-ink-body dark:text-dark-400">{{ t('admin.users.groupConfigHint', { email: user.email }) }}</p>
        </div>
      </div>

      <!-- 加载状态 -->
      <div v-if="loading" class="flex justify-center py-12">
        <svg class="h-10 w-10 animate-spin text-brand-500" fill="none" viewBox="0 0 24 24">
      ...
      <div v-else class="space-y-6">
        ...
        <div v-if="exclusiveGroups.length > 0">
          <div class="mb-3 flex items-center gap-2">
            ...
            <h4 class="text-sm font-semibold text-ink-body dark:text-dark-200">{{ t('admin.users.exclusiveGroups') }}</h4>
            <span class="text-xs text-ink-faint">({{ exclusiveGroupConfigs.filter(c => c.isSelected).length }}/{{ exclusiveGroupConfigs.length }})</span>
          </div>
          <div class="grid gap-3">
            <div
              v-for="config in exclusiveGroupConfigs"
              :key="config.groupId"
              class="group relative overflow-hidden rounded-card border-2 p-4 transition-all duration-150"
              :class="config.isSelected
                ? 'border-brand-400 bg-brand-50/50 shadow-xs dark:border-brand-500 dark:bg-brand-900/20'
                : 'border-line bg-card hover:border-ink-faint dark:border-dark-600 dark:bg-dark-800 dark:hover:border-dark-500'"
            >
              <div class="flex items-center gap-4">
                ...
                <div class="flex-shrink-0">
                  <label class="relative flex h-6 w-6 cursor-pointer items-center justify-center">
                    <input
                      ...
                      class="peer sr-only"
                    />
                    <div class="h-5 w-5 rounded-control border-2 border-line transition-all peer-checked:border-brand-500 peer-checked:bg-brand-500 dark:border-dark-500 peer-checked:dark:border-brand-500">
                ...
                <div class="min-w-0 flex-1">
                  <div class="flex items-center gap-2">
                    <span class="text-base font-semibold text-ink dark:text-white">{{ config.groupName }}</span>
                    <span class="inline-flex items-center rounded-full bg-brand-100 px-2 py-0.5 text-xs font-medium text-brand-700 dark:bg-brand-900/40 dark:text-brand-300">
                      {{ t('admin.groups.exclusive') }}
                    </span>
                  </div>
                  <div class="mt-1.5 flex items-center gap-3 text-sm">
                    <span class="inline-flex items-center gap-1 text-ink-soft dark:text-dark-400">
                    ...
                    <span class="text-ink-soft dark:text-dark-400">
                      {{ t('admin.users.defaultRate') }}: <span class="font-medium text-ink-body dark:text-dark-200">{{ config.defaultRate }}x</span>
                    </span>
                  </div>
                </div>
                ...
                <div class="flex flex-shrink-0 items-center gap-3">
                  <label class="text-sm font-medium text-ink-body dark:text-dark-400">{{ t('admin.users.customRate') }}</label>
                  <input
                    type="number"
                    step="0.001"
                    min="0.001"
                    ...
                    class="hide-spinner w-24 rounded-control border border-line bg-card px-3 py-2 text-sm font-medium transition-colors focus:border-accent focus:outline-none focus:ring-2 focus:ring-accent/25 dark:border-dark-500 dark:bg-dark-700 dark:focus:border-accent"
                  />
                </div>
              </div>
            </div>
        ...
        <div v-if="publicGroups.length > 0">
          <div class="mb-3 flex items-center gap-2">
            ...
            <h4 class="text-sm font-semibold text-ink-body dark:text-dark-200">{{ t('admin.users.publicGroups') }}</h4>
            <span class="text-xs text-ink-faint">({{ publicGroupConfigs.length }})</span>
          </div>
          <div class="grid gap-3">
            <div
              v-for="config in publicGroupConfigs"
              :key="config.groupId"
              class="relative overflow-hidden rounded-card border-2 border-success/40 bg-success-soft/50 p-4 dark:border-success/40 dark:bg-success/10"
            >
              <div class="flex items-center gap-4">
                <!-- 复选框（禁用状态） -->
                <div class="flex-shrink-0">
                  <div class="flex h-5 w-5 items-center justify-center rounded-control border-2 border-success bg-success dark:border-success dark:bg-success">
        ...
        <div v-if="groups.length === 0" class="flex flex-col items-center justify-center py-12 text-center">
          <div class="mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-line dark:bg-dark-700">
            <svg class="h-8 w-8 text-ink-faint" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" />
            </svg>
          </div>
          <p class="text-ink-soft dark:text-dark-400">{{ t('common.noGroupsAvailable') }}</p>
        </div>
      </div>
    </div>
