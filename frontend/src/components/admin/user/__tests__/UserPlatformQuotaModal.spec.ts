...
function makeUser(overrides: { id?: number; email?: string; subscriptions?: UserSubscription[] } = {}) {
...
beforeEach(() => {
  ...
  apiMocks.showError.mockReset()
  apiMocks.showSuccess.mockReset()
})

describe('UserPlatformQuotaModal', () => {
  it('挂载时已经 show=true 也会立即加载当前用户配额', async () => {
    mount(UserPlatformQuotaModal, {
      props: {
        show: true,
        user: makeUser(),
      },
  ...
  it('非法中间态不能静默保存成 null', async () => {
    const w = await mountAndOpen()
    const setupState = (w.vm as any).$?.setupState ?? w.vm
    setupState.quotas[0].daily_limit_usd = '-' as unknown as number
  ...
  it('show 保持开启时切换 user 会重新加载新用户配额', async () => {
    const w = await mountAndOpen()

    await w.setProps({
      user: makeUser({ id: 100, email: 'next@example.com' }),
    })
    await flushPromises()

    expect(apiMocks.getPlatformQuotas).toHaveBeenNthCalledWith(1, 99)
    expect(apiMocks.getPlatformQuotas).toHaveBeenNthCalledWith(2, 100)
  })

  it('用户 A 的迟到 load 响应不会覆盖用户 B 的当前配额', async () => {
    const loadA = deferred<{ platform_quotas: Array<Record<string, unknown>> }>()
    apiMocks.getPlatformQuotas.mockReturnValueOnce(loadA.promise)
    const w = mount(UserPlatformQuotaModal, {
      props: {
        show: true,
        user: makeUser({ id: 99, email: 'a@example.com' }),
      },
    })
    await flushPromises()
    ...
    await w.setProps({
      user: makeUser({ id: 100, email: 'b@example.com' }),
    })
    await flushPromises()

    expect((w.findAll('input[type=number]')[0].element as HTMLInputElement).value).toBe('20')
  ...
  it('用户 A 的迟到 save 响应不会关闭已经切到用户 B 的 modal', async () => {
    const saveA = deferred<{ platform_quotas: Array<Record<string, unknown>> }>()
    const w = mount(UserPlatformQuotaModal, {
      props: {
        show: true,
        user: makeUser({ id: 99, email: 'a@example.com' }),
    ...
    apiMocks.updatePlatformQuotas.mockReturnValueOnce(saveA.promise)
    const saveBtn = w.findAll('button').find((b) => b.text() === 'admin.users.platformQuota.save')
    await saveBtn!.trigger('click')
    await flushPromises()
    ...
    saveA.resolve({ platform_quotas: [] })
    await flushPromises()

    expect(w.emitted('close')).toBeUndefined()
    expect(w.emitted('success')).toBeUndefined()
    expect(apiMocks.showSuccess).not.toHaveBeenCalledWith('admin.users.platformQuota.updateSuccess')
  })

  it('用户 A 的迟到 reset 响应不会改写用户 B 的当前配额', async () => {
    const confirmSpy = vi.spyOn(window, 'confirm').mockReturnValue(true)
    const resetA = deferred<{ platform_quotas: Array<Record<string, unknown>> }>()
    apiMocks.getPlatformQuotas.mockResolvedValueOnce({
      platform_quotas: [
        {
          platform: 'anthropic',
    ...
    apiMocks.resetPlatformQuotaWindow.mockReturnValueOnce(resetA.promise)
    const resetBtn = w.findAll('button').find((b) => b.text() === '↻')
    ...
    expect((w.findAll('input[type=number]')[0].element as HTMLInputElement).value).toBe('40')
