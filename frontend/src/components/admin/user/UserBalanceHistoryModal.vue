<template>
  <BaseDialog :show="show" :title="t('admin.users.balanceHistoryTitle')" width="wide" :close-on-click-outside="true" :z-index="40" @close="$emit('close')">
    <div v-if="user" class="space-y-4">
      ...
      <div class="rounded-card bg-page p-4 dark:bg-dark-700">
        ...
        <div class="flex items-center gap-3">
          ...
          <div class="min-w-0 flex-1">
            <div class="flex items-center gap-2">
              <p class="truncate font-medium text-ink dark:text-white">{{ user.email }}</p>
              ...
              <span
                v-if="user.username"
                class="flex-shrink-0 rounded bg-brand-50 px-1.5 py-0.5 text-xs text-brand-600 dark:bg-brand-900/20 dark:text-brand-400"
              >
                {{ user.username }}
              </span>
            </div>
          ...
          <div class="flex-shrink-0 text-right">
            <p class="text-xs text-ink-soft dark:text-dark-400">{{ t('admin.users.currentBalance') }}</p>
        ...
        <div class="mt-2.5 flex items-center justify-between border-t border-line/60 pt-2.5 dark:border-dark-600/60">
          <p class="min-w-0 flex-1 truncate text-xs text-ink-soft dark:text-dark-400" :title="user.notes || ''">
            <template v-if="user.notes">{{ t('admin.users.notes') }}: {{ user.notes }}</template>
            <template v-else>&nbsp;</template>
          </p>
          <p class="ml-4 flex-shrink-0 text-xs text-ink-soft dark:text-dark-400">
            {{ t('admin.users.totalRecharged') }}: <span class="font-semibold text-success dark:text-success">${{ totalRecharged.toFixed(2) }}</span>
          </p>
        </div>
      </div>
      ...
      <div class="flex items-center gap-3">
        ...
        <button
          v-if="!hideActions"
          @click="emit('deposit')"
          class="flex items-center gap-2 rounded-control border border-line bg-card px-3 py-2 text-sm text-ink-body transition-colors hover:bg-page dark:border-dark-600 dark:bg-dark-800 dark:text-dark-200 dark:hover:bg-dark-700"
        >
      ...
      <div v-else class="max-h-[28rem] space-y-3 overflow-y-auto">
        <div
          ...
          :key="item.id"
          class="rounded-card border border-line bg-card p-4 dark:border-dark-600 dark:bg-dark-800"
        >
...
const getIconBg = (item: BalanceHistoryItem) => {
  ...
  if (isSubscriptionType(item.type)) return 'bg-brand-100 dark:bg-brand-900/30'
  return item.value >= 0
...
const getIconColor = (item: BalanceHistoryItem) => {
  ...
  if (isSubscriptionType(item.type)) return 'text-brand-600 dark:text-brand-400'
  return item.value >= 0
