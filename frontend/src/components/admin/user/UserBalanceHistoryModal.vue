    ...
    <div v-if="user" class="space-y-4">
      ...
      <div class="rounded-xl bg-surface-2 p-4">
        ...
        <div class="flex items-center gap-3">
          <div class="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-[color-mix(in_oklch,var(--accent)_12%,transparent)]">
          ...
          <div class="min-w-0 flex-1">
            <div class="flex items-center gap-2">
              ...
              <span
                v-if="user.username"
                class="flex-shrink-0 rounded bg-[color-mix(in_oklch,var(--accent)_12%,transparent)] px-1.5 py-0.5 text-xs text-accent"
              >
                {{ user.username }}
              </span>
            </div>
          ...
          <div class="flex-shrink-0 text-right">
            <p class="text-xs text-muted">{{ t('admin.users.currentBalance') }}</p>
        ...
        <div class="mt-2.5 flex items-center justify-between border-t border-line/60 pt-2.5">
          <p class="min-w-0 flex-1 truncate text-xs text-muted" :title="user.notes || ''">
            <template v-if="user.notes">{{ t('admin.users.notes') }}: {{ user.notes }}</template>
            <template v-else>&nbsp;</template>
          </p>
          <p class="ml-4 flex-shrink-0 text-xs text-muted">
            {{ t('admin.users.totalRecharged') }}: <span class="font-semibold text-success-text">${{ totalRecharged.toFixed(2) }}</span>
          </p>
        </div>
      </div>
      ...
      <div v-else-if="history.length === 0" class="py-8 text-center">
        <p class="text-sm text-muted">{{ t('admin.users.noBalanceHistory') }}</p>
      </div>
