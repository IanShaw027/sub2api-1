    ...
    <form v-if="user" id="edit-user-form" @submit.prevent="handleUpdateUser" class="space-y-5">
      ...
      <div>
        ...
        <div class="flex gap-2">
          <div class="relative flex-1">
            ...
            <button v-if="form.password" type="button" @click="copyPassword" class="absolute right-2 top-1/2 -translate-y-1/2 rounded-lg p-1 transition-colors hover:bg-surface-2" :class="passwordCopied ? 'text-success-500' : 'text-muted'">
      ...
      <div class="rounded-lg border border-line p-3">
        <div class="flex items-start justify-between gap-3">
          <div>
            <label class="font-medium text-foreground">{{ t('admin.usage.ipPin.enable') }}</label>
            <p class="mt-1 text-xs text-muted">{{ t('admin.usage.ipPin.hint') }}</p>
          </div>
          <button
            type="button"
        ...
        <p class="mt-2 text-xs" :class="form.pin_known_ips ? 'text-warning-text' : 'text-muted'">
