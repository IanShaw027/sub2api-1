    ...
    <div v-if="oldGroup" class="space-y-4">
      ...
      <div class="rounded-lg border border-line bg-page p-3 dark:border-dark-600 dark:bg-dark-800">
        <div class="flex items-center gap-2">
          ...
          <span class="font-medium text-ink dark:text-white">{{ oldGroup.name }}</span>
          <Icon name="arrowRight" size="sm" class="ml-auto text-ink-faint" />
          <span v-if="selectedGroupId" class="font-medium text-brand-600 dark:text-brand-400">
      ...
      <div v-if="availableGroups.length > 0" class="max-h-64 space-y-2 overflow-y-auto">
        <label
          ...
          :class="selectedGroupId === group.id
            ...
            : 'border-line hover:border-ink-faint dark:border-dark-600 dark:hover:border-dark-500'"
        >
          <input
            type="radio"
            :value="group.id"
          ...
          <div class="flex-1">
            ...
            <span class="ml-2 text-xs text-ink-faint">{{ group.platform }}</span>
          </div>
        </label>
      </div>

      <!-- 无可选分组 -->
      <div v-else class="py-6 text-center text-sm text-ink-faint">
