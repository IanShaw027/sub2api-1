<template>
  <BaseDialog :show="show" :title="t('admin.users.userApiKeys')" width="wide" @close="handleClose">
    <div v-if="user" class="space-y-4">
      <div class="flex items-center gap-3 rounded-xl bg-surface-2 p-4">
        <div class="flex h-10 w-10 items-center justify-center rounded-full bg-[color-mix(in_oklch,var(--accent)_12%,transparent)]">
          <span class="text-lg font-medium text-accent">{{ user.email.charAt(0).toUpperCase() }}</span>
        </div>
        <div><p class="font-medium text-foreground">{{ user.email }}</p><p class="text-sm text-muted">{{ user.username }}</p></div>
      </div>
      ...
      <div v-else-if="apiKeys.length === 0" class="py-8 text-center"><p class="text-sm text-muted">{{ t('admin.users.noApiKeys') }}</p></div>
        ...
        <div v-for="key in apiKeys" :key="key.id" class="rounded-xl border border-line bg-surface p-4">
          <div class="flex items-start justify-between gap-3">
            <div class="min-w-0 flex-1">
              ...
              <p class="truncate font-mono text-sm text-muted">{{ key.key.substring(0, 20) }}...{{ key.key.substring(key.key.length - 8) }}</p>
            </div>
            <button
          ...
          <div class="mt-3 flex flex-wrap gap-4 text-xs text-muted">
            <div class="flex items-center gap-1">
              ...
              <button
                ...
                class="-mx-1 -my-0.5 flex cursor-pointer items-center gap-1 rounded-md px-1 py-0.5 transition-colors hover:bg-surface-2"
                :disabled="updatingKeyIds.has(key.id)"
              >
                <GroupBadge
                ...
                />
                <span v-else class="text-muted italic">{{ t('admin.users.none') }}</span>
                ...
                <svg v-else class="h-3 w-3 text-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M8.25 15L12 18.75 15.75 15m-7.5-6L12 5.25 15.75 9" /></svg>
              </button>
            </div>
  ...
  <Teleport to="body">
    <div
      ...
      ref="dropdownRef"
      class="animate-in fade-in slide-in-from-top-2 fixed z-[100000020] w-64 overflow-hidden rounded-xl bg-surface shadow-pop ring-1 ring-black/5 duration-200"
