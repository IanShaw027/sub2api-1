<template>
    ...
    <div class="channel-dialog-body">
      ...
      <form id="channel-form" @submit.prevent="handleSubmit" class="flex-1 overflow-y-auto pt-4">
        ...
        <div v-show="activeTab === 'basic'" class="space-y-5">
          <!-- Name -->
          <div>
            <label class="input-label">{{ t('admin.channels.form.name', 'Name') }} <span class="text-danger-text">*</span></label>
            <input
              v-model="form.name"
              type="text"
              required
 ...
 ? 'bg-[color-mix(in_oklch,var(--accent)_12%,transparent)] border-[color-mix(in_oklch,var(--accent)_40%,transparent)]  '
        ...
        >
          ...
          <div>
            <label class="input-label text-xs">
              {{ t('admin.channels.form.groups', 'Associated Groups') }} <span class="text-danger-text">*</span>
              <span v-if="section.group_ids.length > 0" class="ml-1 font-normal text-muted">
                ({{ t('admin.channels.form.selectedCount', { count: section.group_ids.length }) }})
              </span>
            </label>
            <div class="max-h-40 overflow-auto rounded-lg border border-line bg-surface-2 p-2">
              ...
              <div v-else class="flex flex-wrap gap-1">
                <label
                  ...
                  :key="group.id"
                  class="inline-flex cursor-pointer items-center gap-1.5 rounded-md border border-line px-2 py-1 text-xs transition-colors hover:bg-surface-2"
                  :class="[
 section.group_ids.includes(group.id) ? 'bg-[color-mix(in_oklch,var(--accent)_12%,transparent)] border-[color-mix(in_oklch,var(--accent)_40%,transparent)]  ' : '',
 isGroupInOtherChannel(group.id, section.platform) ? 'opacity-40' : ''
                ...
                >
                  <input
                    ...
                    class="h-3 w-3 rounded border-line text-accent focus:ring-accent"
                    @change="toggleGroupInSection(sIdx, group.id)"
                  />
                  <span :class="['font-medium', platformTextClass(group.platform)]">{{ group.name }}</span>
                  <span
                    :class="['tag rounded-full px-1 py-0 text-[10px]', platformBadgeLightClass(group.platform)]"
                  >{{ group.rate_multiplier }}x</span>
                  <span class="text-[10px] text-muted">{{ group.account_count || 0 }}</span>
                  <span
          ...
          <div v-if="section.platform === 'anthropic' && webSearchGlobalEnabled" class="border-t border-line pt-3">
            <div class="flex items-center justify-between">
              <div>
                ...
                <p class="mt-0.5 text-[11px] text-danger-text ">
          ...
          <div v-if="section.platform === 'openai'" class="border-t border-line pt-3">
            <div class="flex items-center justify-between gap-4">
              <div>
                ...
                <p class="mt-0.5 text-[11px] text-warning-text ">
          ...
          <div>
            ...
            <div
              ...
              class="rounded border border-dashed border-line p-2 text-center text-xs text-muted "
            >
            ...
            <div v-else class="space-y-1">
              ...
              >
                ...
                <button
                  ...
                  class="rounded p-0.5 text-muted hover:text-danger-text"
                >
            ...
            <div class="mb-1 flex items-center justify-between">
              ...
              <div class="flex items-center gap-2">
                <button
                  ...
                  class="text-xs text-muted hover:text-accent disabled:opacity-50"
                >
          ...
          <div class="mt-4 border-t border-line pt-4 space-y-3">
            <div class="flex items-center justify-between">
              ...
              <button
                ...
                class="rounded-lg border border-[color-mix(in_oklch,var(--accent)_40%,transparent)] px-3 py-1 text-xs font-medium text-accent hover:bg-[color-mix(in_oklch,var(--accent)_12%,transparent)]   "
              >
            ...
            >
              <div class="flex items-center justify-between">
                ...
                />
                <button type="button" @click="removeAccountStatsRule(sIdx, ruleIndex)" class="text-xs text-danger-text hover:text-danger-text">
                  {{ t('common.delete') }}
                </button>
              </div>
 ...
 : 'border-line hover:bg-surface-2 '"
                  >
                    <input type="checkbox" :checked="rule.group_ids.includes(gid)" class="h-3 w-3 rounded border-line text-accent focus:ring-accent" @change="rule.group_ids.includes(gid) ? rule.group_ids.splice(rule.group_ids.indexOf(gid), 1) : rule.group_ids.push(gid)" />
                    <span :class="['font-medium', platformTextClass(section.platform)]">{{ getGroupNameById(gid) }}</span>
                  </label>
                </div>
                <p v-if="section.group_ids.length === 0" class="mt-1 text-xs text-muted">
              ...
              <div>
                ...
                <div class="mt-1 flex flex-wrap gap-1">
                  <span
                    ...
                    :key="accountId"
                    class="inline-flex items-center gap-1 rounded-md border border-[color-mix(in_oklch,var(--accent)_40%,transparent)] bg-[color-mix(in_oklch,var(--accent)_12%,transparent)] px-2 py-0.5 text-xs  "
                  >
                    <span :class="['font-medium', platformTextClass(section.platform)]">{{ getRuleAccountLabel(accountId) }}</span>
                    <button type="button" @click="removeRuleAccount(rule, accountId)" class="text-muted hover:text-danger-text">
                      <Icon name="x" size="xs" />
                    </button>
                  </span>
                </div>
                ...
                <div class="relative mt-1 rule-account-search-container">
                  ...
                  >
                    ...
                    >
                      ...
                      <span class="ml-2 text-xs text-muted">#{{ account.id }}</span>
                    </button>
                  </div>
                </div>
                <p class="mt-1 text-xs text-muted">
              ...
              <div>
                ...
                <div v-if="rule.pricing.length === 0" class="rounded border border-dashed border-line p-2 text-center text-xs text-muted ">
    ...
    <template #footer>
      <div class="flex justify-end gap-3">
        <button @click="closeDialog" type="button" class="btn-glass-secondary">
...
import { apiIntervalsToForm, apiTimePricingToForm, createDefaultTimePricingForm, findModelConflict, formIntervalsToAPI, formTimePricingToAPI, isValidPositiveMultiplier, mTokToPerToken, perTokenToMTok, validateIntervals, validateTimePricing } from './types'
...
const groupToChannelMap = computed(() => {
  ...
  for (const ch of allChannelsForConflict.value) {
    if (props.editingChannel && ch.id === props.editingChannel.id) continue
...
function formToAPI(): { group_ids: number[], model_pricing: ChannelModelPricing[], model_mapping: Record<string, Record<string, string>>, features_config: Record<string, unknown> } {
  ...
  const featuresConfig: Record<string, unknown> = props.editingChannel?.features_config
    ? { ...props.editingChannel.features_config }
    : {}
...
.channel-tab-inactive {
  @apply border-transparent text-muted hover:text-foreground hover:border-line;
}
</style>
