  ...
  <div class="rounded-lg border border-line bg-surface-2 p-3">
    ...
    >
      <Icon
        ...
        size="sm"
        :stroke-width="2"
        class="flex-shrink-0 text-muted transition-transform duration-200"
      />
      ...
      <div v-if="collapsed" class="flex min-w-0 flex-1 items-center gap-2 overflow-hidden">
        ...
        <!-- Billing mode badge -->
        <span
          class="flex-shrink-0 rounded-full bg-[color-mix(in_oklch,var(--accent)_12%,transparent)] px-2 py-0.5 text-xs font-medium text-accent"
        >
          {{ billingModeLabel }}
        </span>
      </div>
      ...
      <div class="collapsible-inner">
        ...
        <div v-if="entry.billing_mode === 'token'">
          ...
          <label class="mt-3 block text-xs font-medium text-muted">
            ...
            <span class="ml-1 font-normal text-muted">$/MTok</span>
          </label>
          <div class="pricing-default-grid mt-1 grid gap-2">
            <div>
              <label class="text-xs text-muted">{{ t('admin.channels.form.inputPrice') }}</label>
          ...
          <div v-if="enableTierMultipliers" class="mt-3 grid max-w-md grid-cols-2 gap-2">
            <div>
              <label class="text-xs text-muted">{{ t('admin.channels.form.fastMultiplier') }}</label>
            ...
            <div>
              <label class="text-xs text-muted">{{ t('admin.channels.form.flexMultiplier') }}</label>
          ...
          <div v-if="!hideTokenIntervals" class="mt-3">
            <div class="flex items-center justify-between">
              <label class="text-xs font-medium text-muted">
                ...
                <span class="ml-1 font-normal text-muted">(min, max]</span>
              </label>
        ...
        <div v-else-if="entry.billing_mode === 'per_request'">
          ...
          <label class="mt-3 block text-xs font-medium text-muted">
            ...
            <span class="ml-1 font-normal text-muted">$</span>
          </label>
          <div class="mt-1 w-48">
          ...
          <div v-else class="mt-2 rounded border border-dashed border-line p-3 text-center text-xs text-muted">
        ...
        <div v-else-if="entry.billing_mode === 'image' || entry.billing_mode === 'video'">
          ...
          <div class="mt-3 flex items-center justify-between">
            ...
            <button type="button" @click="addMediaTier" class="text-xs text-accent hover:text-accent">
              + {{ t('admin.channels.form.addTier') }}
            </button>
          </div>
          <div v-if="entry.intervals && entry.intervals.length > 0" class="mt-2 space-y-2">
