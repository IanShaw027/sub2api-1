<template>
  <div class="rounded-lg border border-line bg-page p-3 dark:border-dark-600 dark:bg-dark-800">
    ...
    >
      ...
      <div v-if="collapsed" class="flex min-w-0 flex-1 items-center gap-2 overflow-hidden">
        ...
        <div class="flex min-w-0 flex-1 flex-wrap items-center gap-1">
          ...
          <span
            ...
            class="text-xs italic text-ink-faint"
          >
        ...
        <!-- Billing mode badge -->
        <span
          class="flex-shrink-0 rounded-full bg-brand-100 px-2 py-0.5 text-xs font-medium text-brand-700 dark:bg-brand-900/30 dark:text-brand-300"
        >
          {{ billingModeLabel }}
        </span>
      </div>
      ...
      <div v-else class="flex-1 text-xs font-medium text-ink-soft dark:text-dark-400">
      ...
      <button
        ...
        class="flex-shrink-0 rounded p-1 text-ink-faint hover:text-red-500"
      >
    ...
    >
      <div class="collapsible-inner">
        ...
        <div class="mt-3 flex items-start gap-2">
          <div class="flex-1">
            <label class="text-xs font-medium text-ink-soft dark:text-dark-400">
        ...
        <div v-if="entry.billing_mode === 'token'">
          ...
          <label class="mt-3 block text-xs font-medium text-ink-soft dark:text-dark-400">
            {{ t('admin.channels.form.defaultPrices') }}
            <span class="ml-1 font-normal text-ink-faint">$/MTok</span>
          </label>
          <div class="mt-1 grid grid-cols-2 gap-2 sm:grid-cols-5">
            <div>
              <label class="text-xs text-ink-faint">{{ t('admin.channels.form.inputPrice') }}</label>
          ...
          <div class="mt-3">
            <div class="flex items-center justify-between">
              <label class="text-xs font-medium text-ink-soft dark:text-dark-400">
                ...
                <span class="ml-1 font-normal text-ink-faint">(min, max]</span>
              </label>
              <button type="button" @click="addInterval" class="text-xs text-brand-600 hover:text-brand-700">
        ...
        <div v-else-if="entry.billing_mode === 'per_request'">
          ...
          <label class="mt-3 block text-xs font-medium text-ink-soft dark:text-dark-400">
            ...
            <span class="ml-1 font-normal text-ink-faint">$</span>
          </label>
          <div class="mt-1 w-48">
          ...
          <div v-else class="mt-2 rounded border border-dashed border-line p-3 text-center text-xs text-ink-faint dark:border-dark-500">
        ...
        <div v-else-if="entry.billing_mode === 'image'">
          ...
          <div class="mt-3 flex items-center justify-between">
            ...
            <button type="button" @click="addImageTier" class="text-xs text-brand-600 hover:text-brand-700">
              + {{ t('admin.channels.form.addTier') }}
            </button>
          </div>
          <div v-if="entry.intervals && entry.intervals.length > 0" class="mt-2 space-y-2">
