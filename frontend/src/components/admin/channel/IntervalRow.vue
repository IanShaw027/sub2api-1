<template>
  <div class="flex items-start gap-2 rounded border p-2"
       :class="isEmpty ? 'border-red-400 bg-red-50 dark:border-red-500 dark:bg-red-950/20' : 'border-line bg-card dark:border-dark-500 dark:bg-dark-700'">
    <!-- Token mode: context range + prices ($/MTok) -->
    <template v-if="mode === 'token'">
      <div class="w-20">
        <label class="text-xs text-ink-faint">{{ t('admin.channels.form.minTokens') }}</label>
      ...
      <div class="w-20">
        <label class="text-xs text-ink-faint">{{ t('admin.channels.form.maxTokens') }} <span class="text-ink-faint">{{ t('admin.channels.form.inclusive') }}</span></label>
      ...
      <div class="flex-1">
        <label class="text-xs text-ink-faint">{{ t('admin.channels.form.inputPrice') }} <span v-if="isEmpty" class="text-red-500">*</span> <span class="text-ink-faint">$/M</span></label>
      ...
      <div class="flex-1">
        <label class="text-xs text-ink-faint">{{ t('admin.channels.form.outputPrice') }} <span v-if="isEmpty" class="text-red-500">*</span> <span class="text-ink-faint">$/M</span></label>
        <input :value="interval.output_price" @input="emitField('output_price', ($event.target as HTMLInputElement).value)"
          type="number" step="any" min="0" class="input mt-0.5 text-xs" />
      </div>
      <div class="flex-1">
        <label class="text-xs text-ink-faint">{{ t('admin.channels.form.cacheWritePriceShort') }} <span class="text-ink-faint">$/M</span></label>
        <input :value="interval.cache_write_price" @input="emitField('cache_write_price', ($event.target as HTMLInputElement).value)"
          type="number" step="any" min="0" class="input mt-0.5 text-xs" />
      </div>
      <div class="flex-1">
        <label class="text-xs text-ink-faint">{{ t('admin.channels.form.cacheReadPriceShort') }} <span class="text-ink-faint">$/M</span></label>
    ...
    <template v-else>
      ...
      <div class="flex-1">
        <label class="text-xs text-ink-faint">{{ t('admin.channels.form.perRequestPrice') }} <span v-if="isEmpty" class="text-red-500">*</span> <span class="text-ink-faint">$</span></label>
        <input :value="interval.per_request_price" @input="emitField('per_request_price', ($event.target as HTMLInputElement).value)"
          type="number" step="any" min="0" class="input mt-0.5 text-xs" />
      </div>
    </template>

    <button type="button" @click="emit('remove')" class="mt-4 rounded p-0.5 text-ink-faint hover:text-red-500">
      <Icon name="x" size="sm" />
    </button>
  </div>
</template>
