    ...
    <div v-if="order" class="space-y-4">
      <div class="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <p class="text-sm font-medium text-foreground">{{ paymentAmountSymbol }}{{ baseAmount.toFixed(2) }}</p>
        </div>
        <div v-if="order.fee_rate > 0">
          <p class="text-xs text-muted">{{ t('payment.orders.fee') }} ({{ order.fee_rate }}%)</p>
          <p class="text-sm font-medium text-foreground">{{ paymentAmountSymbol }}{{ feeAmount.toFixed(2) }}</p>
        </div>
        <div>
          <p class="text-xs text-muted">{{ t('payment.orders.payAmount') }}</p>
          <p class="text-sm font-medium text-foreground">{{ paymentAmountSymbol }}{{ order.pay_amount.toFixed(2) }}</p>
        </div>
        <div v-if="order.amount !== order.pay_amount">
          <p class="text-xs text-muted">{{ t('payment.orders.creditedAmount') }}</p>
          <p class="text-sm font-medium text-foreground">{{ creditedAmountSymbol }}{{ order.amount.toFixed(2) }}</p>
        </div>
        <div>
          <p class="text-xs text-muted">{{ t('payment.orders.paymentMethod') }}</p>
        ...
        <div>
          ...
          <p class="text-sm text-foreground">#{{ order.user_id }}</p>
        </div>
        <div>
          <p class="text-xs text-muted">{{ t('payment.orders.createdAt') }}</p>
          <p class="text-sm text-foreground">{{ formatDateTime(order.created_at) }}</p>
        </div>
        <div>
          <p class="text-xs text-muted">{{ t('payment.admin.expiresAt') }}</p>
          <p class="text-sm text-foreground">{{ formatDateTime(order.expires_at) }}</p>
        </div>
        <div v-if="order.paid_at">
          <p class="text-xs text-muted">{{ t('payment.admin.paidAt') }}</p>
          <p class="text-sm text-foreground">{{ formatDateTime(order.paid_at) }}</p>
        </div>
        <div v-if="order.completed_at">
          <p class="text-xs text-muted">{{ t('payment.admin.completedAt') }}</p>
          <p class="text-sm text-foreground">{{ formatDateTime(order.completed_at) }}</p>
        </div>
      </div>
      ...
      >
        <h4 class="mb-2 text-sm font-semibold text-danger-text">
          {{ t('payment.admin.refundInfo') }}
        </h4>
        <div class="grid grid-cols-2 gap-2 text-sm">
          <div>
            <span class="text-danger-text">{{ t('payment.admin.refundAmount') }}:</span>
            <span class="ml-1 font-medium text-danger-text">{{ creditedAmountSymbol }}{{ order.refund_amount.toFixed(2) }}</span>
          </div>
          <div v-if="order.refund_reason" class="col-span-2">
            <span class="text-danger-text">{{ t('payment.admin.refundReason') }}:</span>
            <span class="ml-1 text-danger-text">{{ order.refund_reason }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="flex items-center justify-end gap-2 border-t border-line pt-4">
        ...
        <button
          ...
          class="btn btn-sm rounded-md bg-red-50 px-3 py-1.5 text-sm text-danger-text hover:bg-[color-mix(in_oklch,var(--danger)_14%,transparent)]"
        >
