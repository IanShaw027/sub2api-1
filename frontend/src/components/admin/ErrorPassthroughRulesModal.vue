  ...
  >
      ...
      <div v-else class="max-h-96 overflow-auto rounded-card border border-line dark:border-dark-600">
        <table class="min-w-full divide-y divide-line dark:divide-dark-700">
          ...
          <tbody class="divide-y divide-line bg-card dark:divide-dark-700 dark:bg-dark-800">
            <tr v-for="rule in rules" :key="rule.id" class="hover:bg-page dark:hover:bg-dark-700">
              <td class="whitespace-nowrap px-3 py-2">
                <span class="inline-flex h-5 w-5 items-center justify-center rounded bg-page text-xs font-medium text-ink-body dark:bg-dark-600">
                  {{ rule.priority }}
                </span>
              </td>
              <td class="px-3 py-2">
                <div class="font-medium text-ink dark:text-white text-sm">{{ rule.name }}</div>
                <div v-if="rule.description" class="mt-0.5 text-xs text-ink-soft max-w-xs truncate">
                  {{ rule.description }}
                </div>
              </td>
              <td class="px-3 py-2">
              ...
              <td class="px-3 py-2">
                <div v-if="rule.platforms.length === 0" class="text-xs text-ink-soft">
                ...
                <div v-else class="flex flex-wrap gap-1">
                  ...
                  <span v-if="rule.platforms.length > 2" class="text-xs text-ink-soft">
                ...
                <div class="text-xs space-y-0.5">
                  <div class="flex items-center gap-1">
                    <Icon
                      ...
                      size="xs"
                      :class="rule.passthrough_code ? 'text-success' : 'text-ink-faint'"
                    />
                  ...
                  <div class="flex items-center gap-1">
                    <Icon
                      ...
                      size="xs"
                      :class="rule.passthrough_body ? 'text-success' : 'text-ink-faint'"
                    />
                    <span class="text-ink-soft">
              ...
              <td class="px-3 py-2">
                <button
                  ...
                  :class="[
                    'relative inline-flex h-4 w-7 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-150 ease-in-out focus:outline-none focus:ring-2 focus:ring-accent/25 focus:ring-offset-2',
                    rule.enabled ? 'bg-brand-600' : 'bg-line dark:bg-dark-600'
                  ]"
                >
                  <span
                    :class="[
                      'pointer-events-none inline-block h-3 w-3 transform rounded-full bg-white shadow ring-0 transition duration-150 ease-in-out',
    ...
    >
      <form @submit.prevent="handleSubmit" class="space-y-4">
        ...
        <!-- Match Conditions -->
        <div class="rounded-control border border-line p-3 dark:border-dark-600">
          <h4 class="mb-2 text-sm font-medium text-ink dark:text-white">
            ...
            <div class="mt-1 space-y-2">
              ...
              >
                <input
                  ...
                  class="mt-0.5 h-3.5 w-3.5 border-line text-brand-600 focus:ring-accent/25"
                />
                <div class="flex-1">
                  <span class="text-xs font-medium text-ink-body">{{ option.label }}</span>
                  <p class="text-xs text-ink-soft">{{ option.description }}</p>
                </div>
              </label>
            </div>
          </div>
          ...
          <div class="mt-3">
            ...
            <div class="flex flex-wrap gap-3">
              ...
              >
                ...
                />
                <span class="text-xs text-ink-body">{{ platform.label }}</span>
              </label>
            </div>
            ...
            <div>
              <label class="flex items-center gap-1.5">
                <input
                  ...
                  class="h-3.5 w-3.5 rounded border-line text-brand-600 focus:ring-accent/25"
                />
                <span class="text-xs font-medium text-ink-body">
        ...
        <div class="flex items-center gap-1.5">
          <input
            ...
            class="h-3.5 w-3.5 rounded border-line text-warning focus:ring-warning"
          />
          <span class="text-xs font-medium text-ink-body">
