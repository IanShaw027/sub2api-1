      ...
      <div v-else class="max-h-96 overflow-auto rounded-lg border border-line">
        <table class="min-w-full divide-y divide-line">
          ...
          <tbody class="divide-y divide-line bg-surface">
            <tr v-for="rule in rules" :key="rule.id" class="hover:bg-surface-2">
              <td class="whitespace-nowrap px-3 py-2">
                <span class="inline-flex h-5 w-5 items-center justify-center rounded bg-surface-2 text-xs font-medium text-foreground">
                  {{ rule.priority }}
                </span>
              </td>
              <td class="px-3 py-2">
                <div class="font-medium text-foreground text-sm">{{ rule.name }}</div>
                <div v-if="rule.description" class="mt-0.5 text-xs text-muted max-w-xs truncate">
                  {{ rule.description }}
                </div>
              </td>
              <td class="px-3 py-2">
              ...
              <td class="px-3 py-2">
                <div v-if="rule.platforms.length === 0" class="text-xs text-muted">
                ...
                <div v-else class="flex flex-wrap gap-1">
                  ...
                  <span v-if="rule.platforms.length > 2" class="text-xs text-muted">
