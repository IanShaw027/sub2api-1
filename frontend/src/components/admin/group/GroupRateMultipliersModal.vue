<template>
  <BaseDialog :show="show" :title="t('admin.groups.rateMultipliersTitle')" width="wide" @close="handleClose">
    <div v-if="group" class="space-y-4">
      <!-- 分组信息 -->
      <div class="flex flex-wrap items-center gap-3 rounded-lg bg-surface-2 px-4 py-2.5 text-sm">
      ...
      <div class="rounded-lg border border-line p-3">
          ...
          <div class="relative flex-1">
            ...
            >
              ...
              >
                ...
                <span v-if="user.username" class="text-xs text-muted">{{ user.email }}</span>
              </button>
            </div>
          </div>
          <div class="w-24">
        ...
        <!-- 批量调整 + 全部清空 -->
        <div v-if="localEntries.length > 0" class="mt-3 flex items-center gap-3 border-t border-line pt-3">
          <span class="text-xs font-medium text-muted">{{ t('admin.groups.batchAdjust') }}</span>
          ...
          <div class="ml-auto">
            <button
              type="button"
              class="rounded-lg border border-danger-200 bg-danger-50 px-3 py-1.5 text-sm font-medium text-danger-text transition-colors hover:bg-[color-mix(in_oklch,var(--danger)_14%,transparent)]"
              @click="clearAllLocal"
            >
              ...
              <table class="w-full min-w-max text-sm">
                <thead class="sticky top-0 z-[1]">
                  <tr class="border-b border-line bg-surface-2">
                    <th class="px-3 py-2 text-left text-xs font-medium text-muted">{{ t('admin.groups.columns.userEmail') }}</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-muted">ID</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-muted">{{ t('admin.groups.columns.userName') }}</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-muted">{{ t('admin.groups.columns.userNotes') }}</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-muted">{{ t('admin.groups.columns.userStatus') }}</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-muted">{{ t('admin.groups.columns.rateMultiplier') }}</th>
                ...
                <tbody class="divide-y divide-line">
                  ...
                  >
                    <td class="px-3 py-2 text-muted">{{ entry.user_email }}</td>
                    ...
                    <td class="max-w-[160px] truncate px-3 py-2 text-muted" :title="entry.user_notes">{{ entry.user_notes || '-' }}</td>
 ...
 : 'bg-surface-2 text-muted'
                    ...
                    <td class="px-2 py-2">
                      <button
                        type="button"
                        class="rounded p-1 text-muted transition-colors hover:bg-[color-mix(in_oklch,var(--danger)_14%,transparent)] hover:text-danger-text"
      ...
      <div class="flex items-center gap-3 border-t border-line pt-4">
        ...
        <template v-if="isDirty">
          <span class="text-xs text-warning-text">{{ t('admin.groups.unsavedChanges') }}</span>
          <button
            type="button"
