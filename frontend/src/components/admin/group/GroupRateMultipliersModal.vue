<template>
  <BaseDialog :show="show" :title="t('admin.groups.rateMultipliersTitle')" width="wide" @close="handleClose">
    <div v-if="group" class="space-y-4">
      <!-- 分组信息 -->
      <div class="flex flex-wrap items-center gap-3 rounded-lg bg-page px-4 py-2.5 text-sm dark:bg-dark-700">
        ...
        <span class="text-ink-faint">|</span>
        <span class="text-ink-body dark:text-dark-400">
          {{ t('admin.groups.columns.rateMultiplier') }}: {{ group.rate_multiplier }}x
        </span>
      </div>

      <!-- 操作区 -->
      <div class="rounded-lg border border-line p-3 dark:border-dark-600">
        <!-- 添加用户 -->
        <h4 class="mb-2 text-sm font-medium text-ink-body dark:text-dark-200">
        ...
        <div class="flex items-end gap-2">
          <div class="relative flex-1">
            ...
            <div
              ...
              class="absolute left-0 right-0 top-full z-10 mt-1 max-h-48 overflow-y-auto rounded-lg border border-line bg-white shadow-lg dark:border-dark-500 dark:bg-dark-700"
            >
              <button
                v-for="user in searchResults"
                :key="user.id"
                type="button"
                class="flex w-full items-center gap-2 px-3 py-1.5 text-left text-sm hover:bg-page dark:hover:bg-dark-600"
                @click="selectUser(user)"
              >
                <span class="text-ink-faint">#{{ user.id }}</span>
                <span class="text-ink dark:text-white">{{ user.username || user.email }}</span>
                <span v-if="user.username" class="text-xs text-ink-faint">{{ user.email }}</span>
              </button>
            </div>
          </div>
          <div class="w-24">
        ...
        <!-- 批量调整 + 全部清空 -->
        <div v-if="localEntries.length > 0" class="mt-3 flex items-center gap-3 border-t border-line pt-3 dark:border-dark-600">
          <span class="text-xs font-medium text-ink-soft dark:text-dark-400">{{ t('admin.groups.batchAdjust') }}</span>
          <div class="flex items-center gap-1.5">
            ...
            <input
              ...
              min="0"
              autocomplete="off"
              class="hide-spinner w-20 rounded border border-line bg-white px-2 py-1 text-center text-sm transition-colors focus:border-accent focus:outline-none focus:ring-1 focus:ring-accent/25 dark:border-dark-500 dark:bg-dark-700 dark:focus:border-accent"
              placeholder="0.5"
            />
            <button
              type="button"
      ...
      <div v-else>
        ...
        <div v-if="localEntries.length === 0" class="py-6 text-center text-sm text-ink-faint dark:text-dark-500">
          {{ t('admin.groups.noRateMultipliers') }}
        </div>

        <div v-else>
          <!-- 表格 -->
          <div class="overflow-hidden rounded-lg border border-line dark:border-dark-600">
            <div class="max-h-[420px] overflow-y-auto">
              <table class="w-full text-sm">
                <thead class="sticky top-0 z-[1]">
                  <tr class="border-b border-line bg-page dark:border-dark-600 dark:bg-dark-700">
                    <th class="px-3 py-2 text-left text-xs font-medium text-ink-soft dark:text-dark-400">{{ t('admin.groups.columns.userEmail') }}</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-ink-soft dark:text-dark-400">ID</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-ink-soft dark:text-dark-400">{{ t('admin.groups.columns.userName') }}</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-ink-soft dark:text-dark-400">{{ t('admin.groups.columns.userNotes') }}</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-ink-soft dark:text-dark-400">{{ t('admin.groups.columns.userStatus') }}</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-ink-soft dark:text-dark-400">{{ t('admin.groups.columns.rateMultiplier') }}</th>
                    <th v-if="showFinalRate" class="px-3 py-2 text-left text-xs font-medium text-brand-600 dark:text-brand-400">{{ t('admin.groups.finalRate') }}</th>
                ...
                <tbody class="divide-y divide-line dark:divide-dark-600">
                  <tr
                    ...
                    :key="entry.user_id"
                    class="hover:bg-page dark:hover:bg-dark-700/50"
                  >
                    <td class="px-3 py-2 text-ink-body dark:text-dark-400">{{ entry.user_email }}</td>
                    <td class="whitespace-nowrap px-3 py-2 text-ink-faint dark:text-dark-500">{{ entry.user_id }}</td>
                    <td class="whitespace-nowrap px-3 py-2 text-ink dark:text-white">{{ entry.user_name || '-' }}</td>
                    <td class="max-w-[160px] truncate px-3 py-2 text-ink-soft dark:text-dark-400" :title="entry.user_notes">{{ entry.user_notes || '-' }}</td>
                      ...
                      <span
                        :class="[
                          ...
                          entry.user_status === 'active'
                            ...
                            : 'bg-page text-ink-body dark:bg-dark-600 dark:text-dark-400'
                        ]"
                      >
                        {{ entry.user_status }}
                      </span>
                    ...
                    <td class="whitespace-nowrap px-3 py-2">
                      <input
                        ...
                        class="hide-spinner w-20 rounded border border-line bg-white px-2 py-1 text-center text-sm font-medium transition-colors focus:border-accent focus:outline-none focus:ring-1 focus:ring-accent/25 dark:border-dark-500 dark:bg-dark-700 dark:focus:border-accent"
                        @change="updateLocalRate(entry.user_id, ($event.target as HTMLInputElement).value)"
                      />
                    </td>
                    <td v-if="showFinalRate" class="whitespace-nowrap px-3 py-2 font-medium text-brand-600 dark:text-brand-400">
                      {{ computeFinalRate(entry.rate_multiplier) }}
                    </td>
                    <td class="px-2 py-2">
                      <button
                        type="button"
                        class="rounded p-1 text-ink-faint transition-colors hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/20 dark:hover:text-red-400"
      ...
      <!-- 底部操作栏 -->
      <div class="flex items-center gap-3 border-t border-line pt-4 dark:border-dark-600">
        <!-- 左侧：未保存提示 + 撤销 -->
...
const platformColorClass = computed(() => {
  switch (props.group?.platform) {
    ...
    default: return 'text-accent-700 dark:text-blue-400'
  }
})
