<template>
  <div class="rounded-2xl border border-line bg-surface-2 p-4">
    ...
    <div v-if="mode === 'custom'" class="mt-4 space-y-4">
      ...
      <div v-if="anyOf.length === 0" class="rounded-xl border border-dashed border-line p-4 text-sm text-muted">
        ...
        <div class="flex items-start justify-between gap-3">
          <div class="min-w-0">
            <div class="text-sm font-medium text-foreground">
              ...
              <span class="ml-2 text-xs font-normal text-muted">AND ({{ (group.all_of?.length || 0) }}/50)</span>
            </div>
            <div class="mt-1 text-xs text-muted">
