        ...
        <div class="py-1">
          <template v-if="account">
            ...
            <template v-if="(account.type === 'oauth' || account.type === 'setup-token') && !isShadow">
              <button @click="$emit('reauth', account); $emit('close')" class="flex w-full items-center gap-2 px-4 py-2 text-sm text-accent hover:bg-surface-2">
            ...
            <button v-if="isOpenAIOAuthParent" @click="$emit('create-spark-shadow', account); $emit('close')" class="flex w-full items-center gap-2 px-4 py-2 text-sm text-warning-text hover:bg-surface-2">
              <Icon name="sparkles" size="sm" />
              {{ t('admin.accounts.createSparkShadow') }}
            </button>
            <button v-if="supportsPrivacy" @click="$emit('set-privacy', account); $emit('close')" class="flex w-full items-center gap-2 px-4 py-2 text-sm text-success-text hover:bg-surface-2">
              <Icon name="shield" size="sm" />
              {{ t('admin.accounts.setPrivacy') }}
            </button>
            <div v-if="hasRecoverableState" class="my-1 border-t border-line"></div>
            <button v-if="hasRecoverableState" @click="$emit('recover-state', account); $emit('close')" class="flex w-full items-center gap-2 px-4 py-2 text-sm text-success-text hover:bg-surface-2">
            ...
            <button v-if="!isShadow" @click="$emit('reset-device-profile', account); $emit('close')" class="flex w-full items-center gap-2 px-4 py-2 text-sm text-warning-text hover:bg-surface-2">
              <Icon name="sync" size="sm" />
