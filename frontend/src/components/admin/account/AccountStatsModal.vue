<template>
  ...
  >
    <div class="space-y-6">
      ...
      <div
        v-if="account"
        class="flex items-center justify-between rounded-card border border-brand-200 bg-gradient-to-r from-brand-50 to-brand-100 p-3 dark:border-brand-700/50 dark:from-brand-900/20 dark:to-brand-800/20"
      >
        <div class="flex items-center gap-3">
          ...
          <div>
            <div class="font-semibold text-ink dark:text-ink">{{ account.name }}</div>
        ...
        <span
          :class="[
            'rounded-chip px-2.5 py-1 text-xs font-semibold',
            account.status === 'active'
              ? 'bg-success-soft text-success dark:bg-success/20 dark:text-success'
              : 'bg-page text-ink-soft dark:bg-dark-700'
          ]"
        >
          {{ account.status }}
        </span>
      ...
      <template v-else-if="stats">
        ...
        <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
          <!-- 30-Day Total Cost -->
          <div
            class="card border-success/25 bg-gradient-to-br from-success-soft to-card p-4 dark:border-success/30 dark:from-success/10 dark:to-dark-700"
          >
            <div class="mb-2 flex items-center justify-between">
              <span class="text-xs font-medium text-ink-soft">{{
                t('admin.accounts.stats.totalCost')
              }}</span>
              <div class="rounded-control bg-success-soft p-1.5 dark:bg-success/20">
                <Icon name="dollar" size="sm" class="text-success dark:text-success" />
              </div>
            </div>
            ...
            <p class="mt-1 text-xs text-ink-soft">
              ...
              <span class="text-ink-faint dark:text-ink-soft">
          ...
          <div
            class="card border-accent-200 bg-gradient-to-br from-accent-50 to-card p-4 dark:border-accent-800/30 dark:from-accent-900/10 dark:to-dark-700"
          >
            <div class="mb-2 flex items-center justify-between">
              <span class="text-xs font-medium text-ink-soft">{{
                t('admin.accounts.stats.totalRequests')
              }}</span>
              <div class="rounded-control bg-accent-50 p-1.5 dark:bg-accent-900/30">
          ...
          <!-- Daily Average Cost -->
          <div
            class="card border-warning/25 bg-gradient-to-br from-warning-soft to-card p-4 dark:border-warning/30 dark:from-warning/10 dark:to-dark-700"
          >
            <div class="mb-2 flex items-center justify-between">
              <span class="text-xs font-medium text-ink-soft">{{
                t('admin.accounts.stats.avgDailyCost')
              }}</span>
              <div class="rounded-control bg-warning-soft p-1.5 dark:bg-warning/20">
                <Icon
                  name="calculator"
                  size="sm"
          ...
          <div
            class="card border-brand-200 bg-gradient-to-br from-brand-50 to-card p-4 dark:border-brand-800/30 dark:from-brand-900/10 dark:to-dark-700"
          >
            <div class="mb-2 flex items-center justify-between">
              <span class="text-xs font-medium text-ink-soft">{{
                t('admin.accounts.stats.avgDailyRequests')
              }}</span>
              <div class="rounded-control bg-brand-50 p-1.5 dark:bg-brand-900/30">
                <svg
        ...
        <div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
          ...
          <div class="card p-4">
            <div class="mb-3 flex items-center gap-2">
              <div class="rounded-control bg-brand-50 p-1.5 dark:bg-brand-900/30">
                <Icon name="clock" size="sm" class="text-brand-600 dark:text-brand-cyan" />
              </div>
              <span class="text-sm font-semibold tabular-nums text-ink dark:text-white">{{
                t('admin.accounts.stats.todayOverview')
              }}</span>
            </div>
            <div class="space-y-2">
              <div class="flex items-center justify-between">
                <span class="text-xs text-ink-soft">{{ t('usage.accountBilled') }}</span>
                <span class="text-sm font-semibold tabular-nums text-ink dark:text-white"
                  >${{ formatCost(stats.summary.today?.cost || 0) }}</span
                >
              </div>
              <div class="flex items-center justify-between">
                <span class="text-xs text-ink-soft">{{ t('usage.userBilled') }}</span>
                <span class="text-sm font-semibold tabular-nums text-ink dark:text-white"
          ...
          <div class="card p-4">
            <div class="mb-3 flex items-center gap-2">
              <div class="rounded-control bg-warning-soft p-1.5 dark:bg-warning/20">
                <Icon name="fire" size="sm" class="text-warning dark:text-warning" />
              </div>
              <span class="text-sm font-semibold tabular-nums text-ink dark:text-white">{{
        ...
        <div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
          ...
          <div class="card p-4">
            <div class="mb-3 flex items-center gap-2">
              <div class="rounded-control bg-brand-50 p-1.5 dark:bg-brand-900/30">
                <Icon name="cube" size="sm" class="text-brand-600 dark:text-brand-400" />
              </div>
              <span class="text-sm font-semibold tabular-nums text-ink dark:text-white">{{
          ...
          <div class="card p-4">
            <div class="mb-3 flex items-center gap-2">
              <div class="rounded-control bg-danger-soft p-1.5 dark:bg-danger/20">
                <Icon name="bolt" size="sm" class="text-danger dark:text-danger" />
              </div>
              <span class="text-sm font-semibold tabular-nums text-ink dark:text-white">{{
        ...
        <!-- Usage Trend Chart -->
        <div class="card p-4">
          <h3 class="mb-4 text-sm font-semibold tabular-nums text-ink dark:text-white">
          ...
          <div class="h-64">
            ...
            <div
              v-else
              class="flex h-full items-center justify-center text-sm text-ink-soft"
            >
      ...
      <div
        v-else-if="!loading"
        class="flex flex-col items-center justify-center py-12 text-ink-soft"
      >
