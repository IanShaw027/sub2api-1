    ...
    <div class="space-y-4">
      ...
      >
        ...
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          <div>
            <label class="mb-1 flex items-center gap-1 text-xs font-medium text-muted">
              {{ t('admin.scheduledTests.cronExpression') }}
              <HelpTooltip>
                <template #trigger>
                  <span class="inline-flex h-4 w-4 cursor-help items-center justify-center rounded-full border border-line/70 text-[10px] font-semibold text-muted transition-colors hover:border-accent hover:text-accent">
                    ?
                  </span>
                </template>
        ...
        <div class="mt-3 flex justify-end gap-2">
          <button
            ...
            class="rounded-lg bg-surface-2 px-3 py-1.5 text-sm font-medium text-foreground transition-colors hover:bg-surface-3"
          >
            {{ t('common.cancel') }}
          </button>
          <button
            ...
            class="flex items-center gap-1.5 rounded-lg bg-accent px-3 py-1.5 text-sm font-medium text-white transition-colors hover:bg-primary-600 disabled:cursor-not-allowed disabled:opacity-50"
          >
      ...
      <div v-if="loading" class="flex items-center justify-center py-8">
        <Icon name="refresh" size="md" class="animate-spin text-muted" :stroke-width="2" />
        <span class="ml-2 text-sm text-muted">{{ t('common.loading') }}...</span>
      </div>

      <!-- Empty State -->
      <div
        v-else-if="plans.length === 0"
        class="rounded-xl border border-dashed border-line py-10 text-center"
      >
        <Icon name="calendar" size="lg" class="mx-auto mb-2 text-muted" :stroke-width="1.5" />
        <p class="text-sm text-muted">
      ...
      <div v-else class="space-y-3">
        <div
          v-for="plan in plans"
          :key="plan.id"
          class="rounded-xl border border-line bg-surface transition-all"
        >
          <!-- Plan Header -->
          <div
          ...
          >
            <div class="flex flex-1 items-center gap-4">
              ...
              <span
                ...
                class="inline-flex items-center rounded-full bg-[color-mix(in_oklch,var(--success)_16%,transparent)] px-2 py-0.5 text-xs font-medium text-success-text"
              >
                {{ t('admin.scheduledTests.autoRecover') }}
              </span>
            </div>
            ...
            <div class="flex items-center gap-3">
              <!-- Last Run -->
              <div v-if="plan.last_run_at" class="hidden text-right text-xs text-muted sm:block">
                <div>{{ t('admin.scheduledTests.lastRun') }}</div>
                <div>{{ formatDateTime(plan.last_run_at) }}</div>
              </div>

              <!-- Next Run -->
              <div v-if="plan.next_run_at" class="hidden text-right text-xs text-muted sm:block">
 ...
 expandedPlanId === plan.id ? 'rotate-180' : ''
          ...
          >
            ...
            <div v-if="loadingResults" class="flex items-center justify-center py-4">
              <Icon name="refresh" size="sm" class="animate-spin text-muted" :stroke-width="2" />
              <span class="ml-2 text-xs text-muted">{{ t('common.loading') }}...</span>
            </div>
              ...
              >
                <div class="flex items-center justify-between">
                  <div class="flex items-center gap-2">
                    ...
                    <!-- Latency -->
                    <span v-if="result.latency_ms > 0" class="text-xs text-muted">
                ...
                <div v-if="result.error_message" class="mt-2">
                  <div
                    class="cursor-pointer text-xs font-medium text-danger-text"
                ...
                <div v-else-if="result.response_text" class="mt-2">
                  ...
                  <pre
                    v-if="expandedResultIds.has(result.id)"
                    class="mt-1 max-h-32 overflow-auto whitespace-pre-wrap rounded bg-surface-2 p-2 text-xs text-foreground"
