<template>
  ...
  >
    <div class="space-y-4">
      ...
      <div
        v-if="showAddForm"
        class="rounded-card border border-brand-200 bg-brand-50/50 p-4 dark:border-brand-800 dark:bg-brand-900/20"
      >
        <div class="mb-3 text-sm font-medium text-ink-body">
          {{ t('admin.scheduledTests.addPlan') }}
        </div>
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          <div>
            <label class="mb-1 flex items-center gap-1 text-xs font-medium text-ink-soft">
              {{ t('admin.scheduledTests.cronExpression') }}
              <HelpTooltip>
                <template #trigger>
                  <span class="inline-flex h-4 w-4 cursor-help items-center justify-center rounded-full border border-line text-[10px] font-semibold text-ink-faint transition-colors hover:border-brand-500 hover:text-brand-600 dark:border-line dark:text-ink-soft dark:hover:border-brand-400 dark:hover:text-brand-400">
                    ?
                  </span>
                </template>
          ...
          <div class="flex items-end">
            <div>
              ...
              <p class="mt-0.5 text-xs text-ink-faint dark:text-ink-soft">
        ...
        <div class="mt-3 flex justify-end gap-2">
          ...
          <button
            ...
            class="btn btn-primary btn-sm flex items-center gap-1.5 disabled:cursor-not-allowed disabled:opacity-50"
          >
      ...
      <div v-if="loading" class="flex items-center justify-center py-8">
        <Icon name="refresh" size="md" class="animate-spin text-ink-faint" :stroke-width="2" />
        <span class="ml-2 text-sm text-ink-soft">{{ t('common.loading') }}...</span>
      </div>

      <!-- Empty State -->
      <div
        v-else-if="plans.length === 0"
        class="rounded-card border border-dashed border-line py-10 text-center dark:border-dark-600"
      >
        <Icon name="calendar" size="lg" class="mx-auto mb-2 text-ink-faint" :stroke-width="1.5" />
        <p class="text-sm text-ink-soft">
      ...
      <div v-else class="space-y-3">
        <div
          v-for="plan in plans"
          :key="plan.id"
          class="rounded-card border border-line bg-card transition-all dark:border-dark-600 dark:bg-dark-800"
        >
          <!-- Plan Header -->
          <div
          ...
          >
            <div class="flex flex-1 items-center gap-4">
              <!-- Model -->
              <div class="min-w-0">
                <div class="text-sm font-medium text-ink dark:text-ink">
                  {{ plan.model_id }}
                </div>
                <div class="mt-0.5 font-mono text-xs text-ink-soft">
              ...
              <span
                ...
                class="inline-flex items-center rounded-full bg-success-soft px-2 py-0.5 text-xs font-medium text-success dark:bg-success-soft dark:text-success"
              >
                {{ t('admin.scheduledTests.autoRecover') }}
              </span>
            </div>
            ...
            <div class="flex items-center gap-3">
              <!-- Last Run -->
              <div v-if="plan.last_run_at" class="hidden text-right text-xs text-ink-soft sm:block">
                <div>{{ t('admin.scheduledTests.lastRun') }}</div>
                <div>{{ formatDateTime(plan.last_run_at) }}</div>
              </div>

              <!-- Next Run -->
              <div v-if="plan.next_run_at" class="hidden text-right text-xs text-ink-soft sm:block">
              ...
              <div class="flex items-center gap-1" @click.stop>
                <button
                  ...
                  class="rounded-control p-1.5 text-ink-faint transition-colors hover:bg-accent-50 hover:text-accent-500 dark:hover:bg-accent-900/20"
                ...
                <button
                  ...
                  class="rounded-control p-1.5 text-ink-faint transition-colors hover:bg-danger-soft hover:text-danger dark:hover:bg-danger/20"
          ...
          <div
            ...
            class="border-t border-line px-4 py-3 dark:border-dark-700"
          >
            <div class="mb-2 text-xs font-medium text-ink-soft">
            ...
            <div v-if="loadingResults" class="flex items-center justify-center py-4">
              <Icon name="refresh" size="sm" class="animate-spin text-ink-faint" :stroke-width="2" />
              <span class="ml-2 text-xs text-ink-soft">{{ t('common.loading') }}...</span>
            </div>

            <!-- No Results -->
            <div
              v-else-if="results.length === 0"
              class="py-4 text-center text-xs text-ink-soft"
            >
            ...
            <div v-else class="max-h-64 space-y-2 overflow-y-auto">
              <div
                ...
                :key="result.id"
                class="rounded-control border border-line bg-page p-3 dark:border-dark-700 dark:bg-dark-900"
              >
                <div class="flex items-center justify-between">
                  <div class="flex items-center gap-2">
                      ...
                      :class="[
                        ...
                        result.status === 'success'
                          ...
                          : result.status === 'running'
                            ? 'bg-accent-50 text-accent-700 dark:bg-accent-500/20 dark:text-accent-400'
                    ...
                    <!-- Latency -->
                    <span v-if="result.latency_ms > 0" class="text-xs text-ink-soft">
                ...
                <div v-if="result.error_message" class="mt-2">
                  ...
                  >
                    ...
                    <Icon
                      ...
                      size="sm"
                      :class="[
                        'inline transition-transform duration-150',
                  ...
                  <pre
                    ...
                    class="mt-1 max-h-32 overflow-auto whitespace-pre-wrap rounded bg-danger-soft p-2 text-xs text-danger dark:bg-danger/20 dark:text-danger"
                  >{{ result.error_message }}</pre>
                </div>
                <div v-else-if="result.response_text" class="mt-2">
                  <div
                    class="cursor-pointer text-xs font-medium text-ink-soft"
                    @click="toggleResultDetail(result.id)"
                  >
                  ...
                  <pre
                    v-if="expandedResultIds.has(result.id)"
                    class="mt-1 max-h-32 overflow-auto whitespace-pre-wrap rounded bg-page p-2 text-xs text-ink-body dark:bg-dark-800"
