<template>
 ...
 :key="b.currency"
 :class="['font-medium', b.balance <= 0 ? 'text-red-600' : 'text-muted']"
 >
 {{ b.balance.toFixed(2) }} {{ b.currency }}
 </span>
 </div>

 <div v-if="!snapshot.success" class="truncate text-[10px] text-red-600" :title="snapshot.error" data-testid="monitor-quota-error">
 {{ truncatedError }}
 </div>
 </div>
</template>
