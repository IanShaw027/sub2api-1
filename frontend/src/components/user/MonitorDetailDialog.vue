<template>
  <BaseDialog
    ...
    @close="$emit('close')"
  >
    <div v-if="loading" class="py-8 text-center text-sm text-ink-soft">
      {{ t('common.loading') }}
    </div>
    <div v-else-if="!detail" class="py-8 text-center text-sm text-ink-soft">
      {{ t('channelStatus.detailLoadError') }}
    </div>
    <div v-else class="overflow-x-auto">
      <table class="w-full text-left text-sm">
        <thead class="border-b border-line dark:border-dark-700">
          <tr class="text-xs uppercase tracking-wider text-ink-soft dark:text-dark-400">
        ...
        <tbody>
          <tr
            ...
            :key="m.model"
            class="border-b border-line dark:border-dark-800"
          >
            <td class="py-2 pr-3 font-medium text-ink dark:text-dark-100">{{ m.model }}</td>
            <td class="py-2 pr-3">
              <span
            ...
            <td class="py-2 pr-3 text-ink-body dark:text-dark-300">{{ formatLatency(m.latest_latency_ms) }}</td>
            <td class="py-2 pr-3 text-ink-body dark:text-dark-300">{{ formatPercent(m.availability_7d) }}</td>
            <td class="py-2 pr-3 text-ink-body dark:text-dark-300">{{ formatPercent(m.availability_15d) }}</td>
            <td class="py-2 pr-3 text-ink-body dark:text-dark-300">{{ formatPercent(m.availability_30d) }}</td>
            <td class="py-2 pr-3 text-ink-body dark:text-dark-300">{{ formatLatency(m.avg_latency_7d_ms) }}</td>
          </tr>
        </tbody>
      </table>
    </div>
