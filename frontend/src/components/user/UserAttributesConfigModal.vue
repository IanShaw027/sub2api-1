<template>
 ...
 >
 <!-- Drag Handle -->
 <div class="cursor-move text-muted hover:text-muted" :title="t('admin.users.attributes.dragToReorder')">
 ...
 class="rounded-lg p-1.5 text-muted hover:bg-surface-2 hover:text-accent"
 :title="t('common.edit')"
 >
 ...
 <input v-model="form.required" type="checkbox" class="h-4 w-4 rounded border-line text-accent" />
 <span class="text-sm text-foreground">{{ t('admin.users.attributes.required') }}</span>
 </label>
 <label class="flex items-center gap-2">
 <input v-model="form.enabled" type="checkbox" class="h-4 w-4 rounded border-line text-accent" />
 <span class="text-sm text-foreground">{{ t('admin.users.attributes.enabled') }}</span>
 </label>
 </div>
 </form>
