<template>
 ...
 <div
 role="tablist"
 class="inline-flex p-0.5 rounded-xl bg-surface-2 border border-line/60 text-xs"
 >
 <button
 ...
 ? 'bg-surface shadow-sm text-foreground font-semibold'
 ...
 <button
 type="button"
 class="h-8 w-8 rounded-lg flex items-center justify-center text-muted hover:text-foreground hover:bg-surface-2 transition-colors disabled:opacity-50"
 :disabled="loading"
...
const overallChipClass = computed(() => {
 ...
 case 'operational':
 return 'bg-[color-mix(in_oklch,var(--success)_15%,transparent)] text-[var(--success)]'
 case 'degraded':
 default:
 return 'bg-[color-mix(in_oklch,var(--warning)_15%,transparent)] text-[var(--warning)]'
 }
})
