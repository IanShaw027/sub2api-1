    ...
    <div class="card flex min-h-0 flex-1 flex-col overflow-hidden">
      ...
      >
        <template #cell-model="{ row }">
          <span v-if="row.model" class="text-sm font-medium text-ink dark:text-white">{{ row.model }}</span>
          <span v-else class="text-sm text-ink-faint dark:text-dark-400">-</span>
        </template>

        <template #cell-key_name="{ row }">
          <div class="text-sm">
            <span class="text-ink dark:text-white">{{ row.key_name || '-' }}</span>
            <span
        ...
        <template #cell-endpoint="{ row }">
          <div class="max-w-[320px] text-xs">
            <div class="break-all text-ink-body dark:text-dark-300">
        ...
        <template #cell-category="{ row }">
          <span class="text-sm text-ink dark:text-white">{{ t('usage.errors.categories.' + row.category) }}</span>
        </template>

        <template #cell-message="{ row }">
          <span
            v-if="row.message"
            class="block max-w-[280px] truncate text-sm text-ink-body dark:text-dark-400"
            :title="row.message"
          >{{ row.message }}</span>
          <span v-else class="text-sm text-ink-faint dark:text-dark-400">-</span>
        </template>
        ...
        <template #cell-platform="{ row }">
          <span class="text-sm text-ink dark:text-white">{{ row.platform || '-' }}</span>
        </template>

        <template #cell-client_ip="{ row }">
          <div @click.stop>
            <div v-if="row.client_ip">
              <span class="text-sm font-mono text-ink-body dark:text-dark-400">{{ row.client_ip }}</span>
        ...
        <template #cell-created_at="{ row }">
          <span class="text-sm text-ink-body dark:text-dark-400">{{ formatDateTime(row.created_at) }}</span>
        </template>

        <template #cell-user_agent="{ row }">
          <span
            v-if="row.user_agent"
            class="block max-w-[320px] truncate text-sm text-ink-body dark:text-dark-400"
