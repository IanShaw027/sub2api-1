<template>
  ...
  >
    ...
    >
      <div class="text-sm text-ink-body dark:text-ink-body">
        {{ t('admin.accounts.syncFromCrsDesc') }}
      </div>
      <div
        class="rounded-control bg-page p-3 text-xs text-ink-soft dark:bg-dark-700/60 dark:text-dark-400"
      >
        {{ t('admin.accounts.crsUpdateBehaviorNote') }}
      </div>
      <div
        class="rounded-control border border-warning/25 bg-warning-soft p-3 text-xs text-warning dark:border-warning/30 dark:bg-warning/10 dark:text-warning"
      >
      ...
      <div class="grid grid-cols-1 gap-4">
        ...
        <label class="flex items-center gap-2 text-sm text-ink-body dark:text-ink-body">
          <input
            v-model="form.sync_proxies"
            type="checkbox"
            class="rounded border-line dark:border-dark-600"
          />
    ...
    <div v-else-if="currentStep === 'preview' && previewResult" class="space-y-4">
      ...
      <div
        ...
        class="rounded-control bg-page p-3 dark:bg-dark-700/60"
      >
        <div class="mb-2 text-sm font-medium text-ink-body dark:text-ink-body">
          {{ t('admin.accounts.crsExistingAccounts') }}
          <span class="ml-1 text-xs text-ink-faint">({{ previewResult.existing_accounts.length }})</span>
        </div>
        <div class="max-h-32 overflow-auto text-xs text-ink-soft dark:text-ink-soft">
          <div
      ...
      <div v-if="previewResult.new_accounts.length">
        <div class="mb-2 flex items-center justify-between">
          <div class="text-sm font-medium text-ink dark:text-white">
            ...
            <span class="ml-1 text-xs text-ink-faint">({{ previewResult.new_accounts.length }})</span>
          </div>
          <div class="flex gap-2">
            ...
            <button
              type="button"
              class="text-xs text-ink-soft hover:text-ink-soft dark:text-ink-soft"
              @click="selectNone"
            >{{ t('admin.accounts.crsSelectNone') }}</button>
          </div>
        </div>
        <div
          class="max-h-48 overflow-auto rounded-control border border-line p-2 dark:border-dark-600"
        >
          <label
            v-for="acc in previewResult.new_accounts"
            :key="acc.crs_account_id"
            class="flex cursor-pointer items-center gap-2 rounded px-2 py-1.5 hover:bg-page dark:hover:bg-dark-700/40"
          >
            <input
              type="checkbox"
            ...
            />
            <span
              class="inline-block rounded bg-success-soft px-1.5 py-0.5 text-[10px] font-medium text-success dark:bg-success/15 dark:text-success"
            >{{ acc.platform }} / {{ acc.type }}</span>
            <span class="truncate text-sm text-ink-body dark:text-ink-body">{{ acc.name }}</span>
          </label>
        </div>
      ...
      <div class="flex items-center gap-2 text-xs text-ink-soft dark:text-ink-soft">
        <span>{{ t('admin.accounts.syncProxies') }}:</span>
        <span :class="form.sync_proxies ? 'text-success dark:text-success' : 'text-ink-faint'">
      ...
      <div
        ...
        class="rounded-control bg-page p-4 text-center text-sm text-ink-soft dark:bg-dark-700/60 dark:text-dark-400"
      >
