<template>
  ...
  >
    ...
    >
      ...
      <div
        class="rounded-lg bg-surface-2 p-3 text-xs text-muted"
      >
    ...
    <div v-else-if="currentStep === 'preview' && previewResult" class="space-y-4">
      ...
      >
        <div class="mb-2 text-sm font-medium text-foreground">
          ...
          <span class="ml-1 text-xs text-muted">({{ previewResult.existing_accounts.length }})</span>
        </div>
        <div class="max-h-32 overflow-auto text-xs text-muted">
          ...
          >
            <span
              class="inline-block rounded bg-[color-mix(in_oklch,var(--accent)_12%,transparent)] px-1.5 py-0.5 text-[10px] font-medium text-accent"
      ...
      <div v-if="previewResult.new_accounts.length">
        <div class="mb-2 flex items-center justify-between">
          <div class="text-sm font-medium text-foreground">
            ...
            <span class="ml-1 text-xs text-muted">({{ previewResult.new_accounts.length }})</span>
          </div>
          <div class="flex gap-2">
            ...
            <button
              type="button"
              class="text-xs text-muted hover:text-muted"
              @click="selectNone"
            >{{ t('admin.accounts.crsSelectNone') }}</button>
          </div>
        </div>
        <div
          class="max-h-48 overflow-auto rounded-lg border border-line p-2"
        >
          <label
          ...
          >
            ...
            >{{ acc.platform }} / {{ acc.type }}</span>
            <span class="truncate text-sm text-foreground">{{ acc.name }}</span>
          </label>
        </div>
      ...
      <div
        ...
        class="rounded-lg bg-surface-2 p-4 text-center text-sm text-muted"
      >
    ...
    <div v-else-if="currentStep === 'result' && result" class="space-y-4">
      ...
      >
        ...
        <div v-if="errorItems.length" class="mt-2">
          <div class="text-sm font-medium text-danger-text">
            {{ t('admin.accounts.syncErrors') }}
          </div>
          <div
            class="mt-2 max-h-48 overflow-auto rounded-lg bg-surface-2 p-3 font-mono text-xs"
          >
