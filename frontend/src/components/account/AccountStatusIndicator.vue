  ...
  <div class="flex items-center gap-2">
    ...
    <template v-else>
      <div v-if="isTempUnschedulable" class="flex flex-col items-center gap-1">
        ...
        <span class="max-w-[180px] text-center text-[11px] leading-4 text-muted">
    ...
    <div v-if="hasError && account.error_message" class="group/error relative">
      <svg
        class="h-4 w-4 cursor-help text-danger-500 transition-colors hover:text-danger-text"
        fill="none"
        viewBox="0 0 24 24"
        stroke="currentColor"
        stroke-width="2"
      ...
      <!-- Tooltip - 向下显示 -->
      <div
        class="invisible absolute left-0 top-full z-[100] mt-1.5 min-w-[200px] max-w-[300px] rounded-lg bg-[var(--code-bg)] px-3 py-2 text-xs text-white opacity-0 shadow-glass transition-all duration-200 group-hover/error:visible group-hover/error:opacity-100"
      >
        <div class="whitespace-pre-wrap break-words leading-relaxed text-muted">
          {{ account.error_message }}
        </div>
        <!-- 上方小三角 -->
        <div
          class="absolute bottom-full left-3 border-[6px] border-transparent border-b-[var(--code-bg)]"
        ></div>
      </div>
    </div>
    ...
    <div v-if="isRateLimited" class="group relative">
      <span
        class="inline-flex items-center gap-1 rounded bg-warning-100 px-1.5 py-0.5 text-xs font-medium text-warning-text"
      >
      ...
      <!-- Tooltip -->
      <div
        class="pointer-events-none absolute bottom-full left-1/2 z-50 mb-2 w-56 -translate-x-1/2 whitespace-normal rounded bg-[var(--code-bg)] px-3 py-2 text-center text-xs leading-relaxed text-white opacity-0 transition-opacity group-hover:opacity-100"
      >
        {{ t('admin.accounts.status.rateLimitedUntil', { time: formatDateTime(account.rate_limit_reset_at) }) }}
        <div
          class="absolute left-1/2 top-full -translate-x-1/2 border-4 border-transparent border-t-[var(--code-bg)]"
        ></div>
      </div>
    </div>
    ...
    >
      <div v-for="item in activeModelStatuses" :key="`${item.kind}-${item.model}`" class="group relative mb-1 break-inside-avoid">
        ...
        <span
          ...
          class="inline-flex items-center gap-1 rounded bg-[color-mix(in_oklch,var(--danger)_14%,transparent)] px-1.5 py-0.5 text-xs font-medium text-danger-text"
        >
          <Icon name="exclamationTriangle" size="xs" :stroke-width="2" />
        ...
        <span
          v-else
          class="inline-flex items-center gap-1 rounded bg-accent-100 px-1.5 py-0.5 text-xs font-medium text-accent-700"
        >
          <Icon name="exclamationTriangle" size="xs" :stroke-width="2" />
          {{ formatScopeName(item.model) }}
          <span class="text-[10px] opacity-70">{{ formatCountdown(item.reset_at) }}</span>
        ...
        <!-- Tooltip -->
        <div
          class="pointer-events-none absolute bottom-full left-1/2 z-50 mb-2 w-max max-w-[320px] -translate-x-1/2 whitespace-nowrap rounded bg-[var(--code-bg)] px-3 py-2 text-center text-xs leading-relaxed text-white opacity-0 transition-opacity group-hover:opacity-100"
        >
          {{
