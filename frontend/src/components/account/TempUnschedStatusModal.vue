  ...
  >
    <div class="space-y-4">
      <div v-if="loading" class="flex items-center justify-center py-8">
        <svg class="h-6 w-6 animate-spin text-ink-faint" fill="none" viewBox="0 0 24 24">
          <circle
            class="opacity-25"
            cx="12"
            cy="12"
      ...
      <div v-else-if="!isActive" class="rounded-control border border-line p-4 text-sm text-ink-soft dark:border-dark-600 dark:text-ink-soft">
        {{ t('admin.accounts.tempUnschedulable.notActive') }}
      </div>

      <div v-else class="space-y-4">
        <div class="rounded-control border border-success/25 bg-success-soft p-3 text-sm text-success dark:border-success/30 dark:bg-success/10 dark:text-success">
          {{ t('admin.accounts.recoverStateHint') }}
        </div>

        <div class="rounded-control border border-line p-4 dark:border-dark-600">
          <p class="text-xs text-ink-soft dark:text-ink-soft">
            {{ t('admin.accounts.tempUnschedulable.accountName') }}
          </p>
          <p class="mt-1 text-sm font-medium text-ink dark:text-ink">
            {{ account?.name || '-' }}
          </p>
        </div>
        ...
        <div class="rounded-control border border-line p-3 dark:border-dark-600">
          ...
          <div class="mt-2 rounded bg-page p-2 text-xs text-ink-body dark:bg-dark-700 dark:text-ink-body">
            {{ state?.error_message || '-' }}
          </div>
        </div>
      </div>
