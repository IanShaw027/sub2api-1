...
describe('AccountUsageCell', () => {
  ...
  it('Grok OAuth 无官方窗口时不重复展示独立本地行并展示已用比例', async () => {
    getUsage.mockResolvedValue({
      grok_local_usage: {
        requests: 4,
        tokens: 1200,
    ...
    expect(wrapper.text()).toContain('admin.accounts.usageWindow.grokRequests|100|2026-07-09T16:00:00Z')
  })
  ...
  it('Grok paid plans are not mistaken for Free when weekly usage is temporarily missing', async () => {
    ...
    expect(wrapper.text()).toContain('admin.accounts.usageWindow.grokTokens|75')
    expect(wrapper.text()).not.toContain('2M|')
  })
