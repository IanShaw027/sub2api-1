<template>
 ...
 : 'bg-surface-2 text-muted'"
    ...
    <div v-if="loading" class="flex h-20 items-center justify-center text-muted">
    ...
    <template v-else>
      ...
      <div
        v-if="snapshot"
        class="border-y border-line py-3"
        data-testid="ollama-cloud-usage-details"
      >
        <div class="grid grid-cols-[minmax(4rem,auto)_minmax(0,1fr)] gap-x-3 gap-y-1.5 text-xs">
          <span class="text-muted">{{ t('admin.accounts.ollamaCloud.plan') }}</span>
          <span class="break-words text-foreground">{{ snapshot.data?.plan || '-' }}</span>
          <span class="text-muted">{{ t('admin.accounts.ollamaCloud.fiveHour') }}</span>
          <span class="break-words text-foreground">{{ windowSummary(snapshot.data?.five_hour) }}</span>
          <span class="text-muted">{{ t('admin.accounts.ollamaCloud.sevenDay') }}</span>
          <span class="break-words text-foreground">{{ windowSummary(snapshot.data?.seven_day) }}</span>
          <span class="text-muted">{{ t('admin.accounts.ollamaCloud.balance') }}</span>
          <span class="break-words text-foreground">{{ snapshot.data?.balance || '-' }}</span>
          <span class="text-muted">{{ t('admin.accounts.ollamaCloud.models') }}</span>
          <span class="break-words text-foreground">{{ modelSummary }}</span>
          <span class="text-muted">{{ t('admin.accounts.ollamaCloud.status') }}</span>
          <span class="break-words font-medium text-foreground">{{ statusLabel }}</span>
      ...
      <div v-if="state.configured" class="flex items-center justify-between gap-4 border-t border-line pt-4">
        <div>
