<template>
  <div v-if="visible" class="space-y-1">
    ...
    <div class="flex flex-wrap items-center gap-1.5">
      ...
      <span
        v-if="balanceLow"
        class="inline-flex items-center rounded bg-[color-mix(in_oklch,var(--danger)_14%,transparent)] px-1 py-0.5 text-[10px] font-medium text-danger-text"
      >
    ...
    <div class="flex flex-wrap items-center gap-1.5">
      <button
        ...
        class="inline-flex items-center gap-0.5 whitespace-nowrap rounded px-1.5 py-0.5 text-[10px] font-medium leading-4 text-accent transition-colors hover:bg-[color-mix(in_oklch,var(--accent)_12%,transparent)] disabled:cursor-not-allowed disabled:opacity-50"
        :disabled="loading"
        :title="t('admin.accounts.cnProviders.balanceProbeTooltip')"
        @click="handleProbe"
      >
    ...
    <div v-if="error" class="truncate text-[10px] text-danger-text" :title="error">
      {{ truncatedError }}
    </div>
  </div>
</template>
