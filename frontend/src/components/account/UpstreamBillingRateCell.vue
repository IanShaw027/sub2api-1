<template>
  <div v-if="eligible" class="flex h-6 min-w-[7rem] items-center gap-1">
    <HelpTooltip class="-ml-1" width-class="w-max max-w-[calc(100vw-2rem)]" data-testid="upstream-billing-details">
      <template #trigger>
        <span
          ...
          :class="hasEffectiveRate ? 'font-mono text-foreground' : statusClass || 'text-muted'"
    ...
    <button
      type="button"
      class="inline-flex h-6 w-6 flex-shrink-0 items-center justify-center rounded text-accent transition-colors hover:bg-[color-mix(in_oklch,var(--accent)_12%,transparent)] disabled:cursor-not-allowed disabled:opacity-50"
      :disabled="probing"
...
const statusClass = computed(() => {
  ...
  if (snapshot.value.status === 'unsupported') return 'text-muted'
  if (stale.value) return 'text-warning-text'
  if (snapshot.value.status === 'failed') return 'text-danger-text'
  return ''
})
