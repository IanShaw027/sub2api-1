<template>
 ...
 class="rounded bg-surface-2/60 px-1 text-[10px] uppercase text-muted"
 >
 {{ model.platform }}
 </span>
 {{ model.name }}
 ...
 ref="popoverEl"
 role="tooltip"
 class="pointer-events-none fixed z-[99999] w-80 max-w-[min(22rem,calc(100vw-1rem))] rounded-lg border bg-surface text-xs shadow-sm"
 ...
 <span
 v-if="model.platform"
 class="flex-shrink-0 rounded bg-surface/70 px-1.5 py-0.5 text-[10px] uppercase tracking-wide"
 >
 {{ model.platform }}
 </span>
 </div>
 ...
 <div v-else class="space-y-2 text-foreground">
