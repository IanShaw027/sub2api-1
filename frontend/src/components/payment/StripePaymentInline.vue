<template>
  <div class="space-y-4">
    ...
    <div v-else-if="initError" class="card p-6 text-center">
      <p class="text-sm text-danger dark:text-red-400">{{ initError }}</p>
    ...
    <template v-else-if="success">
      <div class="card p-6">
        <div class="flex flex-col items-center space-y-4 py-4">
          ...
          <div class="w-full rounded-card border border-line bg-page p-4 dark:border-dark-700 dark:bg-dark-800">
            <div class="space-y-2 text-sm">
              <div class="flex justify-between">
                <span class="text-ink-soft">{{ t('payment.orders.orderId') }}</span>
                <span class="font-medium tabular-nums text-ink dark:text-white">#{{ orderId }}</span>
              </div>
              <div v-if="amount > 0" class="flex justify-between">
                <span class="text-ink-soft">{{ t('payment.orders.amount') }}</span>
                <span class="font-medium tabular-nums text-ink dark:text-white">{{ creditedAmountSymbol }}{{ amount.toFixed(2) }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-ink-soft">{{ t('payment.orders.payAmount') }}</span>
                <span class="font-medium tabular-nums text-ink dark:text-white">{{ formattedPayAmount }}</span>
              </div>
            </div>
          </div>
    ...
    <template v-else>
      <!-- Amount (Stripe brand purple kept for recognition) -->
      <div class="card overflow-hidden">
        <div class="bg-gradient-to-br from-[#635bff] to-[#4f46e5] px-6 py-5 text-center">
          <p class="text-sm font-medium text-indigo-200">{{ t('payment.actualPay') }}</p>
          <p class="mt-1 text-3xl font-semibold tabular-nums text-white">{{ formattedPayAmount }}</p>
        </div>
      </div>
      <!-- Stripe Payment Element -->
      <div class="card p-6">
        <div ref="stripeMount" class="min-h-[200px]"></div>
        <p v-if="error" class="mt-4 text-sm text-danger dark:text-red-400">{{ error }}</p>
