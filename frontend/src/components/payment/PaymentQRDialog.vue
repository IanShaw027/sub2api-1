<template>
  <BaseDialog :show="show" :title="dialogTitle" width="narrow" @close="handleClose">
    ...
    <div v-if="!success" class="flex flex-col items-center space-y-4">
      <!-- QR Code mode -->
      <template v-if="qrUrl">
        <div class="rounded-hero border border-line bg-card p-4 shadow-xs dark:border-dark-700 dark:bg-dark-800">
          <canvas ref="qrCanvas" class="mx-auto"></canvas>
        </div>
        <p v-if="scanHint" class="text-center text-sm text-ink-soft dark:text-dark-400">
          {{ scanHint }}
        </p>
      </template>
      ...
      <template v-else>
        <div class="flex flex-col items-center py-4">
          ...
          <p class="mt-4 text-sm text-ink-soft dark:text-dark-400">{{ t('payment.qr.payInNewWindowHint') }}</p>
      ...
      <div v-else class="text-center">
        <p class="text-sm text-ink-soft dark:text-dark-400">{{ qrUrl ? t('payment.qr.expiresIn') : '' }}</p>
    ...
    <div v-else class="flex flex-col items-center space-y-4 py-4">
      ...
      <p class="text-lg font-semibold text-ink dark:text-white">{{ t('payment.result.success') }}</p>
      <div v-if="paidOrder" class="w-full rounded-card border border-line bg-page p-4 dark:border-dark-700 dark:bg-dark-800">
        <div class="space-y-2 text-sm">
          ...
          <div class="flex justify-between">
            ...
            <span class="font-medium tabular-nums text-ink dark:text-white">{{ formatOrderPayAmount(paidOrder) }}</span>
          </div>
        </div>
      </div>
    </div>
