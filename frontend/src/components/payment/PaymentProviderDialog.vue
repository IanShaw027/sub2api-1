    ...
    <form id="provider-form" @submit.prevent="handleSave" class="space-y-4">
      ...
      <div class="flex flex-wrap items-center gap-x-5 gap-y-2">
        ...
        <div v-if="supportsPaymentMode" class="flex items-center gap-2">
          <span class="text-xs font-medium text-ink-soft dark:text-dark-400">{{ t('admin.settings.payment.paymentMode') }}</span>
          <div class="flex gap-1.5">
            <button
              ...
              :class="[
                'rounded-control border px-2.5 py-1 text-xs font-medium transition-all duration-150',
                form.payment_mode === mode.value
                  ? 'border-brand-500 bg-brand-500 text-white shadow-xs'
                  : 'border-line bg-card text-ink-body hover:border-divider hover:bg-page dark:border-dark-600 dark:bg-dark-800 dark:text-dark-300 dark:hover:border-dark-500',
              ]"
            >{{ mode.label }}</button>
          </div>
        </div>
        <div v-if="availableTypes.length > 1" class="flex items-center gap-2">
          <span class="text-xs font-medium text-ink-soft dark:text-dark-400">{{ t('admin.settings.payment.supportedTypes') }}</span>
      ...
      <div v-if="form.provider_key === 'easypay'" class="space-y-3 rounded-card border border-line p-3 dark:border-dark-700">
        <div class="flex items-center justify-between gap-3">
          <div>
            <h5 class="text-sm font-medium text-ink dark:text-white">
        ...
        <div v-if="easyPayCustomMethods.length" class="space-y-2">
          ...
          >
            <div>
              <label class="text-xs text-ink-soft dark:text-dark-400">{{ t('admin.settings.payment.customMethodType') }}</label>
              <input v-model="method.type" type="text" class="input mt-0.5" placeholder="credit_card" />
            </div>
            <div>
              <label class="text-xs text-ink-soft dark:text-dark-400">{{ t('admin.settings.payment.customMethodUpstreamType') }}</label>
              <input v-model="method.upstreamType" type="text" class="input mt-0.5" placeholder="credit_card" />
            </div>
            <div>
              <label class="text-xs text-ink-soft dark:text-dark-400">{{ t('admin.settings.payment.customMethodDisplayName') }}</label>
      ...
      <div class="border-t border-line pt-4 dark:border-dark-700">
        <div class="mb-3 flex items-center gap-2">
          ...
          <HelpTooltip v-if="paymentGuide" trigger="click" width-class="w-80">
            <template #trigger>
              <button
                type="button"
                class="inline-flex h-5 w-5 items-center justify-center rounded-full border border-line text-[11px] font-semibold text-ink-faint transition-colors hover:border-brand-500 hover:text-brand-600 dark:border-dark-500 dark:text-dark-500 dark:hover:border-brand-400 dark:hover:text-brand-400"
        ...
        <p v-if="paymentGuide" class="mb-3 text-xs text-ink-soft dark:text-dark-400">
        ...
        <div class="space-y-3">
          <div v-for="field in resolvedFields" :key="field.key">
            <label class="input-label">
              {{ field.label }}
              <span v-if="field.optional" class="text-xs text-ink-faint">({{ t('common.optional') }})</span>
            ...
            <div v-else-if="field.sensitive" class="relative">
              ...
              <button
                ...
                class="absolute inset-y-0 right-0 flex items-center pr-3 text-ink-faint hover:text-ink-body dark:hover:text-dark-300"
              >
            ...
            />
            <p v-if="field.hintKey" class="mt-1 text-xs leading-relaxed text-ink-soft dark:text-dark-400">
              {{ t(field.hintKey) }}
            </p>
          </div>
        </div>
        ...
        <div v-if="callbackPaths" class="mt-4 space-y-3">
          <div v-if="callbackPaths.notifyUrl">
            ...
            <div class="flex">
              ...
              <span class="inline-flex items-center whitespace-nowrap rounded-r-lg border border-line bg-page px-3 text-xs text-ink-soft dark:border-dark-600 dark:bg-dark-700 dark:text-dark-400">{{ callbackPaths.notifyUrl }}</span>
            </div>
          </div>
          <div v-if="callbackPaths.returnUrl">
            ...
            <div class="flex">
              ...
              <span class="inline-flex items-center whitespace-nowrap rounded-r-lg border border-line bg-page px-3 text-xs text-ink-soft dark:border-dark-600 dark:bg-dark-700 dark:text-dark-400">{{ callbackPaths.returnUrl }}</span>
            </div>
          </div>
        </div>

        <!-- 服务商 Webhook 提示 -->
        <div v-if="providerWebhookUrl" class="mt-3 rounded-lg border border-accent-200 bg-accent-50 p-3 dark:border-blue-800/50 dark:bg-blue-900/20">
          <p class="text-xs text-accent-700 dark:text-blue-300">
            {{ t(providerWebhookHint) }}
          </p>
          <code class="mt-1 block break-all rounded bg-accent-100 px-2 py-1 text-xs text-accent-800 dark:bg-blue-900/40 dark:text-blue-200">
            {{ providerWebhookUrl }}
          </code>
          <p v-if="form.provider_key === 'stripe'" class="mt-2 text-xs leading-relaxed text-accent-700 dark:text-blue-300">
      ...
      <div v-if="limitableTypes.length" class="border-t border-line pt-4 dark:border-dark-700">
        <button type="button" @click="limitsExpanded = !limitsExpanded" class="flex w-full items-center justify-between">
          ...
          <svg :class="['h-4 w-4 text-ink-faint transition-transform', limitsExpanded && 'rotate-180']" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" /></svg>
        </button>
        <div v-show="limitsExpanded" class="mt-3 space-y-3">
          <div
            v-for="lt in limitableTypes"
            :key="lt.value"
            class="rounded-lg border border-line p-3 dark:border-dark-700"
          >
            <p class="mb-2 text-xs font-medium text-ink-body dark:text-dark-300">{{ lt.label }}</p>
            <div class="grid grid-cols-3 gap-3">
              <div>
                <label class="text-xs text-ink-soft dark:text-dark-400">{{ t('admin.settings.payment.limitSingleMin') }}</label>
                <input
                  type="number"
              ...
              <div>
                <label class="text-xs text-ink-soft dark:text-dark-400">{{ t('admin.settings.payment.limitSingleMax') }}</label>
                <input
                  type="number"
              ...
              <div>
                <label class="text-xs text-ink-soft dark:text-dark-400">{{ t('admin.settings.payment.limitDaily') }}</label>
                <input
                  type="number"
          ...
          <p class="text-xs text-ink-faint dark:text-dark-500">{{ t('admin.settings.payment.limitsHint') }}</p>
        </div>
      </div>
    </form>
