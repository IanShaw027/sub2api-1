<template>
  <div class="fixed inset-0 z-50 overflow-y-auto">
    <div class="flex min-h-full items-center justify-center p-4">
      <div class="fixed inset-0 bg-ink/40 backdrop-blur-sm transition-opacity dark:bg-black/50"></div>

      <div class="relative w-full max-w-md transform rounded-card border border-line bg-card p-6 shadow-xl transition-all dark:border-dark-600 dark:bg-dark-800">
        <!-- Header -->
        <div class="mb-6 text-center">
          <div class="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-brand-50 dark:bg-brand-950/40">
            <svg class="h-6 w-6 text-brand-600 dark:text-brand-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
              <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z" />
            </svg>
          </div>
          <h3 class="mt-4 text-xl font-semibold tracking-tight text-ink dark:text-white">
            {{ t('profile.totp.loginTitle') }}
          </h3>
          <p class="mt-2 text-sm text-ink-soft">
            {{ t('profile.totp.loginHint') }}
          </p>
          <p v-if="userEmailMasked" class="mt-1 text-sm font-medium text-ink-body">
            {{ userEmailMasked }}
          </p>
        </div>
        ...
        <div class="mb-6">
          ...
          <div class="flex justify-center gap-2">
            <input
              ...
              pattern="[0-9]"
              autocomplete="off"
              class="h-12 w-10 rounded-control border border-line bg-card text-center text-lg font-semibold text-ink focus:border-accent focus:outline-none focus:ring-[3px] focus:ring-accent/25 dark:border-dark-600 dark:bg-dark-700"
              :disabled="verifying"
          ...
          <!-- Loading indicator -->
          <div v-if="verifying" class="mt-3 flex items-center justify-center gap-2 text-sm text-ink-soft">
