...
async function getTarget() { for (let i = 0; i < 60; i++) { try { const r = await fetch(`http://127.0.0.1:${port}/json/list`); const l = await r.json(); const t = l.find(x => x.type === 'page'); if (t) return t } catch { /* Chrome's debugging endpoint is not ready yet. */ } await sleep(250) } throw new Error('no target') }
(async () => {
